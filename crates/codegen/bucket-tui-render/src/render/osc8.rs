...
//! frame diff (see `bucket_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.bucket`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — bucket
/// session media lives under `~/.bucket/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.bucket/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.bucket/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn scan_detects_bucket_session_media_path() {
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.bucket/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.bucket/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.bucket/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.bucket/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.bucket/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.bucket/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/bucket-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            &*resolve_link_target(&overlay.links()[0].target)
                .and_then(|resolved| resolved.osc8_url)
                .expect("url"),
            "file:///tmp/bucket-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/bucket-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/bucket-tui/src/render` should
        // NOT produce a link for the `/bucket-tui/src/render` substring.
        let line = make_line("find crates/codegen/bucket-tui/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
