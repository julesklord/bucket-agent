...
use bucket_tui::app::app_view::{AuthState, TrustState};
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to Bucket", bold)),
            );
        ...
        MinimalAuthHint::TrustFolder { workspace } => {
            ...
            y = put_line(
                ...
                bottom,
                Line::from(Span::styled(
                    "Bucket Build may run or modify contents in this directory,",
                    gray,
                )),
            );
            y = put_line(
...
mod tests {
    ...
    fn auth_hint_maps_auth_state() {
        use bucket_tui::app::app_view::AuthMode;
    ...
    fn render_auth_shows_url_and_code() {
        ...
        assert!(text.contains("Sign in to Bucket"), "header: {text:?}");
