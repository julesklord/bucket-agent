...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&bucket_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&bucket_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &bucket_chat_state::UsageTotals) -> Self {
        ...
        let bucket_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            reasoning_tokens,
...
impl From<&bucket_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &bucket_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<bucket_hooks_plugins_types::HookInfo>,
...
/// detailed migration guidance (`bucket logout` / `bucket login`) in the
...
pub struct CompactionSegmentFile {
    pub items: Vec<bucket_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: bucket_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_bucket_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for bucket-build and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<bucket_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    /// API's `x_bucket_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub model: String,
    /// Sampling request id sent to the proxy (`bucket-recap-{uuid}`).
    pub x_bucket_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_bucket_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_bucket_req_id: "bucket-recap-abc".into(),
            x_bucket_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_bucket_req_id, "bucket-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use bucket_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.bucket/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.bucket/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.bucket/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.bucket/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.bucket/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("bucket-4".into(), 6_000), ("bucket-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "bucket-4");
    ...
    fn model_changed_serializes_snake_case_with_optional_effort() {
        ...
        assert_eq!(json["sessionUpdate"], "model_changed");
        assert_eq!(json["model_id"], "bucket-3");
        assert!(
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "bucket-4");
    }
