...
//! `bucket_sampling_types::SamplingConfig` so that the sampler crate
...
/// submit.
///
/// # Construction in `bucket-agent-core`
///
...
/// composing chat-state's `bucket_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-BUCKET-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-bucket-doom-loop-check` header on
