...
/// `~/.bucket/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<bucket_agent_core::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    bucket_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.bucket/config.toml`.
///
...
/// `~/.bucket/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no bucket home"))?;
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/bucket");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/bucket");
    }
    ...
    fn completion_reports_full_miss_for_unreadable_file_url() {
        ...
        let completion = state.complete_clipboard_attachment_paste(
            completion_ctx(None, false),
            crate::app::actions::ProbedAttachment::NoRaster,
            Some("file:///definitely/missing/bucket-primary-paste.png".to_owned()),
        );
    ...
    /// per-key serial lock. The `BUCKET_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(BUCKET_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `BUCKET_AGENT_DASHBOARD` key reads it concurrently.
