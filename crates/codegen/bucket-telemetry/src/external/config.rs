...
//! `BUCKET_EXTERNAL_OTEL=1` *and* at least one of `OTEL_METRICS_EXPORTER` /
...
/// Master switch env var. Deliberately *not* `BUCKET_ENABLE_TELEMETRY`: that
/// would be a word-order typo away from the long-standing
/// `BUCKET_TELEMETRY_ENABLED` (product events/Mixpanel mode), and the two control
/// opposite-pointing data flows (to xAI vs. to the customer's collector).
pub const ENV_MASTER_SWITCH: &str = "BUCKET_EXTERNAL_OTEL";
...
pub struct ContentGates {
    /// `OTEL_LOG_USER_PROMPTS=1`: prompt text on `bucket_code.user_prompt`
...
mod tests {
    ...
    fn master_switch_alone_enables_nothing() {
        // RQ7: BUCKET_EXTERNAL_OTEL=1 without an explicit exporter is inert.
        assert!(
            ExternalOtelConfig::resolve_with(env(&[("BUCKET_EXTERNAL_OTEL", "1")]), None).is_none()
        );
    }
    ...
    fn file_config_layered_under_env() {
        ...
        let cfg =
            ExternalOtelConfig::resolve_with(env(&[("BUCKET_EXTERNAL_OTEL", "0")]), Some(&file));
        assert!(cfg.is_none());
    }
