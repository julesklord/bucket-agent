...
//! `BucketToolDyn` / `BucketToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use bucket_tool_protocol::{ToolCapabilities, ToolId};
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&bucket_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
