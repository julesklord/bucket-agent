...
//!
//! The canonical `sample → classify → retry` loop, used by **both** bucket-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Bucket chat's intra `Shared` summarizer
...
//! the winning summary (bucket-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the bucket-build host uses to step down its input size).
