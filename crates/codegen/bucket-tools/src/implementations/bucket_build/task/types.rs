...
//! (in `bucket-tools`) and the subagent coordinator (in `bucket-agent-core`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"bucket-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`bucket_tool_types::is_not_sentinel`] for existing call sites.
pub use bucket_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `bucket-agent-core`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "BucketBuild:run_terminal_cmd" | "BucketBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `bucket-agent-core` relies on
...
register_resource!("bucket_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-bucket-build
...
pub type MonitorEventBuffer = bucket_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("bucket_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// bucket-agent-core produces it, the channel protocol carries it, and the
/// compaction pipeline in bucket-chat-state (via `RunningSubagentSummary`)
...
register_resource!("bucket_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("bucket_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("bucket_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by bucket-agent-core immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by bucket-agent-core at turn start.
...
register_resource!("bucket_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("BucketBuild:run_terminal_cmd", ToolKind::Execute),
                tc("BucketBuild:read_file", ToolKind::Read),
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("BucketBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
