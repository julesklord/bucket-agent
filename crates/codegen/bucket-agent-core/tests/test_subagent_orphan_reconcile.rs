...
//!
//! This test spawns a real `bucket agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p bucket-agent-core --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // BucketStdioClient sets HOME=<temp>; the binary uses <HOME>/.bucket as BUCKET_HOME.
        let bucket_home = shared_home.path().join(".bucket");
        let session_dir = locate_session_dir(&bucket_home, session_id.0.as_ref());
