...
//! `bucket-agent` and `bucket-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `bucket-tools` for external
...
// workspace crate without a direct bucket-tools dependency.
pub use bucket_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use bucket_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use bucket_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use bucket_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`bucket_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = bucket_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = bucket_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
    ...
    files
        .into_iter()
        .map(|mut file| {
            // Strip rules-file YAML frontmatter so it does not leak as raw YAML (matches bucket-build render).
            if file.file_path.contains("/.bucket/rules/")
                || file.file_path.contains("/.claude/rules/")
            {
                file.content = bucket_tools::implementations::skills::skill::extract_skill_body(
                    &file.content,
                );
            }
            file
...
///
/// Delegates to [`bucket_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = bucket_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.bucket/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".bucket").join("config.toml");
    match bucket_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.bucket/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".bucket").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".bucket").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes bucket-build's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = bucket_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: bucket_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.bucket`, so fixtures use test-unique names.
    #[tokio::test]
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".bucket").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"]
                    .as_str()
                    .is_some_and(|p| p.ends_with("/.bucket/rules/xyzzy-discover-agents-md-test.md"))
            })
    ...
    // from `~/.bucket/plugins/`, so tests check for specific plugins.

    #[test]
    fn discover_plugins_finds_manifest_plugin() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".bucket").join("plugins").join("test-plugin");
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".bucket").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),
    ...
    // developer machine with Bucket installed it may return non-Null even
