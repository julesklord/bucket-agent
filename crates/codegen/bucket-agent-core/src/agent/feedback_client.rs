...
const CLIENT_VERSION_HEADER: &str = "x-bucket-client-version";

// ============================================================================
// Turn delta wire types (local to bucket-agent-core until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::bucket_auth_credentials::BucketAuthCredentials,
...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(bucket_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::bucket_auth_credentials::BucketAuthCredentials,
    ) -> Arc<dyn bucket_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(bucket_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, bucket_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-BUCKET-Token-Auth", "bucket-cli")
        } else {
            builder
        }
    }
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = bucket_file_utils::trace_context::inject_trace_context_into_request(request);
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["bucket-3".to_string()],
            primary_model_id: Some("bucket-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("bucket-3".to_string()));
