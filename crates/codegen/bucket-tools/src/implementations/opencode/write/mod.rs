...
impl bucket_tool_runtime::Tool for WriteTool {
    type Args = WriteInput;
    type Output = WriteOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("write").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WriteInput,
    ) -> Result<WriteOutput, bucket_tool_runtime::ToolError> {
            ...
            && !parent.as_os_str().is_empty()
        {
            tokio::fs::create_dir_all(parent).await.map_err(|e| {
                ...
                bucket_tool_runtime::ToolError::execution(
                    bucket_tool_protocol::ToolId::new("write").expect("valid"),
                    ce.to_string(),
                )
            })?;
        }
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = WriteTool;
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "write");
    ...
    async fn missing_filesystem_resource() {
        ...
        let result =
            bucket_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input).await;
