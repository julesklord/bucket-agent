...
use bucket_agent_core::session::{ContextInfo, count_detail};
...
mod tests {
    use super::*;
    use bucket_agent_core::session::TokenUsageCategory;
    ...
    fn build_lines_contains_header_tokens_and_model() {
        let block = ContextInfoBlock::new(snapshot(), "bucket-4");
        ...
        assert_eq!(line_text(&lines, 3), "bucket-4");
    }
    ...
    fn build_lines_includes_compaction_tip_in_warning_band() {
        ...
        snap.usage_pct = 80;
        let block = ContextInfoBlock::new(snap, "bucket-4");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
    ...
    fn bar_and_legend_reconcile_used_with_overhead_excluding_tools() {
        ...
        let block = ContextInfoBlock::new(snap, "bucket-build");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
