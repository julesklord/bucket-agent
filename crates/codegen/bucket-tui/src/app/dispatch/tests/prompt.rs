...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `bucket-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `bucket-build-0.1`
...
/// The always-on pipeline end to end, with `BUCKET_SUGGESTIONS` semantics
