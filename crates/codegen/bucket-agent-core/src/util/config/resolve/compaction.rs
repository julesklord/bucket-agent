...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "BUCKET_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `BUCKET_AUTO_COMPACT_THRESHOLD_PERCENT`
...
///      (populated from `bucket_build_models[i].auto_compact_threshold_percent`;
...
///      (populated from `bucket_build_settings.auto_compact_threshold_percent`)
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "BUCKET_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `BUCKET_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes BUCKET_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
