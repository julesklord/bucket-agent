...
pub(super) struct MemoryFlushSnapshot {
    counts: bucket_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &bucket_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                bucket_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                bucket_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        bucket_telemetry::session_ctx::log_event(
            bucket_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_bucket_req_id: Some(format!("bucket-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: bucket_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_bucket_conv_id: Some(session_id.clone()),
                x_bucket_req_id: Some(format!("bucket-flush-{}", uuid::Uuid::new_v4())),
                x_bucket_session_id: Some(session_id.clone()),
                x_bucket_agent_id: Some(bucket_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: bucket_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        bucket_telemetry::session_ctx::log_event(
            bucket_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => bucket_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => bucket_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => bucket_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => bucket_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::MemoryFlushed {
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            bucket_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    /// LLM call using the `bucket-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("bucket-build".to_owned()),
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    bucket_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                bucket_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = bucket_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = bucket_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                bucket_sampler::collect_response(events).await
            }
        };
