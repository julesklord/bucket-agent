...
/// from `~/.bucket/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        bucket_tools::types::SessionMode::Plan.as_id()
    } else {
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: bucket_telemetry::events::YoloTrigger::Pager,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(bucket_tools::types::SessionMode::Plan.as_id()),
            }]
        }
