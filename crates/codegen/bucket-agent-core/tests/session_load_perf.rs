...
//! `BUCKET_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p bucket-agent-core --test session_load_perf -- --nocapture
//!   cargo test -p bucket-agent-core --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use bucket_agent_core::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use bucket_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("BUCKET_PERF_SCALE", 1).max(1);
        Self {
            turns: g("BUCKET_PERF_TURNS", 80) * scale,
            acu_per_turn: g("BUCKET_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("BUCKET_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("BUCKET_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("BUCKET_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("BUCKET_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("BUCKET_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("BUCKET_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("BUCKET_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `BUCKET_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("BUCKET_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `BUCKET_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("BUCKET_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
async fn full_session_load_e2e() {
    ...
    let server = bucket_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = bucket_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (bucket_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("BUCKET_HOME", bucket_home.path());
        std::env::set_var("BUCKET_INSTRUMENTATION", "log");
        std::env::set_var("BUCKET_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("BUCKET_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("BUCKET_BUCKET_API_BASE_URL", server.url());
        std::env::set_var("BUCKET_API_KEY", "test-key-for-ci");
        std::env::set_var("BUCKET_TELEMETRY_ENABLED", "false");
        std::env::set_var("BUCKET_FEEDBACK_ENABLED", "false");
        std::env::set_var("BUCKET_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(bucket_agent_core::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(bucket_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = bucket_agent_core::instrumentation::finalize();
