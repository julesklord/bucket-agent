...
pub fn is_configured(config: &BucketComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &BucketComConfig) -> Option<ForceLoginTeam> {
...
pub(super) fn build_bucket_auth(
...
mod tests {
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "bucket-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=bucket-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("bucket-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=bucket-desktop"));
        assert!(!url.contains("referrer=bucket-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access bucket-cli:access api:access", "principal_type" : "Team",
