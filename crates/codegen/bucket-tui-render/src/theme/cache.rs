...
//! lookup must be cheaper than re-loading from `~/.bucket/config.toml`.
...
//! `bucket_agent_core::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::BucketNight as u8);
...
/// back to `BucketNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::BucketNight as u8 => ThemeKind::BucketNight,
        x if x == ThemeKind::BucketDay as u8 => ThemeKind::BucketDay,
...
/// When `None`, `to_theme_kind()` defaults to `BucketNight` / `BucketDay`.
...
///
/// On the first call, reads from `~/.bucket/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    bucket_markdown::set_color_level_cap(if locked {
        bucket_markdown::ColorLevel::Basic
    } else {
        bucket_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`BUCKET_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `BucketNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::BucketNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `BucketNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `bucket_agent_core::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = bucket_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = bucket_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::BucketNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::BucketDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::BucketNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::BucketDay);
            assert_eq!(applied, ThemeKind::BucketNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::BucketNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::bucketday().text_primary,
                "must not serve the cached (BucketDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // bucketday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::BucketDay), false),
                ThemeKind::BucketDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::BucketNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_bucketnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::BucketNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_bucketday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::BucketDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_bucketnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_bucketnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::BucketDay), true);
            assert_eq!(result, ThemeKind::BucketDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::BucketDay);
            assert_eq!(current_kind(), ThemeKind::BucketDay);
        });
    }
