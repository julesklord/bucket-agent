...
pub fn summarize_warnings(warnings: &[TerminalWarning]) -> Option<crate::startup::StartupWarning> {
    ...
    let is_ssh = bucket_agent_core::util::clipboard::is_remote_session();
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(bucket_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .bucket/sandbox.toml\nUser config: ~/.bucket/sandbox.toml"
        ),
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && bucket_agent_core::util::clipboard::wayland_data_control_supported(),
        is_wayland && bucket_agent_core::util::clipboard::native_tool_name() == "wl-copy",
    )
}
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart Bucket.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart Bucket.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::BucketDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::BucketDesktop));
    ...
    fn format_themes_env_line_lists_available_below_truecolor() {
        ...
        for level in [ColorLevel::Ansi256, ColorLevel::Basic, ColorLevel::None] {
            ...
            assert!(line.contains("bucketnight") && line.contains("bucketday"));
