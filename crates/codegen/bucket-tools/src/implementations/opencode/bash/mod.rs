...
impl bucket_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(bucket_tool_runtime::ToolError::execution(
                    bucket_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = bucket_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        ...
        let tool = BashTool;
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    //
    // Same shape as bucket_build/bash: the opencode tool inherits the same
