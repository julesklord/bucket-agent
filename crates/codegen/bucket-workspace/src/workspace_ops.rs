...
pub use bucket_workspace_types::rpc::WorkspaceRpc;
pub use bucket_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use bucket_workspace_types::rpc::code_nav::{
...
pub use bucket_workspace_types::rpc::fs::{
...
pub use bucket_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use bucket_workspace_types::rpc::hunks::{
...
pub use bucket_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use bucket_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use bucket_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use bucket_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use bucket_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &bucket_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: bucket_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use bucket_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &bucket_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: bucket_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use bucket_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: bucket_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn session_stats_to_wire(stats: &bucket_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: bucket_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: bucket_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> bucket_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => bucket_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => bucket_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<bucket_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = bucket_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                bucket_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &bucket_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<bucket_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<bucket_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<bucket_codebase_graph::QueryResult, bucket_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<bucket_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](bucket_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: bucket_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<bucket_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](bucket_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(bucket_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = bucket_tool_protocol::ToolId::new(name).map_err(|e| {
                    bucket_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
...
mod tests {
    ...
    /// request types are defined in `bucket-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/bucket-main");
        let window_b = std::path::PathBuf::from("/repos/bucket-main-2");
        assert_eq!(
    ...
    fn hunk_to_wire_serializes_identically() {
        use bucket_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use bucket_hunk_tracker::FileContentEntry;
        use bucket_hunk_tracker::types::FileContentView;
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = bucket_hooks::config::HookSpec {
            ...
            event: bucket_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.bucket/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = bucket_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use bucket_hooks::event::HookEventName as E;
    ...
    /// can't depend on `bucket-hooks`, and `hook_registry_to_wire` only couples
