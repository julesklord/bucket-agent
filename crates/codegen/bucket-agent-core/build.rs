//! Build script for bundling ripgrep for the bucket-shell crate.
//!
//! - If `BUCKET_SHELL_BUNDLE_RG_PATH` is set, always bundle it
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=BUCKET_SHELL_BUNDLE_RG_PATH");
    println!("cargo:rerun-if-env-changed=BUCKET_SHELL_RG_DOWNLOAD_BASE");
    ...
    let path_override = env::var("BUCKET_SHELL_BUNDLE_RG_PATH").ok();
    ...
    // BUCKET_ROOT/target/tmp (which is read-only inside the sandbox). Outside
    // Bazel, prefer BUCKET_ROOT's shared cache dir (monorepo behavior) and fall
    // back to OUT_DIR for standalone checkouts where BUCKET_ROOT is not a thing.
    ...
    } else if let Ok(xai_root) = env::var("BUCKET_ROOT") {
        PathBuf::from(xai_root).join("target/tmp/bucket-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
    ...
    // An explicit BUCKET_SHELL_BUNDLE_RG_PATH still bundles on Windows (the
    ...
    println!("cargo:rustc-env=BUCKET_SHELL_RG_VER={}", RG_VER);
    println!(
        "cargo:rustc-env=BUCKET_SHELL_RG_GEN_DIR={}",
        gen_dir.display()
    );
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=BUCKET_SHELL_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying BUCKET_SHELL_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set BUCKET_SHELL_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=BUCKET_SHELL_RG_TARGET={}", asset_triple);
    ...
    // internal mirror (e.g. BUCKET_SHELL_RG_DOWNLOAD_BASE=http://<mirror>/github/
    // BurntSushi/ripgrep/releases/download). Defaults to the public GitHub
    // releases URL.
    let download_base = env::var("BUCKET_SHELL_RG_DOWNLOAD_BASE")
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet BUCKET_SHELL_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set BUCKET_SHELL_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set BUCKET_SHELL_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
