...
use bucket_sampling_types::truncate_bytes;
...
use bucket_workspace::file_system::CodebaseIndexManager;
use bucket_workspace::permission::{
...
impl bucket_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        bucket_tools::types::resources::ManagedGatewayToolCallResponse,
        bucket_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                bucket_tool_runtime::ToolError::unauthorized(detail)
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            bucket_tool_runtime::ToolError::network_error(format!(
...
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, bucket_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, bucket_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, bucket_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `bucket-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<bucket_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: bucket_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<bucket_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<bucket_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-bucket-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<bucket_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<bucket_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<bucket_agent::Agent>,
    ...
    /// Kill-switch (`BUCKET_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<bucket_tools::implementations::bucket_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, bucket_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: bucket_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<bucket_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<bucket_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<bucket_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: bucket_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `bucket-sampler` actor.
    ///
    ...
    /// (system prompt, [`bucket_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: bucket_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == bucket_tools::implementations::bucket_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<bucket_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `bucket prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &bucket_agent::PromptContext) {
...
) -> Option<bucket_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use bucket_tools::types::output::{MCPOutput, ToolOutput};
    use bucket_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl bucket_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl bucket_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> bucket_tool_protocol::ToolId {
            bucket_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::bucket_tool_runtime::ListToolsContext,
        ) -> bucket_tool_types::ToolDescription {
            bucket_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "BucketBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                assert_eq!(t["namespace"], "bucket_build");
...
mod managed_gateway_tool_tests {
    ...
    use bucket_tools::types::output::{MCPOutput, ToolOutput};
    use bucket_tools::types::tool::{ToolKind, ToolNamespace};
    use bucket_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<bucket_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
