...
/// `bucket-agent-core/src/agent/handlers/session.rs::handle_roster_list`), which
...
mod tests {
    ...
    fn agent_entry() -> bucket_agent_core::agent::roster::RosterEntry {
        use bucket_agent_core::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        assert_eq!(e.model_id.as_deref(), Some("bucket-4"));
        assert!(e.yolo);
