//! Shared API definitions for Bucket tools: protobuf types, config validation,
...
pub mod pb {
    include!(concat!(env!("OUT_DIR"), "/xai.bucket.tools.v1.rs"));
}
...
///
/// Tool ids are colon-separated `Namespace:tool` (e.g. `BucketBuild:grep`); the
...
mod default_client_name_tests {
    ...
    fn pins_first_colon_derivation() {
        assert_eq!(default_client_name("BucketBuild:grep"), "grep");
