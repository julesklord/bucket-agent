...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "bucket-4.5").
    ...
    /// correct today (all current Bucket models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on bucket-4.5) are rejected so we fail in the TUI
    ...
    pub(crate) fn resolve_effort_token_for(
        ...
        // would 400 on the server (e.g. `none` on bucket-4.5).
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("bucket-build"));
        let mut state = ModelState::default();
        state.available.insert(
            id.clone(),
            model_with_effort("bucket-build", "Bucket Build", "high"),
        );
