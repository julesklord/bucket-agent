//! bucket-chat-state — Actor-based chat state management for xAI agents.
//!
//! This crate extracts conversation state management from `bucket-agent-core`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `bucket-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
