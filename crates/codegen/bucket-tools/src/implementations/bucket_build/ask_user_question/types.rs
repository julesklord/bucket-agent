...
//!
//! - **`bucket-tools`** — tool blocks on a oneshot, formats the result.
//! - **`bucket-agent-core`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`bucket-tui`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `bucket-tools`.
...
register_resource!("bucket_build", "UserQuestionSender", UserQuestionSender);
