...
impl WorktreeDb {
    /// Open (or create) the DB at `bucket_home/worktrees.db`.
    pub fn open(bucket_home: &Path) -> Result<Self> {
        Self::open_at(&bucket_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.bucket/worktrees.db`.
    ///
    /// Discovers bucket home via `$BUCKET_HOME`, falling back to the canonicalized
    /// `$HOME/.bucket` (matching `bucket_config::bucket_home`).
...
pub fn resolve_bucket_home() -> Result<PathBuf> {
    ...
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".bucket"))
}

/// Serializes tests that mutate the process-global `BUCKET_HOME` env var so they
...
#[cfg(test)]
static BUCKET_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`BUCKET_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `BUCKET_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `BUCKET_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct BucketHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated bucket home; pass to `WorktreeDb::open` to read the same DB
...
impl BucketHomeFixture {
    pub(crate) fn new() -> Self {
        ...
        // via BUCKET_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until BUCKET_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("BUCKET_HOME");
        unsafe { std::env::set_var("BUCKET_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for BucketHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("BUCKET_HOME", p),
                None => std::env::remove_var("BUCKET_HOME"),
            }
        }
    }
}
