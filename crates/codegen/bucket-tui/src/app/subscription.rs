...
/// `bucket_build_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `BUCKET_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("BUCKET_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: bucket_agent_core::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> bucket_agent_core::auth::GateInfo {
    ...
    fn may_be_free_tier_matrix() {
        ...
        app.subscription_tier = Some("SuperBucket Heavy".into());
        assert!(!app.may_be_free_tier());
    ...
    fn fire_subscription_check_guards_and_debounces() {
        ...
        paid.subscription_tier = Some("SuperBucket".into());
        assert!(
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = bucket_agent_core::auth::GateInfo {
