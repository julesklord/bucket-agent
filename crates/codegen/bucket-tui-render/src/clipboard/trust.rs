...
///
/// - `osc52_sink`: when `bucket wrap` is capturing this process's output (see
...
mod tests {
    ...
    // `bucket wrap` sink: a brand that does NOT natively support OSC 52 (the
    ...
    // container override is deliberately narrow; `bucket wrap` is the SSH path.
    #[test]
