...
//!
//! Each test spawns a real `bucket agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p bucket-agent-core --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.bucket/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".bucket").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let bucket_dir = home.path().join(".bucket");
...
"#,
        ...
        let client = BucketStdioClient::spawn_with_home(&server, workdir.path(), home).await;
