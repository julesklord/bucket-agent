...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: bucket_tools::implementations::bucket_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"bucket-build-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (bucket-build) tool names; guards against accidental wording drift.
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = bucket_file_utils::events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
