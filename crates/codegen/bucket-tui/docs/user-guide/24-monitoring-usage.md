...
> **Status: alpha.** The schema below is versioned (`bucket_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Bucket CLI can export usage **metrics** and **events** to your organization's
...
- **Structurally separate** from SpaceBUCKET-internal telemetry: its exporters carry
...
- **Independent of SpaceXAI data-retention opt-outs**: it works even when
  ...
  settings govern SpaceBUCKET-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export BUCKET_EXTERNAL_OTEL=1                  # master switch
...
`BUCKET_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `BUCKET_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `BUCKET_TELEMETRY_ENABLED`, which controls SpaceBUCKET-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `bucket_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `BUCKET_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`BUCKET_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `bucket setup` managed config /
...
## Metrics (meter scope `ai.xai.bucket_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `bucket_code.session.count` | `{session}` | base attrs only |
| `bucket_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `bucket_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `bucket_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `bucket_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `bucket_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `bucket_code.token.usage` with your own
...
|---|---|
| `bucket_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `bucket_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `bucket_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `bucket_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `bucket_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `bucket_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `bucket_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `bucket_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `bucket_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `bucket_code.permission_mode_changed` | `to_mode`, `trigger` |
| `bucket_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `bucket_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `bucket_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `bucket_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `bucket_code.auth` | `auth_method` |
| `bucket_code.internal_error` | `error_type` (class only — no message, no location) |
| `bucket_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(bucket_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(bucket_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(bucket_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(bucket_code_tool_decision_total[1h]))
```
