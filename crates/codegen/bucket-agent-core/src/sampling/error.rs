...
//!
//! The canonical error types now live in `bucket_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a bucket-shell dependency).

// Re-export everything from the standalone crate.
pub use bucket_sampling_types::error::*;
...
/// This stays in bucket-agent-core because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Bucket subscription")
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (BUCKET_API_KEY). \
                         Your cached OAuth session is being used instead. \
                         To use your API key, run `bucket logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                bucket_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: bucket_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != bucket_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == bucket_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = bucket_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &bucket_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    /// Helper: run a closure with BUCKET_API_KEY temporarily set (or cleared).
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("BUCKET_CODE_BUCKET_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("BUCKET_API_KEY");
            std::env::remove_var("BUCKET_CODE_BUCKET_API_KEY");
            if let Some(k) = key {
                std::env::set_var("BUCKET_API_KEY", k);
            }
        }
        ...
        unsafe {
            ...
            std::env::remove_var("BUCKET_CODE_BUCKET_API_KEY");
            if let Some(v) = prev {
                std::env::set_var("BUCKET_API_KEY", v);
            }
            if let Some(v) = prev_legacy {
                std::env::set_var("BUCKET_CODE_BUCKET_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("bucket-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'bucket-build' requires a Bucket subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
            ...
            assert!(
                ...
                "should suggest bucket logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
