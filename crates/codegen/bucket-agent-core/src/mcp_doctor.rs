//! `bucket mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = bucket_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.bucket/config.toml".to_string(),
...
// ── Managed (bucket.com) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "bucket.com";
...
/// Discover managed `bucket_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `bucket login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use bucket_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `bucket mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `bucket mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `bucket mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `bucket mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
