...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.bucket/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".bucket").join("config.toml");
...
mod tests {
    use super::*;
    use bucket_agent_core::claude_import::PathKind;
    use bucket_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use bucket_agent_core::claude_import::ImportableItem;
