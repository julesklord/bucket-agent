...
pub struct HooksMap {
    ...
    /// Event names present in the JSON but not recognized by Bucket.
...
pub struct RawHandler {
    ...
    /// keys (`BUCKET_HOOK_EVENT`, `BUCKET_HOOK_NAME`, `BUCKET_SESSION_ID`,
    /// `BUCKET_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`BUCKET_HOOK_EVENT`, `BUCKET_HOOK_NAME`, `BUCKET_SESSION_ID`,
    ///    `BUCKET_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`bucket-agent::plugins::hooks_adapter`). The adapter sets
    ///    `BUCKET_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `BUCKET_PLUGIN_DATA`,
    ...
    ///    (`BUCKET_HOOK_EVENT`, `BUCKET_HOOK_NAME`, `BUCKET_SESSION_ID`,
    ///    `BUCKET_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.bucket/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.bucket/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "BUCKET_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "BUCKET_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "BUCKET_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "BUCKET_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "BUCKET_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "BUCKET_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (BUCKET_HOOK_*,
    /// BUCKET_SESSION_ID, BUCKET_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "BUCKET_HOOK_EVENT": "spoofed",
                                    "BUCKET_HOOK_NAME": "spoofed",
                                    "BUCKET_SESSION_ID": "spoofed",
                                    "BUCKET_WORKSPACE_ROOT": "/etc",
