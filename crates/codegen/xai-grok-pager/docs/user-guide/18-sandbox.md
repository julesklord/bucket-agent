...
Create custom sandbox profiles in `~/.bucket/sandbox.toml` (global) or `.grok/sandbox.toml` (per-project):
...
`bucket --continue`, or `bucket -r`), Grok restores that same profile automatically —
