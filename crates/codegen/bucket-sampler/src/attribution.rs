...
//! it" buckets.
//!
//! `bucket-sampler` is intentionally decoupled from `bucket-agent-core`
...
/// tools live in `bucket-tools`
/// (`crates/codegen/bucket-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `bucket_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `bucket_agent_core::auth::token_suffix` (which truncates to 12 chars
...
/// `bucket-agent-core/src/auth/manager.rs` to match, otherwise the
