...
use super::billing::dispatch_open_superbucket_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(bucket_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::StartSessionWithModel(model_id) => dispatch_start_session_with_model(app, model_id),
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, bucket_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(bucket_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(bucket_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
                        ...
                        })
                    {
                        ...
                        if let Ok(content) = std::fs::read_to_string(&provider_file) {
                            if let Ok(toml_val) = content.parse::<toml::Value>() {
                                if let Some(table) =
                                    toml_val.get("providers").and_then(|v| v.as_table())
                                {
                                    for (k, v) in table {
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(bucket_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let bucket_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
