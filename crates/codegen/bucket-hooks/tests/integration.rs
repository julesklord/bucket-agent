//! Integration tests for bucket-hooks.
//!
...
use bucket_hooks::discovery::load_hooks;
use bucket_hooks::dispatcher;
use bucket_hooks::event::*;
use bucket_hooks::result::HookDecision;
use bucket_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        bucket_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$BUCKET_HOOK_EVENT" > {f}; echo "NAME=$BUCKET_HOOK_NAME" >> {f}; echo "SESSION=$BUCKET_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`BUCKET_HOOK_EVENT`, `BUCKET_HOOK_NAME`,
/// `BUCKET_SESSION_ID`, `BUCKET_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$BUCKET_HOOK_EVENT" > {f}; echo "NAME=$BUCKET_HOOK_NAME" >> {f}; echo "SESSION=$BUCKET_SESSION_ID" >> {f}; echo "ROOT=$BUCKET_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "BUCKET_HOOK_EVENT": "spoofed_event",
                                "BUCKET_HOOK_NAME": "spoofed_name",
                                "BUCKET_SESSION_ID": "spoofed_session",
                                "BUCKET_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "BUCKET_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed BUCKET_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "BUCKET_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed BUCKET_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "BUCKET_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed BUCKET_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use bucket_hooks::result::HookRunResult;
    let info = match run {
