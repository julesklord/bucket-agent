...
use bucket_agent_core::extensions::notification::RetryState;
use bucket_agent_core::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> bucket_agent_core::extensions::notification::ImageCompressedEntry {
    bucket_agent_core::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    bucket_acp::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<bucket_acp::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(bucket_acp::AcpArgs {
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use bucket_agent_core::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(bucket_acp::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `BUCKET_HOME` for disk-replay tests. `bucket_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `BUCKET_HOME`
...
/// Runs `f` with a thread-local bucket home override so disk replay tests do not
/// depend on process-wide `bucket_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_bucket_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_bucket_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    bucket_home: &std::path::Path,
    child_sid: &str,
    content: &str,
) {
    let sessions_dir = bucket_home
        .join("sessions")
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(bucket_acp::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = bucket_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: bucket_agent_core::extensions::mcp::McpServerStatus,
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = bucket_agent_core::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "bucket_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "bucket_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("x.ai/mcp/tools_changed", std::sync::Arc::from(raw))
}
