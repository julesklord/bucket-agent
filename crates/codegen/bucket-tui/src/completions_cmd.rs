//! `bucket completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "bucket" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("bucket");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "bucket", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "bucket", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `bucket worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:bucket-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:bucket-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("bucket");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "bucket", &mut buf);
    ...
    // so `bucket worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
