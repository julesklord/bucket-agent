...
use bucket_tui_pty_harness::PtyHarness;
pub(crate) use bucket_tui_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`bucket-tui-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `bucket-tui-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `BUCKET_SCROLL_SPEED`) appended to the controller's env.
