...
/// `bucket_tools::reminders` (free-fn shape because this module has no
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        bucket_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !bucket_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = bucket_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn make_ico_content(width: u32, height: u32) -> ImageContent {
        let png = make_test_png(width, height);
        let buf = bucket_test_utils::image::ico_with_png_frame(&png, width as u8, height as u8);
        ImageContent::new(
    ...
    fn re_encode_fallback_notice_picks_tag_per_harness() {
        ...
        let bucket = render_re_encode_fallback_notice(&notes, false);
        assert!(bucket.contains("<system-reminder>"));
        assert!(bucket.contains("</system-reminder>"));
        assert!(!bucket.contains("<system_reminder>"));
        assert!(bucket.contains("<image_re_encode_fallback>"));
    }
    #[test]
    fn display_format() {
    ...
    fn persisted_image_reject_reason_verdicts() {
        ...
        let ico = bucket_test_utils::image::ico_with_png_frame(&make_test_png(16, 16), 16, 16);
    ...
    fn image_dropped_notice_picks_tag_per_harness() {
        ...
        let bucket = render_image_dropped_notice(&notes, false);
        assert!(bucket.contains("<system-reminder>"));
        assert!(bucket.contains("</system-reminder>"));
        assert!(!bucket.contains("<system_reminder>"));
        assert!(bucket.contains("<image_dropped_notice>"));
        assert!(bucket.contains("Image 5"));
