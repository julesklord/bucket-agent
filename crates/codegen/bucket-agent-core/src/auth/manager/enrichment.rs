...
use crate::auth::model::{BucketAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: BucketAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.bucket_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-BUCKET-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut BucketAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: BucketAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut BucketAuth, user_info: UserInfo) {
