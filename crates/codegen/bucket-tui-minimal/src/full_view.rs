...
use bucket_tui::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = bucket_tui::appearance::cache::load_show_thinking_blocks();
        ...
        bucket_tui::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: bucket_tui::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(bucket_tui::scrollback::block::RenderBlock::system(
