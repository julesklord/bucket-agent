//! Leader-follower IPC architecture for bucket-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.bucket/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use bucket_agent_core::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::BucketBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = bucket_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`bucket -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<BucketBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(BucketBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<BucketBuildEnvironment> {
    let environments: &[BucketBuildEnvironment] = &[BucketBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<BucketBuildEnvironment> {
    let environments: &[BucketBuildEnvironment] = &[BucketBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::bucket_home::bucket_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : bucket_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::BucketComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::BucketComConfig) -> Self {
        Self {
            bucket_ws_url: c.bucket_ws_url.clone(),
            bucket_ws_origin: c.bucket_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`bucket -p` where hanging forever is unacceptable.
...
///     "bucket-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.bucket_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "bucket-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.bucket_ws_url);
...
///
/// For a **managed install** — the running binary lives under `bucket_home`
/// (e.g. `~/.bucket/...`) — prefer the managed `~/.bucket/bin/bucket` symlink. After an
/// auto-update or `bucket update` atomically swaps that symlink, `current_exe()`
...
/// `bucket_updater::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `bucket_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.bucket/bin/bucket` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::bucket_home::bucket_home())
}
fn resolve_binary_with_home(bucket_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(bucket_home, std::env::current_exe().ok())
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = bucket_home.join("bin").join(managed_bucket_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, bucket_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let log_path = crate::util::bucket_home::bucket_home().join("leader.log");
    ...
    let leader_log = std::env::var("BUCKET_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "bucket_agent_core=info,bucket_acp=warn,bucket_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            bucket_ws_url: "wss://test.invalid".into(),
            bucket_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("bucket"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("bucket-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("bucket")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("bucket-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("bucket");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("bucket-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_bucket_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_bucket_bin_name());
