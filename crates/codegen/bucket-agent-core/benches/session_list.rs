...
//!
//! Run: `cargo bench -p bucket-agent-core --bench session_list`
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = bucket_agent_core::util::bucket_home::encode_cwd_dirname(&cwd);
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/bucket-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = bucket_agent_core::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(bucket_agent_core::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:bucket-org/benchmark.git".to_owned()],
