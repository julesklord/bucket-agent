...
//! primitive consumed cross-crate by `bucket-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.bucket/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.bucket`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `bucket-workspace`, which depends on THIS crate) to keep the dep edge
