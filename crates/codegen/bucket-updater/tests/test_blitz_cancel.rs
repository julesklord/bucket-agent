...
//!
//! > `~/.bucket/bin/bucket` resolves to a binary that passes the smoke-test, OR it
...
/// (`bucket` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `bucket` should resolve to after an install attempt.
...
/// to both `bucket` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p bucket-updater --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("BUCKET_BLITZ_ITERS")
        .ok()
