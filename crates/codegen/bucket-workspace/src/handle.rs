...
/// preStop-hook drain marker; override via `BUCKET_WORKSPACE_DRAINING_FILE`.
...
static DRAIN_STARTED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "bucket_workspace_drain_started_total",
...
static DRAIN_COMPLETED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "bucket_workspace_drain_completed_total",
...
static DRAIN_DURATION: std::sync::LazyLock<Histogram> = std::sync::LazyLock::new(|| {
    register_histogram!(
        "bucket_workspace_drain_duration_seconds",
...
static DRAIN_LOST_ITEMS_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "bucket_workspace_drain_lost_items_total",
...
static PRODUCER_SPAWNED_AFTER_DRAIN_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "bucket_workspace_producer_spawned_after_drain_total",
...
static WORKSPACE_BIND_ZERO_TOOLS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_bind_zero_tools_total",
            "session.bind resolutions advertising zero model-facing tools, by reason",
            &["reason"]
        )
        .unwrap()
...
static WORKSPACE_BIND_FAILED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_bind_failed_total",
...
static WORKSPACE_BIND_UNSERVED_TOOLS_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "bucket_workspace_bind_unserved_tools_total",
...
static WORKSPACE_BIND_ADVERTISED_TOOLS: std::sync::LazyLock<Histogram> =
    std::sync::LazyLock::new(|| {
        register_histogram!(
            "bucket_workspace_bind_advertised_tools",
...
pub(crate) static WORKSPACE_TERMINAL_BACKEND_ORPHANED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_terminal_backend_orphaned_total",
...
static ENV_CAPTURE_PANIC_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "bucket_workspace_env_capture_panic_total",
...
use bucket_file_utils::events::types::CancellationCategory;
use bucket_file_utils::events::{Event, SessionRelationship, TurnOutcomeLabel};
use bucket_file_utils::queue::EnqueueOutcome;
use bucket_tool_protocol::turn_hook::{AfterTurnAckPayload, AfterTurnAckStatus};
...
pub(crate) static REWIND_CHECKPOINT_CAPTURE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rewind_checkpoint_capture_total",
...
pub(crate) static REWIND_CHECKPOINT_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rewind_checkpoint_finalize_total",
...
pub(crate) static REWIND_RESTORE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rewind_restore_total",
...
pub(crate) static REWIND_CHECKPOINT_DURATION: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "bucket_workspace_rewind_checkpoint_duration_seconds",
...
pub(crate) static REWIND_NON_COMPLETED_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rewind_non_completed_finalize_total",
...
impl WorkspaceHandle {
    ...
    pub async fn trace_donation_reporter(
        ...
        bucket_hub_sdk::HubDonatingReporter,
        bucket_hub_sdk::TraceDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    /// teardown.
    ///
    /// [`LogDonationSender`]: bucket_hub_sdk::LogDonationSender
    pub async fn log_donation_layer(
        &self,
        service_name: &str,
    ) -> Option<(
        bucket_hub_sdk::LogDonationSender,
        bucket_hub_sdk::LogDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    pub async fn metric_donation_reporter(
        &self,
        service_name: &str,
    ) -> Option<bucket_hub_sdk::MetricDonationPump> {
        self.shared
            .hub_handle
            .lock()
            .await
    ...
    /// Construct a handle with an explicit `$BUCKET_WORKSPACE_HOME` and a
    /// pre-spawned [`UploadQueue`](bucket_file_utils::queue::UploadQueue).
    ///
    ...
    pub(crate) fn new_with_data_collection(
        ...
        upload_queue: Arc<bucket_file_utils::queue::UploadQueue>,
    ...
    fn build(
        config: WorkspaceConfig,
        workspace_home: std::path::PathBuf,
        upload_queue: Option<Arc<bucket_file_utils::queue::UploadQueue>>,
        ...
        let local_registry = bucket_hub_sdk::LocalRegistry::new();
        ...
        let (hook_registry, hook_load_errors) = {
            use bucket_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn bucket_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                bucket_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                bucket_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use bucket_tools::implementations::lsp::{
                ...
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    ...
                    true,
                    bucket_tools::notification::ToolNotificationHandle::noop(),
                )));
        ...
        let session_event_writers: Arc<
            dashmap::DashMap<String, bucket_file_utils::events::EventWriter>,
    ...
    /// The [`ToolServer`](bucket_hub_sdk::ToolServer) for this
    ...
    pub fn hub_server(&self) -> Option<bucket_hub_sdk::ToolServer> {
    ...
    pub async fn hub_server_blocking(&self) -> Option<bucket_hub_sdk::ToolServer> {
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<bucket_tools::registry::types::ToolServerConfig>,
        capability: CapabilityMode,
        viewer_ctx: Option<bucket_tool_runtime::WorkspaceViewerContext>,
    ...
    pub fn create_session_with_tracker_and_viewer_ctx(
        ...
        let system_notify_channel = system_notifications
            .then(bucket_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: bucket_tools::registry::types::ToolServerConfig,
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<bucket_tools::registry::types::ToolServerConfig>,
    ...
    pub async fn on_before_turn(
        &self,
        session_id: &str,
        payload: &bucket_tool_protocol::turn_hook::BeforeTurnPayload,
    ) {
    ...
    pub async fn compute_turn_injections(
        &self,
        session_id: &str,
        request: &bucket_tool_protocol::turn_hook::TurnHookRequest,
    ) -> bucket_tool_protocol::turn_hook::HookReply {
        use bucket_tool_protocol::turn_hook::{HookReply, TurnHookRequest};
        match request {
    ...
    /// `BUCKET_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `BUCKET_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn cancel_tool_call(&self, session_id: &str, call_id: &str) {
        self.shared.activity_tracker.tool_call_completed(
            call_id,
            Some(session_id),
            bucket_file_utils::events::ToolOutcome::Cancelled,
        );
    ...
    pub fn hook_registry(&self) -> bucket_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[bucket_hooks::error::HookError] {
    ...
    pub fn get_or_create_codebase_index(
        &self,
        cwd: std::path::PathBuf,
    ) -> (Arc<bucket_codebase_graph::IndexManagerHandle>, bool) {
        self.shared.codebase_indexes.lock().get_or_create(cwd)
    }
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<Arc<bucket_codebase_graph::IndexManagerHandle>> {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(bucket_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(bucket_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(bucket_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    async fn emit_environment_artifact(
    ...
    ) -> Option<bucket_file_utils::queue::EnqueueOutcome> {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(bucket_tracing::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        let outcome = upload_queue
            ...
            .await;
        match &outcome {
            bucket_file_utils::queue::EnqueueOutcome::Failed { reason: _ } => {
                dc_log!(
    ...
    pub async fn start_session_mcp_servers(
        ...
        use bucket_hub_mcp_adapter::McpBridge;
        use bucket_hub_sdk::ToolServerHandler as _;
        use bucket_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        ...
        > = tokio::task::spawn_blocking(move || {
            ...
            rt_handle.block_on(bucket_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                bucket_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
        ...
        for result in mcp_results {
            match result {
                Ok(client) => {
                    ...
                    let transport: Arc<dyn bucket_hub_mcp_adapter::McpTransport> =
                        ...
                        Ok(handle) => {
                            for handler in handle.bridge.handlers() {
                                    ...
                                    .await
                                {
                                    ...
                                    bucket_tool_protocol::ToolId::new(&qualified_name)
                                {
    ...
    pub async fn teardown_session_mcp(&self, session_id: &str) {
        ...
        let sid = match bucket_tool_protocol::SessionId::new(session_id) {
            Ok(s) => s,
            Err(_) => return,
        };
        let mut tool_ids = session.mcp_tool_ids.lock().await;
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<bucket_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<bucket_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
    ...
    pub(crate) fn session_bind_resolver(
        &self,
        catalog: Arc<Vec<Arc<dyn bucket_hub_sdk::ToolServerHandler>>>,
        rpc_tool_id: bucket_tool_protocol::ToolId,
    ) -> bucket_hub_sdk::SessionHandlerResolver {
        let weak_shared = Arc::downgrade(&self.shared);
        Arc::new(
            move |sid: bucket_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(bucket_tracing::local_or_random_span_ctx);
                ...
                async move {
                    let Some(shared) = weak_shared.upgrade() else {
                        WORKSPACE_BIND_FAILED_TOTAL
                            ...
                            .inc();
                        return Err(
                            bucket_tool_runtime::ToolError::service_unavailable(
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || bucket_tools::registry::types::ToolServerConfig {
                        tools: vec![],
                    ...
                    Ok(bucket_hub_sdk::ResolvedSessionHandlers {
                        handlers,
                        unserved_tool_ids,
                        resolve_error,
                    })
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let catalog: Arc<Vec<Arc<dyn bucket_hub_sdk::ToolServerHandler>>> =
        ...
        let (activity_notify_handle, activity_notify_rx) =
            bucket_tools::notification::types::ToolNotificationHandle::channel();
        ...
        let listener_task = tokio::spawn(async move {
            while let Some(notification) = notification_rx.recv().await {
                match notification {
                    bucket_hub_sdk::HubNotification::ToolsChanged {
                        added,
                        removed,
                        updated,
                        ..
            ...
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        ...
                        let frame = bucket_tool_protocol::ToolNotificationFrame::custom(
                            bucket_tool_protocol::ToolId::new(
        ...
        let status_publisher_task = tokio::spawn(async move {
            ...
            async fn send_status(
                conn: &bucket_hub_sdk::HubConnection,
                ...
                let req = bucket_tool_protocol::JsonRpcRequest {
                    jsonrpc: bucket_tool_protocol::JsonRpcVersion,
                    id: bucket_tool_protocol::JsonRpcId::from_request_id(&request_id),
                    session_id: None,
                    method: bucket_tool_protocol::Method::ToolServerStatus
                        .as_wire_str()
                        .to_owned(),
                    params,
                };
...
fn build_session_routed_handlers(
    toolset: &bucket_tools::registry::types::FinalizedToolset,
    ws: &WorkspaceHandle,
) -> Vec<Arc<dyn bucket_hub_sdk::ToolServerHandler>> {
    ...
    for def in toolset.tool_definitions() {
        ...
        let mut desc = bucket_tool_types::ToolDescription::new(
        ...
        match crate::hub::SessionRoutedToolHandler::new(
            ...
            Ok(handler) => {
                handlers.push(Arc::new(handler) as Arc<dyn bucket_hub_sdk::ToolServerHandler>)
            }
            Err(e) => {
                tracing::warn!(
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &bucket_tools::notification::types::ToolNotification,
) {
    use bucket_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        bucket_tools::notification::types::ToolNotification,
    >,
) {
...
impl DrainReason {
    /// Stable `reason` label for `bucket_workspace_drain_started_total`.
...
impl DrainOutcome {
    /// Stable `outcome` label for `bucket_workspace_drain_completed_total`.
    pub fn as_str(self) -> &'static str {
        match self {
...
/// The SIGTERM drain budget from `BUCKET_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("BUCKET_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`BUCKET_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("BUCKET_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
/// sandbox; override via `BUCKET_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    cwd: std::path::PathBuf,
    hub_url: url::Url,
    auth: bucket_hub_sdk::SharedAuthProvider,
    ...
    let api_base_url = std::env::var("BUCKET_CLI_CHAT_PROXY_BASE_URL")
        .unwrap_or_else(|_| "https://cli-chat-proxy.bucket.com/v1".to_string());
    let data_collection_disabled =
        std::env::var("BUCKET_WORKSPACE_DATA_COLLECTION_DISABLED").as_deref() != Ok("false");
    ...
    let tool_config = bucket_agent::workspace_bucket_build_toolset();
    ...
    if let Ok(dir) = std::env::var("BUCKET_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("BUCKET_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("BUCKET_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
    ...
    let trace_source: Arc<dyn bucket_file_utils::queue::TraceExportSource> = Arc::new(
        crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()),
    );
    let upload_queue = Arc::new(bucket_file_utils::queue::UploadQueue::spawn(
        &workspace_home,
        trace_source,
        bucket_file_utils::queue::UploadRetryPolicy::default(),
    ));
    ...
    upload_queue.cleanup_orphans(bucket_file_utils::queue::DEFAULT_MAX_AGE);
...
/// Resolve `$BUCKET_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$BUCKET_WORKSPACE_HOME` (operator override).
/// 2. `<bucket_home>/workspace`, where `<bucket_home>` honours `$BUCKET_HOME` and
///    otherwise falls back to `~/.bucket` (see [`bucket_config::bucket_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("BUCKET_WORKSPACE_HOME")
        && !p.trim().is_empty()
    {
        return std::path::PathBuf::from(p);
    }
    bucket_config::bucket_home().join("workspace")
}
...
/// (`BUCKET_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
/// keeps the legacy behaviour: [`WorkspaceShared::session_event_writer`] hands
/// back [`EventWriter::noop()`](bucket_file_utils::events::EventWriter::noop)
/// and no `events.jsonl` is ever opened.
fn events_enabled() -> bool {
    std::env::var("BUCKET_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `BUCKET_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("BUCKET_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`BUCKET_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("BUCKET_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
async fn enqueue_workspace_tool_definitions(
    upload_queue: &bucket_file_utils::queue::UploadQueue,
    session_id: &str,
    object_path: &str,
    bytes: &[u8],
) -> bucket_file_utils::queue::EnqueueOutcome {
    use bucket_file_utils::queue::EnqueueOutcome;
    let outcome = upload_queue
        .enqueue_bytes_blocking(
            bytes,
...
/// backing upload queue (tests, local mode). Never the real bucket home —
/// only [`connect_local_workspace`] resolves `$BUCKET_WORKSPACE_HOME` — so the
/// queue-less default path can never collide with a real workspace's state dir.
fn ephemeral_workspace_home() -> std::path::PathBuf {
    std::env::temp_dir().join(format!("bucket-workspace-ephemeral-{}", std::process::id()))
}
/// Resolve `workspace_rewind_all_outcomes` from `BUCKET_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    bucket_config::env_bool("BUCKET_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
struct SessionToolHandle {
    tool_id: bucket_tool_protocol::ToolId,
    desc: bucket_tool_types::ToolDescription,
...
impl SessionToolHandle {
    fn new(
        ...
        workspace: WorkspaceHandle,
        session_id: String,
    ) -> Result<Self, bucket_tool_protocol::IdError> {
        Ok(Self {
            tool_id: bucket_tool_protocol::ToolId::new(tool_name)?,
            desc,
            workspace,
            session_id,
        })
...
impl bucket_tool_runtime::ToolDyn for SessionToolHandle {
    fn id(&self) -> bucket_tool_protocol::ToolId {
        self.tool_id.clone()
    }
    fn description(
        &self,
        _ctx: &::bucket_tool_runtime::ListToolsContext,
    ) -> bucket_tool_types::ToolDescription {
        self.desc.clone()
    }
    async fn execute(
        &self,
        ...
        use bucket_tool_runtime::{ToolError, ToolErrorKind, ToolStreamItem, terminal_only};
...
impl WorkspaceHandle {
    ...
    pub fn create_local_harness(
        &self,
        session_id: &str,
    ) -> WorkspaceResult<bucket_hub_sdk::ToolHarness> {
        let session = self
            .session(session_id)
            .ok_or_else(|| WorkspaceError::SessionNotFound(session_id.to_string()))?;
        let toolset = session.toolset();
        let registry = bucket_hub_sdk::LocalRegistry::new();
        for def in toolset.tool_definitions() {
            let tool_name = def.function.name.clone();
            let desc = bucket_tool_types::ToolDescription::new(
                tool_name.clone(),
                def.function.description.clone().unwrap_or_default(),
            );
            match SessionToolHandle::new(tool_name, desc, self.clone(), session_id.to_string()) {
                Ok(tool) => {
                    registry.register_dyn(Arc::new(tool) as Arc<dyn bucket_tool_runtime::ToolDyn>);
                }
                Err(e) => {
                    tracing::warn!(
                        tool = % def.function.name, error = % e,
        ...
        let session_id = bucket_tool_protocol::SessionId::new(session_id.to_string())
            .map_err(|e| WorkspaceError::HubError(format!("invalid session id: {e}")))?;
        Ok(bucket_hub_sdk::ToolHarness::local_only_with(
            registry,
            session_id,
            bucket_tool_runtime::TypedExtensions::default(),
        ))
    }
}
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: bucket_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    fn make_handle_inner(
        ...
        let config = WorkspaceConfig {
            ...
            plugin_discovery_config: Default::default(),
            hub_config: None,
            auth_provider: Some(Arc::new(bucket_hub_sdk::AuthCredential::bearer(
                "test-token",
            ))),
            server_metadata: None,
            status_config,
    ...
    impl bucket_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> bucket_tools::types::tool::ToolNamespace {
            bucket_tools::types::tool::ToolNamespace::MCP
        }
        fn description_template(&self) -> &str {
            "bash cco stub"
        }
    }
    impl bucket_tool_runtime::Tool for BashCcoStub {
        type Args = serde_json::Value;
        type Output = bucket_tools::types::output::ToolOutput;
        fn id(&self) -> bucket_tool_protocol::ToolId {
            bucket_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::bucket_tool_runtime::ListToolsContext,
        ) -> bucket_tool_types::ToolDescription {
            bucket_tool_types::ToolDescription::new(BASH_CCO_STUB_NAME, "bash cco stub")
        }
        async fn run(
            &self,
            _ctx: bucket_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<bucket_tools::types::output::ToolOutput, bucket_tool_runtime::ToolError>
        {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(bucket_tools::types::output::ToolOutput::Bash(
                bucket_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        bucket_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    pub(crate) fn assert_bash_cco_terminal(typed: &bucket_tool_runtime::TypedToolOutput) {
        use bucket_tool_runtime::ToolOutput as _;
        let resp = typed
    ...
    async fn local_harness_preserves_bash_chat_completion_output() {
        use bucket_tool_runtime::ToolCallContext;
        let handle = make_handle();
        register_bash_cco_stub(&handle);
        let harness = handle.create_local_harness("main").expect("local harness");
        let tool_id = bucket_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id");
        let stream = harness
    ...
    async fn donation_entry_points_are_inert_without_a_hub() {
        ...
        assert!(
            handle
                .trace_donation_reporter("prod_bucket_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .log_donation_layer("prod_bucket_workspace")
                .await
                .is_none(),
        ...
        assert!(
            handle
                .metric_donation_reporter("prod_bucket_workspace")
                .await
                .is_none(),
    ...
    async fn build_session_routed_handlers_skips_invalid_client_name_without_panic() {
        let handle = make_handle();
        let mut renamed = tc("BucketBuild:read_file", Some(ToolKind::Read));
        ...
        let session = handle
            .create_session_with_config(
                ...
                Some(ToolServerConfig {
                    tools: vec![renamed, tc("BucketBuild:grep", Some(ToolKind::Read))],
                    behavior_preset: None,
                }),
                CapabilityMode::All,
                None,
    ...
    /// ABSENT from that (bucket-build) catalog is still advertised. The old
    /// `catalog ∩ session-names` filter silently dropped exactly such tools
    /// (bucket-build renames → 6/11).
    #[tokio::test]
    ...
    async fn create_fingerprint_write_does_not_clobber_concurrent_rebind() {
        ...
        let fp_a = serde_json::to_value(&ToolServerConfig {
            tools: vec![tc("BucketBuild:list_dir", Some(ToolKind::ListDir))],
            behavior_preset: None,
        })
        .ok();
    ...
    async fn toolset_terminal(
        toolset: &Arc<bucket_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn bucket_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<bucket_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    pub(crate) fn background_capable_cfg() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                tc("BucketBuild:read_file", Some(ToolKind::Read)),
                tc("BucketBuild:run_terminal_cmd", Some(ToolKind::Execute)),
                tc(
                    "BucketBuild:get_task_output",
                    Some(ToolKind::BackgroundTaskAction),
                ),
                tc("BucketBuild:kill_task", Some(ToolKind::KillTaskAction)),
            ],
            behavior_preset: None,
        }
    }
    ...
    ///
    /// [`TerminalRunRequest`]: bucket_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> bucket_tools::computer::types::TerminalRunRequest {
        bucket_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: bucket_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: bucket_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> bucket_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn bucket_tools::computer::types::TerminalBackend>,
        ) -> bucket_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                bucket_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> bucket_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let bucket_tools::types::output::ToolOutput::TaskOutput(
            bucket_tool_types::TaskOutputOutput::TaskNotFound(msg),
        ) = &result.output
        else {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        ...
        handle.shared().session_event_writer(sid).emit(
            bucket_file_utils::events::Event::McpToolCallStarted {
        ...
        assert_eq!(ts["model_id"], "bucket-4");
    ...
    async fn before_turn_hooks_sync_session_yolo_mode() {
        use bucket_tool_protocol::turn_hook::{BeforeTurnPayload, TurnHookRequest};
        let handle = make_handle();
        ...
        assert_eq!(
            reply,
            bucket_tool_protocol::turn_hook::HookReply::default(),
    ...
    async fn before_turn_yolo_transition_emits_yolo_toggled_event() {
        use bucket_tool_protocol::turn_hook::BeforeTurnPayload;
        let (handle, home) = make_handle_with_events();
        let sid = "sess-yolo";
        let _session = handle
    ...
    fn turn_outcome_label_maps_every_variant() {
        use bucket_file_utils::events::TurnOutcomeLabel;
        use bucket_tool_protocol::turn_hook::TurnHookOutcome;
        assert!(matches!(
    ...
    impl bucket_file_utils::queue::TraceExportSource for UnreachableSource {
        fn resolve(&self) -> bucket_file_utils::TraceExportConfig {
            bucket_file_utils::TraceExportConfig {
                bucket_url: None,
                service_account_key: None,
                upload_method: bucket_file_utils::UploadMethod::Proxy {
    ...
    fn spawn_test_queue(home: &std::path::Path) -> Arc<bucket_file_utils::queue::UploadQueue> {
        let policy = bucket_file_utils::queue::UploadRetryPolicy {
            initial_delay: std::time::Duration::from_secs(3600),
            ..Default::default()
        };
        Arc::new(bucket_file_utils::queue::UploadQueue::spawn(
            home,
    ...
    /// `$BUCKET_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $BUCKET_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("BUCKET_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("BUCKET_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    fn make_queue_backed_handle_with(
        ...
        let source: Arc<dyn bucket_file_utils::queue::TraceExportSource> =
            Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy));
        let policy = bucket_file_utils::queue::UploadRetryPolicy {
            max_attempts: 1,
            ..Default::default()
        };
        let queue = Arc::new(bucket_file_utils::queue::UploadQueue::spawn(
            home.path(),
            source,
            policy,
        ));
    ...
    async fn environment_artifact_enqueued_when_queue_present() {
        ...
        assert!(
            matches!(
                outcome,
                Some(bucket_file_utils::queue::EnqueueOutcome::Enqueued)
            ),
    ...
    async fn fork_session_inherits_parent_tool_config_when_none() {
        ...
        let new_parent_baseline = ToolServerConfig {
            tools: vec![tc("BucketBuild:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
    ...
    async fn fork_session_uses_explicit_tool_config_when_provided() {
        ...
        let custom = ToolServerConfig {
            tools: vec![
                tc("BucketBuild:read_file", Some(ToolKind::Read)),
                tc("BucketBuild:list_dir", Some(ToolKind::ListDir)),
            ],
            behavior_preset: None,
        };
        let child = handle
    ...
    async fn fork_session_uses_main_session_when_parent_session_id_is_none() {
        ...
        let baseline_ids: Vec<String> = child
            ...
            .map(|t| t.id.clone())
            .collect();
        assert_eq!(baseline_ids, vec!["BucketBuild:read_file".to_string()]);
    }
    #[tokio::test]
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("bucket-workspace-test-cwd-override");
    ...
    async fn on_mcp_snapshot_changed_emits_per_session_events_and_rebuilds() {
        ...
        handle
            ...
            .expect("subB ok");
        let mut rx = handle.shared.events.subscribe();
        let mcp_tool = tc("BucketBuild:read_file", Some(ToolKind::Read));
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = bucket_hooks::config::HookSpec {
                name: "injected".into(),
                event: bucket_hooks::event::HookEventName::SessionStart,
    ...
    async fn fork_session_inherits_viewer_ctx_from_parent() {
        ...
        let parent = handle
            .create_session_with_tracker_and_viewer_ctx(
                ...
                bucket_hunk_tracker::HunkTrackerHandle::noop(),
                None,
                CapabilityMode::All,
                Some(bucket_tool_runtime::WorkspaceViewerContext {
    ...
    fn handler_names(resolved: &bucket_hub_sdk::ResolvedSessionHandlers) -> Vec<String> {
        resolved
            .handlers
            .iter()
            .map(|h| h.tool_id().as_str().to_owned())
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        let resolved = resolver(
            bucket_tool_protocol::SessionId::new("bind-e2e-strict").unwrap(),
            Some(serde_json::json!(
                { "metadata" : { "preset" : "bucket-computer", "capability_mode" :
                "all" }, }
            )),
        )
        .await
        ...
        assert!(
            reason.contains(bucket_version::VERSION),
    ...
    async fn strict_rpc_only_bind_fails_closed_with_resolve_error_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            bucket_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap(),
            Some(serde_json::json!(
    ...
    async fn strict_bind_with_explicit_toolset_serves_it_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            bucket_tool_protocol::SessionId::new("bind-e2e-tools").unwrap(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "BucketBuild:read_file" }] },
                }
            )),
        )
        .await
    ...
    async fn lax_bind_without_metadata_uses_default_catalog_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            bucket_tool_protocol::SessionId::new("bind-e2e-lax").unwrap(),
            None,
        )
        .await
        .expect("bind must succeed");
    ...
    async fn rejected_rebind_config_keeps_resolve_error_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-rejected").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "BucketBuild:read_file" }] },
    ...
    async fn explicit_empty_toolset_rebind_never_swaps_session_tools() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "BucketBuild:read_file" }] },
    ...
    async fn strict_rebind_with_corrected_toolset_heals_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-heal").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({ "metadata" : { "preset" : "bucket-computer" } })),
        )
        .await
    ...
    fn owner_full_bind_metadata() -> serde_json::Value {
        serde_json::json!(
            ...
            "BucketBuild:read_file" }, { "id" : "BucketBuild:search_replace" }, { "id" :
            "BucketBuild:grep" }, { "id" : "BucketBuild:list_dir" },], }, }
        )
    }
    ...
    async fn owner_toolset_survives_concurrent_consumer_shaped_rebinds() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-consumer-storm").unwrap();
    ...
    async fn restored_server_first_bind_ordering_decides_capability_and_toolset() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-restore-read-first").unwrap();
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-restore-write-first").unwrap();
        resolver(
            sid.clone(),
            Some(serde_json::json!(
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-restore-owner-first").unwrap();
    ...
    async fn bind_flow_rebinds_keep_backend_and_task_alive_end_to_end() {
        ...
        let handle = make_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = bucket_tool_protocol::SessionId::new("bind-e2e-bg").unwrap();
        let bg_metadata = serde_json::json!(
            { "metadata" : { "tools" : [{ "id" : "BucketBuild:read_file" }, { "id" :
            "BucketBuild:run_terminal_cmd" }, { "id" : "BucketBuild:get_task_output" }, {
            "id" : "BucketBuild:kill_task" },] }, }
        );
    ...
    async fn compute_turn_injections_after_returns_skipped_ack_without_queue() {
        use bucket_tool_protocol::turn_hook::{AfterTurnPayload, TurnHookOutcome, TurnHookRequest};
        let handle = make_handle();
    ...
    async fn compute_turn_injections_before_runs_turn_start_and_replies_noop() {
        use bucket_tool_protocol::turn_hook::{BeforeTurnPayload, HookReply, TurnHookRequest};
        let handle = make_handle();
        let reply = handle
            .compute_turn_injections(
                "main",
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.bucket`.
    #[tokio::test]
    ...
    fn make_handle_with_queue_routing(
        ...
        WorkspaceHandle,
        Arc<bucket_file_utils::queue::UploadQueue>,
        tempfile::TempDir,
    ) {
    ...
    async fn wait_enqueued(queue: &bucket_file_utils::queue::UploadQueue, want: u64) {
    ...
    async fn enqueue_workspace_tool_definitions_reports_enqueued_at_session_root() {
        ...
        assert_eq!(outcome, bucket_file_utils::queue::EnqueueOutcome::Enqueued);
        assert_eq!(
            queue
                .stats()
                .enqueued
    ...
    async fn two_phase_drain_no_queue_marks_draining_and_returns_zero() {
        ...
        assert_eq!(
            snap.status,
            bucket_tool_protocol::ToolServerLifecycleStatus::Draining
        );
        assert!(
