...
/// Extract user-guide docs to `<bucket_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(bucket_home: &std::path::Path) {
    let docs_dir = bucket_home.join("docs").join("user-guide");
