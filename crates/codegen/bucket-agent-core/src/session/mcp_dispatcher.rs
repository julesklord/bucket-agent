...
//!
//! Receives [`bucket_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`bucket_mcp::liveness`]),
//! - the [`bucket_mcp::servers::BucketClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`bucket_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `bucket_com_` prefix)
    /// or `local` (user `.bucket/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`bucket_mcp::servers::ClientStateKind::Ready`]
...
/// [`bucket_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit bucket.com"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && bucket_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`bucket_acp::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "bucket_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `bucket_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("bucket_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use bucket_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("bucket_com_slack"), // managed
    ...
    fn discard_gateway() -> bucket_acp::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        bucket_acp::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = bucket_acp::AcpAgentGatewaySender::new(gw_tx);
