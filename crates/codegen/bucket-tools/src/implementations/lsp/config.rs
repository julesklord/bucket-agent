//! LSP server configuration from `.bucket/lsp.json`.
...
/// This is the canonical merge function — both session startup and `bucket inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::bucket_home::bucket_home().join("lsp.json");
    let project_path = cwd.join(".bucket").join("lsp.json");
...
/// Load LSP server configs from `~/.bucket/lsp.json` and `<cwd>/.bucket/lsp.json`.
