...
///
/// Reads `stable_version` from `~/.bucket/version.json` (written by the
...
///
/// This is a lightweight duplicate of `bucket_updater::channel_name()` for
/// use in `bucket-agent-core` which cannot depend on `bucket-updater`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::bucket_home::bucket_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(bucket_version::VERSION).ok()?;
