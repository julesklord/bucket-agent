...
//! release:
//!
//! 1. TUI startup: `check_update_background` spawns a detached `bucket update`
//!    (the Ctrl+U path now adopts this child instead of spawning a second).
//! 2. Explicit `bucket update` (incl. the Ctrl+U fallback when there is no
//!    live child).
...
//!   temp-name fix, every `0.1.x` download shared one `bucket-0.1.tmp`
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("bucket");
    assert!(link.is_symlink(), "bucket must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active bucket symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("bucket-{version}-{platform}"),
        "active bucket must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active bucket must pass the smoke-test");
}

/// Lay down a managed-install layout in the test BUCKET_HOME:
/// `bin/bucket -> ../downloads/bucket-<version>-<platform>` (what
...
fn fake_managed_install(version: &str) {
    ...
    let name = format!("bucket-{version}-{}", host_platform());
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `bucket update` (the Ctrl+U fallback path) finds the
...
// installers that actually maintain the managed `~/.bucket/bin/bucket` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.bucket/downloads cleanup), the probe must report None — otherwise
    ...
    std::fs::remove_file(
        home.join("downloads")
            .join(format!("bucket-0.2.7-{platform}")),
    )
    .unwrap();
...
// shared one `bucket-0.1.tmp`, so a concurrent racer could atomically rename a
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    // Pre-fix, BOTH of these wrote to downloads/bucket-0.1.tmp concurrently
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("bucket-{version}-{platform}"));
        assert_eq!(
