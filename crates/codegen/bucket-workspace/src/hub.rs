...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<bucket_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(bucket_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: bucket_file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: bucket_file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: bucket_file_utils::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<bucket_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "BUCKET_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
        ...
        Box::pin(async_stream::stream! {
            ...
            .set_outcome(bucket_file_utils::events::ToolOutcome::Success); yield
            ...
            .set_outcome(bucket_file_utils::events::ToolOutcome::Error); yield
...
fn parse_server_id(id: &str) -> Result<bucket_tool_protocol::ServerId, ClientError> {
    bucket_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![bucket_tool_runtime::ContentBlock::Text {
    ...
    use bucket_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    impl bucket_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> bucket_tool_protocol::ToolId {
            bucket_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::bucket_tool_runtime::ListToolsContext,
        ) -> bucket_tool_types::ToolDescription {
            bucket_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use bucket_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("BucketBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&bucket_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> bucket_tool_protocol::ToolServerStatusPayload {
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use bucket_tools::computer::types::{TaskKind, TaskSnapshot};
