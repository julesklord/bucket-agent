...
/// environment (`BUCKET_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `BUCKET_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "bucket_workspace_upload_outcome_total",
        ...
        assert!(has(
            "bucket_workspace_toolset_swap_rejected_total",
            &[("reason", "turn_active"), ("trigger", "update_tool_config")]
        ));
        assert!(has(
            "bucket_workspace_orphan_lost_total",
            &[("reason", "sha_mismatch")]
        ));
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "bucket_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "bucket_workspace_permission_timeout_total")
        );
    }
}
