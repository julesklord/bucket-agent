...
//!
//! 1. [`bucket_telemetry::unified_log::warn`] for the local
//!    `~/.bucket/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`bucket_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`bucket_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`bucket_sampler::SamplerConfig::attribution_callback`] at every
...
impl ShellAttribution {
    ...
    /// [`bucket_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `bucket_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn bucket_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `bucket_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `bucket-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `bucket-agent-core` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.bucket/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `BucketAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `BucketAuth` (or
    ...
    // `bucket_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.bucket/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> BucketAuth {
        BucketAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `bucket_tools::Auth401AttributionCallback`
