...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::BucketPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / BucketWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use bucket_workspace::permission::types::{PermissionConfig, PromptPolicy};
