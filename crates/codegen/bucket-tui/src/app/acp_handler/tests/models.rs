#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn models_update_preserves_active_agent_model() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_3 = acp::ModelId::new(std::sync::Arc::from("bucket-3"));
        agent
            .session
            .models
            .available
            .insert(id_3.clone(), make_model_info("bucket-3"));
        agent.session.models.current = Some(id_3);

        let notif = make_models_update_notif("bucket-4", &["bucket-3", "bucket-4"]);
    ...
    fn models_update_uses_shell_default_when_agent_model_removed() {
        ...
        // bucket-3 removed from catalog.
        let notif = make_models_update_notif("bucket-4.3", &["bucket-4.3", "bucket-4.5"]);
        handle_models_update(&notif, &mut app);
    ...
    fn models_update_noop_when_agent_matches_shell_default() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_4 = acp::ModelId::new(std::sync::Arc::from("bucket-4"));
        agent
            .session
            .models
            .available
            .insert(id_4.clone(), make_model_info("bucket-4"));
        ...
        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("bucket-4"),
            "app.models.current must be bucket-4 when agent and shell agree"
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
            agent
                ...
                .map(|id| id.0.as_ref()),
            Some("bucket-4"),
            "agent model must remain bucket-4"
        );
    }
    ...
    fn models_update_non_active_agent_uses_shell_fallback_not_active_model() {
...

        {
            ...
            let id_5 = acp::ModelId::new(std::sync::Arc::from("bucket-4.5"));
            agent_b
                .session
                .models
                .available
                .insert(id_5.clone(), make_model_info("bucket-4.5"));
            agent_b.session.models.current = Some(id_5);
        }

        // bucket-5 removed from catalog.
        let notif = make_models_update_notif("bucket-4", &["bucket-3", "bucket-4"]);
        handle_models_update(&notif, &mut app);
        ...
        // B's bucket-5 was removed — must fall back to shell's bucket-4, not A's bucket-3.
    ...
    fn model_changed_dropped_when_model_unknown_to_catalog() {
        ...
        let notif = model_changed_ext("sess-1", "bucket-99-unknown", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(
            !changed,
    ...
    fn model_changed_applies_reasoning_effort_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "bucket-4", Some("high"));
    ...
    fn model_changed_dropped_for_unknown_session_id() {
        ...
        let notif = model_changed_ext("sess-OTHER", "bucket-4", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(!changed);
