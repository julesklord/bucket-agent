...
This crate extracts a first-class `Agent` type from `bucket-agent-core`.
...
`bucket-agent-core`, another in-process host, or a headless batch runner.
...
in `.bucket/agents/` (project-level) or `~/.bucket/agents/` (user-level).

```rust
use bucket_agent::{AgentDefinition, AgentBuilder};
use bucket_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".bucket/agents/code-reviewer.md")?;
...
```rust
use bucket_agent::discovery;

// Find all .md files in .bucket/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  bucket-agent-core  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`bucket-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `bucket-agent` depends on it for tool setup.
- **`bucket-agent-core`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `bucket-agent` (AGENTS.md
...
|---|---|---|
| `bucket-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p bucket-agent

# Test
cargo test -p bucket-agent

# Clippy
cargo clippy -p bucket-agent --fix --allow-dirty
