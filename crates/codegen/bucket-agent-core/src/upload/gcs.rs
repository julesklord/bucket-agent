...
//! `StorageClient` constructed inside `bucket_file_utils::gcs::*` helpers.
//!
...
//! to an `bucket_file_utils::gcs::*` helper.
...
///
/// ```ignore
/// bucket_file_utils::gcs::upload_bytes(
...
/// `BUCKET_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("BUCKET_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match bucket_file_utils::gcs::upload_bytes(
