...
/// after we stop waiting. Mirrors the bound the `bucket-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("BUCKET_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for bucket_acp::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
