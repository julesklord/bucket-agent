...
//! (the envelope types are canonical in `bucket_workspace_types::rpc`).
//!
//! `error_code` uses a non-wildcard match so the compiler enforces
//! coverage of new `WorkspaceError` variants.

pub use bucket_workspace_types::rpc::{RpcEnvelope, RpcError};
...
pub fn error_code(err: &WorkspaceError) -> &'static str {
    match err {
        ...
        WorkspaceError::TurnActive(_) => bucket_workspace_types::rpc::envelope::TURN_ACTIVE,
...
mod tests {
    ...
    fn error_code_round_trip_all_variants() {
        ...
        variants.extend(
            bucket_workspace_types::rpc::deploy::DeployError::ALL
                .into_iter()
