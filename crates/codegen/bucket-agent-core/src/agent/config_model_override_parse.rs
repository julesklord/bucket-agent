...
pub(crate) fn log_model_override_warnings(warnings: &[ModelOverrideWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "model_override: parsed with warnings; run `bucket inspect` for details"
        );
    }
}
...
mod tests {
    ...
    fn duplicate_compactions_keys_keeps_model() {
        ...
        let model = cfg
            .config_models
            .get("bucket-4.5")
            .expect("bucket-4.5 must remain in catalog");
        assert_eq!(
    ...
    fn legacy_alias_alone_parses_without_warning() {
        ...
        let model = cfg.config_models.get("bucket-4.5").unwrap();
        assert_eq!(
            model.compactions_remaining,
    ...
    fn invalid_reasoning_effort_skips_field_keeps_model() {
        ...
        let model = cfg
            ...
            .get("bucket-4.5")
            .expect("bucket-4.5 must remain in catalog");
        assert_eq!(model.model.as_deref(), Some("bucket-4.5"));
    ...
    fn unknown_field_warns_but_keeps_known_fields() {
        ...
        let entry = models.get("bucket-4.5").unwrap();
        assert_eq!(entry.model.as_deref(), Some("bucket-4.5"));
        assert_eq!(
        ...
        assert_eq!(
            warnings,
            vec![ModelOverrideWarning {
                model_key: Some("bucket-4.5".to_owned()),
    ...
    fn non_table_model_section_warns_and_is_ignored() {
        let (models, warnings) = parse_raw(r#"model = "bucket-4""#);
