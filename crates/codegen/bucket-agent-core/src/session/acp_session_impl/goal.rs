...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &bucket_tools::implementations::bucket_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<bucket_tools::implementations::bucket_build::update_goal::UpdateGoalEnvelope>,
    ) {
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run bucket-build flavor. Non-strict names (bucket-build
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if bucket_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default bucket-build
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default bucket-build
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use bucket_sampling_types::conversation::SyntheticReason;
