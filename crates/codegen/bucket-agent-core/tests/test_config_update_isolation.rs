...
/// All tests share this as BUCKET_HOME (the `OnceLock` in bucket-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("BUCKET_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = bucket_agent_core::config::load_effective_config().unwrap();
    let effective_cfg = bucket_agent_core::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    bucket_agent_core::util::config::update_config(|cfg| {
    ...
    let user_cfg = bucket_agent_core::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("bucket-3"));
}
