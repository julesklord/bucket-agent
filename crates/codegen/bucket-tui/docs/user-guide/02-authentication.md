...
Authenticate developers through your own Identity Provider (Okta, Azure AD, Auth0, etc.) instead of bucket.com.
...
export BUCKET_AUTH_TOKEN_TTL=3600
```

### Token refresh

When Bucket needs to refresh an expired token, it re-runs your binary with `BUCKET_AUTH_EXPIRED=1` set:
...
3. **`BUCKET_API_KEY`** — fallback when no session token is active.

When more than one login flow is configured:

1. External auth provider (`auth_provider_command`)
2. Enterprise OIDC
3. xAI/bucket.com OAuth2 browser login
...
- **Login screen at startup** — configure a local model or set `BUCKET_API_KEY`; see [Custom Models](11-custom-models.md).
