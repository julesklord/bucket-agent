...
/// - `file_path` — bucket_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), bucket_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — bucket_build (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `bucket_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<bucket_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    bucket_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = bucket_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(bucket_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from bucket-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, bucket-shell would sanitize
