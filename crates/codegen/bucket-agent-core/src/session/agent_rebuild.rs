...
//! [`bucket_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`bucket_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`bucket_agent::Agent`] owns an [`bucket_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`bucket_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use bucket_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use bucket_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use bucket_tools::implementations::bucket_build::ask_user_question::types::UserQuestionRequest;
...
use bucket_tools::implementations::lsp::LspBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<bucket_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<bucket_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use bucket_tools::implementations::bucket_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(bucket_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(bucket_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use bucket_tools::implementations::bucket_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            bucket_tools::computer::local::LocalTerminalBackend::new_local(
                bucket_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: bucket_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(bucket_tools::types::tool::ToolKind::Task)
            .expect("BucketBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("BucketBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_bucket_build())
                    .await
