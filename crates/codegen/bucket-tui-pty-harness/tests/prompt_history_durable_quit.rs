...
//!
//! ```bash
//! cargo test -p bucket-tui-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use bucket_tui_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.bucket/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".bucket").join("sessions");
