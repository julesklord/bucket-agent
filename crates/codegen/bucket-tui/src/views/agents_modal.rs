...
/// User-level vs project-level config files (`~/.bucket` vs `{cwd}/.bucket`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `BUCKET_AGENT`,
    /// model `agentType`, then `bucket-build`).
...
/// Skips internal variants (BucketBuildConcise, BucketBuildPlan,
/// BucketBuildPlanNoSubagents, BucketBuildAskUser, Codex, Opencode,
/// CursorExtended, BucketBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = bucket_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.bucket/personas` and `{cwd}/.bucket/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = bucket_home.join("bundled").join("personas");
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match bucket_agent_core::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => bucket_config::bucket_home().join("personas"),
...
/// True when `path` is a deletable local persona file (user or project `.bucket/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.bucket/{subdir}` or `{cwd}/.bucket/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let bucket_home = bucket_config::bucket_home();
    let in_user = dunce::canonicalize(bucket_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".bucket").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    bucket_agent_core::config::load_effective_config()
        .ok()
        .and_then(|root| bucket_agent_core::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in bucket-agent-core.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    bucket_agent_core::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        bucket_agent::config::PromptMode::Extend => "extend",
        bucket_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use bucket_agent_core::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".bucket").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".bucket").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.bucket/bundled/personas/b.toml".into()),
            scope_label: None,
        };
