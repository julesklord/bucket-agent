...
fn voice_final_appends_to_dashboard_dispatch() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        bucket_voice::VoiceEvent::UtteranceFinal {
            text: "the build".into(),
        },
    );
    assert_eq!(
...
fn voice_dashboard_dispatch_submit_tears_down_voice() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(bucket_voice::VoiceCommand::PttRelease)
    ));
...
fn voice_target_bound_at_start_dispatch_vs_peek() {
    if !bucket_voice::AUDIO_SUPPORTED {
        return;
    }
    use crate::views::dashboard::DashboardRowId;
    use crate::views::dashboard::peek::{PeekFields, PeekPanelState};
...
/// `bucket dashboard` before login: the startup hook consumes the
/// `BUCKET_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `bucket dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "bucket-4.5", "Bucket 4.5");
    open_dashboard(&mut app);
    app.cwd_has_git_ancestor = true;
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(bucket_agent_core::sampling::types::ReasoningEffort::High),
            display: "Bucket 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(bucket_agent_core::sampling::types::ReasoningEffort::High)
        )),
        "effort must be stashed for the shell",
    );
    assert_eq!(
        agent.deferred_session_mode,
        Some(bucket_tools::types::SessionMode::Plan),
    );
...
/// `bucket dashboard`, and the startup hook.
#[serial_test::serial(BUCKET_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    seed_model(&mut app, "bucket-4.5", "Bucket 4.5");
    open_dashboard(&mut app);
    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/model bucket-4.5".into());
    assert!(
        effects.is_empty(),
    ...
    assert_eq!(pending.id.0.as_ref(), "bucket-4.5");
    assert_eq!(pending.display, "Bucket 4.5");
...
fn dashboard_slash_restricted_command_upsells_via_toast() {
    ...
    assert!(
        toast.contains("/imagine") && toast.contains("SuperBucket"),
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("bucket-4.5")),
            effort: None,
            display: "Bucket 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_bucket_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("BUCKET_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
...
fn dashboard_peek_reply_with_image_sends_blocks() {
    ...
    let reply_text = {
        ...
        let img = crate::prompt_images::PastedImage {
            element_id: bucket_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
            mime_type: "image/png".into(),
