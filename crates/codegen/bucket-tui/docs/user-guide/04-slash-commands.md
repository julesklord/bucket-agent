...
- **Shell builtins** -- handled by the agent backend (bucket-agent-core)
- **Pager builtins** -- handled by the TUI frontend (bucket-tui)
...
When the context window fills up, Bucket auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `bucket` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `BUCKET_MEMORY=1`. `/remember` is always available.
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, Bucket prompts you.
...
Set, manage, or check an autonomous goal. Bucket works toward the objective across turns and reports progress.
