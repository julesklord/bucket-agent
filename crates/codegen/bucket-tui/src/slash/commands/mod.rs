...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        let id_fast = acp::ModelId::new(Arc::from("bucket-4.5"));
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "Bucket 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("bucket-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "Bucket 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "Bucket 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "bucket-4.5");
            }
    ...
    fn model_resolves_by_model_id() {
        ...
        let mut ctx = make_ctx(&models);
        let cmd = model::ModelCommand;
        let result = cmd.run(&mut ctx, "bucket-4.3");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "bucket-4.3");
            }
            other => panic!("expected Action(SetDefaultModel), got {other:?}"),
        }
    }
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("Bucket 4.5") && i.insert_text == "Bucket 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "Bucket 4.3" && i.insert_text == "Bucket 4.3")
        );
    }
    #[test]
    ...
    fn usage_manage_returns_open_url() {
        match run_usage("manage") {
            CommandResult::Action(Action::OpenUrl(url)) => {
                assert_eq!(url, "https://bucket.com/?_s=usage");
            }
