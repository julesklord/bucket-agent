...
fn voice_on_welcome_creates_session_and_records() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(bucket_voice::VoiceCommand::PttPress)
    ));
}
#[test]
...
fn worktree_session_created_sets_session_and_cwd() {
    ...
    let id = AgentId(0);
    let worktree_path = PathBuf::from("/tmp/bucket-worktrees/pager-123");
...
fn worktree_session_preserves_subdirectory_offset() {
    let mut app = test_app();
    app.cwd = PathBuf::from("/repo/crates/codegen/bucket-tui");
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.bucket/worktrees/repo/pager-123");
    let session_cwd = worktree_root.join("crates/codegen/bucket-tui");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
fn worktree_session_created_drains_queued_prompts() {
    ...
    assert_eq!(app.agents[&id].session.queue_len(), 1);
    let worktree_path = PathBuf::from("/tmp/bucket-worktrees/pager-abc");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
            session_id: acp::SessionId::new("wt-drain-1"),
            worktree_path,
            session_cwd: PathBuf::from("/tmp/bucket-worktrees/pager-abc"),
            models: None,
        }),
        &mut app,
    );
...
/// trust. BUCKET_HOME-isolated so the write hits a temp store, not the real one.
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(bucket_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    app.yolo_policy_block = Some(POLICY_WARNING);
    {
        ...
        agent.deferred_session_mode = Some(bucket_tools::types::SessionMode::Plan);
    }
