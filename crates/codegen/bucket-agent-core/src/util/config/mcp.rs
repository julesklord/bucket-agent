...
use bucket_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use bucket_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `bucket-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use bucket_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use bucket_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use bucket_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.bucket/config.toml` at each level.
/// Project-scoped `.bucket/config.toml` entries override global `~/.bucket/config.toml`
...
/// `.bucket/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.bucket/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.bucket/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.bucket/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .bucket/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.bucket/config.toml`.
...
/// Upsert an MCP server entry in `~/.bucket/config.toml`.
///
...
/// config file, e.g. a project-scoped `.bucket/config.toml`.
...
/// Delete an MCP server entry from `~/.bucket/config.toml`.
///
...
/// config file, e.g. a project-scoped `.bucket/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = bucket_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.bucket/config.toml` files.
...
// `.mcp.json` discovery moved to `bucket-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use bucket_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.bucket/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::bucket_home::bucket_home().join("config.toml")
}

/// Path to the user-level config file (`~/.bucket/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.bucket/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".bucket").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.bucket/skills", "~/.bucket/skills/special/SKILL.md"]
ignore = ["~/.bucket/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.bucket/skills", "~/.bucket/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.bucket/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${BUCKET_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
