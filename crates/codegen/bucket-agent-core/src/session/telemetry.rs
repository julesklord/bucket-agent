use bucket_telemetry::events::SessionHarness;
use bucket_workspace::permission::Decision;
...
pub(crate) fn skill_source_label(skill_path: &str, cwd: &str) -> &'static str {
    ...
    let user = crate::util::bucket_home::bucket_home()
        .parent()
...
pub(crate) fn format_hook_name(spec: &bucket_hooks::config::HookSpec) -> String {
...
/// project dirs contain `/.bucket/`.)
fn format_hook_source(spec: &bucket_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &bucket_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: bucket_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: bucket_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: bucket_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        SessionHarness {
            ...
            is_git_repo: bucket_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
