...
//! inside a cloned repository (`.mcp.json`, project `.bucket/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.bucket/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`bucket_workspace::trust::TrustStore`]
//! reads/writes — lives in `bucket-workspace` (client-side); this module keeps
...
//! on [`bucket_workspace::folder_trust::decide`]; the consume-side nuance is
...
// `bucket_agent_core::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use bucket_workspace::folder_trust::grant_folder_trust;
...
// NOTE: this folder-trust store (`~/.bucket/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`bucket_agent::plugins::TrustStore` at `~/.bucket/trusted-plugins`, plus the
...
/// [`bucket_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = bucket_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`bucket_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.bucket/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    bucket_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: bucket_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != bucket_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `bucket mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `bucket --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `bucket mcp doctor`). It MUST
...
///
/// Sources: project `.bucket/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.bucket/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.bucket/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            bucket_tools::implementations::lsp::config::LspServerConfig,
            bucket_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, bucket_tools::implementations::lsp::config::LspServerConfig>
{
    bucket_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use bucket_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(bucket_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; BUCKET_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because BUCKET_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. BUCKET_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because BUCKET_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. BUCKET_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because BUCKET_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; BUCKET_HOME-isolated store; BUCKET_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".bucket").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. BUCKET_HOME-isolated so the trust store is empty;
        // BUCKET_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = bucket_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. BUCKET_HOME-isolated so the trust store is empty;
        // BUCKET_FOLDER_TRUST unset so the default-on feature flag applies.
        use bucket_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). BUCKET_HOME-isolated so the trust store is empty.
        use bucket_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. BUCKET_HOME-isolated (empty store).
        use bucket_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".bucket").join("agents");
        ...
        .unwrap();
        let def = bucket_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.bucket/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). BUCKET_HOME-isolated (empty
        // store); BUCKET_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".bucket").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + BUCKET_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. BUCKET_HOME-isolated so the seeded store is the temp one;
        // BUCKET_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // BUCKET_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(bucket_version::TEST_VERSION_ENV);
        if option_env!("BUCKET_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.bucket/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. BUCKET_HOME-isolated (empty store);
        // BUCKET_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".bucket").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // BUCKET_HOME-isolated (empty store); BUCKET_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".bucket").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // BUCKET_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); BUCKET_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use bucket_agent::plugins::discovery::DiscoveryConfig;
        use bucket_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".bucket").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // BUCKET_HOME-isolated so the grant writes to a temp store; BUCKET_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".bucket").join("hooks");
        ...
        let has_project_hook = |reg: &bucket_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = bucket_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &bucket_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.bucket/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.bucket/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use bucket_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use bucket_tools::types::config_source::ConfigSource;

        // A `<cwd>/.bucket/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.bucket/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.bucket/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let bucket = tmp.path().join(".bucket");
        std::fs::create_dir_all(&bucket).unwrap();
        std::fs::write(bucket.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.bucket/config.toml [mcp_servers]`).
    ...
    /// `.bucket/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `bucket --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(bucket_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("BUCKET_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no BUCKET_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(bucket_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(bucket_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("BUCKET_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // BUCKET_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("BUCKET_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".bucket").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".bucket").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with BUCKET_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. BUCKET_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(bucket_version::TEST_VERSION_ENV);
        if option_env!("BUCKET_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. BUCKET_HOME-isolated so the store starts
        // empty; `#[serial]` because BUCKET_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // BUCKET_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(bucket_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; BUCKET_HOME-isolated and
        // BUCKET_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".bucket").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".bucket").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.bucket/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
