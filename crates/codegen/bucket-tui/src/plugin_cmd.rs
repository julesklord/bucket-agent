//! `bucket plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `bucket_agent_core::plugin` (shared orchestration)
//! and lower crates (`bucket-agent`, `bucket-plugin-marketplace`). This
...
/// Typed entry for `bucket plugin list --json`. The `status` field acts as a
...
/// Typed entry for `bucket plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `bucket plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  bucket plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `bucket plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &bucket_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = bucket_agent_core::config::load_effective_config()
        .ok()
    ...
    for source in &sources {
        ...
        for plugin in bucket_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                bucket_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &bucket_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = bucket_plugin_marketplace::git::default_cache_root();
            bucket_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> bucket_telemetry::events::InstallKind {
    if is_git {
        bucket_telemetry::events::InstallKind::Git
    } else {
        bucket_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: bucket_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    ...
    if !trust {
        use bucket_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &bucket_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `bucket plugin update {}` to update it.",
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               bucket plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `bucket plugin list` to see installed plugins."
        );
    }
    if let Err(e) = bucket_agent_core::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    bucket_agent_core::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = bucket_agent_core::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    bucket_agent_core::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. Bucket discovers skills, agents, and hooks \
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `bucket plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `bucket plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use bucket_agent_core::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        bucket_agent_core::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &bucket_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match bucket_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  bucket plugin install sentry@bucket-org/plugin-marketplace --trust"
        ));
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = bucket_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = bucket_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
