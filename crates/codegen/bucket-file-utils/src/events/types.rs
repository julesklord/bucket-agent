...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `bucket-agent-core::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `bucket-agent-core::session::events`.
    ...
    /// in `bucket-agent-core::session::events`.
    ...
    /// discriminator constants in `bucket-agent-core::session::events`.
    ...
    /// `bucket-agent-core::session::events`.
    ...
    /// `bucket-agent-core::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `bucket-agent-core::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `bucket-agent-core`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `bucket-agent-core/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `bucket-agent-core`'s `GoalPauseReason`. The two types
