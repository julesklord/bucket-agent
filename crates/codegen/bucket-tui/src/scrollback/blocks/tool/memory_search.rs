...
fn shorten_path(path: &str) -> &str {
    let memory_root = bucket_config::bucket_home().join("memory");
...
**File:** /root/.bucket/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.bucket/memory/MEMORY.md");
...
**File:** /root/.bucket/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.bucket/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured bucket memory root keep one trailing segment group.
        let memory_root = bucket_config::bucket_home().join("memory");
        let session = memory_root.join("bucket-50aa78f0/sessions/2026-05-01.md");
