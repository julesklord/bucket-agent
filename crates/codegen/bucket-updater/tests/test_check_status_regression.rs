...
//! exact JSON shape produced by `bucket update --check --json` for the failure
//! modes that real users have hit in the wild.
//!
//! Seen when a user is behind a corporate npm registry mirror:
//!
//! ```text
//! # Mirror returns 403 for the @bucket-official scope
...
async fn check_status_surfaces_npm_403_in_error_field() {
    ...
    g.set_stderr(
        "npm error code E403\n\
         npm error 403 403 Forbidden - GET https://registry-mirror.example.invalid/api/npm/js-virtual/@bucket-official%2fbucket\n\
