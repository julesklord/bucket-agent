...
//! points that can still be reached directly (`bucket login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::BucketAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<BucketAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<BucketAuth> {
    anyhow::bail!(UNAVAILABLE)
}
