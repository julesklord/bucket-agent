...
pub(crate) struct RoleToolNames {
    ...
    /// (the default bucket-build host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default bucket-build host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default bucket-build host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_bucket_build_names() {
        // A bucket-build toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default bucket-build host (the stock `web_search`/`web_fetch`).
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_bucket_build_host() {
        // Default bucket-build host: the pre-spawn describe probe exposes only
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default bucket-build parent bridge: no `Write`, only `Edit`
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_bucket_build_mutator() {
        ...
        // default bucket-build host (Edit-only toolset), so a fail-open retry
