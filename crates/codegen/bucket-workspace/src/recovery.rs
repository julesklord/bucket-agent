...
static ORPHAN_RECOVERED: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "bucket_workspace_orphan_recovered_total",
...
static ORPHAN_EXPIRED: LazyLock<IntCounter> = LazyLock::new(|| {
    register_int_counter!(
        "bucket_workspace_orphan_expired_total",
...
pub async fn run_startup_recovery(workspace_home: &Path, queue: &UploadQueue) -> RecoveryReport {
    ...
    for sidecar_path in sidecars {
        ...
        let actual_sha = bucket_file_utils::sha256_hex(&bytes);
...
mod tests {
    ...
    use bucket_file_utils::queue::{TraceExportSource, UploadRetryPolicy, sidecar_path_for};
    ...
    fn write_orphan_pair(
        ...
        let sidecar = QueueItemSidecar {
            ...
            enqueued_at,
            sha256: bucket_file_utils::sha256_hex(content),
        };
