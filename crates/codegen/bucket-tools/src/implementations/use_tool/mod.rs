...
crate::register_resource!("bucket_build", "UseTool", UseToolParams);
...
fn is_local_tool_id_rejection(err: &bucket_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == bucket_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != bucket_tool_runtime::ToolErrorKind::NotFound
...
impl bucket_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    impl bucket_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
    ...
    #[async_trait::async_trait]
    impl bucket_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: bucket_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return bucket_tool_runtime::terminal_only(Err(
                    bucket_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (bucket_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl bucket_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: bucket_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            bucket_tool_runtime::terminal_only(Err(bucket_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl bucket_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: bucket_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: bucket_tool_runtime::ToolCallContext,
        ) -> bucket_tool_runtime::ToolStream<bucket_tool_runtime::TypedToolOutput> {
    ...
    #[async_trait::async_trait]
    impl bucket_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: bucket_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = bucket_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> bucket_tool_runtime::ToolCallContext {
        let call_id = bucket_tool_protocol::ToolCallId::new_v7();
        bucket_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl bucket_tool_runtime::ToolDispatch + 'static,
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (bucket_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
