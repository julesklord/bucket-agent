...
pub(crate) const ENV_CRASH_HANDLER: &str = "BUCKET_CRASH_HANDLER";
...
/// Precedence: requirements > env (`BUCKET_CRASH_HANDLER`) >
...
/// Merge system-managed policy (`/etc/bucket`) under home `managed_config.toml`
...
fn load_managed_toml_layers() -> Option<TomlValue> {
    ...
    match (system, managed) {
        ...
        (None, Some(m)) => Some(m),
        (Some(mut s), Some(m)) => {
            bucket_config::deep_merge_toml(&mut s, &m);
            Some(s)
        }
    }
}
...
mod crash_handler_gate_tests {
    ...
    // `BUCKET_CRASH_HANDLER` is process-global; serialize and force it unset at
