...
/// in `config.toml`. When `None`, defaults to `BucketNight` / `BucketDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::BucketDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::BucketNight),
    }
}
