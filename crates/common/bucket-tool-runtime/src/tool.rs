...
/// `filename`, `path`, `metadata`) that the Bucket SLOP converter uses to
...
pub enum ContentBlock {
    ...
    ///
    /// Optional metadata fields are used by the Bucket SLOP converter to
    ...
    /// - `path`: file path on the Bucket Computer filesystem
