...
// rmcp is quarantined in bucket-mcp; see that crate's docs.
use bucket_mcp::rmcp;
...
/// `bucket_mcp::wire`), these methods are private to the agent↔client channel and
...
pub async fn notify_servers_updated(
    gateway: &bucket_acp::AcpAgentGatewaySender,
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<bucket_tools::bridge::ToolBridge>,
    event_writer: Option<&bucket_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(bucket_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = bucket_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from bucket-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `bucket-mcp` directly.
...
impl bucket_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<bucket_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(bucket_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<bucket_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(bucket_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(bucket_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(bucket_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "bucket_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Bucket CLI".to_string()),
                    },
                    source_label: None,
                    session: None,
                },
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Bucket CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("bucket_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["bucket_com_slack", "bucket_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["bucket_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "bucket_com_slack");
    ...
    fn test_disabled_session_state_serialization() {
        let entry = McpServerEntry {
            name: "bucket_com_slack".to_string(),
            display_name: None,
            source: McpServerSource::Managed,
            source_label: None,
            config: McpServerConfig::Http {
