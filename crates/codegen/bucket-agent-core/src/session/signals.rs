...
pub use bucket_telemetry::enums::PrCreationSource;
...
pub struct SessionSignals {
    ...
    // === Bash tool patterns (bucket_build) ===
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &bucket_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: bucket_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    bucket_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("bucket-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("bucket-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["bucket-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("bucket-4");
        handle.record_model_usage("bucket-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"bucket-3".to_string()));
        assert!(snapshot.models_used.contains(&"bucket-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["bucket-3".to_string(), "bucket-4".to_string()],
        );
        ...
        handle.record_model_usage("bucket-3"); // existing model
        handle.record_model_usage("bucket-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // bucket-3 not duplicated, bucket-5 added
        assert!(snapshot.models_used.contains(&"bucket-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("bucket-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("bucket-4");
        ...
        assert!(restored.models_used.contains(&"bucket-3".to_string()));
        assert!(restored.models_used.contains(&"bucket-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for bucket-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the bucket-critique bug describes: the
        ...
        handle2.record_model_usage("bucket-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // bucket-3 not duplicated
