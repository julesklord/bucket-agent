...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: bucket_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn bucket_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<bucket_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<bucket_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(bucket_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// [`bucket_tool_runtime::WorkspaceBindMetadata`] (same type the emitter
    /// serializes); `tool_config` is a consumer-only raw escape hatch read
    /// separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: bucket_tool_runtime::WorkspaceBindMetadata =
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = bucket_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match bucket_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = bucket_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "bucket-computer", "bogus"] {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        let v = serde_json::json!(
            { "preset" : "explore", "tools" : [{ "id" : "BucketBuild:grep", "params_json" :
        ...
        assert_eq!(grep.id, "BucketBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "BucketBuild:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.bucket/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.bucket/hooks/`).
    ...
    /// `<cwd>/.bucket/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `BUCKET_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `BUCKET_SESSION_ID`; empty is normalized to
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: bucket_hub_sdk::SharedAuthProvider,
...
/// `*.json` hook files. Maps 1:1 to [`bucket_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.bucket/hooks/`).
    Directory(PathBuf),
}
