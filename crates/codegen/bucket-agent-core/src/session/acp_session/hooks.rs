...
//! both matched by the agent ([`bucket_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = bucket_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    bucket_telemetry::session_ctx::log_event(
                        bucket_telemetry::events::ClientHookGate {
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    bucket_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
