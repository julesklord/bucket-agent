...
// Re-export the shared BucketIntegerSchema from types module
pub use crate::types::BucketIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("bucket_build", "Grep", GrepParams);
...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(bucket_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<bucket_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| bucket_tool_protocol::ToolCapabilities {
        is_read_only: true,
        tool_scope: Some(bucket_tool_protocol::ToolScope::Read),
...
impl bucket_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> bucket_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return bucket_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: bucket_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, bucket_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: bucket_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static bucket_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> bucket_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield bucket_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
                yield bucket_tool_runtime::ToolStreamItem::Terminal(Err(e));
                return;
            }
        };
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield bucket_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield bucket_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &bucket_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, bucket_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a bucket_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a bucket_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<bucket_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<bucket_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<bucket_tool_runtime::ToolProgress> {
        ...
        bucket_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    async fn tool_grep_no_matches() {
        ...
        let tool = GrepTool;
        let output = bucket_tool_runtime::Tool::run(
            &tool,
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                bucket_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &bucket_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            bucket_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, bucket_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                bucket_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                bucket_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
