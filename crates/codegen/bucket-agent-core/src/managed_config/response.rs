...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that BUCKET_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `bucket login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.bucket. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<bucket_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        &self,
    ) -> Option<bucket_config::signed_policy::SignatureEnvelope> {
        self.signature_sidecar_with(bucket_config::signed_policy::embedded_key_id_trusted)
    }
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: bucket_config::signed_policy::SignatureEnvelope,
    pub(super) payload: bucket_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use bucket_config::signed_policy;
...
mod tests {
    ...
    fn signature_sidecar_picks_trusted_envelope_then_falls_back() {
        use bucket_config::signed_policy::SignatureEnvelope;
