...
use crate::implementations::bucket_build::search_replace::CONTEXT_LINES;
use crate::implementations::bucket_build::search_replace::helpers::{
...
impl bucket_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: EditInput,
    ) -> Result<SearchReplaceOutput, bucket_tool_runtime::ToolError> {
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), bucket_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            bucket_tool_runtime::ToolError::execution(
                bucket_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `bucket_build::search_replace::helpers` — shared across
// both the bucket_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "edit");
