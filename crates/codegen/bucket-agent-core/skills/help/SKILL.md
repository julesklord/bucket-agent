...
name: help
description: >
  Bucket documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any Bucket feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "Bucket docs — config, MCP, auth, skills, commands"
---

# Bucket Help

Answer the user's question about Bucket setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.bucket/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.bucket/docs/user-guide/`. The available guides are:
   ...
   `~/.bucket/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.bucket/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.bucket/skills/<name>/SKILL.md`
   (read `~/.bucket/docs/user-guide/08-skills.md` for the SKILL.md format).