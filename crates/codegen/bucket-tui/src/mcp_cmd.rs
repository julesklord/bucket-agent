//! `bucket mcp` — manage MCP server configurations from the command line.
...
Examples:
  ...
  bucket mcp add xcode -- xcrun mcpbridge
  ...
  # Add to the project config (./.bucket/config.toml) instead of ~/.bucket/config.toml
  bucket mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.bucket/config.toml`, available in all your projects
    User,
    /// `./.bucket/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of bucket.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.bucket/config.toml) or project (./.bucket/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = bucket_agent_core::util::config::load_mcp_server_configs_with_project(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `bucket mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    bucket_agent_core::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: bucket mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: bucket mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => bucket_agent_core::util::config::user_config_path(),
        McpScope::Project => {
            bucket_agent_core::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_bucket_path("config.toml"),
...
fn select_remove_site(
    ...
    use bucket_agent_core::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    bucket_agent_core::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        bucket_agent_core::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .bucket/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = bucket_agent_core::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        bucket_agent_core::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["bucket", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["bucket", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["bucket", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "bucket", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "bucket", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a bucket scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "bucket", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        ...
        let add = parse_add(&["bucket", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["bucket", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["bucket", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["bucket", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["bucket", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = bucket_agent_core::util::config::user_config_path();
        let project = PathBuf::from("/repo/.bucket/config.toml");
