...
mod tests {
    ...
    fn unset_params_is_ok_none() {
        assert_eq!(parse_params_json(0, "BucketBuild:grep", None), Ok(None));
    }
    ...
    fn empty_string_is_a_parse_error() {
        let err = parse_params_json(3, "BucketBuild:grep", Some("")).unwrap_err();
    ...
    fn name_override_unset_or_valid_is_ok() {
        assert_eq!(validate_name_override(0, "BucketBuild:grep", None), Ok(()));
        for name in ["search", "BucketBuild:grep", "a-b_C9"] {
            assert_eq!(
                validate_name_override(0, "BucketBuild:grep", Some(name)),
                Ok(()),
                "name={name:?}"
            );
        }
    ...
    fn name_override_outside_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "dot.name"] {
            let err = validate_name_override(2, "BucketBuild:grep", Some(name)).unwrap_err();
    ...
    fn empty_entries_returns_none() {
        assert_eq!(
            first_unknown_tool_id(&[], &allowed(&["BucketBuild:grep"])),
            None
        );
    }
    ...
    fn first_unknown_id_is_returned_with_index() {
        let entries = [
            entry("BucketBuild:grep"),
            entry("BucketBuild:nonexistent"),
            entry("BucketBuild:also_missing"),
        ];
        let allowed = allowed(&["BucketBuild:grep"]);
        assert_eq!(
            first_unknown_tool_id(&entries, &allowed),
            Some((1, "BucketBuild:nonexistent"))
        );
    }
}
