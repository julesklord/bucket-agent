...
//! OS-level sandboxing for Bucket Build via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use bucket_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__BUCKET_INSIDE_BWRAP";
...
/// Zero-permission placeholder (file or dir) under `bucket_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent bucket
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::bucket_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.bucket/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("bucket-{tag}-{}-{nanos}", std::process::id()));
        let bucket = ws.join(".bucket");
        std::fs::create_dir_all(&bucket).unwrap();
        std::fs::write(bucket.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("bucket-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::bucket_home()
