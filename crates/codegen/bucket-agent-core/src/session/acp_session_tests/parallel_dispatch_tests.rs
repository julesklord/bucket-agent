...
//!
//! These tests verify the parallel dispatch path (BUCKET_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and bucket_build's
...
#[test]
fn lock_path_for_args_matches_bucket_build_file_path() {
    // bucket_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_bucket_build_target_file() {
    // bucket_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_bucket_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. bucket_build search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&bucket), lock_path_for_args(&compat));
}
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use bucket_sampling_types::{ConversationItem, SyntheticReason};
