...
//! `.bucket/config.toml` files by walking from `cwd` up to the git root.
//!
...
use bucket_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$BUCKET_HOME/config.toml` (user tier, not project).
fn is_user_bucket_config_file(config_path: &Path) -> bool {
    let Some(user_home) = bucket_config::user_bucket_home() else {
        return false;
    };
...
/// Find all `.bucket/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.bucket/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.bucket/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.bucket/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".bucket").join("config.toml"))
        .filter(|config_path| config_path.is_file() && !is_user_bucket_config_file(config_path))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn find_project_configs_excludes_user_bucket_config_file() {
        let Some(user_home) = bucket_config::user_bucket_home() else {
            return;
        };
        let user_config = user_home.join("config.toml");
        if user_config.is_file() {
            ...
            assert!(
                !from_home.iter().any(|p| is_user_bucket_config_file(p)),
                "user config leaked into project configs: {from_home:?}"
            );
            assert!(is_user_bucket_config_file(&user_config));
        }

        let tmp = tempfile::tempdir().unwrap();
        let project = tmp.path().join("repo");
        std::fs::create_dir_all(project.join(".bucket")).unwrap();
        std::fs::write(project.join(".bucket/config.toml"), "# project\n").unwrap();
        let found = find_project_configs(&project);
        assert_eq!(found.len(), 1);
        assert!(!is_user_bucket_config_file(&found[0]));
    }
}
