...
//!
//! Persists per-folder trust decisions to `~/.bucket/trusted_folders.toml`.
...
//!
//! The store is rooted at [`bucket_config::user_bucket_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.bucket`) when
//! neither `$BUCKET_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.bucket/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.bucket/`.
...
impl TrustStore {
    /// Load the trust store from `<user_bucket_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_bucket_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`bucket_config::user_bucket_home`], never
    /// [`bucket_config::bucket_home`], so it never falls back to a cwd-relative
    /// `./.bucket` — that fallback would let an untrusted cloned repo's `.bucket`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(bucket_config::user_bucket_home())
    }

    /// Map a resolved user-bucket-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_bucket_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_bucket_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user bucket home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A bucket-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.bucket/worktrees.db` registry), so every `bucket -w` worktree
...
/// `bucket -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `bucket -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A bucket-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir bucket -w was launched from (parity with
...
/// sharing `~/.bucket/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.bucket/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = bucket_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global bucket-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match bucket_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.bucket");
        assert_eq!(
        ...
        // cwd-relative `./.bucket` that bucket_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.bucket/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_bucket_home() {
        // Thin source-pin: the production accessor reads user_bucket_home()
        // (Option, no cwd fallback), not bucket_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            bucket_config::user_bucket_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `bucket -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (bucket-managed worktrees) ─────────
    ...
    /// Point `BUCKET_HOME` at an isolated tempdir and register one bucket-managed
    ...
    fn register_bucket_worktree(
        ...
        let env = LockedTestEnv::lock().set("BUCKET_HOME", &home);
    ...
    #[test]
    fn workspace_key_collapses_standalone_bucket_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_bucket_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone bucket worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_bucket_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<bucket_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under bucket HOME but not
        ...
        let (_env, _wt) = register_bucket_worktree(&temp, "wt", &source_repo, "standalone");

        // Under bucket HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root.join("bucket-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <bucket_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
