...
/// Wire schema version, exported as resource attr `bucket_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.xai.bucket_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "bucket_code.session_start",
            Self::SessionEnd => "bucket_code.session_end",
            Self::UserPrompt => "bucket_code.user_prompt",
            Self::TurnCompleted => "bucket_code.turn_completed",
            Self::ApiRequest => "bucket_code.api_request",
            Self::ApiError => "bucket_code.api_error",
            Self::ToolResult => "bucket_code.tool_result",
            Self::ToolDecision => "bucket_code.tool_decision",
            Self::McpServerConnection => "bucket_code.mcp_server_connection",
            Self::PermissionModeChanged => "bucket_code.permission_mode_changed",
            Self::SkillActivated => "bucket_code.skill_activated",
            Self::PluginLoaded => "bucket_code.plugin_loaded",
            Self::Compaction => "bucket_code.compaction",
            Self::Subagent => "bucket_code.subagent",
            Self::Auth => "bucket_code.auth",
            Self::InternalError => "bucket_code.internal_error",
            Self::ModelSwitched => "bucket_code.model_switched",
            Self::ContextualTip => "bucket_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `bucket_code.session.count`.
    SessionCount,
    /// `bucket_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `bucket_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `bucket_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `bucket_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `bucket_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "bucket_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "bucket_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "bucket_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "bucket_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "bucket_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "bucket_code.error.count";
...
/// Built-in tool names (closed enum surface in `bucket-tools`) that pass
...
fn tool_outcome_label(o: &bucket_file_utils::events::types::ToolOutcome) -> &'static str {
    use bucket_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `bucket_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `bucket_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `bucket_code.session_end`.
...
/// `PromptSubmitted` → `bucket_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `bucket_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `bucket_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `bucket_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `bucket_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `bucket_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `bucket_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `bucket_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `bucket_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `bucket_code.permission_mode_changed`.
...
/// `ContextualTip` → `bucket_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `bucket_code.permission_mode_changed`.
...
/// `SkillDispatched` → `bucket_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `bucket_code.plugin_loaded`.
...
/// `PluginUsed` → `bucket_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `bucket_code.compaction`.
...
/// `CompactionTriggered` → `bucket_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `bucket_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `bucket_code.subagent` (`phase=completed`).
...
/// `InternalError` → `bucket_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `bucket_code.model_switched`.
