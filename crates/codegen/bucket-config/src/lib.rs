//! Config file loading for Bucket.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/bucket/managed_config.toml`
//! 2. `$BUCKET_HOME/managed_config.toml`
//! 3. `$BUCKET_HOME/config.toml`
//! 4. `$BUCKET_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/bucket/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.bucket`, admin-forced) — macOS only
//!
...
pub use paths::{
    bucket_application, bucket_application_in, bucket_home, claude_managed_settings_path,
    claude_managed_settings_probe_path, decode_cwd_from_dirname, default_bucket_home,
    encode_cwd_dirname, ensure_sessions_cwd_dir, sessions_cwd_dir, system_config_dir,
    user_bucket_home,
};
pub use validation::{
