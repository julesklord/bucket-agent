...
use bucket_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .bucket/hooks/ directories are still scanned (they hold
    ...
    // user_bucket_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .bucket.
    ...
    if let Some(ref bucket) = bucket {
        global.push(bucket.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = bucket
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".bucket").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    bucket_hooks::discovery::HookRegistry,
    Vec<bucket_hooks::error::HookError>,
) {
