...
Headless mode runs Bucket non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Bucket processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, Bucket emits an error object (process exit non-zero). Prompt-level
...
Bucket may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `bucket -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
bucket -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
bucket -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
bucket -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
bucket -p "What's the secret number?" --resume abc123
```
...
bucket -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
bucket --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  bucket -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Bucket's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(bucket -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
bucket -p "Format all files" --yolo

# Run tests and fix failures
bucket -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `BUCKET_API_KEY`        | API key for authentication (required when no browser login)   |
| `BUCKET_HOME`                    | Override config directory (default: `~/.bucket`)                |
| `BUCKET_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `BUCKET_API_KEY` with an API key from [console.x.ai](https://console.x.ai):

```bash
export BUCKET_API_KEY="bucket-..."
bucket -p "Run the test suite" --yolo
```
...
- **`BUCKET_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`bucket login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`bucket login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `bucket -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug bucket -p "..." 2> debug.log`.

---

## Project Root Discovery

When Bucket starts, it discovers the project root by walking upward from `--cwd`
...
Bucket discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Bucket stores data in `~/.bucket` (override with `BUCKET_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.bucket` read-only:

- Pre-populate `auth.json` or use `BUCKET_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export BUCKET_API_KEY="bucket-..."
export BUCKET_DISABLE_AUTOUPDATER=1
bucket -p "..." --no-auto-update
```
...
| `BUCKET_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`BUCKET_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `BUCKET_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$BUCKET_HOME/bin/bucket`).
...
- Resume: `bucket -p "continue" --resume "<id>"` or `bucket -p "continue" --continue`
