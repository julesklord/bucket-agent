...
//! Delegates to `bucket-plugin-marketplace` crate for scanning and install logic.
...
fn load_filtered_marketplace_sources() -> Vec<bucket_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                bucket_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                bucket_plugin_marketplace::SourceKind::Local { path } => {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        bucket_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let bucket_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = bucket_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = bucket_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
            ...
            bucket_hooks_plugins_types::ActionOutcome {
                status: bucket_hooks_plugins_types::OutcomeStatus::Success,
                message: msg,
...
async fn handle_update(
    ...
    let source_identity = |s: &bucket_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            bucket_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            bucket_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let marketplace_root = match &source.kind {
        ...
        bucket_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = bucket_plugin_marketplace::git::default_cache_root();
            match bucket_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                bucket_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = bucket_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = bucket_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, bucket_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(bucket_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            bucket_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match bucket_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    bucket_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
async fn auto_install_defaults(
    agent: &MvpAgent,
    sources: &[bucket_plugin_marketplace::MarketplaceSource],
    ...
    for (source, scan) in sources.iter().zip(results.iter()) {
        ...
        let mut reg = bucket_agent::plugins::install_registry::InstallRegistry::load();
        ...
        if let Some((existing_key, _)) = existing {
            ...
            let _ = reg.save();
            reg = bucket_agent::plugins::install_registry::InstallRegistry::load();
        }
...
fn scan_source(
    source: &bucket_plugin_marketplace::MarketplaceSource,
    ...
    let scan = bucket_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = bucket_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match bucket_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: bucket_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.bucket/config.toml`.
async fn handle_add_source(url: &str) -> bucket_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &bucket_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if bucket_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        bucket_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = bucket_config::bucket_home().join("config.toml");
    let bucket_home = bucket_config::bucket_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&bucket_home).ok();
...
async fn handle_remove_source(
    ...
    match tokio::task::spawn_blocking(move || remove_source_locked(&src)).await {
        Ok(outcome) => outcome,
        Err(e) => bucket_hooks_plugins_types::ActionOutcome {
            status: bucket_hooks_plugins_types::OutcomeStatus::InternalError,
            message: format!("Config write task failed: {e}"),
            requires_reload: false,
            requires_restart: false,
        },
...
fn remove_source_locked(source_url_or_path: &str) -> bucket_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use bucket_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let config_path = bucket_home.join("config.toml");
    let is_official = bucket_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<bucket_home>/.config-init.lock`,
...
fn acquire_init_lock(bucket_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(bucket_home);
    let lock_path = bucket_home.join(".config-init.lock");
...
pub fn ensure_official_marketplace_source(bucket_home: &std::path::Path) {
    let config_path = bucket_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(bucket_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %bucket_home.join(".config-init.lock").display(),
    ...
    // (settings.json / known_marketplaces.json) under bucket_home. Scoped to
    // bucket_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = bucket_plugin_marketplace::load_sources(&parsed);
    let json_sources = bucket_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&bucket_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, bucket_plugin_marketplace::SourceKind::Git { url, .. }
            if bucket_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            bucket_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<bucket_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        bucket_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            bucket_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                bucket_plugin_marketplace::SourceKind::Git { url, .. }
                    if bucket_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            bucket_plugin_marketplace::SourceKind::Git { url, .. }
                if url == bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                bucket_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"bucket-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"bucket-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = bucket_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(bucket_hooks_plugins_types::PluginComponents {
                skills: vec![bucket_hooks_plugins_types::ComponentItem::new(
                    "code-review",
