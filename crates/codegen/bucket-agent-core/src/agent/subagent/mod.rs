...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<bucket_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn bucket_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: bucket_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::BucketAuth>,
    ...
    pub parent_terminal_backend: Option<Arc<dyn bucket_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<bucket_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<bucket_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: bucket_tools::implementations::bucket_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: bucket_tools::implementations::bucket_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<bucket_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<bucket_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<bucket_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<bucket_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<bucket_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<bucket_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: bucket_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<bucket_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "bucket-build").
    ...
    pub parent_tool_snapshot: Option<Vec<bucket_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<bucket_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: bucket_agent::prompt::skills::SkillsConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut bucket_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("BUCKET_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("BUCKET_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use bucket_subagent_resolution::ResumeSourceData;
...
use bucket_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &bucket_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (bucket_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &bucket_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (bucket_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &bucket_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &bucket_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = bucket_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(bucket_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[bucket_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<bucket_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<bucket_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        bucket_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[bucket_sampling_types::conversation::ConversationItem],
) -> bool {
    use bucket_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<bucket_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = bucket_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        bucket_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &bucket_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use bucket_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<bucket_agent::config::AgentDefinition> {
    let mut def = bucket_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<bucket_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || bucket_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = bucket_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// agent definition.
///
/// The harness flavor (alternate vs bucket-build) normally follows the PARENT
/// agent: `BucketBuildOrchestrator` parents give children
...
/// (so a bucket-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut bucket_agent::config::AgentDefinition,
) {
...
fn resolve_subagent_permission_mode(
    requested: bucket_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> bucket_agent::config::PermissionMode {
    use bucket_agent::config::PermissionMode;
    if is_plugin {
...
/// auto-backgrounded (non-destructively). Env override: `BUCKET_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("BUCKET_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn cancellation_error_message(
    category: Option<bucket_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<bucket_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = bucket_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = bucket_tools::reminders::wrap_reminder(&message);
