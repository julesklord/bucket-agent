//! Notification bridge: translates `bucket-tools` `ToolNotification` events
//! into `bucket-agent-core`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use bucket_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    pub auto_wake_delivered: bucket_tools::reminders::task_completion::AutoWakeDeliveredIds,
    ...
    /// next-turn exit reminder. Bucket-build leaves this `false` — its
...
async fn emit_current_mode_update(
    ...
    mode: bucket_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor = task_snapshot.kind == bucket_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    bucket_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    bucket_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = bucket_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, bucket_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, bucket_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    ...
    use bucket_tools::types::TaskSnapshot;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>,
        mpsc::UnboundedReceiver<PersistenceMsg>,
        mpsc::UnboundedReceiver<SessionCommand>,
    ) {
        let (gateway_tx, gateway_rx) = mpsc::unbounded_channel();
        let gateway = bucket_acp::AcpAgentGatewaySender::new(gateway_tx);
    ...
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let bucket_acp::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "x.ai/task_completed"
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(bucket_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            bucket_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification =
            ToolNotification::BashOutputChunk(bucket_tools::notification::types::BashOutputChunk {
                base: bucket_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            bucket_acp::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            bucket_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            bucket_tools::notification::types::BashExecutionBackgrounded {
                base: bucket_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            bucket_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn legacy_monitor_event_without_owner_is_injected() {
        // Legacy / non-bucket-build backends record no owner; such events must
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(bucket_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let bucket_acp::AcpClientMessage::SessionNotification(args) = msg
    ...
    /// Default (bucket) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(bucket_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-bucket".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification =
            ToolNotification::PlanModeEntered(bucket_tools::notification::types::PlanModeEntered {
