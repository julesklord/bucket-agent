...
//!
//! Core worktree lifecycle logic lives in [`bucket_workspace::worktree`].
...
/// manual `create_from_worktree_sync` path used by `bucket -w --ref`).
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| bucket_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        match tokio::task::spawn_blocking(move || bucket_fast_worktree::remove_worktree(&wt_path))
            .await
        {
            Ok(Ok(_)) => {}
            Ok(Err(e)) => {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = bucket_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> bucket_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return bucket_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    bucket_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<bucket_workspace::session::git::RestoreDegree>,
}
...
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &bucket_workspace::session::git::CheckoutSessionOutcome,
    kind: bucket_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = bucket_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub async fn resume_session_in_worktree(
    ...
    ops: &bucket_workspace::WorkspaceOps,
    ...
    use bucket_workspace::session::git::effective_worktree_path;
    tracing::info!(
    ...
    let restore_degree = if codebase_ok {
        Some(bucket_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &bucket_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| bucket_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if bucket_workspace::session::git::should_warn_registry_disabled(
                is_jj,
...
pub async fn rehydrate_session_in_worktree(
    ...
    let session_summary_exists = crate::util::bucket_home::sessions_cwd_dir(&req.source_cwd)
        .join(&req.session_id)
        .join("summary.json")
        .exists();
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(bucket_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(bucket_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(bucket_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn make_wt_record(path: &str, source_repo: &str) -> bucket_fast_worktree::WorktreeRecord {
        bucket_fast_worktree::WorktreeRecord {
            id: format!("wt-{}", path.replace('/', "-")),
            path: std::path::PathBuf::from(path),
            source_repo: std::path::PathBuf::from(source_repo),
            repo_name: "repo".into(),
            kind: bucket_fast_worktree::WorktreeKind::Session,
    ...
    fn repo_wide_resolve_finds_session_in_sibling_cwd_skipping_remote() {
        ...
        let encoded = crate::util::bucket_home::encode_cwd_dirname(sibling_cwd);
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("bucket: pre-restore-code sess-dirty-wt"),
