...
//! - `BUCKET_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/bucket-build-public-artifacts/cli/bucket-<ver>-linux-x86_64`).
//! - `BUCKET_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! BUCKET_BINARY_LEADER=/path/to/bucket-old BUCKET_BINARY_CLIENT=/path/to/bucket-new \
//!   cargo test -p bucket-agent-core --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
use bucket_test_support::leader::{
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: BUCKET_BINARY_LEADER/BUCKET_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".bucket").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set BUCKET_BINARY_LEADER/BUCKET_BINARY_CLIENT and run with --ignored"]
...
/// `bucket update`'s relaunch signal against a REAL old leader: connect,
/// require `relaunch_v1`, send `RelaunchForUpdate`, and the leader exits so
/// the surviving client re-elects. Mirrors the private
/// `signal_leaders_to_relaunch` in `bucket-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".bucket").join("leader.sock"),
                "bucket-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".bucket").join("auth.json");
