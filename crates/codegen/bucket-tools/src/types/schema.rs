...
///
/// Use with `#[schemars(with = "BucketIntegerSchema")]` on `Option<usize>` fields.
pub struct BucketIntegerSchema;
impl schemars::JsonSchema for BucketIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "bucket_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `bucket-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use bucket_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
