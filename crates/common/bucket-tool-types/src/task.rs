...
/// A built-in subagent type shared by the CLI (`bucket-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "bucket-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("bucket-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("bucket-3".into())).as_deref(),
            Some("bucket-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  bucket-3  ".into())).as_deref(),
    ...
    // match what the bucket-shell MiniJinja templates render for the default
    // bucket-build toolset (monitor + task + bash + read present, POSIX). The
