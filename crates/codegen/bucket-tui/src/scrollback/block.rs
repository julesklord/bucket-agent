...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: bucket_agent_core::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[bucket_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("bucket-test"), "got: {text:?}");
    ...
    fn context_info_indexes_model_only() {
        ...
        let block = RenderBlock::context_info(snapshot, "bucket-4.5");
        // Only the model name is source text; the rest is a numeric breakdown.
        assert_eq!(block.searchable_text().as_deref(), Some("bucket-4.5"));
    }
