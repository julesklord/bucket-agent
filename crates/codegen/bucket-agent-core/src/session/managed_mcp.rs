...
//! `bucket_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use bucket_shell_session_support::managed_mcp::*;
...
pub fn merge_managed_mcp_servers(
    ...
    plugin_registry: Option<&bucket_agent::plugins::PluginRegistry>,
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &bucket_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &bucket_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &bucket_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: bucket_tools::util::bucket_home::bucket_home().join("config.toml"),
    };
...
/// Auto-create `bucket_com_*` entries for managed configs not already in `merged`.
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = bucket_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = bucket_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn auto_inject_creates_server_for_unmatched_managed_config() {
        ...
        let slack = merged
            .iter()
            .find(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "bucket_com_slack"));
        let slack = slack.expect("should have auto-injected bucket_com_slack");
        match slack {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use bucket_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
    ...
    fn merge_drops_server_denied_by_name_including_managed_prefix() {
        ...
        let tagged = apply_mcp_server_policy(
            vec![
                acp::McpServer::Http(
                    acp::McpServerHttp::new("bucket_com_slack", "https://mcp.slack.com/sse")
                        .headers(vec![]),
                ),
        ...
        let slack = tagged
            .iter()
            .find(|s| mcp_server_name(&s.server) == "bucket_com_slack")
    ...
    fn auto_inject_dedup_by_display_name_first_scope_wins() {
        ...
        let linear_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "bucket_com_linear"))
            .count();
    ...
    fn auto_inject_skips_existing_server() {
        ...
        let slack_count = merged
            .iter()
            .filter(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "bucket_com_slack"))
            .count();
    ...
    fn auto_inject_skips_disabled() {
        ...
        let disabled: std::collections::HashSet<String> =
            ["bucket_com_slack".to_string()].into_iter().collect();
        let mut merged = vec![];
        auto_inject_managed_servers_with_disabled(&mut merged, &managed, &disabled);

        let has_slack = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "bucket_com_slack"));
        let has_linear = merged
            .iter()
            .any(|s| matches!(s, acp::McpServer::Http(acp::McpServerHttp { name, .. }) if name == "bucket_com_linear"));
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".bucket")).unwrap();
        std::fs::write(
            cwd.path().join(".bucket").join("config.toml"),
            r#"
[mcp_servers.github]
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${BUCKET_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.bucket/plugins/team-tool",
            "/home/user/.bucket/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.bucket/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use bucket_agent::plugins::PluginRegistry;
        use bucket_agent::plugins::PluginScope;
        use bucket_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use bucket_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: bucket_agent::plugins::PluginOrigin::UserBucket,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use bucket_agent::plugins::PluginScope;
        use bucket_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use bucket_agent::plugins::manifest::PluginManifest;
