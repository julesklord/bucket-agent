...
//! `LspTool` (Tool trait impl) under the `BucketBuild` namespace.
...
impl bucket_tool_runtime::ToolOutput for LspToolOutput {}
...
impl bucket_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, bucket_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    bucket_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.bucket/lsp.json or <cwd>/.bucket/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
