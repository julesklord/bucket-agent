...
pub(crate) struct ExternalAuthOutput {
    ...
    /// see [`BucketAuth::is_xai_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `BucketAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<BucketAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<BucketAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &BucketAuth) -> Option<BucketAuth> {
...
mod tests {
    ...
    #[test]
    fn sync_sets_bucket_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $BUCKET_AUTH_EXPIRED", true).unwrap();
