#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn xai_session_update_replay_gating_and_cursor() {
        ...
        let replay_meta = serde_json::json!({ "isReplay": true, "eventId": "sess-bucket-7" });
        assert!(!handle_ext_notification(
    ...
    fn ignored_model_changed_does_not_advance_cursor_applied_one_does() {
        ...
        assert!(!handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "bucket-99-unknown", "sess-1-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
        ...
        assert!(handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "bucket-4", "sess-1-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
