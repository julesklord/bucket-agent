...
/// Path format: `~/.bucket/sessions/{session_id}/tasks/{task_id}.log`
...
pub struct BackgroundTaskManifestEntry {
    ...
    pub cwd: String,
    #[serde(default)]
    pub kind: bucket_tools::computer::types::TaskKind,
}
...
pub fn format_resumed_tasks_reminder(entries: &[BackgroundTaskManifestEntry]) -> String {
    ...
    for entry in entries {
        ...
        let kind_label = match entry.kind {
            bucket_tools::computer::types::TaskKind::Monitor => " [monitor]",
...
mod tests {
    ...
    fn format_reminder_labels_monitor_tasks() {
        ...
        entry.kind = bucket_tools::computer::types::TaskKind::Monitor;
        let reminder = format_resumed_tasks_reminder(&[entry]);
