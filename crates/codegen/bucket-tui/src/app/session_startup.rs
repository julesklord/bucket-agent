...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Bucket session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: bucket_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = bucket_agent_core::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = bucket_agent_core::util::bucket_home::bucket_home().join("sessions");
    let encoded = bucket_agent_core::util::bucket_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    bucket_agent_core::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    /// Process-wide flag: resume targets are bucket.com conversations, not
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = bucket_agent_core::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'bucket' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// `AuthManager` for direct bucket.com calls made outside the agent (pre-ACP
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &bucket_agent_core::agent::config::Config,
) -> std::sync::Arc<bucket_agent_core::auth::AuthManager> {
    let auth = std::sync::Arc::new(bucket_agent_core::auth::AuthManager::new(
        &bucket_agent_core::util::bucket_home::bucket_home(),
        agent_config.bucket_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.bucket_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if bucket_agent_core::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = bucket_agent_core::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    let agent_config = bucket_agent_core::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use bucket_agent_core::agent::session_registry_client::SessionRegistryClient;
    use bucket_agent_core::auth::{AuthManager, ensure_authenticated_or_noninteractive};
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["bucket"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["bucket", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["bucket", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["bucket", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["bucket", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["bucket", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["bucket", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["bucket"])).chat_mode);
    }
