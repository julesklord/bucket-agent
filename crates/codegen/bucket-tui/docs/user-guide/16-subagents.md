...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.bucket/agents/` or `~/.bucket/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.bucket/personas/`; applied during subagent resolution |
...
| **Examples** | `bucket-build`, `explore`, `plan` | `researcher`, `concise` |
...
Bucket Agent discovers file-based personas from these locations, in priority order:

- `.bucket/personas/*.toml` (project)
- `~/.bucket/personas/*.toml` (user)
...
> **Note:** Bucket Agent applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Bucket Agent resolves the effective model and reasoning effort in this order, highest priority first:
...
Bucket Agent manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "bucket-build"               # route explore to a specific model
```
...
# instructions_file = ".bucket/personas/concise.md"  # or load from a file
```

Bucket Agent also discovers roles from `.bucket/roles/*.toml` and personas from `.bucket/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Bucket Agent shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · bucket-3) — Thinking`
