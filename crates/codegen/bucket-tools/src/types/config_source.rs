...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.bucket/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.bucket/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.bucket/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.bucket/ or ~/.claude/.
    User { path: PathBuf },
