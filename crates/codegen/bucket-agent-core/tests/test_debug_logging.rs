...
//!
//! Runs the built bucket binary against the mock inference server with a
//! caller-owned `$BUCKET_HOME`, then inspects `~/.bucket/debug/`:
...
//!   `~/.bucket/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `BUCKET_LOG_SAMPLING`/`BUCKET_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `BUCKET_DEBUG_LOG=1`) writes a per-session
//!   `~/.bucket/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.bucket/debug/` files).
//! - `BUCKET_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p bucket-agent-core --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$BUCKET_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".bucket").join("debug")
}

/// List firehose `*.txt` files under `~/.bucket/debug` (excluding the `latest.txt`
...
/// Build a headless `bucket -p` command with a pinned `$BUCKET_HOME` so the firehose
/// lands under `<home>/.bucket/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    let mut cmd = tokio::process::Command::new(bucket_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    bucket_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("BUCKET_HOME", home.join(".bucket"));
    cmd.env_remove("BUCKET_DEBUG_LOG");
    cmd.env_remove("BUCKET_LOG_FILE");
    cmd.env_remove("BUCKET_LOG_SAMPLING");
    cmd.env_remove("BUCKET_HOOKS_LOG");
    cmd
}
...
/// assert it carries first-party (`xai_bucket`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &BucketStdioClient) -> String {
...
/// headless `bucket -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "bucket --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "bucket headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.bucket/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let bucket_home = home.path().join(".bucket");
        let bucket_home_str = bucket_home.to_string_lossy().into_owned();

        let client = BucketStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
            &[("BUCKET_DEBUG_LOG", "1"), ("BUCKET_HOME", &bucket_home_str)],
        )
        .await;
        ...
        let session_file = bucket_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.bucket/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = bucket_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `BUCKET_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `BUCKET_LOG_SAMPLING`/`BUCKET_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `bucket --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // BUCKET_DEBUG_LOG env. The spawn helper clears inherited firehose toggles,
        // so the `--debug` flag is the only thing that can enable logging here.
        let client = BucketStdioClient::spawn_with_home_env_and_args(
            &server,
            workdir.path(),
            home,
            &[("BUCKET_HOME", &bucket_home_str)],
            &["--debug"],
        )
        .await;
        client.initialize_with_timeout().await;
        ...
        // clears BUCKET_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.bucket/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // BUCKET_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = bucket_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.bucket/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "bucket --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `BUCKET_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn bucket_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    cmd.env("BUCKET_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "bucket BUCKET_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit BUCKET_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "BUCKET_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
