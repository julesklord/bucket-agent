//! bucket-build's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so bucket-build's full-replace
//! prompt lives alongside the rest of its [`code_compaction`](crate::code_compaction)
//! subsystem. The Bucket chat's step-level intra prompt
//! ([`format_compaction_prompt`](crate::prompt::format_compaction_prompt))
//! stays at the crate root.

/// Build bucket-build's session-level summarization prompt (no chat history).
///
...
/// `bucket-agent-core::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `bucket-agent-core`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (bucket-build) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// bucket-build's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes bucket-build's prompt.
        assert_eq!(
