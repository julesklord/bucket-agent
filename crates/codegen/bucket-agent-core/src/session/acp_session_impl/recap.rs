...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let mut items: Vec<ConversationItem> =
            bucket_chat_state::compaction_utils::strip_reasoning_blocks(
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_bucket_conv_id: Some(btw_session_id.clone()),
            x_bucket_req_id: Some(format!("bucket-btw-{}", uuid::Uuid::new_v4())),
            x_bucket_session_id: Some(parent_session_id.clone()),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_bucket_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_bucket_req_id = format!("bucket-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_bucket_conv_id: Some(x_bucket_conv_id.clone()),
            x_bucket_req_id: Some(x_bucket_req_id.clone()),
            x_bucket_session_id: Some(self.session_info.id.to_string()),
            x_bucket_agent_id: Some(bucket_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Bucket
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_bucket_req_id: x_bucket_req_id.to_owned(),
            x_bucket_conv_id: x_bucket_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`BUCKET_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`bucket-build-0.1`). Every
    ...
    /// `bucket-build-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `bucket-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_bucket_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_bucket_req_id: Some(format!("bucket-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_bucket_session_id: Some(self.session_info.id.to_string()),
            x_bucket_agent_id: Some(bucket_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %bucket_tools::util::truncate_str(raw.trim(), 60),
