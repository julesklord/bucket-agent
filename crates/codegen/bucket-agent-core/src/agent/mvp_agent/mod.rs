...
use bucket_telemetry::id::{agent_id, agent_instance_id};
use bucket_telemetry::session_ctx::log_event;
use bucket_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use bucket_workspace::permission::{ClientType, PermissionEvent};
...
use bucket_sampler::SamplerConfig as SamplingConfig;
...
use bucket_workspace::session::git::GitDiscoveryResult;
use bucket_hunk_tracker::HunkTrackerActor;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::BucketAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperBucketPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "BucketPro" => jwt_claim == "superbucket",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperBucketPro" => jwt_claim == "superbucket_heavy",
        "SuperBucketLite" => jwt_claim == "superbucket_lite",
        _ => false,
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a bucket_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[bucket_announcements::RemoteAnnouncement]>,
    last_emitted: &[bucket_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<bucket_announcements::RemoteAnnouncement>> {
    let current = bucket_announcements::filter_expired_at(
...
/// Override with `BUCKET_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("BUCKET_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// bucket.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_bucket_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.bucket/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<bucket_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        bucket_tools::implementations::bucket_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: bucket_tools::implementations::bucket_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        bucket_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        bucket_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: bucket_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled bucket-desktop's first
    ...
    workspace_ops: RefCell<Option<bucket_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<bucket_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &bucket_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<bucket_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !bucket_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = bucket_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `bucket-build` →
/// `bucket-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `bucket-build` →
/// `bucket-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use bucket_agent::config::is_strict_harness_agent_type;
    match (
...
use bucket_chat_state::conversation_util::replace_or_insert_system_head;
...
///
/// Headers injected:
///  - `x-bucket-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-BUCKET-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-bucket-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<bucket_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(bucket_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<bucket_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<bucket_acp::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        let is_xai = method == "_x.ai/session/update" || method == "_bucket/session/update";
        if is_xai {
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<bucket_acp::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = bucket_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_bucket_code_access(&self, auth: &crate::auth::BucketAuth) {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings bucket_build_access_gate)"
            );
            ...
            .map(|auth| {
                    ...
                    && is_first_party_model
                {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://bucket.com/superbucket?referrer=bucket-build".to_string(),
                        ),
    ...
    /// cached_token, bucket.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.bucket/bundled/` and the manifest.
...
/// absent — matching the `bucket_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_bucket_code_access` (initial login gate) and
