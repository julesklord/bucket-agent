...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = bucket_agent_core::config::load_effective_config().ok();
    ...
    // Always recompute is_api_key_auth from the tier so a later Free/SuperBucket
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(bucket_agent_core::tier::is_restricted_tier_name)
        {
            app.voice_reset();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("BUCKET_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(bucket_agent_core::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        bucket_agent_core::config::load_merged_requirements(),
        bucket_agent_core::config::load_from_disk().ok(),
        bucket_agent_core::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = bucket_agent_core::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = bucket_agent_core::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = bucket_agent_core::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use bucket_agent_core::util::config::resolve_tips;
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = bucket_agent_core::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        bucket_agent_core::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<bucket_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `BUCKET_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[bucket_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = bucket_agent_core::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = bucket_announcements::filter_expired(merged);
    ...
    if bucket_announcements::prune_hidden_announcement_ids(
...
pub(super) fn pick_random_announcement(
    announcements: &[bucket_announcements::RemoteAnnouncement],
) -> Option<bucket_announcements::RemoteAnnouncement> {
...
/// `bucket-agent-core/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `bucket-agent-core/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
