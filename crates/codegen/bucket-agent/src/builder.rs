...
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    bucket_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.bucket/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<bucket_tools::implementations::bucket_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn bucket_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: bucket_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn bucket_tools::implementations::lsp::LspBackend>>,
    image_gen_config: bucket_tools::implementations::bucket_build::image_gen::ImageGenConfig,
    video_gen_config: bucket_tools::implementations::bucket_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        bucket_tools::implementations::bucket_build::deploy_app::AppBuilderDeployerConfig,
    ...
    api_key_provider: Option<bucket_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<bucket_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in bucket-agent-core).
    ...
    preloaded_skills: Option<Vec<bucket_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut bucket_tools::registry::types::ToolServerConfig) {
    use bucket_tools::implementations::bucket_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("BucketBuild:enter_plan_mode");
    let missing_exit = !existing.contains("BucketBuild:exit_plan_mode");
    let missing_ask = !existing.contains("BucketBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&bucket_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&bucket_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&bucket_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut bucket_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(bucket_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<bucket_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn bucket_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: bucket_tools::implementations::bucket_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: bucket_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `BUCKET_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn bucket_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: bucket_tools::implementations::bucket_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: bucket_tools::implementations::bucket_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: bucket_tools::implementations::bucket_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: bucket_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the BucketBuild Task description.
    ...
    ///
    /// When disabled, `BucketBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.bucket/skills, ~/.bucket/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_fetch_config.is_enabled() {
                use bucket_tools::implementations::bucket_build;
                tool_config.tools.push((&bucket_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&bucket_tools::implementations::bucket_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&bucket_tools::implementations::bucket_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&bucket_tools::implementations::bucket_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&bucket_tools::implementations::bucket_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&bucket_tools::implementations::bucket_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&bucket_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let bucket_build_ns = bucket_tools::types::tool::ToolNamespace::BucketBuild.to_string();
            let mem_search_id = format!(
                "{bucket_build_ns}:{}",
                bucket_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{bucket_build_ns}:{}",
                bucket_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                bucket_tools::types::tool::ToolNamespace::BucketBuild,
            );
        ...
        if task_stripped {
            use bucket_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::BucketBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::BucketBuildConcise, "run_terminal_cmd", true)
        ...
        if let bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["BucketBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "BucketBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full bucket toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                bucket_tools::types::resources::TruncationCfg(
                    bucket_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(bucket_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(bucket_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI [`bucket_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: bucket_tool_types::SubagentToolNaming<'static> =
    bucket_tool_types::SubagentToolNaming {
...
/// shared [`bucket_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => bucket_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => bucket_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => bucket_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`bucket_tool_types::SubagentDescriptor`] and defers to
/// [`bucket_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<bucket_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            bucket_tool_types::SubagentDescriptor {
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        bucket_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: bucket_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(bucket_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(bucket_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: bucket_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("bucket", " models")));
    }
    #[test]
    ...
    async fn build_pager_agent(
        ...
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "bucket-build / subagents+ask_user",
                profile: AgentDefinition::default_bucket_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build / subagents / no-ask-user",
                profile: AgentDefinition::default_bucket_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "bucket-build / no-subagents / ask_user",
                profile: AgentDefinition::default_bucket_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_bucket_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "bucket-build-ask-user / subagents",
                profile: AgentDefinition::bucket_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build-ask-user / no-subagents",
                profile: AgentDefinition::bucket_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build-plan",
                profile: AgentDefinition::bucket_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build-plan / no-ask-user",
                profile: AgentDefinition::bucket_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "bucket-build-plan-no-subagents",
                profile: AgentDefinition::bucket_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "bucket-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::bucket_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_bucket_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::implementations::bucket_build::ask_user_question::AskUserQuestionParams;
        use bucket_tools::notification::ToolNotificationHandle;
        use bucket_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_bucket_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "BucketBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_bucket_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("BucketBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::implementations::bucket_build::bash::BashParams;
        use bucket_tools::notification::ToolNotificationHandle;
        use bucket_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_bucket_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Bucket
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_bucket_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their bucket
    ...
    /// treat `mcp__*` as always-on, so bucket keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use bucket_tools::computer::local::LocalTerminalBackend;
        use bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig;
        use bucket_tools::implementations::web_search::WebSearchConfig;
        use bucket_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_bucket_build();
    ...
    /// bucket-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to bucket's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, bucket_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, bucket_sampling_types::HostedTool::XSearch)),
