...
mod tests {
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("x.ai/queue/remove", &p, Some("bucket-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("bucket-tui"));
            }
        ...
        match parse_queue_edit_command("x.ai/queue/edit", &p, Some("bucket-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("bucket-vscode"));
            }
        ...
        match parse_queue_edit_command("x.ai/queue/interject", &p, Some("bucket-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
