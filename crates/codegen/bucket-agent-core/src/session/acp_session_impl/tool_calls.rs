...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => bucket_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use bucket_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Bucket-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as bucket edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &bucket_tools::implementations::bucket_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
///
/// Uses `bucket_acp`'s TYPED [`AcpChannelFailure`](bucket_acp::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        bucket_acp::acp_channel_failure(err),
        Some(bucket_acp::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        bucket_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    bucket_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(bucket_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => bucket_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && bucket_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(bucket_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(bucket_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            bucket_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            bucket_hooks::event::HookEventName::PostToolUseFailure,
                            bucket_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    bucket_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    bucket_hooks::event::HookEventName::PostToolUse,
                    bucket_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                .emit(
                    bucket_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if bucket_telemetry::external::is_active() {
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(bucket_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                bucket_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                bucket_hooks::event::HookEventName::PreToolUse,
                None,
                bucket_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    bucket_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let bucket_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                bucket_workspace::permission::AccessKind::Read(p) => (
                    bucket_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                bucket_workspace::permission::AccessKind::Edit(p) => {
                    (bucket_telemetry::events::AccessKind::Edit, p.clone())
                }
                bucket_workspace::permission::AccessKind::Bash(cmd) => {
                    (bucket_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                bucket_workspace::permission::AccessKind::Grep { path, glob } => (
                    bucket_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                bucket_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (bucket_telemetry::events::AccessKind::Mcp, name.clone())
                }
                bucket_workspace::permission::AccessKind::WebFetch(u) => {
                    (bucket_telemetry::events::AccessKind::Web, u.clone())
                }
                bucket_workspace::permission::AccessKind::WebSearch(q) => {
                    (bucket_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        bucket_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        bucket_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        bucket_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        bucket_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (bucket_telemetry::events::PermissionOutcome::Allow, None)
                }
            ...
            .in_scope(|| {});
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        bucket_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        bucket_hooks::event::HookEventName::PermissionDenied,
                        bucket_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        bucket_tools::implementations::bucket_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use bucket_tools::implementations::bucket_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                bucket_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        bucket_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        bucket_tool_types::WaitMode::WaitAny => "wait_any",
                        bucket_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
    ...
    async fn handle_tool_parse_error(
        ...
        call_id: &str,
        function_name: &str,
        err: bucket_tool_runtime::ToolError,
        raw_arguments: &str,
        model_id: &str,
    ) -> Result<(), acp::Error> {
        tracing::error!(
    ...
    /// `bucket_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use bucket_telemetry::enums::PrCreationSource;
        use bucket_tools::util::git_detect;
        match &result.output {
            bucket_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
    ...
    fn record_pr_created(
        &self,
        pr: bucket_tools::util::git_detect::PrRef,
        source: bucket_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            bucket_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                bucket_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let bucket_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                bucket_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | bucket_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            bucket_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            bucket_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`bucket_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: bucket_sampler::SamplingEvent,
    ) {
        use bucket_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == bucket_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                bucket_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = bucket_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use bucket_tools::types::ToolInput;
    use bucket_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use bucket_tools::implementations::bucket_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use bucket_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// Bucket edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn bucket_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "bucket tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use bucket_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use bucket_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use bucket_tools::implementations::bucket_build::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&bucket_acp::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use bucket_interjection_core::PendingInterjection;
