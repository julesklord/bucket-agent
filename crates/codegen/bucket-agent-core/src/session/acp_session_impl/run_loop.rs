...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<bucket_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<bucket_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            bucket_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : bucket_telemetry::memory_log::TARGET,
            ...
            bucket_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(bucket_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(bucket_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            bucket_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(bucket_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(bucket_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(bucket_hooks::event::HookEventName::SessionEnd, None,
            bucket_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            bucket_hooks::dispatcher::dispatch_non_blocking(& registry,
            bucket_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(bucket_hooks::event::HookEventName::Stop,
            None, bucket_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            bucket_hooks::dispatcher::dispatch_non_blocking(& registry,
            bucket_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            bucket_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::SessionEnded
            ...
            bucket_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(bucket_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(bucket_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .push_capped(bucket_tools::implementations::bucket_build::task::types::MonitorEventNotification
            ...
            bucket_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(bucket_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(bucket_file_utils::events::Event::McpServerToggled { server_name :
            ...
            session.fire_hook(bucket_hooks::event::HookEventName::SessionStart, None,
            bucket_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            bucket_hooks::dispatcher::dispatch_non_blocking(& registry,
            bucket_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(bucket_hooks::event::HookEventName::SessionEnd, None,
            bucket_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            bucket_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(bucket_hooks::event::HookEventName::Stop,
            None, bucket_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            bucket_hooks::dispatcher::dispatch_non_blocking(& registry,
            bucket_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[bucket_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = bucket_chat_state::compaction_utils::extract_user_query(&raw);
