...
/// `"namespace.Name"` (e.g., `"bucket_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"bucket_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("bucket_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "bucket_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"bucket_build.ReadFile"`).
    id: String,
...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, bucket_tool_runtime::ToolError> {
...
/// tool falls back to `Cwd/.bucket/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".bucket/plan.md";
...
/// [`Cwd`]`/.bucket/plan.md`, else the bare relative `.bucket/plan.md`.
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), bucket_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn bucket_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn bucket_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, bucket_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
register_resource!("bucket_build", "WebCitation", WebCitationCounter);
...
/// depending on `bucket-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("bucket_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("bucket_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("bucket_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "bucket_build.ReadFile".to_string(),
            serde_json::json!({ "files_read" : ["loaded.rs"] }),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "bucket_build.Edit".to_string(),
            serde_json::json!(
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "bucket_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "bucket_build.Edit").is_none());
    }
    #[test]
