...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (bucket build harness) ─────────────
...
///
/// `"/path/.bucket/skills/my-skill/SKILL.md"` -> `"/path/.bucket/skills/"`
...
/// Used by the budgeted (bucket build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Bucket build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the bucket
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // bucket-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: bucket build harness (budgeted XML) ───────────
