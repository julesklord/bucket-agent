...
impl bucket_tool_runtime::Tool for HashlineEditTool {
    type Args = HashlineEditInput;
    type Output = crate::types::output::SearchReplaceOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("hashline_edit").expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<crate::types::output::SearchReplaceOutput, bucket_tool_runtime::ToolError> {
        ...
        let (cwd, display_cwd, fs, scheme, hints_enabled) = {
            ...
            let cwd = match ctx.extensions.get::<bucket_tool_runtime::Cwd>() {
