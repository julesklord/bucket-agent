...
pub const MANAGED_ARTIFACT_FILES: [&str; 3] = [
    bucket_config::MANAGED_CONFIG_FILENAME,
    bucket_config::REQUIREMENTS_FILENAME,
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            bucket_config::MANAGED_CONFIG_CACHE_FILE,
...
/// would just 401).
fn eligible_team_principal(auth: BucketAuth) -> Option<BucketAuth> {
...
/// config is a bucket.com feature with one bucket.com auth.
fn read_active_team_auth() -> Option<BucketAuth> {
    let home = crate::util::bucket_home::bucket_home();
...
/// files (TUI tick vs `bucket login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `bucket setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `BUCKET_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("BUCKET_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `bucket-http` classifier.
...
/// Override with `BUCKET_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("BUCKET_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** BucketBuildDeployment UUID (the id
...
/// Resolve deployment key from `BUCKET_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (bucket setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("BUCKET_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.bucket/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<BucketAuth>,
...
/// read-only `bucket setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if bucket_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(bucket_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        bucket_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
    ...
    clear_squatting_dir(&home.join(bucket_config::MANAGED_CONFIG_CACHE_FILE));
...
/// Post-login hook for `bucket login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<BucketAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `bucket setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(BucketAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `bucket setup`; ceasing to serve
...
/// Outcome of the `bucket setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.bucket`.
    Installed,
...
/// Result of `bucket setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `bucket setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `bucket setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `bucket setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `bucket setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && bucket_config::signed_policy::verification_active()
...
/// Run the `bucket setup` sync for the current principal. The caller must check
