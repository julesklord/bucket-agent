// Re-export all types from the standalone bucket-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use bucket_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `bucket-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `bucket-sampler` crate can reference them without a circular
// dep on `bucket-agent-core`.

// Tests for the types now live in bucket-sampling-types crate.

use bucket_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `bucket-sampling-types` or
/// `bucket-tools`) so neither of those crates needs to depend on
