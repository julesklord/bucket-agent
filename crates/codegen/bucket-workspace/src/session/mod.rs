...
pub struct WorkspaceSession {
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, bucket_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<bucket_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<bucket_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<bucket_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<bucket_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<bucket_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<bucket_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: bucket_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn bucket_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `BUCKET_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$BUCKET_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<bucket_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<bucket_file_utils::queue::UploadQueue>>,
    ...
    /// (`BUCKET_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](bucket_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`BUCKET_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, bucket_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<bucket_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<bucket_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](bucket_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> bucket_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<bucket_file_utils::events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&bucket_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](bucket_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<bucket_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, bucket_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> bucket_file_utils::events::EventWriter {
