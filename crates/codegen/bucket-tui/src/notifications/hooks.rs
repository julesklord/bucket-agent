...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("BUCKET_EVENT", event_str)
        .env("BUCKET_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("BUCKET_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'BUCKET_EVENT=%s\\nBUCKET_MESSAGE=%s\\nBUCKET_SESSION_ID=%s\\n' \
             \"$BUCKET_EVENT\" \"$BUCKET_MESSAGE\" \"$BUCKET_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("BUCKET_EVENT=Turn complete"),
            "missing BUCKET_EVENT: {content}"
        );
        assert!(
            content.contains("BUCKET_MESSAGE=hello world"),
            "missing BUCKET_MESSAGE: {content}"
        );
        assert!(
            content.contains("BUCKET_SESSION_ID=sess-42"),
            "missing BUCKET_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("BUCKET_SESSION_ID"),
            "BUCKET_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("BUCKET_EVENT=Turn complete"));
        assert!(content.contains("BUCKET_MESSAGE=test body payload"));
        assert!(content.contains("BUCKET_SESSION_ID=test-session-123"));
    }
}
