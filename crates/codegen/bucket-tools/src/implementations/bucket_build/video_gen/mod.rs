...
const BUCKET_VIDEO_BASE_MODEL: &str = "bucket-imagine-video";
const BUCKET_VIDEO_QUALITY_MODEL: &str = "bucket-imagine-video-1.5-preview";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "bucket-videos/";
...
pub use bucket_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> bucket_file_utils::s3::S3StaticCredentials {
        bucket_file_utils::s3::S3StaticCredentials {
...
pub struct VideoGenClient {
    ...
    /// and return the SuperBucket upsell prose. See [`VideoGenClient::is_tier_restricted`].
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, bucket_tool_runtime::ToolError> {
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), bucket_tool_runtime::ToolError>(())
        })?;
    ...
    /// SuperBucket upsell instead of issuing a doomed request.
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, bucket_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(bucket_tool_runtime::ToolError::new(
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(bucket_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, bucket_tool_runtime::ToolError> {
        ...
        let upload_url = bucket_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, bucket_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = bucket_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
pub enum VideoGenConfig {
    ...
    Enabled {
        ...
        /// at call time with the SuperBucket upsell prose. Set by the host from
...
/// the deliberate `/imagine-video` slash command shows the SuperBucket upsell
/// modal instead.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Video generation is a SuperBucket feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperBucket: https://bucket.com/superbucket?referrer=bucket-build. Do not retry this tool.";
...
async fn resolve_image_reference(value: &str) -> Result<String, bucket_tool_runtime::ToolError> {
...
fn validate_one_of(
    ...
    Err(bucket_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), bucket_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), bucket_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, bucket_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, bucket_tool_runtime::ToolError> {
    match outcome {
...
impl bucket_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
    ...
    fn capabilities(&self) -> bucket_tool_protocol::ToolCapabilities {
        bucket_tool_protocol::ToolCapabilities {
            is_read_only: false,
...
impl bucket_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn image_and_reference_payload_fields_are_serialized() {
        let payload = GenerateVideoPayload {
            model: BUCKET_VIDEO_QUALITY_MODEL,
            prompt: "animate",
