...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<bucket_file_utils::UploadMethod>) -> Self {
        match method {
            Some(bucket_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(bucket_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(bucket_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use bucket_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `bucket-shell-doom_loop_recovery` Mixpanel event's name and
