...
use crate::auth::model::BucketAuth;
use crate::auth::token_type::TokenType;
use bucket_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&BucketAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        bucket_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<BucketAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<BucketAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<BucketAuth, AuthError> {
        loop {
            match self.step {
    ...
    async fn try_reload_from_disk(&self) -> Option<BucketAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            bucket_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<BucketAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<BucketAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &BucketAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, BucketAuth};
    ...
    fn rejected_cred() -> Option<BucketAuth> {
        Some(BucketAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
        (dir, m)
    }
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.bucket_com_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.bucket_com_config().auth_scope();
        let expired_different = BucketAuth {
