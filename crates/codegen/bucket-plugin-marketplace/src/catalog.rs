...
//! `.bucket-plugin/plugin-index.json` (preferred), then
...
use bucket_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_bucket_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".bucket-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_bucket_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".bucket-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".bucket-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".bucket-plugin",
            r#"{
                "$schema": "https://x.ai/bucket/plugin-index.schema.json",
                "version": 1,
