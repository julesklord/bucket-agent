...
// (`~/.bucket/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_bucket_home>/trusted-hook-projects`), or `None` when no user bucket home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(bucket_config::user_bucket_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user bucket home (set $BUCKET_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$BUCKET_HOME/disabled-hooks`, or `None` when no user bucket
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(bucket_config::user_bucket_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        ...
        bucket_dir.join("trusted-hook-projects")
    }
