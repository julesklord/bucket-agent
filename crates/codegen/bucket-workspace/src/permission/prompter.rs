...
use bucket_file_utils::events::{Event, EventWriter, PermissionDecision};
use bucket_tools::implementations::bucket_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell Bucket what to do differently";
...
///        `~/.bucket/config.toml` via the `Effect::PersistPermissionMode` effect
...
/// (global yolo) option that is prepended for BucketTUI / BucketPager / Desktop.
///
...
/// (`BucketTUI`, `BucketPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::BucketTUI | ClientType::BucketPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `bucket_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `bucket-mcp` can
...
/// reached `bucket_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"bucket_com_notion"` →
/// `"Bucket Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"bucket_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server segment (everything before the single `__` separator,
    /// e.g. `"bucket_com_notion"`). `None` if the tool name has no `__`,
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for BucketTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For BucketTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::BucketPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::BucketPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::BucketWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::BucketPager, true);
        ...
        let off = prompter_with_gate(ClientType::BucketPager, false);
        assert!(off.bash_selection_meta(&access).is_none());
        // Generic client never gets the fancy-UI meta, even with the gate on.
        let generic = prompter_with_gate(ClientType::BucketWeb, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::BucketPager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::BucketTUI);
        let access = AccessKind::MCPTool {
            name: "linear__list".to_owned(),
            input: serde_json::Value::Null,
        };
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / BucketWeb / Extension / …) see `fallback_options`. The
    ...
    /// Same pin as above for `BucketTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::BucketTUI, ClientType::Desktop] {
            let p = prompter(ct);
