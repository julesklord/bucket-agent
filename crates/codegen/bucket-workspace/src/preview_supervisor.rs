...
//! spawns the unchanged `/usr/local/bin/bucket-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/bucket-preview-proxy";
...
/// `bucket_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "bucket_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `bucket-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`BUCKET_SERVER_KEY` /
/// `BUCKET_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT bucket_tty_utils::detach_command: the proxy must stay in
...
/// `bucket-preview-proxy`'s `/__control/activity` route).
...
/// the default in `bucket-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://bucket.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match bucket-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "BUCKET_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
