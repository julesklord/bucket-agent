...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `BUCKET_PLUGIN_ROOT` and `BUCKET_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
...
| `BUCKET_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

Bucket sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (Bucket also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.
...
| `--plugin-dir` (CLI flag, `bucket agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.bucket/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.bucket/plugins/` | User -- personal plugins for every project | Trusted automatically |
...
bucket plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `bucket plugin install <source>` without `--trust` and Bucket prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
bucket plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
bucket plugin marketplace add my-org/team-plugins
bucket plugin marketplace list
bucket plugin install my-org/team-plugins --trust
bucket plugin list
bucket plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `bucket plugin` CLI.
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `bucket plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.
...
```bash
bucket plugin install <source> --trust
```
...
Run `bucket inspect` to see every discovered plugin and what it provides:

```bash
bucket inspect          # Show plugins with their skills, agents, hooks, and MCP servers
bucket inspect --json   # Emit machine-readable JSON
```
