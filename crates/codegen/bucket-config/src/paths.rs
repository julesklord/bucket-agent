//! Filesystem locations for bucket config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static BUCKET_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user bucket directory (`~/.bucket`, canonicalized) used when
/// `BUCKET_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`bucket_home()`] is the default without duplicating the computation.
///
...
/// `~/.bucket/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `bucket_fast_worktree::db::resolve_bucket_home` (deliberately standalone crate).
pub fn default_bucket_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".bucket")
}

/// Per-user config directory: `$BUCKET_HOME` or `~/.bucket`. Created if needed.
pub fn bucket_home() -> PathBuf {
    BUCKET_HOME
        .get_or_init(|| {
            let bucket_home = if let Ok(v) = std::env::var("BUCKET_HOME") {
                PathBuf::from(v)
            } else {
                default_bucket_home()
            };
...
/// that *scan* user-global bucket resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.bucket` tree for the user-global one when no home resolves.
pub fn user_bucket_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("BUCKET_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(bucket_home)
}

/// Canonical bucket application path: `$BUCKET_HOME/bin/bucket` (Unix) or `bucket.exe` (Windows).
pub fn bucket_application() -> PathBuf {
    bucket_application_in(&bucket_home())
}
...
/// System-wide config directory: `/etc/bucket/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/bucket"))
    } else {
        None
    }
}
...
/// `bucket_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    bucket_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = bucket_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_bucket_home_has_no_verbatim_prefix() {
        ...
        let home = default_bucket_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".bucket"));
    }
