...
/// Vendor API keys with `sk-`/`sk_` prefixes and xAI (`bucket-`) keys. `\b`-anchored so
/// `task-`/`disk-`/`risk-` don't fold a stray `sk-`.
static API_KEY_PREFIX_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile(r"\b(?:sk[-_]|bucket-)[A-Za-z0-9_-]{20,}"));
...
mod tests {
    ...
    fn no_match_returns_borrowed() {
        ...
        assert!(matches!(redact_secrets("model=bucket-3"), Cow::Borrowed(_)));
    }
    ...
    fn redacts_known_secret_shapes() {
        let cases = [
            (
                fixture(&["key: bucket-", "abc123XYZdef456GHIjkl789"]),
                "xai api key",
            ),
            (
