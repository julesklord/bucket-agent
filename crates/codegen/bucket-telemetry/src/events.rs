...
//!
//! These structs were extracted from `bucket-agent-core` so they can be
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = bucket_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: bucket_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from bucket-tui via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Bucket Build is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperBucketUpsellShown {
    pub source: SuperBucketUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperBucketUpsellClicked {
    pub source: SuperBucketUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperBucketUpsell`]; lets the funnel attribute the click to the
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Bucket Build's upsell surface.
...
/// `bucket-shell-manual_auth` (the `manual_auth` binding gets the `bucket-shell-`
...
telemetry_event!(SuperBucketUpsellShown, "superbucket_upsell_shown");
telemetry_event!(SuperBucketUpsellClicked, "superbucket_upsell_clicked");
