...
const BUCKET_IMAGINE_MODEL: &str = "bucket-imagine-image-quality";
// Some Imagine models (e.g. `bucket-imagine-image`, selectable via `model_override`)
...
/// SuperBucket upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperBucket feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperBucket: https://bucket.com/superbucket?referrer=bucket-build. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// [`BUCKET_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    ...
    /// HTTP call and return the SuperBucket upsell prose instead. See
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| BUCKET_IMAGINE_MODEL.to_owned());
    ...
    /// with the SuperBucket upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`BUCKET_IMAGINE_MODEL`]). Driven by the remote
        ...
        /// SuperBucket upsell prose instead of a doomed request. Set by the
...
impl ImageGenConfig {
    ...
    /// default quality model ([`BUCKET_IMAGINE_MODEL`]) is used.
...
impl bucket_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("bucket-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("bucket-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("bucket-imagine-image")), None)
                .unwrap()
                .model,
            "bucket-imagine-image"
        );
    }
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperBucket upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperBucket"), "got: {}", t.text);
                assert!(t.text.contains("superbucket?referrer=bucket-build"));
            }
