...
//! FsNotify adapter functions bridging [`bucket_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](bucket_codebase_graph::IndexManagerHandle).
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> bucket_codebase_graph::FileEvent {
    use bucket_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`bucket_fsnotify::FsEventKind`] to the wire-type
/// [`bucket_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> bucket_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => bucket_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => bucket_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => bucket_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => bucket_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => bucket_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](bucket_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<bucket_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<bucket_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    bucket_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<bucket_codebase_graph::FileEvent> {
    ...
    match status.chars().next()? {
        ...
        _ => Some(bucket_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &bucket_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<bucket_workspace_types::WorkspaceEvent>,
) {
    ...
    bucket_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(bucket_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            bucket_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: bucket_workspace_types::FsEventKind,
) -> bucket_codebase_graph::FileEvent {
    use bucket_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        bucket_workspace_types::FsEventKind::Created => FileEventKind::Created,
        bucket_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        bucket_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        bucket_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.bucket/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use bucket_workspace_types::FsEventKind as WsKind;
        assert_eq!(
