...
/// (`bucket-agent-core::extensions::task::KillTaskResponse`) and deserialized
/// by clients (bucket-tui), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in bucket-tools, spawns processes)
/// - `AcpTerminalBackend` (in bucket-agent-core, calls ACP protocol)
#[async_trait::async_trait]
