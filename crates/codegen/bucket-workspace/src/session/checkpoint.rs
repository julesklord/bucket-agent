...
/// Resolve `workspace_rewind_hunks` from `BUCKET_WORKSPACE_REWIND_HUNKS` (default off).
pub(crate) fn rewind_hunks_enabled() -> bool {
    bucket_config::env_bool("BUCKET_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
/// Resolve `workspace_rewind_durable` from `BUCKET_WORKSPACE_REWIND_DURABLE`
/// (default off). Off ⇒ the legacy in-memory-only path with no disk I/O.
pub(crate) fn rewind_durable_enabled() -> bool {
    bucket_config::env_bool("BUCKET_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
...
impl WorkspaceHandle {
    ...
    ) -> Option<tokio::task::JoinHandle<bucket_file_utils::queue::EnqueueOutcome>> {
        match boundary {
            TurnBoundary::Start {
...
mod tests {
    ...
    async fn turn_end_with_durable_flag_off_writes_nothing_to_disk() {
        ...
        handle
            .on_turn_boundary("main", TurnBoundary::rewind_finalize(0))
            .await;
        let store_root = cwd.join(".bucket").join("rewind-checkpoints");
        assert!(
            !store_root.exists(),
