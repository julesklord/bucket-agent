...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("BUCKET_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `bucket_tui_pty_harness::flows` (re-exported above).
...
/// the harness's default `BUCKET_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
/// would never fetch `/v1/settings`. Spawns WITHOUT `BUCKET_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("BUCKET_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`BUCKET_TEST_VERSION`) and an explicit `BUCKET_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/BUCKET_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    // A self-built (unstamped) bucket auto-trusts and never prompts; simulate a
    ...
    // auto-trust.
    env.push(("BUCKET_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set BUCKET_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("BUCKET_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".bucket")
        .join(bucket_workspace::trust::TRUST_FILE_NAME);
    let store = bucket_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&bucket_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `bucket_tui_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&bucket_home).expect("create fake BUCKET_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(bucket_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in bucket-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.bucket/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{BUCKET_HOME|HOME}/.bucket/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let bucket_home = content.home().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create .bucket");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let bucket_home = content.home().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create .bucket");
    // Minimal opt-in only — matches load_config's `{BUCKET_HOME|HOME}/.bucket/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("BUCKET_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "BUCKET_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        bucket_tui::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `bucket wrap` run budget. Same contention math as the requirements-version
...
/// Run `bucket wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `BUCKET_HOME`, returning the exit code (`None` if it never exited within
...
/// `bucket wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$BUCKET_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("BUCKET_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use bucket_tui_pty_harness::host_clipboard::{
...
pub(crate) use bucket_tui_pty_harness::host_clipboard::clipboard_roundtrip_works;
