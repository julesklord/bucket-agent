...
use crate::implementations::bucket_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the bucket-build `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `BucketBuild` or `Codex` search,
...
impl bucket_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, bucket_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> bucket_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = bucket_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
