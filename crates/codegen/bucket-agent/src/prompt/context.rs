...
//! `TemplateRenderer` in `bucket-tools`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary bucket-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity on trim-tool-descriptions (`You are Bucket released by xAI`).
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "Bucket Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "Bucket Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.bucket/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Bucket Build agent"),
