...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `bucket update` to get the latest version.";
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Bucket Build is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Bucket Build is available.");
        }
        return Ok(());
    }
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_bucket_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_bucket_version();
...
/// background download, explicit `bucket update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_bucket_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_bucket_version();
...
/// managed `~/.bucket/bin/bucket` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("BUCKET_INSTALLER") {
    ...
    if std::env::var_os("BUCKET_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("BUCKET_MANAGED_BY_INTERNAL").is_some() {
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `bucket update` child, `Some` only when a
...
/// (spawns `bucket update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `bucket update` child resolves
...
/// Launch "bucket update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("bucket update failed with {}", status);
            }
            Ok(None)
        }
        UpdateRunMode::NonBlocking => {
            ...
            bucket_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Restart bucket with the original command-line arguments to pick up the update.
pub fn restart_bucket() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "BUCKET_AUTO_UPDATE"));
    eprintln!("Restarting Bucket...");
...
/// versioned name as the extension (`bucket-0.1.181-linux-x86_64` →
/// `bucket-0.1.tmp`) and therefore collides for every `0.1.x` version. The PID
...
/// versioned base like `bucket-0.1.181` doesn't collide via `with_extension`.
...
/// Delete `~/.bucket/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = bucket_home().join("models_cache.json");
...
/// Remove the stale `bucket-pager` symlink/binary from `~/.bucket/bin/` left by
...
/// config and writes to `~/.bucket/bin/`, so callers must isolate
/// `BUCKET_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.bucket/downloads/`, not yet
/// activated as the managed `bucket`/`agent`.
...
async fn download_verified_from_base(
    ...
    let download_dir = bucket_home.join("downloads");
    tokio::fs::create_dir_all(&download_dir).await?;

    let binary_name = format!("bucket-{}-{}", version, platform);
    let binary_path = download_dir.join(&binary_name);

    eprintln!("  Downloading bucket v{} ({})...", version, platform);
    ...
    // corrupt download is caught here and never becomes the active bucket.
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let bucket_home = bucket_home();
    let download_dir = bucket_home.join("downloads");
    let bin_dir = bucket_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.bucket/bin/{bucket,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "bucket", &download.version).await;
    cleanup_old_downloads(&download_dir, "bucket-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &bucket_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, bucket_home: &std::path::Path) {
    // Derive $HOME independently — bucket_home may be overridden via BUCKET_HOME
    // env var, so bucket_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
...
///
/// When both paths share a grandparent (e.g. `~/.bucket/bin/bucket` and
/// `~/.bucket/downloads/bucket-0.1.203-linux-x86_64`), returns a relative path
/// like `../downloads/bucket-0.1.203-linux-x86_64`.  When they share the same
/// parent directory, returns just the filename.  Falls back to the absolute
/// `target` path for any other layout.
///
/// Relative symlinks survive Docker bind-mounts where `~/.bucket/` is mapped
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Same directory — just the filename (e.g. bucket-latest -> bucket-0.1.203-…)
    if target_parent == link_parent
        && let Some(name) = target.file_name()
    {
        return std::path::PathBuf::from(name);
    }
    // Sibling directories — ../target_dir/filename (e.g. bin/bucket -> ../downloads/bucket-…)
...
/// Swap `~/.bucket/bin/{bucket,agent}` to point at `binary_path`. Returns the
/// `bucket` link path (for [`regenerate_completions`]).
///
/// `bucket` and `agent` are first-class entry points that the bootstrap
/// installers (`install.sh`, `install.ps1`, `install-enterprise.sh`)
/// maintain in lockstep, and so must the updater — otherwise `bucket update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.bucket/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let link_paths: [std::path::PathBuf; 2] = [bucket_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running bucket sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"bucket"` or `"bucket-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `bucket-*` matching `bucket-pager-*` or `bucket-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // Skip symlinks (e.g. bucket-latest).
        ...
        // binary (avoids `bucket-latest`, `bucket-pager-*` when prefix is `bucket`).
        let suffix = &name[prefix.len()..];
        if !suffix.starts_with(|c: char| c.is_ascii_digit()) {
            continue;
        }
        // Extract the version portion via the shared parser (handles the
        // internal `bucket-0.1.150-macos-aarch64`, pre-release, and npm
        // `bucket-0.1.150` layouts — see `version_from_versioned_binary_name`).
...
async fn gh_release_download(
    ...
    .stderr(Stdio::piped());
    bucket_tools::util::detach_command(&mut cmd);
    cmd.envs(bucket_tools::util::pager_env());
...
async fn install_gh_release(target: Option<&str>, update_config: &UpdateConfig) -> Result<()> {
    ...
    eprintln!(
        "  Downloading bucket v{} ({}) from GitHub Releases...",
        version, platform
    );
    ...
    // Update bucket-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/bucket -> ~/.bucket/downloads/bucket-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("bucket-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update bucket-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{bucket,agent} if either points directly into
    // ~/.bucket/downloads/ (legacy layout — skips the bucket-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["bucket", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".bucket/downloads/") && !target_str.ends_with("bucket-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "bucket", &version).await;
    cleanup_old_downloads(&download_dir, "bucket-pager", &version).await;
...
/// Check if other bucket processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any bucket process running from that vendored path will be SIGKILL'd by the
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.bucket/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_bucket_processes_running() {
    ...
    cmd.args(["-f", "bucket"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    bucket_tools::util::detach_std_command(&mut cmd);
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other bucket process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other bucket sessions before updating.");
            eprintln!();
        }
    }
}
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_bucket_processes_running();

    let version_arg = match target {
        Some(ver) => format!("@bucket-official/bucket@{ver}"),
        None => {
...
/// Run the `bucket update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ bucket v{} installed successfully!", version);
        eprintln!("  Please restart Bucket.");
    ...
    {
        eprintln!(
            "Forcing reinstall of Bucket {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating Bucket {} → {}", effective_current, install_target);
        &install_target
    };
    ...
    if !force && std::env::var_os("BUCKET_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Bucket.");
    }
...
async fn refresh_deployment_config() {
    if !bucket_agent_core::managed_config::has_principal() {
        return;
    }
    if !bucket_agent_core::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    bucket_agent_core::managed_config::clear_orphan();
    if !bucket_agent_core::config::is_managed_config_stale_for(
        &bucket_agent_core::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match bucket_agent_core::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `bucket setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `bucket setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        assert!(
            name.starts_with("bucket-0.1.181-linux-x86_64."),
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.bucket/downloads").into(),
    ...
    async fn test_atomic_symlink_swap_replaces_regular_file() {
        ...
        let link = dir.path().join("bucket");
        // Simulate an old installation where bucket is a regular file.
    ...
    async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
        ...
        let leftover_old = dir.path().join("bucket.tmp-link");
        let leftover_new = dir.path().join("bucket.123-0.tmp-link");
        ...
        // `bucket` link (no `.tmp-link` suffix) is untouched.
        ...
        let fresh = dir.path().join("bucket.999-9.tmp-link");
    ...
    async fn test_atomic_symlink_swap_with_absolute_target() {
        ...
        let binary = dir.path().join("bucket-0.1.141");
    ...
    async fn test_atomic_symlink_swap_with_relative_target() {
        ...
        std::fs::write(downloads.join("bucket-0.1.203"), "v203").unwrap();

        let rel_target = std::path::Path::new("../downloads/bucket-0.1.203");
        let link = bin.join("bucket");
        atomic_symlink_swap(rel_target, &link).await.unwrap();

        assert!(link.is_symlink());
        assert_eq!(
            std::fs::read_link(&link).unwrap(),
            std::path::PathBuf::from("../downloads/bucket-0.1.203")
        );
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        // bin/bucket -> ../downloads/bucket-0.1.203
        let target = std::path::Path::new("/home/alice/.bucket/downloads/bucket-0.1.203");
        let link = std::path::Path::new("/home/alice/.bucket/bin/bucket");
    ...
    fn test_relative_symlink_target_same_dir() {
        // downloads/bucket-latest -> bucket-0.1.203 (same directory)
        let target = std::path::Path::new("/home/alice/.bucket/downloads/bucket-0.1.203");
        let link = std::path::Path::new("/home/alice/.bucket/downloads/bucket-latest");
        let result = relative_symlink_target(target, link);
        assert_eq!(result, std::path::PathBuf::from("bucket-0.1.203"));
    }

    #[cfg(unix)]
    #[test]
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/bucket -> /home/alice/.bucket/downloads/bucket-0.1.203
        // Different grandparents — should stay absolute.
        let target = std::path::Path::new("/home/alice/.bucket/downloads/bucket-0.1.203");
        let link = std::path::Path::new("/usr/local/bin/bucket");
        let result = relative_symlink_target(target, link);
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.bucket/downloads/bucket-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.bucket/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".bucket");
        ...
        let bob_link = bob.join("bin").join("bucket");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        // Simulate 5 old bucket binaries in downloads dir.
        for v in ["0.1.140", "0.1.141", "0.1.142", "0.1.143", "0.1.144"] {
            std::fs::write(d.join(format!("bucket-{}-macos-aarch64", v)), v).unwrap();
        }
        // Current version.
        std::fs::write(d.join("bucket-0.1.145-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "0.1.145").await;

        // Current must survive.
        assert!(d.join("bucket-0.1.145-macos-aarch64").exists(), "current");
        // Newest old version (0.1.144) must survive.
        assert!(d.join("bucket-0.1.144-macos-aarch64").exists(), "N-1");
        // Everything else should be deleted.
        assert!(
            !d.join("bucket-0.1.143-macos-aarch64").exists(),
            "0.1.143 should be deleted"
        );
        assert!(
            !d.join("bucket-0.1.142-macos-aarch64").exists(),
            "0.1.142 should be deleted"
        );
        assert!(
            !d.join("bucket-0.1.141-macos-aarch64").exists(),
            "0.1.141 should be deleted"
        );
        assert!(
            !d.join("bucket-0.1.140-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // bucket and bucket-pager should not interfere with each other.
        ...
        cleanup_old_downloads(d, "bucket", "0.1.141").await;

        assert!(d.join("bucket-0.1.141-macos-aarch64").exists());
        assert!(d.join("bucket-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
        assert!(
            d.join("bucket-pager-0.1.140-macos-aarch64").exists(),
            "pager untouched"
        );
        assert!(
            d.join("bucket-pager-0.1.141-macos-aarch64").exists(),
            "pager untouched"
        );
    }
    ...
    async fn test_cleanup_old_downloads_removes_stale_tmp_keeps_fresh_tmp() {
        ...
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64.tmp"), "partial").unwrap();
        make_stale(&d.join("bucket-0.1.140-macos-aarch64.tmp"));
        // Fresh tmp: a concurrent updater's in-flight download — kept, or
        // its atomic rename would fail with ENOENT.
        std::fs::write(d.join("bucket-0.1.142-macos-aarch64.77-0.tmp"), "inflight").unwrap();
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "current").unwrap();

        cleanup_old_downloads(d, "bucket", "0.1.141").await;

        assert!(
            !d.join("bucket-0.1.140-macos-aarch64.tmp").exists(),
            "stale tmp cleaned up"
        );
        assert!(
            d.join("bucket-0.1.142-macos-aarch64.77-0.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_keeps_fresh_versioned_binary() {
        ...
        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("bucket-{v}-macos-aarch64")), v).unwrap();
        }
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "current").unwrap();
        make_all_stale(d);
        ...
        std::fs::write(d.join("bucket-0.1.138-macos-aarch64"), "in-flight").unwrap();

        cleanup_old_downloads(d, "bucket", "0.1.141").await;

        assert!(d.join("bucket-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("bucket-0.1.140-macos-aarch64").exists(), "N-1 kept");
        assert!(
            d.join("bucket-0.1.138-macos-aarch64").exists(),
            "fresh just-renamed binary must NOT be deleted"
        );
        assert!(
            !d.join("bucket-0.1.139-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_skips_symlinks() {
        ...
        // bucket-latest is a symlink — must be skipped.
        let target = d.join("bucket-0.1.141-macos-aarch64");
        std::fs::write(&target, "current").unwrap();
        std::os::unix::fs::symlink(&target, d.join("bucket-latest")).unwrap();
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "bucket", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        std::fs::write(d.join("bucket-0.1.14-macos-aarch64"), "current").unwrap();
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64"), "old-140").unwrap();
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "old-141").unwrap();
        std::fs::write(d.join("bucket-0.1.13-macos-aarch64"), "old-13").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "0.1.14").await;

        // Current must survive.
        assert!(
            d.join("bucket-0.1.14-macos-aarch64").exists(),
            "current 0.1.14"
        );
        ...
        assert!(
            !d.join("bucket-0.1.13-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for bucket-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.148", "0.1.149", "0.1.150"] {
            std::fs::write(d.join(format!("bucket-pager-{}-linux-x64", v)), v).unwrap();
        }
        std::fs::write(d.join("bucket-pager-0.1.151-linux-x64"), "current").unwrap();
        ...
        assert!(
            !d.join("bucket-pager-0.1.148-linux-x64").exists(),
            "0.1.148 deleted"
        );
    }
    ...
    async fn test_cleanup_old_downloads_npm_layout() {
        // npm layout: files are just `bucket-{version}` (no platform suffix).
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("bucket-{}", v)), v).unwrap();
        }
        std::fs::write(d.join("bucket-0.1.141"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "0.1.141").await;

        assert!(d.join("bucket-0.1.141").exists(), "current");
        assert!(d.join("bucket-0.1.140").exists(), "N-1 kept");
        assert!(!d.join("bucket-0.1.139").exists(), "0.1.139 deleted");
        assert!(!d.join("bucket-0.1.138").exists(), "0.1.138 deleted");
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_alpha_versions() {
        // Alpha version filenames include pre-release tags:
        //   bucket-0.1.150-alpha.1-macos-aarch64
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        ...
        cleanup_old_downloads(d, "bucket", "0.1.150-alpha.1").await;

        // Current must survive.
        assert!(
            d.join("bucket-0.1.150-alpha.1-macos-aarch64").exists(),
            "current alpha"
        );
        // Newest old (0.1.149-alpha.1) kept as N-1.
        assert!(
            d.join("bucket-0.1.149-alpha.1-macos-aarch64").exists(),
            "N-1 alpha"
        );
        // Older alphas deleted.
        assert!(
            !d.join("bucket-0.1.148-alpha.2-macos-aarch64").exists(),
            "0.1.148-alpha.2 deleted"
        );
        assert!(
            !d.join("bucket-0.1.148-alpha.1-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        std::fs::write(d.join("bucket-0.1.148-macos-aarch64"), "stable-148").unwrap();
        std::fs::write(d.join("bucket-0.1.149-alpha.1-macos-aarch64"), "alpha-149").unwrap();
        std::fs::write(d.join("bucket-0.1.149-macos-aarch64"), "stable-149").unwrap();
        // Current is a stable release.
        std::fs::write(d.join("bucket-0.1.150-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "0.1.150").await;

        // Current must survive.
        assert!(d.join("bucket-0.1.150-macos-aarch64").exists(), "current");
        // Newest old is 0.1.149 stable (semver: 0.1.149 > 0.1.149-alpha.1).
        assert!(
            d.join("bucket-0.1.149-macos-aarch64").exists(),
        ...
        assert!(
            !d.join("bucket-0.1.148-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "v141").unwrap();

        // Invalid version string → cleanup must early-return without deleting.
        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "not-a-version").await;
        assert!(d.join("bucket-0.1.140-macos-aarch64").exists());
        assert!(d.join("bucket-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "bucket", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. bucket-latest, bucket-pager-* when prefix is bucket).
        std::fs::write(d.join("bucket-latest"), "alias").unwrap();
        std::fs::write(d.join("bucket-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        // bucket-latest and bucket-pager-* must be untouched.
        assert!(d.join("bucket-latest").exists());
        assert!(d.join("bucket-pager-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_unparseable_version_skipped() {
        ...
        std::fs::write(d.join("bucket-9garbage-macos-aarch64"), "junk").unwrap();
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(
            d.join("bucket-9garbage-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_multiplatform_in_same_dir() {
        ...
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "mac").unwrap();
        std::fs::write(d.join("bucket-0.1.141-linux-x86_64"), "linux").unwrap();
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64"), "old-mac").unwrap();
        std::fs::write(d.join("bucket-0.1.139-macos-aarch64"), "older-mac").unwrap();
        ...
        assert!(d.join("bucket-0.1.141-macos-aarch64").exists());
        assert!(d.join("bucket-0.1.141-linux-x86_64").exists());
        // N-1 (0.1.140) kept, older deleted.
        assert!(d.join("bucket-0.1.140-macos-aarch64").exists());
        assert!(!d.join("bucket-0.1.139-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_tmp_files_deleted_even_when_unparseable() {
        ...
        std::fs::write(d.join("bucket-junk.tmp"), "partial").unwrap();
        make_stale(&d.join("bucket-junk.tmp"));
        std::fs::write(d.join("bucket-0.1.140-macos-aarch64.tmp"), "partial2").unwrap();
        make_stale(&d.join("bucket-0.1.140-macos-aarch64.tmp"));
        std::fs::write(d.join("bucket-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(!d.join("bucket-junk.tmp").exists(), "junk tmp deleted");
        assert!(
            !d.join("bucket-0.1.140-macos-aarch64.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_three_olds_keeps_only_newest() {
        ...
        assert!(d.join("bucket-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("bucket-0.1.140-macos-aarch64").exists(), "N-1 only");
        assert!(!d.join("bucket-0.1.139-macos-aarch64").exists());
        assert!(!d.join("bucket-0.1.138-macos-aarch64").exists());
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_darwin_platform_recognized() {
        // The `darwin` alias for macOS is in PLATFORM_OS — versions on
        // bucket-X.Y.Z-darwin-* layouts must split correctly.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("bucket-0.1.140-darwin-arm64"), "v140").unwrap();
        std::fs::write(d.join("bucket-0.1.141-darwin-arm64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "bucket", "0.1.141").await;

        assert!(d.join("bucket-0.1.141-darwin-arm64").exists(), "current");
        assert!(d.join("bucket-0.1.140-darwin-arm64").exists(), "N-1");
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert_eq!(
            ...
            "Run `bucket update` to get the latest version."
        );
    }
    ...
    //   1. BUCKET_INSTALLER (npm | internal | gh-release | gh)
    //   2. BUCKET_MANAGED_BY_NPM       → npm
    //   3. BUCKET_MANAGED_BY_INTERNAL  → internal
    ...
    impl InstallerEnvGuard {
        fn isolate() -> Self {
            const VARS: &[&str] = &[
                "BUCKET_INSTALLER",
                "BUCKET_MANAGED_BY_NPM",
                "BUCKET_MANAGED_BY_INTERNAL",
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `BUCKET_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("BUCKET_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit BUCKET_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("BUCKET_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("BUCKET_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // BUCKET_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("BUCKET_INSTALLER", "internal");
            std::env::set_var("BUCKET_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    async fn test_windows_replace_exe_creates_dest_when_missing() {
        ...
        let dest = dir.path().join("bucket.exe");
    ...
    async fn test_windows_replace_exe_cleans_stale_old_backup() {
        ...
        let old = dir.path().join("bucket.exe.old");
    ...
    async fn test_windows_replace_exe_rollback_on_copy_failure() {
        // No stale .old: the aside IS bucket.exe.old, so this pins the
    ...
    async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
        ...
        let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with("bucket.exe.old.") && n.ends_with(".old"))
            })
            .collect();
        assert_eq!(
            asides.len(),
    ...
    async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
        ...
        let leftover_asides = std::fs::read_dir(dir.path())
            ...
            .filter(|e| {
                ...
                name.starts_with("bucket.exe.old.") && name.ends_with(".old")
            })
            .count();
    ...
    async fn test_windows_replace_exe_sweeps_accumulated_asides() {
        ...
        let aside_a = dir.path().join("bucket.exe.old.1234-0.old");
        let aside_b = dir.path().join("bucket.exe.old.1234-1.old");
