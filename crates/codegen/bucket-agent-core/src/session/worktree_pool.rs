...
//!   under `~/.bucket/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.bucket/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.bucket/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.bucket/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.bucket/worktree_pool/`, reads each `.pid` file, and checks
...
/// `CleanAll` mode. Placed under `~/.bucket/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(bucket_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(bucket_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                bucket_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
