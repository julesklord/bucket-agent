...
//!
//! Tests mutate `BUCKET_GOAL_CLASSIFIER` so they carry `serial`.
...
async fn answer_strategist(
    ...
    req: Box<bucket_tools::implementations::bucket_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        bucket_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
