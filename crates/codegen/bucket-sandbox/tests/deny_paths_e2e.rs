...
fn profile_from_env() -> bucket_sandbox::ProfileName {
    bucket_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !bucket_sandbox::is_inside_bwrap() {
            ...
            match bucket_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = bucket_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = bucket_sandbox::SandboxManager::support_info();
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".bucket")).expect("mkdir .bucket");
    fs::write(
        tmp.join(".bucket").join("sandbox.toml"),
