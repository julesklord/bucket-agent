//! AcpFsAdapter: implements `bucket-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates bucket-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use bucket_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps bucket-agent-core's ACP gateway to satisfy bucket-tools' AsyncFileSystem.
///
