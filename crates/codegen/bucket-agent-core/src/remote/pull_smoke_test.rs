...
//!
//! Run with: `cargo test -p bucket-agent-core -- pull_smoke --ignored --nocapture`
...
mod tests {
    ...
    fn load_prod_auth() -> Option<BucketAuth> {
        let path = crate::util::bucket_home::bucket_home().join("auth.json");
        let contents = std::fs::read_to_string(&path).ok()?;
        let store: BTreeMap<String, BucketAuth> = serde_json::from_str(&contents).ok()?;
    ...
    async fn smoke_push_pull_round_trip() {
        ...
        let auth = load_prod_auth().expect("No auth.json — run `bucket login`");
