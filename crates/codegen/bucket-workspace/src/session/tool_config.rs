...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<bucket_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<bucket_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<bucket_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<bucket_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn bucket_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            bucket_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`BUCKET_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("BUCKET_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = bucket_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `BUCKET_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: bucket_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<bucket_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$BUCKET_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: bucket_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
    ...
    /// (`$BUCKET_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(bucket_tools::computer::local::LocalFs)
            as Arc<dyn bucket_tools::computer::types::AsyncFileSystem>;
        let notification_handle = bucket_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    bucket_hub_sdk::AuthCredential::Bearer { token, .. } => {
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = bucket_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("bucket-workspace/{version}"),
    );
    headers.insert("x-bucket-client-version".to_string(), version.to_string());
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-BUCKET-Token-Auth".to_string(), "bucket-cli".to_string());
        headers.insert(
...
/// `BUCKET_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig
{
    use bucket_tools::implementations::bucket_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("BUCKET_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("BUCKET_WEB_FETCH_PROXY") {
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("BucketBuild:search_replace", Some(ToolKind::Edit)),
                tc("BucketBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("BucketBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("BucketBuild:search_replace".to_owned(), ToolKind::Edit),
            ("BucketBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("BucketBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("BucketBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("BucketBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("BucketBuild:read_file", None),
                test_support::tc("BucketBuild:grep", None),
                test_support::tc("BucketBuild:list_dir", None),
                test_support::tc("BucketBuild:search_replace", None),
                test_support::tc("BucketBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("BucketBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"BucketBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "BUCKET_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use bucket_tools::types::resources::{State, WebCitationCounter};
