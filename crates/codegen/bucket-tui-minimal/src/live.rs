...
use bucket_tui::render::Renderable;
use bucket_tui::scrollback::state::ScrollbackState;
use bucket_tui::scrollback::wrappers::EntryRenderer;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &bucket_tui::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    bucket_tui::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                bucket_tui::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(bucket_tui::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a bucket_tui::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
...
fn minimal_watchers(agent: &bucket_tui::app::agent_view::AgentView) -> turn_status::Watchers {
...
fn minimal_advance_phase_timer(
    agent: &mut bucket_tui::app::agent_view::AgentView,
) -> Option<bucket_tui::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &bucket_tui::app::agent_view::AgentView,
    activity: &Option<bucket_tui::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = bucket_tui::app::minimal_auto_set_for_mouse_leak();
    let switch_back = bucket_tui::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use bucket_tui::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = bucket_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<bucket_tui::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use bucket_tui::app::agent::AgentState;
        use bucket_tui::scrollback::RenderBlock;
        use bucket_tui::scrollback::entry::ScrollbackEntry;
        use bucket_tui::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &bucket_tui::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use bucket_tui::acp::tracker::TurnActivity;
        use bucket_tui::app::agent::AgentState;
        let theme = Theme::current();
        ...
        bucket_tui::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        bucket_tui::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            bucket_tui::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(bucket_agent_core::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &bucket_tui::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
