...
Agent mode runs Grok as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`bucket -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `bucket agent` command and apply to every mode. Pass them before the mode name, for example `bucket agent --model grok-build stdio`. The `stdio` subcommand itself takes no options.
...
```bash
bucket agent headless --grok-ws-url wss://your-relay.example.com/ws
```
