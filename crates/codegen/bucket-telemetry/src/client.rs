...
//!
//! Extracted from `bucket-agent-core::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`bucket-shell-` /
/// `bucket-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperBucket Heavy")
/// and JWT-derived keys ("superbucket_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperBucket Heavy" | "superbucket_heavy" => "superbucket_heavy",
        "SuperBucket" | "superbucket" => "superbucket",
        "SuperBucket Lite" | "superbucket_lite" => "superbucket_lite",
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("bucket-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("bucket-shell-turn"), "turn");
        assert_eq!(
            event_value("bucket-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("bucket-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("bucket-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperBucket"), "superbucket");
        assert_eq!(normalize_tier("SuperBucket Heavy"), "superbucket_heavy");
        assert_eq!(normalize_tier("superbucket_heavy"), "superbucket_heavy");
        ...
        assert_eq!(normalize_tier("SuperBucket Lite"), "superbucket_lite");
    ...
    /// (e.g. a future `"bucket-shell-ext-"` origin), an earlier `ALL` entry could
