...
//!
//! Extracted from the `OpenSuperbucketUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("BUCKET_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "BUCKET_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    bucket_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`bucket_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    bucket_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the SuperBucket upsell flow to attribute clicks to `referrer=bucket-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/bucket session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        ...
        assert_eq!(out, "https://bucket.com/superbucket?referrer=bucket-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://bucket.com/superbucket?referrer=other",
            "referrer",
            "bucket-build",
        );
        assert_eq!(out, "https://bucket.com/superbucket?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://bucket.com/superbucket?heavy=1",
            "referrer",
            "bucket-build",
        );
        assert_eq!(
            out,
            "https://bucket.com/superbucket?heavy=1&referrer=bucket-build"
        );
    }
    ...
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "bucket-build");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://bucket.com/superbucket?referrer=bucket-build";
