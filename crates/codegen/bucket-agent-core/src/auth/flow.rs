...
fn is_cached_credential_compatible(auth: &BucketAuth, bucket_com_config: &BucketComConfig) -> bool {
    let expected_issuer = bucket_com_config
        ...
        .or_else(|| bucket_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(bucket_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("BUCKET_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `BUCKET_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `bucket_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("BUCKET_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.bucket_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    bucket_com_config: &BucketComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = bucket_com_config.oidc.is_some(),
        has_oauth2 = bucket_com_config.oauth2.is_some(),
        has_external_auth = bucket_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, bucket_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, bucket_com_config)
        }) {
            bucket_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = bucket_com_config.auth_provider_command {
    ...
    // `bucket login --devbox` uses run_devbox_login and is not gated here.
    if !bucket_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / BUCKET_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(bucket_com_config) {
        return crate::auth::oidc::run_login_flow(bucket_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = bucket_com_config.oauth2 {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `bucket login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(bucket_com_config: &BucketComConfig) -> Option<BucketAuth> {
    ...
    auth_manager.configure_refresher(bucket_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(bucket_com_config).await {
        return Some(auth);
    }
    let bucket_home = bucket_home::bucket_home();
    let auth_manager = Arc::new(AuthManager::new(&bucket_home, bucket_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, bucket_com_config).await
}
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = bucket_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: BucketAuth) -> BucketAuth {
...
fn report_signed_in(auth: &BucketAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<BucketAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(bucket_com_config).await)
    } else {
        ensure_authenticated(bucket_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `bucket login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `BUCKET_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `bucket login` on an
    ...
    let authenticated = if devbox {
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.bucket_com_config) {
            eprintln!(
    ...
    // config was actually applied; `bucket setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `BUCKET_API_KEY` / `BUCKET_CODE_BUCKET_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        bucket_telemetry::external::set_identity(
            bucket_telemetry::external::IdentityAttrs::default(),
        );
        bucket_telemetry::external::flush();
...
/// `bucket logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let bucket_home = bucket_home::bucket_home();
    let auth_manager = AuthManager::new(&bucket_home, config.bucket_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via BUCKET_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("BUCKET_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    /// Run `f` with `BUCKET_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("BUCKET_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("BUCKET_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("BUCKET_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A bucket.com first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> BucketAuth {
        BucketAuth {
            key: key.into(),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), BucketComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only bucket_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = BucketComConfig {
            auth_provider_command: Some("printf '%s' bucket-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("bucket-ext-token".to_string()));
    }
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), BucketComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("BUCKET_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("BUCKET_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `bucket login` must use loopback, not device —
    ...
    fn oidc_auth(issuer: &str) -> BucketAuth {
        BucketAuth {
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = BucketAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // BUCKET_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Bucket: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `bucket login` passes `on_stderr=None`; stderr must be inherited so
