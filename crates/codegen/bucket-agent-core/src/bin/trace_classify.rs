...
//!       [--api-key <key> | $BUCKET_API_KEY | <bucket-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "bucket-4.5")]
    model: String,
    ...
    /// API key. Overrides `$BUCKET_API_KEY` when set; falls back to
    /// `$BUCKET_API_KEY`, then `<bucket-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$BUCKET_HOME` or `~/.bucket`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    bucket_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.bucket_home.is_none());
    }
    ...
    #[test]
    fn cli_bucket_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.bucket_home, Some(PathBuf::from("/tmp/scratch-bucket")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["bucket-4.5"]);
