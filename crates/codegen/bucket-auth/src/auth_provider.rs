...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`bucket-desktop`, `bucket login`). The `token` field MUST mirror
    ...
    /// Whether `X-BUCKET-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `BucketAuthCredentials::apply()` for the wire format contract.
