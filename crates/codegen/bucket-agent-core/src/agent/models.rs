...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `BUCKET_API_KEY` so a stray env key
...
struct Inner {
    ...
    gateway: RwLock<Option<bucket_acp::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&bucket_home, BucketComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: bucket_acp::AcpAgentGatewaySender) {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<bucket_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<bucket_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `bucket-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-bucket-build`
    /// → slug `bucket-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.bucket/models_cache.json` after an
    ...
    /// When another bucket process sharing `~/.bucket` (a `--no-leader` run, a
    /// newer client, bucket-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `bucket_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::bucket_home::bucket_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.bucket_version.as_deref() != Some(bucket_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            bucket_version: Some(bucket_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Bucket Build" both route to `bucket-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a bucket.com session; skip for BYOK.
...
fn resolve_prefetch_env_with_auth(auth: Option<BucketAuth>) -> Option<PrefetchEnv> {
...
/// explicit off switch must hold even when a stray login, `BUCKET_API_KEY`, or
...
fn resolve_prefetch_env(bucket_com_config: Option<BucketComConfig>) -> Option<PrefetchEnv> {
    let bucket_home = crate::util::bucket_home::bucket_home();
    let auth_manager = AuthManager::new(&bucket_home, bucket_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<BucketAuth>) -> Option<EarlyPrefetchHandle> {
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `bucket-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-bucket-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto bucket-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("bucket-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, BucketComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("bucket-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("bucket-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("bucket-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, BucketComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("bucket-4.5"),
            api_key: None,
            env_key: None,
            api_base_url: None,
        };
        ...
        prefetched.insert("bucket-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["bucket-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("bucket-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, BucketComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("bucket-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["bucket-3", "bucket-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "bucket-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with bucket-4 removed.
        let prefetched = make_prefetched(&["bucket-3", "bucket-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("bucket-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("bucket-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["bucket-3", "bucket-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "bucket-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("bucket-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "bucket-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["bucket-4.5", "bucket-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("bucket-4.5"));
        assert!(mgr.models().contains_key("bucket-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["bucket-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("bucket-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["bucket-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("bucket-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["bucket-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("bucket-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["bucket-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("bucket-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["bucket-4.5", "bucket-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("BUCKET_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("BUCKET_API_KEY");
        let _unset_legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `BUCKET_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("BUCKET_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient BUCKET_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `BUCKET_API_KEY`, `deployment_key`, AND a
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "bucket-build", Some("Auto")),
            make_entry_config_with_id(Some("bucket-build"), "bucket-build", Some("Bucket Build")),
            make_entry_config_with_id(
                Some("bucket-composer-2.5-fast"),
                "bucket-composer-2.5-fast",
                Some("Bucket Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("bucket-build"));
        assert!(map.contains_key("bucket-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "bucket-build",
            "auto entry should still route to bucket-build"
        );
        assert_eq!(map["bucket-build"].info.model, "bucket-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("bucket-build"), "bucket-build", Some("First")),
            make_entry_config_with_id(Some("bucket-build"), "bucket-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["bucket-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-bucket-build".to_string(),
            make_model_entry("bucket-build"),
        );
        catalog.insert("bucket-build".to_string(), make_model_entry("bucket-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("bucket-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "bucket-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        ...
        let persisted = acp::ModelId::new("bucket-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-bucket-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("bucket-build".to_string(), make_model_entry("bucket-build"));
        models.insert(
            "enterprise-bucket-build".to_string(),
            make_model_entry("bucket-build"),
        );
        ...
        let available = test_available_keys(&["enterprise-bucket-build", "bucket-4.3"]);

        let persisted = acp::ModelId::new("bucket-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("bucket-build".to_string(), make_model_entry("bucket-4.5"));
        models.insert("other".to_string(), make_model_entry("bucket-build"));

        let available = test_available_keys(&["bucket-build", "other"]);

        let persisted = acp::ModelId::new("bucket-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "bucket-build");
    }
