...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"BucketBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"BucketBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `BucketBuild:run_terminal_cmd` vs.
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("BucketBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("BucketBuild:read_file", "legacy-0.4.10"),
            ("BucketBuild:search_replace", "legacy-0.4.10"),
            ("BucketBuild:get_task_output", "legacy-0.4.10"),
            ("BucketBuild:kill_task", "legacy-0.4.10"),
            ("BucketBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("BucketBuild:run_terminal_cmd", "current"),
            ("BucketBuild:read_file", "current"),
            ("BucketBuild:search_replace", "current"),
            ("BucketBuild:list_dir", "current"),
            ("BucketBuild:grep", "current"),
            ("BucketBuild:kill_task", "current"),
            ("BucketBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "BucketBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "BucketBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "BucketBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "BucketBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "BucketBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "BucketBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // BucketBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("BucketBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "BucketBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("BucketBuild:run_terminal_cmd"));
        assert!(is_version_managed("BucketBuild:read_file"));
        assert!(is_version_managed("BucketBuild:search_replace"));
        assert!(is_version_managed("BucketBuild:list_dir"));
        assert!(is_version_managed("BucketBuild:grep"));
        assert!(is_version_managed("BucketBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("BucketBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "BucketBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "BucketBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("BucketBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("BucketBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "BucketBuild:web_search", None).unwrap();
