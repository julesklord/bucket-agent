...
use bucket_tools::reminders::task_completion::consumed_completion_ids;
use bucket_tools::types::output::{BashOutput, TextOutput, ToolOutput};
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use bucket_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use bucket_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
...
struct OneTaskTerminal {
    tasks: Vec<bucket_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl bucket_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: bucket_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        bucket_tools::computer::types::TerminalRunResult,
        bucket_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<bucket_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> bucket_tools::computer::types::KillOutcome {
        bucket_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<bucket_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<bucket_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> bucket_tools::computer::types::TaskSnapshot {
    bucket_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
