...
/// `BUCKET_OSC52_SINK=1` (the signal an inner `bucket` uses to trust OSC 52 over
...
fn wrap_osc52_sink_env_advertised_through_shell() {
    let (code, raw) = run_wrap(
        &["echo sink=$BUCKET_OSC52_SINK"],
        &[("SHELL", "/bin/sh"), ("BUCKET_OSC52_SINK", "0")],
    );

    assert!(
        raw.contains("sink=1"),
        "wrapped child must see BUCKET_OSC52_SINK=1 through the shell route\nraw:\n{raw}"
    );
    assert!(
        !raw.contains("sink=0"),
