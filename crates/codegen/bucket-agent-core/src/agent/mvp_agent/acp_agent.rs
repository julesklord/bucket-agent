...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        bucket_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::bucket_home::bucket_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    bucket_workspace::permission::cleanup_stale_permission_state(
        ...
        bucket_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_bucket_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("bucket-web") => client_type = ClientType::BucketWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("bucket-code-extension") => client_type = ClientType::Extension,
                Some("bucket-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.bucket_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.bucket_com_config.auth_provider_label.clone(),
                cfg.bucket_com_config.auth_provider_command.is_some(),
                cfg.bucket_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising bucket.com auth method",
            );
        }
        let preferred_method = self.cfg.borrow().bucket_com_config.preferred_method;
        ...
        bucket_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "bucket_home" : crate ::util::bucket_home::bucket_home().display()
        ...
        debug_assert!(
            ...
            Some(auth_method::AuthMethodKind::ApiKey)),
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("BUCKET_AGENT_METADATA");
                    serde_json::json!(
                        { "bucketShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (bucket_mcp::wire::MCP_SDK) :
                        ...
                        bucket_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().bucket_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::BUCKET_API_KEY_METHOD_ID => {
                if self.cfg.borrow().bucket_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set BUCKET_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(bucket_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().bucket_com_config.preferred_method, Some(crate
            ...
            auth_method::BUCKET_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let bucket_ctx = self.auth_manager.bucket_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_bucket_oauth(cli_oauth);
                tracing::debug!(
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: bucket.com/oidc handler set api_key (SessionToken)"
                    );
                    bucket_telemetry::unified_log::debug(
                        "auth: bucket.com/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        bucket_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || bucket_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = bucket_telemetry::context::collect_git_context(&cwd_str);
                let ev = bucket_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                bucket_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match bucket_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        bucket_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        bucket_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        bucket_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => bucket_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_checkout_allowed = bucket_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : bucket_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a bucket worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            bucket_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use bucket_workspace::session::git::RestoreKind;
            let outcome = bucket_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = bucket_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let is_bucket_build = persisted_model.0.starts_with("bucket-build");
        let same_family_fallback = if is_bucket_build {
            available.keys().find(|id| id.0.starts_with("bucket-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("bucket-build")).cloned()
        };
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    bucket_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = bucket_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = bucket_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(bucket_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            bucket_file_utils::trace_context::link_current_span_to_meta(
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: bucket_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                respond_to: tx,
                persist_ack: None,
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<bucket_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = bucket_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = bucket_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = bucket_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            bucket_file_utils::trace_context::link_current_span_to_meta(meta);
        }
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| bucket_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| bucket_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == bucket_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                bucket_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            bucket_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
