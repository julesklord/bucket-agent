...
//! TTY/console detach via `bucket_tty_utils::detach_std_command`); this module only
...
/// direct child.
///
/// `bucket_tty_utils::detach_std_command` runs `setsid` (EPERM fallback
/// `setpgid(0,0)`), so the child is its own group leader and its pgid equals its
/// pid. We send the signal directly because `bucket_tty_utils::ProcessGroup` only
