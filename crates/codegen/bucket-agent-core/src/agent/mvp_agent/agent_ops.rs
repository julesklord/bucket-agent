...
impl MvpAgent {
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::BucketAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
                if self.is_session_based_auth() {
                    ...
                    if auth.is_some() {
                        bucket_telemetry::unified_log::info(
    ...
    /// must not block the ACP response (bucket-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled bucket-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            bucket_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                bucket_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current BucketAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::BucketAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &bucket_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on bucket-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<bucket_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    bucket_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    bucket_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match bucket_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => bucket_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     `bucket.com`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    ...
    pub(crate) fn auth_type(&self) -> bucket_chat_state::AuthType {
    ...
    /// iff `should_advertise_xai_api_key`; otherwise `bucket.com`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().bucket_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_xai_api_key(
                self.cfg.borrow().bucket_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `bucket.com`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::BUCKET_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::BucketAuth) {
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<bucket_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == bucket_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = bucket_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == bucket_chat_state::AuthType::ApiKey
    ...
    /// Whether the current session is a personal bucket.com account on a gated
    ...
    /// SuperBucket upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    pub(super) fn prepare_image_gen_config(
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| bucket_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("bucket-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use bucket_tools::implementations::bucket_build::deploy_app::AppBuilderDeployerConfig;
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `BUCKET_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `BUCKET_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> bucket_tools::implementations::bucket_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: bucket_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: bucket_tools::implementations::bucket_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().bucket_com_config.auth_provider_command.clone(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<bucket_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> bucket_tools::implementations::bucket_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<bucket_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or BUCKET_RELAY_SYNC_ENABLED)
    ...
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<bucket_hooks_plugins_types::HooksListResponse> {
    ...
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: bucket_hooks_plugins_types::HooksAction,
    ) -> Option<bucket_hooks_plugins_types::ActionOutcome> {
        if matches!(action, bucket_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&bucket_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub async fn execute_plugins_action(
        ...
        let is_reload = matches!(action, bucket_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(bucket_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<bucket_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::BucketSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<bucket_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<bucket_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, bucket_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(bucket_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = bucket_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                bucket_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let bucket_home = crate::util::bucket_home::bucket_home();
    ...
    /// 5. `BUCKET_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `BUCKET_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`bucket_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<bucket_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> bucket_agent::AgentDefinition {
        use bucket_agent::AgentDefinition;
        let bucket_agent_env_set = std::env::var("BUCKET_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(bucket_agent::config::is_strict_harness_agent_type);
        if !bucket_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = bucket_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = bucket_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("BUCKET_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("bucket-build-concise") | Some("bucket_build_concise") => {
                AgentDefinition::bucket_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::bucket_build_plan()
                    }
                }
            }
            Some(name) => {
                bucket_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::bucket_build_plan)
            }
            None => AgentDefinition::bucket_build_plan(),
        };
        if !bucket_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = bucket_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn bucket_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                .unwrap_or(false)
                || std::env::var("BUCKET_LOC_TRACKING")
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = bucket_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = bucket_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    bucket_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = bucket_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                bucket_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            bucket_agent::config::ModelOverride::Override(id) => {
            ...
            bucket_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = bucket_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = bucket_tools::util::bucket_home::bucket_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".bucket").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = bucket_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                bucket_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = bucket_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = bucket_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    bucket_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        bucket_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == bucket_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        bucket_hunk_tracker::LocAggregate::LinesReverted {
