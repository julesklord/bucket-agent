//! Tracing layer for `target: "sampling_log"` → `~/.bucket/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `BUCKET_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "BUCKET_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = bucket_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
