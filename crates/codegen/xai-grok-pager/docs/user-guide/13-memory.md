...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.bucket/memory/MEMORY.md` for cross-project preferences:
...
By default, Grok watches `~/.bucket/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
2. Check the flag: `bucket --experimental-memory` or `GROK_MEMORY=1`.
