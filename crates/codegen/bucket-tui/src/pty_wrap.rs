...
//! system clipboard. This is the engine behind `bucket wrap` (see
...
///
/// This is the engine behind `bucket wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `bucket` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner bucket
    // reads this (see `bucket_tui_render::clipboard::osc52_sink_active`) to
    ...
    // unknown brands look incapable and the inner bucket would otherwise report
    // "Copy failed" despite the copy actually working).
    //
    // `CommandBuilder::new` inherits the full parent environment; `env` overlays
    // these two without clearing it. The canonical `BUCKET_OSC52_SINK` is
    ...
    cmd.env("BUCKET_OSC52_SINK", "1");
    cmd.env("LC_BUCKET_OSC52_SINK", "1");
    ...
    // detached — `bucket wrap` is short-lived and exits with the child.
...
///
/// Delegates to [`bucket_agent_core::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = bucket_agent_core::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = bucket_tui_render::clipboard::system_clipboard_get_image();
