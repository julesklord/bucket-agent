...
/// Resolved absolute path to bash. On Unix uses the `bucket_config` shell
/// resolution cascade (`$BUCKET_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        bucket_config::shell::unix_shell_path(bucket_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `bucket-tools`.
pub use bucket_tools::util::pager_env;
...
/// bucket-tui which renders its own UI and doesn't need raw ANSI codes).
///
