...
///
/// - `FullReplace` (default): bucket-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from bucket-shell's
...
/// time (token threshold only, matching bucket-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as bucket-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<bucket_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "bucket-4.20";
