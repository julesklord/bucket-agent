...
use bucket_tools::implementations::bucket_build::web_fetch::{
...
fn permission_mode_artifact_str(mode: bucket_telemetry::enums::PermissionMode) -> &'static str {
    use bucket_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.bucket/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new bucket process, new agent
            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && bucket_sandbox::should_auto_allow_bash()
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / BucketWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `BucketTUI | BucketPager | Desktop`).
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = bucket_acp::acp_gateway::<acp::AgentSide, _>(client);
    ...
    async fn auto_classifier_block_prompt_still_offers_always_approve() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // BucketPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::BucketPager);
                mgr.set_auto_mode(true);
