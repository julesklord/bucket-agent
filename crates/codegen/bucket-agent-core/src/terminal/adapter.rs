//! AcpTerminalAdapter: implements `bucket-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates bucket-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (bucket-desktop) spawns with `shell: true`
...
/// Wraps bucket-agent-core's ACP gateway to satisfy bucket-tools' TerminalBackend.
