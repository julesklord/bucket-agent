...
///
/// Parsed from `[session_search.remote_sync]` in `~/.bucket/config.toml`.
/// Default: disabled.
...
pub async fn maybe_upload_index(
    ...
    gcs_config: bucket_file_utils::TraceExportConfig,
...
impl ProxyHttpClient for crate::upload::gcs::TraceExportConfigWithAuth {
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        <Self as bucket_file_utils::gcs::StorageConfig>::proxy_http_client(self)
    }
}
