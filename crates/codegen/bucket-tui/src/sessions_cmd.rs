...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // If the user has previously run the interactive `bucket` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.bucket_com_config).await;
    ...
    let client = bucket_agent_core::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = bucket_agent_core::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = bucket_agent_core::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
