...
///
/// Stored here (rather than only in `bucket-agent-core`) so that query-extraction
...
/// `ActiveSubagentSummary` in bucket-tools. The mapping between them
/// happens in `run_compact_inner()` (bucket-agent-core).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (bucket-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in bucket-tools.
...
/// handled by the consumer (e.g. `bucket-agent-core`), which has access to
...
impl CompactionStateContext {
    ...
    /// model to re-read the same files. bucket-build retains
...
/// `bucket-chat-state` and `bucket-agent-core` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (bucket-build
    /// ordering).
    ...
    /// carrier and the bucket-build summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the bucket-build
...
/// `run_compact_inner` in `bucket-agent-core` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = bucket_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = bucket_sampling_types::repair_dangling_tool_calls(
        items,
        bucket_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use bucket_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    fn call(id: &str) -> bucket_sampling_types::ToolCall {
        bucket_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.bucket/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.bucket/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by bucket-agent-core:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use bucket_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use bucket_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use bucket_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use bucket_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
        ...
        assert!(
            ...
            "reasoning must be kept when strip_reasoning = false (Bucket backends)"
        );
