...
//! after every tool call to check for SKILL.md files in `.bucket/skills/`,
...
/// Directories that contain skill definitions (`.bucket/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".bucket", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.bucket/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.bucket/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.bucket/) are outside the git root so the walk breaks early.
