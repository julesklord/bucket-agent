...
crate::register_resource!("bucket_build", "Todo", TodoState);
...
impl bucket_tool_runtime::Tool for TodoWriteTool {
    ...
    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("bucket_build.Todo").is_some(),
            "TodoState should serialize under 'bucket_build.Todo'"
        );
