//! bucket-build's L5 wiring onto the shared full-replace engine
//! (`bucket_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](bucket_compaction::sample_full_replace_summary);
//! this module adapts bucket-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](bucket_compaction::FullReplaceError).
...
use bucket_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use bucket_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for bucket-build's full-replace pass.
///
...
/// else the structured bucket-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the bucket-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured bucket-build), ignoring the shared
...
/// Map bucket-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces bucket-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    bucket_telemetry::session_ctx::log_event(CompactionRetryDegraded {
