...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: bucket_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `bucket-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-bucket-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<bucket_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<bucket_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.bucket/hooks`,
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, bucket_chat_state::UsageTotals)>,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<bucket_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<bucket_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<bucket_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<bucket_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: bucket_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<bucket_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// tool-free model call (default `bucket-build-0.1` when available via
    ...
    /// `bucket-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<bucket_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<bucket_sampling_types::conversation::ConversationItem>>>,
    },
