...
use bucket_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use bucket_sampling_types::ApiBackend;

/// Process-level fallback for the `x-bucket-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "bucket-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "bucket-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-bucket-*` headers. Optional fields are skipped when empty/`None`.
struct BucketRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl BucketRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-bucket-conv-id", self.conv_id)
            .header("x-bucket-req-id", self.req_id)
            .header("x-bucket-model-override", self.model_id)
            .header("x-bucket-session-id", self.session_id)
            .header("x-bucket-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-bucket-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-bucket-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-bucket-user-id", id);
        }
        b
    }
}
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = bucket_sampling_types::reported_cost_ticks(
        value
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-bucket-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-bucket-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<bucket_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-bucket-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-bucket-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-bucket-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-bucket-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-bucket-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    fn format_request_headers(
        ...
        req_headers.push(Self::format_header("x-bucket-conv-id", x_bucket_conv_id));
        req_headers.push(Self::format_header("x-bucket-req-id", x_bucket_req_id));
        req_headers.push(Self::format_header("x-bucket-model-override", model_id));
        if include_accept {
    ...
    pub async fn chat_completion(
        ...
        let x_bucket_conv_id = &payload.x_bucket_conv_id.clone().unwrap_or_default();
        let x_bucket_req_id = &payload.x_bucket_req_id.clone().unwrap_or_default();
        ...
        let bucket_headers = BucketRequestHeaders {
            conv_id: x_bucket_conv_id,
            req_id: x_bucket_req_id,
            model_id: &model_id,
            session_id: payload.x_bucket_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_bucket_turn_idx.as_deref(),
            agent_id: payload.x_bucket_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_bucket_deployment_id.as_deref(),
            user_id: payload.x_bucket_user_id.as_deref(),
        };
        let http_request = bucket_headers
    ...
    pub async fn chat_completion_stream(
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_bucket_conv_id, x_bucket_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_bucket_conv_id = request.x_bucket_conv_id.as_deref().unwrap_or_default();
        let x_bucket_req_id = request.x_bucket_req_id.as_deref().unwrap_or_default();
        ...
        let bucket_headers = BucketRequestHeaders {
            ...
            req_id: x_bucket_req_id,
            model_id: &model_id,
            session_id: request.x_bucket_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_bucket_turn_idx.as_deref(),
            agent_id: request.x_bucket_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_bucket_deployment_id.as_deref(),
            user_id: request.x_bucket_user_id.as_deref(),
        };
        ...
        bucket_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = bucket_headers
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_bucket_conv_id, x_bucket_req_id, &model_id, false);
            let server_message = parse_error_bytes(bytes.as_ref());
    ...
    /// gate that adds the opt-in `x-bucket-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = bucket_headers
            .apply(self.post(self.endpoint("responses")))
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_bucket_conv_id = request.x_bucket_conv_id.clone();
        let x_bucket_req_id = request.x_bucket_req_id.clone();
        let x_bucket_session_id = request.x_bucket_session_id.clone();
        let x_bucket_turn_idx = request.x_bucket_turn_idx.clone();
        let x_bucket_agent_id = request.x_bucket_agent_id.clone();
        ...
        let extra_tools = bucket_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_bucket_conv_id = x_bucket_conv_id;
        wrapper.x_bucket_req_id = x_bucket_req_id;
        wrapper.x_bucket_session_id = x_bucket_session_id;
        wrapper.x_bucket_turn_idx = x_bucket_turn_idx;
        wrapper.x_bucket_agent_id = x_bucket_agent_id;
...
mod tests {
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_bucket_conv_id: None,
            x_bucket_req_id: None,
            x_bucket_session_id: None,
            x_bucket_turn_idx: None,
            x_bucket_agent_id: None,
            x_bucket_deployment_id: None,
            x_bucket_user_id: None,
            trace: None,
        };
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-BUCKET-token-auth".to_string(), "bucket-cli".to_string());
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the bucket-shell agent product.
        assert!(ua.starts_with("my-client bucket-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = bucket_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "bucket-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
