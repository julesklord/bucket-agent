...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&bucket_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("BUCKET_API_KEY") };
        } else {
            crate::auth::store_api_key(&bucket_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("BUCKET_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&bucket_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `bucket-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a bucket-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("bucket-asset:///{key}")),
        None => None,
    };
