...
/// Record the source repo root in `<worktree>/.git/bucket-worktree-source`.
///
...
fn record_main_repo_marker(source: &Path, worktree: &Path) {
    ...
    let marker = git_dir.join("bucket-worktree-source");
    if marker.exists() {
        return;
    }
...
mod tests {
    ...
    fn test_marker_written_for_standalone_git_dir() {
        ...
        bucket_test_utils::git::init_git_repo(&source);
        ...
        let marker = dest.join(".git/bucket-worktree-source");
    ...
    fn test_marker_skipped_for_linked_worktree() {
        ...
        assert!(!dest.join(".git/bucket-worktree-source").exists());
    }
