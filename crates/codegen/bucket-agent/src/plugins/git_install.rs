...
fn run_git_in_capture(cwd: &Path, args: &[&str]) -> Result<std::process::Output, String> {
    ...
    cmd.args(args)
        ...
        .envs(bucket_tools::util::pager_env());
    let output = cmd
        .output()
...
///
/// Each plugin has a data dir at `~/.bucket/plugin-data/<plugin_id>/`.
...
pub fn cleanup_plugin_data(repo: &InstalledRepo, scope: super::discovery::PluginScope) {
    let plugin_data_base = bucket_config::bucket_home().join("plugin-data");
