...
fn write_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &BucketAuth) {
...
async fn oidc_refresher_e2e_full_refresh_cycle() {
    ...
    let mgr = Arc::new(
        AuthManager::new(dir.path(), BucketComConfig::default()).with_proxy_base_url(&base_url),
    );
...
fn expired_oidc_for(base_url: &str) -> BucketAuth {
    BucketAuth {
        key: "old-expired-token".into(),
        create_time: Utc::now() - Duration::hours(2),
        user_id: "user-42".into(),
...
impl AuthSnapshot for EmptySnapshot {
    fn current(&self) -> Option<BucketAuth> {
        None
    }
    fn expired_auth(&self) -> Option<BucketAuth> {
        None
    }
    fn read_disk_auth(&self) -> Option<BucketAuth> {
        None
    }
