...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match bucket_http::shared_client()
        .get(url)
...
impl ManagedMcpState {
    ...
    /// connector re-authorized on bucket.com can retry. The reactive path must NOT
...
///
/// Servers with names starting with this prefix are managed by bucket.com —
...
///
/// Examples:
///   `bucket_com_linear`  → managed by bucket.com
///   `bucket_com_slack`   → managed by bucket.com
///   `my_company_api`   → user-managed (local)
///
/// Single source of truth lives in `bucket-workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use bucket_workspace::permission::resolution::MANAGED_MCP_PREFIX;

/// `"Linear"` -> `"bucket_com_linear"`. Shares normalization and the
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    bucket_agent_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
...
/// 1. Server name starts with `MANAGED_MCP_PREFIX` ("bucket_com_")
/// 2. Server URL matches a managed config endpoint
///
/// This prevents false injection if a user accidentally names a server `bucket_com_*`
...
/// Inject managed OAuth headers into `bucket_com_`-prefixed MCP servers.
///
...
mod tests {
    ...
    fn reauth_first_attempt_allowed_then_backs_off() {
        let mut state = ManagedMcpState::default();
        let now = Utc::now();
        assert!(state.reauth_allowed("bucket_com_slack", now));
        assert!(!state.reauth_is_terminal("bucket_com_slack"));

        state.record_reauth_failure("bucket_com_slack", now);
        // 2^1 = 2s backoff: not eligible now, eligible after the window.
        assert!(!state.reauth_allowed("bucket_com_slack", now));
        assert!(state.reauth_allowed("bucket_com_slack", now + chrono::Duration::seconds(2)));
        assert!(!state.reauth_is_terminal("bucket_com_slack"));
    }
    ...
    fn reauth_goes_terminal_after_max_attempts() {
        ...
        assert!(!state.reauth_allowed("bucket_com_slack", now + chrono::Duration::seconds(3600)));
    }
    ...
    fn reauth_success_resets_cooldown() {
        ...
        state.record_reauth_success("bucket_com_slack");
        assert!(state.reauth_allowed("bucket_com_slack", now));
        assert!(!state.reauth_is_terminal("bucket_com_slack"));
    }
    ...
    fn complete_fetch_preserves_cooldown_clear_resets_it() {
        ...
        let now = Utc::now();
        {
            ...
            for _ in 0..MAX_REACTIVE_REAUTH_ATTEMPTS {
                state.record_reauth_failure("bucket_com_slack", now);
                state.record_reauth_failure("bucket_com_linear", now);
            }
            assert!(state.reauth_is_terminal("bucket_com_slack"));
            assert!(state.reauth_is_terminal("bucket_com_linear"));
            ...
            assert!(state.reauth_allowed("bucket_com_slack", now));
            assert!(state.reauth_allowed("bucket_com_linear", now));
            assert!(!state.reauth_is_terminal("bucket_com_slack"));
            assert!(!state.reauth_is_terminal("bucket_com_linear"));
        }
    }
