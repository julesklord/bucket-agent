...
//! stay in each product host (for example `bucket-agent-core`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `bucket-sampling-types`. It is decoupled from both Bucket chat and
//! bucket-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — bucket-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Bucket chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Bucket chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Bucket chat and
//! bucket-build; hosts still own snapshotting and host-only sections).
...
// bucket-build's full-replace subsystem now lives under `code_compaction`;
