...
pub struct ConversationRequest {
    ...
    pub x_bucket_conv_id: Option<String>,
    pub x_bucket_req_id: Option<String>,
    pub x_bucket_session_id: Option<String>,
    pub x_bucket_turn_idx: Option<String>,
    pub x_bucket_agent_id: Option<String>,
    pub x_bucket_deployment_id: Option<String>,
    pub x_bucket_user_id: Option<String>,
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-bucket-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Bucket Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/bucket-compaction` operate over bucket-build's
...
impl bucket_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> bucket_compaction::CompactionRole {
        use bucket_compaction::CompactionRole;
        // bucket-build has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // bucket-build has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing bucket-build history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<bucket_compaction::CompactionFileRef> {
        // bucket-build `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `BucketTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<bucket_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl bucket_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/bucket-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`bucket-agent-core`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/bucket-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_bucket_conv_id: req.x_bucket_conv_id,
            x_bucket_req_id: req.x_bucket_req_id,
            x_bucket_session_id: req.x_bucket_session_id,
            x_bucket_turn_idx: req.x_bucket_turn_idx,
            x_bucket_agent_id: req.x_bucket_agent_id,
            x_bucket_deployment_id: req.x_bucket_deployment_id,
            x_bucket_user_id: req.x_bucket_user_id,
            trace: None,
        }
    }
}
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_bucket_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_bucket_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The bucket-shell agent does NOT use this path — a
...
/// ([crates/codegen/bucket-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use bucket_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // bucket-build has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "bucket-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("bucket-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("bucket-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("bucket-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        let assistant = AssistantItem {
            ...
            model_id: Some("bucket-3".to_string()),
            model_fingerprint: None,
            reasoning_effort: None,
        };
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("bucket-3".to_string()));
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in bucket-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.bucket/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.bucket/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.bucket/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("bucket-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_bucket_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_bucket_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("bucket-3");

        assert_eq!(item.model_id, Some("bucket-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("bucket-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("bucket-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("bucket-4.5"));
    ...
    //      (earlier bucket-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_bucket_build_shape() {
