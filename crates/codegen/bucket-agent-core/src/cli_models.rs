//! Data APIs for `bucket models`. Clients own display.
...
/// Status for the `bucket models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `bucket_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.bucket_com_config.bucket_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_xai_api_key(
            agent_config.bucket_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    ///
    /// Uses `BUCKET_AUTH_PATH` (not `BUCKET_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(BUCKET_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_BUCKET_API_KEY_ENV_VAR),
            EnvGuard::unset("BUCKET_AUTH"),
            EnvGuard::set("BUCKET_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("BUCKET_DEPLOYMENT_KEY"),
            EnvGuard::unset("BUCKET_WS_ORIGIN"),
            EnvGuard::unset("BUCKET_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, "bucket-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_BUCKET_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        ...
        assert_eq!(
            ...
            AuthStatus::LoggedIn("bucket.com".to_owned())
        );
    }
