//! `scroll-matrix` — scroll validation matrix sweep for bucket-tui.
//!
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against bucket-tui",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_bucket-tui,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves BUCKET_SCROLL_LOG against ITS cwd (the
