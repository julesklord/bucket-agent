...
use crate::auth::{AuthManager, AuthMode, BucketAuth, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (bucket.com
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(bucket_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = bucket_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`bucket_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    bucket_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    bucket_telemetry::unified_log::set_version(bucket_version::VERSION);
    ...
    bucket_file_utils::queue::cleanup_orphaned_uploads(
        &bucket_home::bucket_home(),
        bucket_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by bucket-desktop when spawning `bucket agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("BUCKET_CLIENT_VERSION") {
        crate::unified_log::info(
            "BUCKET_CLIENT_VERSION",
            None,
    ...
    // blocking stdin reads (see `bucket_acp::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `bucket agent stdio` is a local/interactive entrypoint (spawned by
            // bucket-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    bucket_telemetry::unified_log::set_version(bucket_version::VERSION);
    // `bucket agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `bucket agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a bucket.com session. \
        Run `bucket login` to sign in, or use `bucket agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.bucket_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'bucket login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `bucket login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&bucket_home::bucket_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the bucket build URL for the first-connection callback
    let bucket_code_url = format!("{}/build", ctx.bucket_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open Bucket Build: {} (press Enter to open in browser)",
                bucket_code_url
            );
            eprintln!();
            let url_for_open = bucket_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&BucketAuth>, session: &BucketAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's bucket.com relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `bucket agent leader`
...
///   duplication of mirroring every agent message to bucket.com.
///
...
/// When a bucket.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the bucket.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: bucket_version::VERSION.to_string(),
    })
    ...
    let auth: Option<BucketAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<BucketAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<BucketAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from bucket.com relay)
            ...
            // Start (or arm) the bucket.com relay. Eager by default — a bare
            // `bucket agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no bucket.com session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.bucket_com_config.auth_scope();
            // Gated on user_bucket_home() so a cwd-relative .bucket/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = bucket_config::user_bucket_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.bucket/models_cache.json
                            // (another bucket process fetched a fresher /v1/models
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> BucketAuth {
        BucketAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::BUCKET_OAUTH2_ISSUER.to_string()),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::BucketComConfig {
            bucket_ws_url: format!("ws://{addr}"),
            bucket_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    /// `bucket agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the bucket.com relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `bucket agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
