...
//! Backend environment presets for the Bucket CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `BUCKET_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct BucketBuildEndpoints {
...
pub enum BucketBuildEnvironment {
    #[default]
    Production,
}
impl BucketBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        BucketBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            BucketBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, BucketBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            BucketBuildEnvironment::Production => "BUCKET_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> BucketBuildEndpoints {
        match self {
            BucketBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `bucket.com/code` driving a
    ...
    /// `BUCKET_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for BucketBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BucketBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            BucketBuildEnvironment::Production.env_prefix(),
            "BUCKET_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "BUCKET_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn test_from_flags() {
        assert_eq!(
            BucketBuildEnvironment::from_flags(false, false),
            BucketBuildEnvironment::Production
        );
    }
}
