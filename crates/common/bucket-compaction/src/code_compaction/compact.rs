//! bucket-build's full-replace compaction pass.
//!
//! bucket-build does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `bucket-agent-core`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). bucket-build keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (bucket-build's
...
/// Run bucket-build's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam bucket-build's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // bucket-build appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce bucket-build's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_bucket_build_history_shape() {
        let llm_turns = vec![
