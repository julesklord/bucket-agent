...
//!
//! Extracted from `bucket-agent-core::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `bucket-agent-core` (and the pager/TUI that emit through it).
    Shell,
    /// `bucket-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "bucket-shell-",
            EmitterOrigin::Workspace => "bucket-workspace-",
        }
    }
}
...
/// event-name prefix (`bucket-shell-*` vs `bucket-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "bucket-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "bucket-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("bucket-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("bucket-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "bucket-workspace-turn");
    }
