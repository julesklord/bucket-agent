...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct BucketSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("bucket-build", "Bucket Build"),
            model("bucket-4.5", "Bucket 4.5"),
        ];
        let current = acp::ModelId::from("bucket-build");
        ...
        assert_eq!(selected_models[0].id, "bucket-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("bucket-build", "Bucket Build")];
        let current = acp::ModelId::from("bucket-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("bucket-build", "")];
        let current = acp::ModelId::from("bucket-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "bucket-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "bucket-build".to_string(),
            category: "model".to_string(),
            label: "Bucket Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "bucket-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Bucket Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn bucket_session_detail_serializes_camel_case() {
        let detail = BucketSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "bucket-build");
