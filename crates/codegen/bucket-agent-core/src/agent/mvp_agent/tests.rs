...
fn jwt_tier_claim_maps_free_and_paid() {
    ...
    assert_eq!(
        ...
        Some("superbucket_heavy")
    );
    assert_eq!(
        jwt_tier_claim(&jwt_with_tier(6)).as_deref(),
        Some("superbucket_lite")
    );
    assert_eq!(jwt_tier_claim(&jwt_with_tier(99)).as_deref(), Some("99"));
}
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::BucketAuth {
    crate::auth::BucketAuth {
        key: key.into(),
        auth_mode: mode,
...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "bucket-not-a-jwt");
    assert_eq!(
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ("superbucket", "BucketPro"),
        ("x_basic", "XBasic"),
        ("x_premium", "XPremium"),
        ("x_premium_plus", "XPremiumPlus"),
        ("superbucket_heavy", "SuperBucketPro"),
        ("superbucket_lite", "SuperBucketLite"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "BucketPro"));
...
mod hunk_tracking_mode {
    ...
    use bucket_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<bucket_sampling_types::conversation::ConversationItem> {
    use bucket_sampling_types::ToolCall;
    use bucket_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_bucket_build() {
    let prev = std::env::var("BUCKET_AGENT").ok();
    unsafe {
        std::env::remove_var("BUCKET_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("BUCKET_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return bucket-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = bucket_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `bucket-build-plan`, models in the catalog that still declare
/// `agent_type = "bucket-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `bucket-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_bucket_build_family() {
    let prev = std::env::var("BUCKET_AGENT").ok();
    unsafe {
        std::env::remove_var("BUCKET_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["bucket-build", "bucket-build-plan", "bucket-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for bucket-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("BUCKET_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use bucket_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".bucket").join("agents");
    ...
    assert_eq!(
        def.model,
        bucket_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("BUCKET_AGENT", v) },
        None => unsafe { std::env::remove_var("BUCKET_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `bucket-build` (a client profile name) to
/// `bucket-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("bucket-build", "bucket-build-plan"));
    assert!(harnesses_are_compatible("bucket-build-plan", "bucket-build"));
    assert!(harnesses_are_compatible("bucket-build", "bucket-build"));
    assert!(harnesses_are_compatible(
        "bucket-build-concise",
        "bucket-build-plan"
    ));
    assert!(harnesses_are_compatible(
        "remote-sidebar",
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("bucket-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "bucket-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_bucket_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "bucket-build-plan"),
        "bucket-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_bucket_build() {
    assert_eq!(
        resolve_required_agent_type(None, "bucket-build"),
        "bucket-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "bucket-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "bucket-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "bucket-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    let builder = bucket_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: bucket_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: bucket_tools::implementations::bucket_build::image_gen::ImageGenConfig::default(),
        video_gen_config: bucket_tools::implementations::bucket_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: bucket_tools::implementations::bucket_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: bucket_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = bucket_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        bucket_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: bucket_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            bucket_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(bucket_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: bucket_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "bucket-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: bucket_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("bucket-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("bucket-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("bucket-3", false, None)),
        (sid_b.clone(), make_test_handle("bucket-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use bucket_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("bucket-3", false, Some("bucket-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("bucket-3", false, Some("bucket-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("bucket-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("bucket-3", true, Some("bucket-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("bucket-3", true, Some("bucket-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("bucket-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("bucket-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("bucket-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::BucketWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::BucketAuth {
                key: "eligible".into(),
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, BucketComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), BucketComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::BucketAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, BucketComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let bucket_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("BUCKET_HOME", bucket_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), BucketComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("bucket-3", false, None);
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let bucket_acp::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("bucket-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("bucket-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("bucket-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("bucket-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("bucket-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("bucket-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::BucketWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::BucketTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::BucketWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::BUCKET_COM_METHOD_ID,
...
async fn auth_type_xai_api_key_no_current_returns_api_key() {
    ...
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::BUCKET_API_KEY_METHOD_ID,
    ));
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    agent.auth_manager.hot_swap(BucketAuth::test_default());
    assert!(agent.auth_manager.current().is_some());
    assert_eq!(agent.auth_type(), bucket_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), bucket_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `bucket_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.bucket_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `BUCKET_API_KEY` resolves and the kill
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{BUCKET_API_KEY_ENV_VAR, BUCKET_API_KEY_METHOD_ID};
    use bucket_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("BUCKET_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(BUCKET_API_KEY_METHOD_ID),
        "deployment-key user (BUCKET_API_KEY set, no kill switch) must fall \
         through to xai.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `BUCKET_API_KEY` present, the admin kill
/// switch keeps the fallthrough on interactive `bucket.com` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{BUCKET_COM_METHOD_ID, BUCKET_API_KEY_ENV_VAR};
    use bucket_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("BUCKET_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(BUCKET_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive bucket.com so BUCKET_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `bucket.com`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_bucket_com_without_credentials() {
    use crate::agent::auth_method::{
        BUCKET_COM_METHOD_ID, LEGACY_BUCKET_API_KEY_ENV_VAR, BUCKET_API_KEY_ENV_VAR,
    };
    use bucket_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("BUCKET_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(BUCKET_API_KEY_ENV_VAR);
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(BUCKET_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive bucket.com login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use bucket_tools::implementations::bucket_build::video_gen::{
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::BucketAuth::test_default());
    assert!(
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("bucket-4.5"), Some("bucket-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("bucket-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = bucket_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = bucket_workspace::WorkspaceOps::for_test();
    let toolset =
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<bucket_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::BUCKET_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("bucket-tui"));
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, BucketComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let bucket_acp::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use bucket_test_support::EnvGuard;
    use bucket_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("BUCKET_HOME", home.path());
    let _sim = EnvGuard::set(bucket_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, bucket_acp::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(bucket_acp::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use bucket_test_support::EnvGuard;
    use bucket_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("BUCKET_HOME", home.path());
    let _sim = EnvGuard::set(bucket_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(bucket_acp::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> bucket_announcements::RemoteAnnouncement {
    bucket_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<bucket_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = bucket_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = bucket_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<bucket_acp::AcpClientMessage>| {
            ...
            let bucket_acp::AcpClientMessage::ExtNotification(args) = msg else {
