...
impl SessionActor {
    ...
    /// sees "Bucket is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `bucket-4.5` (and
            // other tool-flavoured Bucket variants) reject the field at
            ...
            x_bucket_conv_id: Some(session_id_str.clone()),
            x_bucket_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_bucket_session_id: Some(session_id_str),
