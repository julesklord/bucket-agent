...
Bucket reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `BUCKET_API_KEY`, `BUCKET_MEMORY`)
3. **config.toml** (`~/.bucket/config.toml`)
...
Location: `~/.bucket/config.toml`

If the file does not exist, Bucket uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); Bucket writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `BUCKET_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). Bucket writes the change to
`[ui] vim_mode` in `~/.bucket/config.toml` immediately and applies it to every
...
`bucket` launches. Configure it from `/settings` → **Default screen mode**
...
`BUCKET_SCROLL_SPEED`, `BUCKET_SCROLL_MODE`, `BUCKET_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `BUCKET_SCROLL_LINES`. Precedence: env var →
...
(`BUCKET_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`BUCKET_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
env_key = "BUCKET_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Credential resolution: `api_key` > `env_key` > signed-in session token > `BUCKET_API_KEY`.
...
MCP servers can also be configured per-project in `.bucket/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.bucket/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.bucket/config.toml` (current dir) > `<repo-root>/.bucket/config.toml` > `~/.bucket/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `BUCKET_MEMORY=1`).
...
[subagents.models]
explore = "bucket-build"               # route to different models
```
...
`bucket inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. Bucket writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.bucket/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "bucket"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `bucket`. |
...
| Bucket Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, Bucket detects the terminal brand and selects the best
...
`$BUCKET_EVENT`, `$BUCKET_MESSAGE`, and `$BUCKET_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Bucket' -message '$BUCKET_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$BUCKET_MESSAGE' ntfy.sh/my-bucket-alerts"
...
fires, try `condition = "always"` as a fallback. Bucket supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by Bucket; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= BUCKET_EXTERNAL_OTEL)
...
|----------|-------------|
| `BUCKET_API_KEY` | API key from console.x.ai |
| `BUCKET_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `BUCKET_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `BUCKET_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `BUCKET_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `BUCKET_OIDC_ISSUER` | OIDC issuer URL |
| `BUCKET_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `BUCKET_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `BUCKET_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `BUCKET_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `BUCKET_AGENT` | Custom agent definition path or name |
| `BUCKET_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `BUCKET_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `BUCKET_LOG_FILE` log and headless stderr output |
...
| `BUCKET_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `BUCKET_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `BUCKET_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `BUCKET_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.bucket/config.toml` | Main configuration file |
| `~/.bucket/pager.toml` | TUI appearance configuration |
...
| `~/.bucket/lsp.json` | LSP server configuration (user-scoped) |
| `~/.bucket/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.bucket/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.bucket/` within your repository:

| File | What it configures |
|------|--------------------|
| `.bucket/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.bucket/config.toml`) |
| `.bucket/skills/` | Project-specific skills |
| `.bucket/hooks/` | Project-specific lifecycle hooks |
| `.bucket/agents/` | Project-specific agent definitions |
| `.bucket/lsp.json` | LSP server configuration |
| `.bucket/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.bucket/lsp.json` | All projects |
| Project | `.bucket/lsp.json` | Current repository |
...
1. **Project** -- `.bucket/lsp.json`
2. **User** -- `~/.bucket/lsp.json`
