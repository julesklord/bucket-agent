...
pub(crate) const BUCKET_SCREEN_MODE_ENV: &str = "BUCKET_SCREEN_MODE";
...
/// `bucket "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `bucket "fix the bug"`). Must not re-fire
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit BUCKET_SCREEN_MODE.
    if want_minimal {
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{BUCKET_SCREEN_MODE_ENV}={mode} bucket {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(BUCKET_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`BUCKET_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value
...
/// Consume the one-shot screen-mode override env (see [`BUCKET_SCREEN_MODE_ENV`]).
///
...
/// (tool shells, workers, nested `bucket` invocations) would inherit a forced
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(BUCKET_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(BUCKET_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["bucket", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["bucket", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_bare_worktree_without_eating_next_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--worktree", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn strips_positional_prompt() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--no-leader", "fix the bug"]),
            "sid",
            true,
        );
        assert_eq!(
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `bucket --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn keeps_equals_form_and_short_model_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "-m", "bucket-4", "--cwd=/tmp/proj", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `bucket --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
            args(&["bucket", "--resume", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry BUCKET_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "BUCKET_SCREEN_MODE=fullscreen bucket --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "BUCKET_SCREEN_MODE=minimal bucket --minimal --resume abc-sid"
        );
    }
