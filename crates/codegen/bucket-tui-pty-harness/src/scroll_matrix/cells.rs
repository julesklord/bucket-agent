...
pub struct ExpectedProfile {
    ...
    pub invert: bool,
    /// Speed multiplier (NOT the 1-100 setting): `BUCKET_SCROLL_SPEED=100`
...
pub struct MatrixCell {
    ...
    /// appends `BUCKET_SCROLL_LOG`; the harness's env strips guarantee the
...
const SPEED100: (&str, &str) = ("BUCKET_SCROLL_SPEED", "100");
const MODE_WHEEL: (&str, &str) = ("BUCKET_SCROLL_MODE", "wheel");
const MODE_TRACKPAD: (&str, &str) = ("BUCKET_SCROLL_MODE", "trackpad");
const LINES1: (&str, &str) = ("BUCKET_SCROLL_LINES", "1");
const INVERT: (&str, &str) = ("BUCKET_INVERT_SCROLL", "1");
...
mod tests {
    ...
    /// pager would (`from_terminal_context` + the `BUCKET_SCROLL_*` env
    ...
    fn derive_expected(env: &'static [(&'static str, &'static str)]) -> ExpectedProfile {
        ...
        if let Some(lines) = get("BUCKET_SCROLL_LINES") {
        ...
        let speed = match get("BUCKET_SCROLL_SPEED") {
            None | Some("50") => 1.0,
            Some("100") => 6.0,
            Some(other) => panic!("unmapped BUCKET_SCROLL_SPEED {other:?}"),
        };
        ExpectedProfile {
            mode: get("BUCKET_SCROLL_MODE").unwrap_or("auto"),
            ept,
            wheel_lpt,
            trackpad_lpt,
            invert: get("BUCKET_INVERT_SCROLL") == Some("1"),
            speed,
        }
    }
