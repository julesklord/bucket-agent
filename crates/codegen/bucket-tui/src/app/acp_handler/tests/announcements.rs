#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.bucket` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.bucket` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.bucket` reads).
    #[test]
