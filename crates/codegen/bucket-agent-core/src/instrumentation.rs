//! Shim — see `bucket_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `bucket_agent_core::instrumentation_timer!`). Keeping the macro here
...
pub use bucket_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `bucket_agent_core::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `bucket_agent_core`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `bucket_agent_core::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `bucket_telemetry::instrumentation`.
#[macro_export]
