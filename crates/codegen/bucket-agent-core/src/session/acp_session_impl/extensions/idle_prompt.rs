...
use bucket_agent_lifecycle::LocalExtensionRegistryBuilder;
use bucket_agent_lifecycle::{LocalSessionLifecycleContributor, LocalTurnLifecycleContributor};
use bucket_agent_lifecycle::{
...
/// `BUCKET_IDLE_NOTIFICATION_DELAY_MS` overrides it (used by E2E tests).
fn idle_notification_delay() -> Duration {
    resolve_idle_notification_delay(std::env::var("BUCKET_IDLE_NOTIFICATION_DELAY_MS").ok())
}
...
/// Covered by the headless E2E via `BUCKET_IDLE_NOTIFICATION_DELAY_MS`.
...
mod idle_notification_delay_tests {
    ...
    /// Pins the public `BUCKET_IDLE_NOTIFICATION_DELAY_MS` contract: a valid override is interpreted as milliseconds (the E2E seam depends on this).
    #[test]
