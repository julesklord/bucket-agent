...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "bucket-shell", alias = "bucket_shell")]
    Generic,
    /// Bucket TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "bucket-tui", alias = "bucket_tui")]
    BucketTUI,
    /// Bucket Web client - identified by clientIdentifier "bucket-web"
    #[serde(rename = "bucket_web")]
    BucketWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "bucket-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Bucket Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to BucketTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"bucket-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"bucket_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "bucket-pager", alias = "bucket_pager")]
    BucketPager,
    /// Bucket Desktop (Electron) client - identified by clientIdentifier "bucket-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "bucket_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `bucket-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "bucket-shell",
            Self::BucketTUI => "bucket-tui",
            Self::BucketWeb => "bucket-web",
            Self::Nebula => "nebula",
            Self::Extension => "bucket-code-extension",
            Self::BucketPager => "bucket-pager",
            Self::Desktop => "bucket-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"bucket-web"`, `"bucket-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("bucket-web") => Self::BucketWeb,
            Some("nebula") => Self::Nebula,
            Some("bucket-code-extension") => Self::Extension,
            Some("bucket-desktop") => Self::Desktop,
            Some("bucket-pager") => Self::BucketPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::BucketTUI | Self::BucketPager => "tui",
            Self::BucketWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&bucket_tools::types::ToolInput> for AccessKind {
    fn from(input: &bucket_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.bucket/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use bucket_tools::implementations::bucket_build_hashline::edit::types::HashlineEditInput;
        use bucket_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use bucket_tools::implementations::bucket_build::bash::BashToolInput;
        use bucket_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use bucket_tools::implementations::use_tool::UseToolInput;
        use bucket_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use bucket_tools::implementations::bucket_build::monitor::types::MonitorInput;
        use bucket_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use bucket_tools::implementations::bucket_build::web_fetch::WebFetchInput;
        use bucket_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use bucket_tools::implementations::bucket_build::web_search::WebSearchInput;
        use bucket_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_bucket_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("bucket-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("bucket_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
