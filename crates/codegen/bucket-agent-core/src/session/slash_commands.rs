...
use bucket_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's Bucket Build flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use bucket_tools::implementations::skills::skill::{
...
/// `bucket-tools` so it stays identical to the pager's `LoopCommand` and the
