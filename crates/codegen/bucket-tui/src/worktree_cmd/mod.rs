...
async fn dispatch(command: WorktreeCommand, tx: &bucket_acp::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &bucket_acp::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &bucket_acp::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
...
mod tests {
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.bucket/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.bucket/worktrees.db");
    }
