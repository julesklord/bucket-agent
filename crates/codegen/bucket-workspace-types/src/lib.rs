//! Wire types for the `bucket-workspace` API.
//!
...
//! `bucket-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `bucket-workspace-types` (instead of `bucket-workspace` or
/// `bucket-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `bucket_workspace::permission`
