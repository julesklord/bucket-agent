...
//!
//! Uses `bucket-tui-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.bucket/auth.json` from `BUCKET_AUTH_JSON` (path) or the
//! developer's `~/.bucket/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p bucket-tui --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use bucket_tui_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.bucket/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("BUCKET_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        let dest = bucket.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.bucket/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("BUCKET_HOME".into(), bucket.display().to_string()),
        ...
        ("TMUX".into(), "".into()),
        // Do not set BUCKET_API_KEY — prefer OIDC entry in auth.json (pty_e2e pattern).
    ];
    // Pin the feature gate explicitly so the cycle is deterministic regardless
    // of the developer's shell. `BUCKET_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (bucket_config::
    ...
    env.push((
        "BUCKET_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("BUCKET_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (bucket-tui-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "BUCKET_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
