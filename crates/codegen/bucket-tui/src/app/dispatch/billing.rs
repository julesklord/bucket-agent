...
/// Whether the user is at the highest subscription tier (SuperBucket Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("superbucket_heavy") and CCP display names ("SuperBucket Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "superbucket_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://bucket.com/superbucket?referrer=bucket-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://bucket.com?_s=usage";
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(bucket_agent_core::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Bucket Build usage limit for now. Get SuperBucket for much higher limits, or try again later: https://bucket.com/superbucket?referrer=bucket-build";
...
///
/// **`max_tier = true`** (positively identified as SuperBucket Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        bucket_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Bucket Build",
            CreditLimitCardAction::PurchaseCredits,
            bucket_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            bucket_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(bucket_telemetry::events::CreditLimitUpsellShown {
            surface: bucket_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(bucket_telemetry::events::CreditLimitUpsellShown {
        surface: bucket_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            bucket_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperBucketUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_superbucket_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperBucket upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_superbucket_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperBucket upsell modal. Controls the heading
...
fn open_superbucket_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperBucketUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperBucket.",
            SuperBucketUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(bucket_telemetry::events::SuperBucketUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperBucket".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperBucket Heavy".into(),
            description: "Get the most out of Bucket Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /superbucket page lists
...
pub(super) fn handle_gate_refreshed(
    app: &mut AppView,
    settings: Option<bucket_agent_core::util::config::RemoteSettings>,
) -> Vec<Effect> {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<bucket_agent_core::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<bucket_agent_core::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_superbucket_url(app: &mut AppView) -> Vec<Effect> {
    ...
    let url = app
        ...
        .unwrap_or("https://bucket.com/superbucket?referrer=bucket-build");
    // Funnel attribution: tag CLI-originated SuperBucket upsell clicks
    // with `referrer=bucket-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "bucket-build");
