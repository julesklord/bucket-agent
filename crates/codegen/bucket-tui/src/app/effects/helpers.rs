...
pub(super) fn format_acp_error(err: &acp::Error, is_api_key_auth: bool) -> String {
    ...
    if err.code == acp::ErrorCode::InvalidParams && let Some(data) = &err.data
        && let Some(msg) = bucket_agent_core::sampling::error::error_detail_from_data(data)
        && !msg.is_empty()
    {
        return msg;
    }
...
) -> (bool, Option<String>, Option<bucket_workspace::session::git::RestoreDegree>) {
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(bucket_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(bucket_fast_worktree::ENOSPC_OS_MESSAGE)
    {
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `bucket-build` (default)         | `false`            |
/// | false | true      | false    | `bucket-build` (default)         | `false`            |
/// | false | false     | true     | `bucket-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `bucket-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `bucket-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `bucket-build-plan`              | `false`            |
/// | true  | false     | true     | `bucket-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `bucket-build-plan`              | omitted (shell gate) |
///
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `bucket-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("bucket-build-plan"),
            (true, false, _) => Some("bucket-build-plan-no-subagents"),
            (false, _, true) => Some("bucket-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `BUCKET_AGENT` — the env var chooses the *agent*, but the tool-strip is
    ...
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        ...
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("BUCKET_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("BUCKET_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
...
pub(super) fn extract_first_user_prompt(
    info: &bucket_agent_core::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = bucket_agent_core::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use bucket_agent_core::sampling::{AssistantItem, ConversationItem, UserItem};
...
pub(super) fn parse_session_picker_entries(
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            use bucket_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = bucket_agent_core::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && bucket_agent_core::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            bucket_agent_core::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            bucket_agent_core::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            bucket_agent_core::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            bucket_agent_core::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            bucket_agent_core::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            bucket_agent_core::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            bucket_agent_core::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            bucket_agent_core::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            bucket_agent_core::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            bucket_agent_core::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            bucket_agent_core::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            bucket_agent_core::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            bucket_agent_core::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            bucket_agent_core::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            bucket_agent_core::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            bucket_agent_core::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            bucket_agent_core::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            bucket_agent_core::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            bucket_agent_core::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            bucket_agent_core::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            bucket_agent_core::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            bucket_agent_core::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            bucket_agent_core::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            bucket_agent_core::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            bucket_agent_core::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            bucket_agent_core::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            bucket_agent_core::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            bucket_agent_core::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            bucket_agent_core::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            bucket_agent_core::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            bucket_agent_core::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            bucket_agent_core::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            bucket_agent_core::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = bucket_agent_core::util::config::update_config(|cfg| {
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &bucket_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == bucket_hooks_plugins_types::OutcomeStatus::Success
}
...
/// serializes (`bucket_agent_core::extensions::task::KillTaskResponse` +
/// `bucket_agent_core::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<bucket_tools::types::KillOutcome> {
    use bucket_agent_core::extensions::task::KillTaskResponse;
    use bucket_agent_core::session::result::ExtMethodResult;
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use bucket_agent_core::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: bucket_agent_core::extensions::billing::BillingConfig,
...
pub(super) fn parse_auto_topup_response(
    ...
    use bucket_agent_core::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.bucket` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match bucket_agent_core::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
