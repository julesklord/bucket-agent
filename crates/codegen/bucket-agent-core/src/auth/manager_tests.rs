...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> BucketAuth {
    BucketAuth {
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older bucket version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = BucketAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = BucketAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(BucketAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = BucketAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = BucketAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `bucket login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `bucket logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `bucket login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_bucket_code_access`. The `&mut BucketAuth` signature already
...
/// contributor adding a token-like field to both `BucketAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = BucketAuth {
        key: "ROT_KEY".into(),
        ...
        has_bucket_code_access: Some(true),
    ...
    assert_eq!(disk.has_bucket_code_access, snapshot.has_bucket_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    let _xai = EnvGuard::unset("BUCKET_API_KEY");
    let _legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
    mgr.hot_swap(BucketAuth {
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = BucketAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), BucketComConfig::default()).with_proxy_base_url(&base);

    let mut auth = BucketAuth {
        key: "tok".into(),
        ..BucketAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> BucketAuth {
    BucketAuth {
        key: team_jwt(principal_id),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        oidc_issuer: Some(crate::auth::config::BUCKET_OAUTH2_ISSUER.to_string()),
        oidc_client_id: Some("client".into()),
        ..BucketAuth::test_default()
    }
}
...
async fn auth_rejects_token_refreshed_into_wrong_team() {
    ...
    impl TokenRefresher for WrongTeamRefresher {
        async fn refresh(&self, _reason: RefreshReason) -> crate::auth::refresh::RefreshOutcome {
            crate::auth::refresh::RefreshOutcome::Success(Box::new(BucketAuth {
                key: self.jwt.clone(),
                auth_mode: AuthMode::Oidc,
                refresh_token: Some("rt-new".into()),
                expires_at: Some(Utc::now() + Duration::hours(1)),
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || BucketAuth {
        key: "bucket-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..BucketAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), BucketComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("bucket-cached-key".to_string()),
...
async fn shared_api_key_provider_static_fallthrough() {
    ...
    {
        let _legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
        let _key = EnvGuard::set("BUCKET_API_KEY", "env-only-key");
        assert_eq!(
    ...
    {
        let _key = EnvGuard::set("BUCKET_API_KEY", "env-should-lose");
        mgr.hot_swap(BucketAuth {
...
async fn shared_api_key_provider_kill_switch_blocks_static() {
    ...
    let _key = EnvGuard::set("BUCKET_API_KEY", "blocked");
    let dir = tempfile::tempdir().unwrap();
...
async fn shared_api_key_provider_oidc_preferred_blocks_static() {
    ...
    let _key = EnvGuard::set("BUCKET_API_KEY", "should-not-use");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        BucketComConfig {
...
async fn shared_api_key_provider_api_key_preferred_skips_session() {
    ...
    let _legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
    let _key = EnvGuard::set("BUCKET_API_KEY", "static-preferred");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        BucketComConfig {
...
async fn shared_api_key_provider_sync_falls_through_when_session_expired() {
    ...
    let _legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
    let _key = EnvGuard::set("BUCKET_API_KEY", "static-after-expiry");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
    mgr.hot_swap(BucketAuth {
        key: "expired-oidc".into(),
...
async fn shared_api_key_provider_sync_buffered_session_beats_static() {
    ...
    let _legacy = EnvGuard::unset("BUCKET_CODE_BUCKET_API_KEY");
    let _key = EnvGuard::set("BUCKET_API_KEY", "leftover-static");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use bucket_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use bucket_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, BucketComConfig::default()));
