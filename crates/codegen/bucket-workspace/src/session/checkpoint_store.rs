...
/// Directory (under `<cwd>/.bucket`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.bucket/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
