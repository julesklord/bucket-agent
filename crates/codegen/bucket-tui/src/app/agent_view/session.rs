...
impl AgentView {
    ...
    pub fn apply_full_context_info(&mut self, next: bucket_agent_core::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = bucket_token_estimation::usage_percentage_u8(used, snap.total);
