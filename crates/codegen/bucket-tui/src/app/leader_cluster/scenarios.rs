...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + bucket_home OnceLock in the shared lib test binary); run: cargo test -p bucket-tui --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<bucket_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_bucket_home().join("sessions");
