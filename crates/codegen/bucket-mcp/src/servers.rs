...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"bucket_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.bucket/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: bucket_file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: bucket_file_utils::events::EventWriter::noop(),
    ...
    /// `BucketClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: bucket_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &bucket_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `bucket-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use bucket_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> bucket_file_utils::events::McpErrorCategory {
        use bucket_file_utils::events::McpErrorCategory;
        match self {
...
/// `bucket_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl bucket_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> bucket_tool_protocol::ToolId {
        ...
        bucket_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| bucket_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        bucket_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(bucket_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(bucket_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(bucket_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(bucket_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        match result {
            ...
            Ok(Err(e)) => Err(bucket_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(bucket_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(bucket_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(bucket_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(bucket_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(bucket_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(bucket_file_utils::events::Event::McpTransportDecodeError {
...
/// Uses [`BucketClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, BucketClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`BucketClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`BucketClientHandler`]** constructed by
...
/// and the [`BucketClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to bucket-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: bucket_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, BucketClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("bucket-shell-{server_name}"),
                bucket_version::VERSION.to_string(),
            ),
        )
    ...
    /// Build the [`BucketClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> BucketClientHandler {
        BucketClientHandler {
    ...
    /// references (the [`BucketClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _list_tools_timer = bucket_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.bucket/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = bucket_config::bucket_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`bucket_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "bucket_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("bucket-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = bucket_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = bucket_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        bucket_telemetry::session_ctx::log_event(
                            bucket_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: bucket_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    bucket_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            bucket_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = bucket_telemetry::instrumentation::timer("mcp_start_servers");
...
#[derive(Debug)]
pub struct BucketClientHandler {
...
impl ClientHandler for BucketClientHandler {
    ...
    // See the [`BucketClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            bucket_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("bucket_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("BUCKET_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("bucket_com_FIGMA", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_bucket_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use bucket_hub_sdk::LocalRegistry;
        use bucket_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = bucket_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = bucket_file_utils::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = BucketClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'bucket_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            bucket_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
