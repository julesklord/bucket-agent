...
//! `ToolInput`, proto, or runtime). A future `bucket-tool-taxonomy` leaf crate
...
///   share it (bucket `read_file` → `"Read"`).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::BucketBuild,
