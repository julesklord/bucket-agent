...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use bucket_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(bucket_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = bucket_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != bucket_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != bucket_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("bucket-subagent-worktrees").join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: bucket_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = bucket_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        bucket_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(bucket_fast_worktree::WorktreeKind::Subagent)
    ...
    {
        use bucket_tools::implementations::bucket_build::task::MAX_SUBAGENT_DEPTH;
    ...
    {
        let reminder = bucket_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = bucket_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            bucket_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = bucket_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = bucket_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<bucket_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = bucket_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<bucket_tools::registry::types::ToolConfig> = vec![
            (& bucket_build::ReadFileTool).into(), (& bucket_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = bucket_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == bucket_hooks::event::HookEventName::Stop {
                            s.event = bucket_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    bucket_agent::config::McpServerRef::Named(name) => {
                    ...
                    bucket_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            bucket_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            bucket_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            bucket_chat_state::CompactionMode::Summary,
            ...
            if inherit_skills {
                ...
                bucket_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    bucket_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            bucket_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        bucket_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        bucket_telemetry::events::Outcome::Cancelled
    } else {
        bucket_telemetry::events::Outcome::Error
    };
    bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/bucket/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
