//! `paste-latency` — clipboard paste latency benchmark for `bucket-tui`.
//!
...
//!
//! ```bash
//! cargo bench -p bucket-tui-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p bucket-tui-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/bucket-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for bucket-tui (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
