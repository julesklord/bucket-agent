...
//! `BUCKET_HOME` (the `bucket_home` `OnceLock` allows a single value per process)
...
/// Shared temp dir used as BUCKET_HOME for the whole test binary (the bucket_home
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("BUCKET_HOME", &path);
            ...
            for var in [
                "BUCKET_DEPLOYMENT_KEY",
                "BUCKET_MANAGED_CONFIG",
                "BUCKET_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS",
                "BUCKET_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
                "HTTP_PROXY",
                "HTTPS_PROXY",
                "ALL_PROXY",
                "http_proxy",
            ...
            std::env::set_var("BUCKET_DEPLOYMENT_CONFIG_BACKOFF_MS", "10");
        }
        path
    })
}
...
fn write_team_auth_expiry(home: &std::path::Path, team_id: &str, expires_at: &str) {
    let scope = bucket_agent_core::auth::BucketComConfig::default().auth_scope();
...
async fn team_sync_writes_files() {
    ...
    let wrote = bucket_agent_core::managed_config::sync()
        .await
...
async fn marker_dir_squat_is_cleared_and_marker_written() {
    ...
    let marker_path = home.join(bucket_config::MANAGED_CONFIG_CACHE_FILE);
...
async fn padded_team_id_is_one_identity() {
    ...
    bucket_agent_core::managed_config::sync()
        .await
        .expect("sync should succeed");
    let marker =
        std::fs::read_to_string(home.join(bucket_config::MANAGED_CONFIG_CACHE_FILE)).unwrap();
    let v: serde_json::Value = serde_json::from_str(&marker).unwrap();
    assert_eq!(
        v["principal"].as_str(),
        Some("team-007"),
    ...
    assert_eq!(
        bucket_config::confirmed_team_switch("team-007"),
        None,
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    assert!(!bucket_agent_core::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !bucket_agent_core::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = bucket_agent_core::managed_config::sync()
        .await
...
async fn served_then_deleted_refetches_best_effort() {
    ...
    let auth_manager = std::sync::Arc::new(bucket_agent_core::auth::AuthManager::new(
        &home,
        bucket_agent_core::auth::BucketComConfig::default(),
    ));
    bucket_agent_core::managed_config::ensure_managed_policy_present(&auth_manager).await;
    assert!(
        home.join("requirements.toml").exists(),
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !bucket_agent_core::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        bucket_agent_core::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        bucket_agent_core::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    bucket_agent_core::config::mark_managed_config_synced(bucket_agent_core::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = bucket_agent_core::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(bucket_agent_core::auth::AuthManager::new(
        &home,
        bucket_agent_core::auth::BucketComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match bucket_agent_core::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
/// Live wiring guard: an offline `BUCKET_DEPLOYMENT_KEY` switch on a fail_closed install must FAIL CLOSED, else a
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    unsafe { std::env::set_var("BUCKET_DEPLOYMENT_KEY", "KEY-AAA") };
    bucket_agent_core::managed_config::sync()
        .await
    ...
    // BUCKET_MANAGED_CONFIG=0 disables any incidental background fetch (the gate is sync anyway).
    // SAFETY: #[serial] test; restored before any assertion below.
    unsafe { std::env::set_var("BUCKET_MANAGED_CONFIG", "0") };

    // Same key A → matching fingerprint → ALLOW (exact-equality vs recorded blake3).
    let gate_same_key = bucket_agent_core::managed_config::managed_policy_gate();

    // Switch to a different key B → REFUSE: exercises the full offline wiring.
    // SAFETY: #[serial] test; restored immediately below.
    unsafe { std::env::set_var("BUCKET_DEPLOYMENT_KEY", "KEY-BBB") };
    let gate_switched_key = bucket_agent_core::managed_config::managed_policy_gate();

    // SAFETY: #[serial] test; restore env BEFORE asserting so a failed assert can't leak it to later tests.
    unsafe {
        std::env::remove_var("BUCKET_DEPLOYMENT_KEY");
        std::env::remove_var("BUCKET_MANAGED_CONFIG");
    }
...
async fn identity_change_permits_offline_team_switch_and_purges_prior_team() {
    ...
    for f in bucket_agent_core::managed_config::MANAGED_ARTIFACT_FILES
        .into_iter()
        .chain([bucket_config::MANAGED_CONFIG_CACHE_FILE])
    {
        assert!(
            !home.join(f).exists(),
...
/// The session-start gate reads no env: `BUCKET_MANAGED_CONFIG_FAIL_CLOSED=0` must NOT disarm a fail_closed
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    unsafe {
        std::env::set_var("BUCKET_MANAGED_CONFIG", "0"); // offline (the gate is sync anyway)
        std::env::set_var("BUCKET_MANAGED_CONFIG_FAIL_CLOSED", "0"); // attempt a local disarm
    }
    let gate = bucket_agent_core::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("BUCKET_MANAGED_CONFIG");
        std::env::remove_var("BUCKET_MANAGED_CONFIG_FAIL_CLOSED");
    }

    assert!(
        gate.is_err(),
        "BUCKET_MANAGED_CONFIG_FAIL_CLOSED=0 must not disarm the session-start gate (no local bypass)"
    );
}
...
/// `bucket setup` with config served but the lock held by another writer reports the
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = bucket_agent_core::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            bucket_agent_core::managed_config::SetupOutcome::Skipped
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = bucket_agent_core::http::shared_client();
...
/// `BUCKET_MANAGED_CONFIG=0` is an explicit opt-out: the post-login sync must
/// make zero requests.
#[tokio::test]
#[serial]
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("BUCKET_MANAGED_CONFIG", "0") };
    let outcome = bucket_agent_core::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("BUCKET_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        bucket_agent_core::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: bucket_agent_core::auth::BucketAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = bucket_agent_core::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        bucket_agent_core::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        bucket_agent_core::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
...
async fn purge_crash_prefixes_stay_armed_and_converge() {
    let home = test_home().clone();
    let artifacts = bucket_agent_core::managed_config::MANAGED_ARTIFACT_FILES;
    ...
    for prefix_len in 0..=artifacts.len() {
        ...
        write_team_auth(&home, "team-b");
        assert_eq!(
            bucket_config::confirmed_team_switch("team-b").as_deref(),
            Some("team-a"),
...
async fn contended_sync_writes_no_marker() {
    ...
    let lock = std::fs::OpenOptions::new()
        ...
        .unwrap();
    lock.lock().unwrap();
    let synced = bucket_agent_core::managed_config::sync()
        .await
...
async fn dk_synced_marker_survives_config_blip_with_team_signed_in() {
    ...
    write_team_auth(&home, "team-b");
    assert_eq!(
        bucket_config::confirmed_team_switch("team-b"),
        None,
