...
//!
//! Behavior is byte-for-byte identical for Bucket chat (`T = Arc<BucketTurn>`).
...
///   summaries so their `<bucket_user_queries>` sections can be split out.
...
/// Extracts **all** `<bucket_user_queries>...</bucket_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<bucket_user_queries>";
    let end_tag = "</bucket_user_queries>";
...
/// Extract a `<bucket_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<bucket_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<bucket_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<bucket_user_queries>\n");
    ...
    result.push_str("</bucket_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<bucket_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<bucket_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<bucket_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<bucket_user_queries>` block.
...
/// `<bucket_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<bucket_user_queries>` from earlier rounds.
...
///   would see the prior `<bucket_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<bucket_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<bucket_user_queries>"));
        assert!(out.ends_with("</bucket_user_queries>"));
        assert!(out.contains("<bucket_query>hello world</bucket_query>"));
    }
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<bucket_user_queries>\n<bucket_query>Q1</bucket_query>\n</bucket_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<bucket_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<bucket_user_queries>"));
