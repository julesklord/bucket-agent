...
//!
//! Disk writes live in `bucket_agent_core::util::config::set_<field>()`.
...
/// Default ON when unset. The `BUCKET_PROMPT_SUGGESTIONS` env var overrides
...
/// `BUCKET_SCROLL_MODE` overrides `[ui].scroll_mode` (the `BUCKET_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("BUCKET_SCROLL_MODE")
                .ok()
...
/// `BUCKET_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
...
/// its own lines-per-tick. `BUCKET_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("BUCKET_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match bucket_config::load_effective_config_disk_only() {
        Ok(r) => r,
