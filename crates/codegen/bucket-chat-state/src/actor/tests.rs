...
async fn update_sampling_config_is_queryable() {
    ...
    assert_eq!(config.model, "bucket-3");
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_bucket_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_bucket_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use bucket_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("bucket-3".to_string()));
...
/// Simulates the exact sequence that `bucket-agent-core`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools = ConversationItem::Assistant(bucket_sampling_types::AssistantItem {
...
async fn dangling_tool_calls_after_crash_are_repaired_on_load() {
    ...
    let crashed_conversation = vec![
        ...
        ConversationItem::Assistant(bucket_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use bucket_sampling_types::{ContentPart, UserItem};
...
async fn get_last_model_metadata_returns_both_fields() {
    let h = TestHarness::with_conversation(vec![
        ...
        ConversationItem::Assistant(bucket_sampling_types::AssistantItem {
            content: "hello".into(),
            tool_calls: vec![],
            model_id: Some("bucket-4.5".into()),
            model_fingerprint: Some("fp_abc123".into()),
            reasoning_effort: None,
        }),
    ]);
    let meta = h.handle.get_last_model_metadata().await;
    assert_eq!(meta.resolved_model_id.as_deref(), Some("bucket-4.5"));
...
async fn sampling_config_survives_compaction_replacement() {
    use bucket_sampling_types::ApiBackend;
    ...
    assert_eq!(pre.model, "bucket-build");
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("bucket-4.5"));
...
/// If that model slug is e.g. "bucket-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            bucket_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    // Initial config: 500k context, Responses backend (matches bucket-4.5)
    let config = SamplingConfig {
    ...
    assert_eq!(post.model, "bucket-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(req: &bucket_sampling_types::ConversationRequest) -> serde_json::Value {
    use bucket_sampling_types::rs;
    let create_response: rs::CreateResponse = req.into();
    let mut body = serde_json::to_value(&create_response).unwrap();
    bucket_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &bucket_sampling_types::ConversationRequest,
    extended: &bucket_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    use bucket_sampling_types::ContentPart;
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(bucket_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(bucket_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use bucket_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
