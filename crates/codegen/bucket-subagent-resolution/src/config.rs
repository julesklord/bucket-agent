...
//! `bucket_agent_core::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `bucket-agent-core` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("bucket-3"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".bucket/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(
            ...
            Some(".bucket/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("bucket-3-fast"));
