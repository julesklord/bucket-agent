...
//! connection to the bucket.com relay server with automatic reconnection.
...
/// Config for the bucket.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a bucket.com first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.bucket_ws_url.clone(),
            ws_origin: ctx.bucket_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to bucket.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `bucket login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Bucket WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(bucket_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                bucket_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test BucketAuth with the given key.
    fn test_auth(key: &str) -> BucketAuth {
        BucketAuth {
            key: key.to_string(),
    ...
    /// Helper: write a BucketAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &BucketAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let cfg = crate::auth::BucketComConfig::default();
