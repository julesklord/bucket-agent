...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = bucket_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                bucket_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    bucket_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                bucket_workspace::permission::AccessKind::Read(Some(".env".into())),
                bucket_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        bucket_workspace::permission::Decision::PolicyDeny(_)
                    ),
