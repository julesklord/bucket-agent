...
//! inside [`bucket_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `bucket-tui __mermaid-render` (see
...
//!    via [`bucket_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`bucket_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `bucket-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, bucket_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `BUCKET_HOME` (whose `bucket_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.bucket`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_BUCKET_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `BUCKET_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["bucket"])));
        assert!(!is_render_subcommand(&argv(&["bucket", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    /// hermetic, with no process-global `BUCKET_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `bucket_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/bucket-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::BucketNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::BucketDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
