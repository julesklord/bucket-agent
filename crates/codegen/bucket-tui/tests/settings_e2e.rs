...
use bucket_tui::views::settings_modal::{
...
fn make_state() -> SettingsModalState {
    ...
    bucket_tui::app::set_voice_mode_enabled_for_test(true);
...
fn filter_enter_commits_and_preserves_query() {
    ...
    assert_eq!(s.query, "stamp", "Enter must PRESERVE the query (Issue 1)");
    assert_eq!(
...
fn defaults_round_trip_through_registry() {
    use bucket_tui::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    bucket_tui::appearance::cache::set_keep_text_selection(
        bucket_tui::appearance::TextSelection::Flash,
    );
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("bucketnight"),
            "auto_dark_theme" => SettingValue::Enum("bucketnight"),
            "auto_light_theme" => SettingValue::Enum("bucketday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = bucket_tui::settings::default_value_for(meta);
...
// `crates/codegen/bucket-tui/src/app/dispatch.rs::tests` (next to
...
fn pr4_mouse_click_in_theme_picker_is_no_op() {
    ...
    s.mode = SettingsModalMode::PickingEnum {
        key: "theme",
        choices_idx: 0,
        original_value: SettingValue::Enum("bucketnight"),
        supports_preview: true,
    };
    synth_rects(&mut s);
    let outcome = handle_settings_mouse(
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use bucket_tui::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use bucket_tui::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
/// "Ask Bucket" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Bucket"),
            "[{fixture_label}] docs footer (`Ask Bucket`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to bucketday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    bucket_tui::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn pr15_int_stepper_commit_dispatches_typed_setter() {
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "120", "buffer must seed from default");
        }
        other => panic!("expected EditingValue mode, got {other:?}"),
    }
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => assert_eq!(buffer, "200"),
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            (
                "Bucket 4.5".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("bucket-4.5")),
            ),
            (
                "Bucket 3".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("bucket-3")),
            ),
        ],
        ..PagerLocalSnapshot::default()
    };
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("Bucket 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr15_int_stepper_up_down_left_right_steps_and_clamps() {
    ...
    let _ = handle_settings_key(&mut s, &press(KeyCode::Up));
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => assert_eq!(buffer, "125"),
        _ => panic!("expected EditingValue"),
    }
    // Right: 125 + 10 = 135.
    let _ = handle_settings_key(&mut s, &press(KeyCode::Right));
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => assert_eq!(buffer, "135"),
        _ => panic!("expected EditingValue"),
    }
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "40", "must clamp to min");
        }
        _ => panic!("expected EditingValue"),
    }
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "500", "must clamp to max");
        }
        _ => panic!("expected EditingValue"),
    }
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "40", "Left must clamp to min");
        }
        _ => panic!("expected EditingValue"),
    }
}
...
fn pr15_int_stepper_rejects_text_input_keys() {
    ...
    navigate_to(&mut s, "max_thoughts_width");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
    let initial_buffer = match &s.mode {
    ...
    for k in reject_keys {
        ...
        match &s.mode {
            ...
            _ => panic!("mode must remain EditingValue"),
        }
    }
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        bucket_tui::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        bucket_tui::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_parses_aliases() {
    use bucket_tui::slash::commands::privacy::parse_privacy_arg;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use bucket_tui::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use bucket_tui::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use bucket_tui::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use bucket_tui::appearance::TextSelection;
    ...
    bucket_tui::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    bucket_tui::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn scroll_speed_int_stepper_commit_dispatches_typed_setter() {
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "50", "buffer must seed from default 50");
        }
        other => panic!("expected EditingValue mode, got {other:?}"),
    }
    ...
    let _ = handle_settings_key(&mut s, &press(KeyCode::Right));
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => assert_eq!(buffer, "56"),
        other => panic!("expected EditingValue, got {other:?}"),
    }
...
fn scroll_lines_int_stepper_commit_dispatches_typed_setter() {
    ...
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => {
            assert_eq!(buffer, "3", "buffer must seed from default 3");
        }
        other => panic!("expected EditingValue mode, got {other:?}"),
    }
    ...
    let _ = handle_settings_key(&mut s, &press(KeyCode::Up));
    match &s.mode {
        SettingsModalMode::EditingValue { buffer, .. } => assert_eq!(buffer, "4"),
        other => panic!("expected EditingValue, got {other:?}"),
    }
...
fn invert_scroll_enter_dispatches_typed_setter() {
    bucket_tui::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    bucket_tui::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    bucket_tui::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    bucket_tui::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    bucket_tui::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    bucket_tui::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
