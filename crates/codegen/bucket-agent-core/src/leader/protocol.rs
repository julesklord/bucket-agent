...
pub enum ClientMode {
    /// Headless mode (bucket agent, bucket agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (bucket agent stdio, bucket -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `bucket update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `bucket update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `bucket update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["bucket-a".into(), "bucket-b".into()],
            pid: 4242,
        };
