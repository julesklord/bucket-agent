...
async fn fetch_gh_release_stable_returns_tag_stripped() {
    ...
    let v = fetch_gh_release_version("stable", &make_update_config("stable"))
        .await
        .unwrap();
    assert_eq!(v, "0.1.181");
...
async fn fetch_gh_release_alpha_returns_max_of_pre_and_stable() {
    ...
    let v = fetch_gh_release_version("alpha", &make_update_config("alpha"))
        .await
        .unwrap();
    assert_eq!(v, "0.1.182-alpha.1");
...
async fn fetch_gh_release_propagates_gh_failure() {
    ...
    let err = fetch_gh_release_version("stable", &make_update_config("stable"))
        .await
        .unwrap_err();
    let msg = format!("{err:#}");
...
async fn fetch_gh_release_passes_repo_flag() {
    ...
    let _ = fetch_gh_release_version("stable", &make_update_config("stable"))
        .await
        .unwrap();
    let log = g.args_log();
...
async fn fetch_gh_release_does_not_hang_on_quick_responses() {
    ...
    let res = tokio::time::timeout(
        ...
        fetch_gh_release_version("stable", &make_update_config("stable")),
    )
    .await;
