...
//! The default theme is BucketNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! BucketNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::BucketNight, ThemeKind::BucketDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::BucketNight => "bucketnight",
            Self::TokyoNight => "tokyonight",
            Self::BucketDay => "bucketday",
    ...
    /// when quantized to 256 or 16 colors. BucketNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::BucketNight => false,
            Self::TokyoNight => true,
            Self::BucketDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "bucketnight" | "bucket-night" | "dark" => Some(Self::BucketNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "bucketday" | "bucket-day" | "light" | "day" => Some(Self::BucketDay),
...
/// `"bucketnight"` → `"Bucket Night"`). Falls back to `value` verbatim.
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.bucket/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::BucketNight => Self::bucketnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::BucketDay => Self::bucketday(),
            ...
            // if reached, fall back to BucketNight.
            ThemeKind::Auto => Self::bucketnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `BUCKET_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on bucketnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., BucketNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // BucketNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    bucket_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::BucketNight.is_auto());
        assert!(!ThemeKind::BucketDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::bucketday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::bucketnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::bucketday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. BucketNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::BucketNight => Theme::bucketnight(),
                ThemeKind::BucketDay => Theme::bucketday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_bucketnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark BucketNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::bucketnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("bucketnight"),
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("bucketnight", ThemeKind::BucketNight),
            ("bucket-night", ThemeKind::BucketNight),
            ("dark", ThemeKind::BucketNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("bucketday", ThemeKind::BucketDay),
            ("bucket-day", ThemeKind::BucketDay),
            ("light", ThemeKind::BucketDay),
            ("day", ThemeKind::BucketDay),
