...
pub const BUCKET_API_KEY_ENV_VAR: &str = "BUCKET_API_KEY";

/// Legacy env var name. Checked as a fallback when `BUCKET_API_KEY` is not set,
/// so existing deployments that use the old name keep working.
pub const LEGACY_BUCKET_API_KEY_ENV_VAR: &str = "BUCKET_CODE_BUCKET_API_KEY";
...
/// exist (no global `BUCKET_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// `disable_api_key_auth` (`[bucket_com_config] disable_api_key_auth` /
/// `BUCKET_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `BUCKET_API_KEY` can't bypass a deployment's forced IdP login.
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// Optional display label for the login method (`bucket.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `bucket_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `bucket.com` method).
    ...
    /// True if any local or custom models (like Ollama) are configured.
    pub has_local_or_custom_models: bool,
}
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        ...
        default_auth_method_id: Some(acp::AuthMethodId::new(BUCKET_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(
    ...
    login_label: Option<&str>,
    has_auth_provider_command: bool,
    has_local_or_custom_models: bool,
) -> BuiltAuthMethods {
    let mut methods: Vec<acp::AuthMethod> = Vec::new();
    let mut default_auth_method_id: Option<acp::AuthMethodId> = None;
    ...
    // ponytail: always include local auth method by default so the CLI never forces interactive login on startup
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.bucket_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.bucket_com_config.oidc.as_ref().unwrap()`
...
/// `true` for session-based ACP methods (cached_token, bucket.com, oidc).
...
/// `BUCKET_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `bucket login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `bucket login`, set BUCKET_API_KEY, or add api_key to ~/.bucket/config.toml.";
...
pub fn method_id_after_cached_token_unavailable(
    ...
    match preferred_method {
        ...
        None => Some(if has_external_api_key {
            BUCKET_API_KEY_METHOD_ID
        } else {
            BUCKET_COM_METHOD_ID
        }),
    }
}

/// Error when `preferred_method=api_key` but no key/BYOK credentials exist.
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set BUCKET_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `bucket login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.bucket/auth.json".to_string())),
    )
}

pub const BUCKET_COM_METHOD_ID: &str = "bucket.com";

/// xAI OAuth2/OIDC auth. Method id `"bucket.com"` kept for ACP wire-compat.
pub fn bucket_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Bucket");
...
mod tests {
    ...
    /// No advertiseable API-key credentials → interactive `bucket.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_bucket_com_without_api_key() {
        assert_eq!(
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(BUCKET_API_KEY_METHOD_ID);
    ...
    /// `bucket.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): local and bucket.com are
    /// advertised, defaulting to local so no interactive login is forced on startup.
    #[test]
    fn fresh_user_only_advertises_bucket_com_and_requires_login() {
        ...
        assert_eq!(built.methods.len(), 2);
    }

    /// Enterprise OIDC replaces `bucket.com` (mutually exclusive). xai.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_bucket_com_but_xai_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::BucketCom),
            "bucket.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `bucket.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let bucket = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::BucketCom)
            .expect("bucket.com must be advertised");
        assert_eq!(bucket.name(), "Acme Corp");
        let meta = bucket.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.bucket/config.toml` skeleton from the bug report, walks it through
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(BUCKET_API_KEY_ENV_VAR);
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // bucket.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    /// `BUCKET_API_KEY` alone (no per-model creds) also triggers
    ...
    fn global_external_api_key_advertises_xai_api_key_first() {
        let _set = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, "bucket-external-key");
    ...
    fn disable_api_key_auth_suppresses_xai_api_key_method() {
        ...
        assert_eq!(
            ...
            "with api-key auth disabled and no cached token, the local method leads",
        );
        assert_eq!(
            built
                .default_auth_method_id
    ...
    /// Legacy `BUCKET_CODE_BUCKET_API_KEY` env var is accepted as a fallback
    /// when `BUCKET_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_xai_api_key() {
        let _unset_new = EnvGuard::unset(BUCKET_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_BUCKET_API_KEY_ENV_VAR, "bucket-legacy-key");
        assert!(has_xai_api_key_env());
        assert_eq!(read_xai_api_key_env().unwrap(), "bucket-legacy-key");
    ...
    /// When both `BUCKET_API_KEY` and `BUCKET_CODE_BUCKET_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_BUCKET_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_xai_api_key_env().unwrap(), "new-key");
    }

    // -- bucket login --legacy regression coverage ------------------------
    //
    // `bucket login --legacy` produces a BucketAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `BUCKET_AUTH` env var (or via legacy
    ...
    // to-end. A regression in BUCKET_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `BUCKET_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn bucket_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, BucketAuth, BucketComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("BUCKET_AUTH_PATH");
        let _g2 = EnvGuard::unset(BUCKET_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `bucket login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = BucketAuth {
        ...
        // Provide it via BUCKET_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("BUCKET_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in BUCKET_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("BUCKET_AUTH");
        let _g2 = EnvGuard::unset("BUCKET_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::Local),
            "no cached token AND no api key: local method leads",
        );
    }
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![BUCKET_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(BUCKET_API_KEY_METHOD_ID));
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, BUCKET_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![BUCKET_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
