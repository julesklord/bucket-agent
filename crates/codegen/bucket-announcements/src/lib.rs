//! Shared announcement types, persistence, and formatting for Bucket CLI apps.
//!
//! This crate provides the common logic used by `bucket-agent-core` and
//! `bucket-tui` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.bucket/announcements.json`.
...
/// Write hidden announcement ids to `~/.bucket/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    bucket_tools::util::bucket_home::bucket_home().join("announcements.json")
}
...
///
/// Precedence: `BUCKET_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("BUCKET_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid BUCKET_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("BUCKET_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("BUCKET_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get SuperBucket","url":"https://x.ai/bucket","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get SuperBucket"));
        assert_eq!(cta.url.as_deref(), Some("https://x.ai/bucket"));
