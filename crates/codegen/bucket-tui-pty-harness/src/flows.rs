...
/// shell has session auth (the harness's `BUCKET_API_KEY` is ApiKey/BYOK mode
...
/// [`ContentController::env_for_pager`] minus `BUCKET_API_KEY`, so the entry
...
pub fn oauth_env_for_pager(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.retain(|(k, _)| k != "BUCKET_API_KEY");
    env
}
