...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p bucket-agent-core --test test_leader_soak -- --ignored --nocapture
//! ```
...
/// Copied from `bucket-codebase-graph/tests/memory_integration.rs`.
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = bucket_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            // the same wiring: bucket-tui/src/app/leader_cluster/mod.rs
