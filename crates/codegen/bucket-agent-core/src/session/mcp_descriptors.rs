...
/// sanitizer shared with `bucket-mcp` so the advertised folder matches disk.
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("bucket_com_linear"),
            "bucket_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.bucket/projects/enc/mcps");
