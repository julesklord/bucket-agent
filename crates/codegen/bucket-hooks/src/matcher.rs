...
///   **exact** match against each name (after external→Bucket alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Bucket tool `run_terminal_command`).
///
...
/// plus any Bucket tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Bucket registry in
/// `bucket-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for bucket_name in bucket_names_for(term) {
            push(bucket_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_bucket_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Bucket name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_bucket_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Bucket equivalent
        assert!(m.is_match("hashline_edit")); // second Bucket alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_bucket_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_bucket_tool() {
        // A regex written against an external alias still matches the Bucket tool
