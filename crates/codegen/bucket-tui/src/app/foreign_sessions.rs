...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, bucket_home: &Path) -> [PathBuf; 2] {
        ...
        [
            bucket_home
                ...
                .join(skill)
                .join("SKILL.md"),
            bucket_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(bucket_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, bucket_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, bucket_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        bucket_home: bucket_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use bucket_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/bucket"), |path| {
