...
impl WorkspaceIdentity {
    ...
    /// `BucketAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
/// [`AuthIdentity`](bucket_hub_sdk::AuthIdentity). The two types carry the
...
impl From<bucket_hub_sdk::AuthIdentity> for WorkspaceIdentity {
    fn from(id: bucket_hub_sdk::AuthIdentity) -> Self {
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `bucket-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$BUCKET_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("BUCKET_SANDBOX_PROFILE").ok(),
            bucket_sandbox::is_inside_bwrap(),
...
mod tests {
    ...
    fn assemble_populates_every_field() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("git@github.com:bucket-org/example.git".to_string()),
        );
        ...
        assert_eq!(
            ...
            Some("git@github.com:bucket-org/example.git")
        );
    ...
    fn serializes_with_all_fields_present() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("host-1".to_string()),
            Some("/work/repo".to_string()),
            Some("https://github.com/bucket-org/example".to_string()),
        );
    ...
    fn git_repo_facts_strips_remote_url_credentials() {
        ...
        assert_eq!(url, "https://github.com/bucket-org/example.git");
    }
}
