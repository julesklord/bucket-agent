...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party OAuth (`is_first_party_auth()`:
    // AuthMode::Oidc + first-party issuer); the harness's BUCKET_API_KEY is ApiKey mode
    ...
    env_refs.retain(|(k, _)| *k != "BUCKET_TRACE_UPLOAD");
    env_refs.push(("BUCKET_TRACE_UPLOAD", "true"));
    env_refs.push(("BUCKET_TELEMETRY_TRACE_UPLOAD", "true"));
    env_refs.push(("BUCKET_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"));
    ...
    //
    // Accounting (bucket-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `BUCKET_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
