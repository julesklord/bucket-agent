...
use crate::auth::config::BucketComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use bucket_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, BucketAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<BucketAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<bucket_system_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<BucketAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(bucket_home: &Path, bucket_com_config: BucketComConfig) -> Self {
        let scope = bucket_com_config.auth_scope();
        let proxy_base_url =
        ...
        bucket_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "bucket_home": bucket_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "BUCKET_HOME": std::env::var("BUCKET_HOME").unwrap_or_else(|_| "(unset)".into()),
                "BUCKET_AUTH_PATH": std::env::var("BUCKET_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "BUCKET_AUTH": std::env::var("BUCKET_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // BUCKET_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("BUCKET_AUTH") {
            if let Ok(auth) = serde_json::from_str::<BucketAuth>(&inline_json) {
                return Self::assemble(
                    Some(auth),
                    bucket_home.join("auth.json"),
                    scope,
                    bucket_com_config,
                    proxy_base_url,
                    None,
            ...
            tracing::warn!("BUCKET_AUTH set but failed to parse as JSON, falling back to file");
        }

        // BUCKET_AUTH_PATH: custom file path (overrides default $BUCKET_HOME/auth.json).
        let path = std::env::var("BUCKET_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| bucket_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &BucketAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.bucket_com_config)?;
    ...
    fn vet_cached(&self, auth: BucketAuth) -> Option<BucketAuth> {
    ...
    pub(crate) fn current(&self) -> Option<BucketAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<BucketAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&BucketAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<BucketAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<BucketAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<BucketAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &BucketAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &BucketAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &BucketAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.bucket_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `BucketAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: BucketAuth) -> std::io::Result<BucketAuth> {
        ...
        match &write_result {
            Ok(()) => bucket_telemetry::unified_log::info(
            ...
            Err(e) => bucket_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: BucketAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut BucketAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn bucket_com_config(&self) -> &BucketComConfig {
        &self.bucket_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<BucketAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: BucketAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<BucketAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> BucketAuth {
        self.inner.read().clone().unwrap_or_else(|| BucketAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: BucketAuth) -> Option<BucketAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<BucketAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<BucketAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<BucketAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in BUCKET_AUTH mode).
            DiskAuthState::Ok => {
                bucket_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<BucketAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<BucketAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<BucketAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.bucket_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`bucket login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.bucket_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<BucketAuth, AuthError> {
        if self.bucket_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        bucket_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `bucket_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &BucketAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<bucket_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[bucket_com] auth_token_ttl` to drive a real schedule.
...
///
/// Static fallthrough (`BUCKET_API_KEY` / `auth.json` `xai::api_key`) makes voice
...
impl bucket_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.bucket_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.bucket_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.bucket_com_config.preferred_method,
...
) -> bucket_tools::types::SharedApiKeyProvider {
