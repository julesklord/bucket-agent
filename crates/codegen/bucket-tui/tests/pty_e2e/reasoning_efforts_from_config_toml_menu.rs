...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("bucket-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.bucket/config.toml` with a per-model reasoning-effort menu.
    let bucket_home = content.home().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create .bucket");
    // Quote the dotted model id: bare `[model.bucket-4.5]` is TOML key-path syntax (model.bucket-4.5), not the id "bucket-4.5".
    std::fs::write(
