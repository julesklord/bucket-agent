...
///
/// ```bash
/// BUCKET_API_KEY='bucket-some-key' cargo run --bin bucket-tui
/// ```
///
...
///   prefix: ["BUCKET_API_KEY=bucket-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "bucket-tui"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "BUCKET_API_KEY='bucket-some-key' cargo run --bin bucket-tui";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["BUCKET_API_KEY=bucket-some-key".to_owned()],
        ...
        let redirection_command = "cargo build --bin bucket-tui 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package bucket-agent-core --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
