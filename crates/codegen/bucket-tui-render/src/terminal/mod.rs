...
pub enum TerminalName {
    ...
    JetBrains,
    /// Bucket Desktop (Electron app).
    #[strum(to_string = "Bucket Desktop")]
    BucketDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` bucket is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches bucket, which
    ...
    pub fn telemetry_snapshot(&self) -> bucket_telemetry::events::TerminalTelemetry {
        ...
        bucket_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: bucket_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> bucket_shared::session::FeedbackTerminalInfo {
        use bucket_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(bucket_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `bucket ssh` can
    // forward env vars.
...
/// `bucket-tui-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "bucketdesktop" => Some(TerminalName::BucketDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.bucket/pager.toml` and
