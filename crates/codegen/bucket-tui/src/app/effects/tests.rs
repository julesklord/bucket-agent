...
use super::*;
use bucket_agent_core::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
/// Untitled bucket.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn parse_kill_outcome_reads_result_envelope() {
    use bucket_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use bucket_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<bucket_acp::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let bucket_acp::AcpAgentMessage::ExtNotification(args) = msg {
...
/// Redirect `BUCKET_HOME` to a tempdir for test isolation.
fn setup_bucket_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("BUCKET_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use bucket_agent_core::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        bucket_agent_core::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        bucket_agent_core::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_bucket_home_in_tempdir();
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use bucket_acp::AcpAgentMessage;
    use bucket_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: bucket_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: bucket_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            bucket_home: PathBuf::from("/path/that/must/not/be-read"),
...
fn agent_profile_names_are_valid_builtins() {
    ...
    let test_cases: &[(SessionFlags, &str)] = &[
        ...
        (
            ...
            "bucket-build-plan-no-subagents",
        ),
        (
            SessionFlags {
                plan_mode: true,
...
/// Default flags produce no agent profile (uses bucket-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (bucket-build already has TaskTool).
#[test]
...
/// Neutralize `BUCKET_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(BUCKET_AGENT)]` (process-global env).
fn without_bucket_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("BUCKET_AGENT", "")
}
...
#[serial_test::serial(BUCKET_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_bucket_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "bucket-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "bucket-build-plan-no-subagents");
...
/// --ask-user alone selects the bucket-build-ask-user profile.
#[serial_test::serial(BUCKET_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "bucket-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `bucket-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce bucket-build-plan
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> bucket_agent_core::session::SessionInfoResponse {
    use bucket_agent_core::session::acp_types::{ContextInfo, SessionInfoData};
    bucket_agent_core::session::SessionInfoResponse {
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("bucket-4.5", Some("bucket-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: bucket-4.5 (bucket-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("bucket-4.5", Some("bucket-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model: bucket-4.5"));
    assert!(! text.contains("bucket-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("bucket-4.5", Some("bucket-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: bucket-4.5"));
    assert!(! text.contains("(bucket-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("bucket-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("bucket-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("bucket-4"));
