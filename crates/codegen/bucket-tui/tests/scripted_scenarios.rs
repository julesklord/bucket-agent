...
//!
//! These tests exercise the real `bucket-tui` binary through a PTY using
...
/// enterprise deploy report: with `BUCKET_GOAL=1`, `/goal` must
...
/// explicitly off (`BUCKET_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `BUCKET_FOLDER_TRUST=1` and a git repo that
...
/// also pins `BUCKET_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
