//! Conversion from the gRPC wire config types (`bucket-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`bucket_tools_api::config_validation`] so every consumer (this
...
pub use bucket_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`bucket_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: bucket_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let bucket_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
...
pub fn tool_server_config_from_entries(
    entries: Vec<bucket_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> bucket_tools_api::ToolConfigEntry {
        bucket_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
    ...
    fn minimal_entry_converts_with_defaults() {
        let cfg = tool_config_from_entry(0, entry("BucketBuild:read_file")).unwrap();
        assert_eq!(cfg.id, "BucketBuild:read_file");
    ...
    fn fully_populated_entry_converts_field_by_field() {
        let mut e = entry("BucketBuild:grep");
    ...
    fn name_override_valid_tool_id_charset_is_accepted() {
        for name in ["search", "BucketBuild:grep", "a-b_C9"] {
            let mut e = entry("BucketBuild:grep");
    ...
    fn name_override_outside_tool_id_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "emoji✨", "dot.name"] {
            ...
            assert_eq!(err.tool_id, "BucketBuild:grep");
    ...
    fn server_config_conversion_rejects_invalid_name_override_entry() {
        let mut bad = entry("BucketBuild:grep");
    ...
    fn server_config_conversion_preserves_valid_name_overrides() {
        let mut a = entry("BucketBuild:grep");
        a.name_override = Some("search".to_owned());
        let cfg = tool_server_config_from_entries(vec![a, entry("BucketBuild:bash")]).unwrap();
