...
use bucket_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl bucket_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        bucket_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
...
mod tests {
    use super::*;
    use crate::implementations::bucket_build::task_output::test_helpers::{
