...
/// (`BUCKET_WORKSPACE_TERMINATION_GRACE_MS`, default 45s): a takeover only
...
mod tests {
    ...
    fn process_name_matches_own_argv0() {
        ...
        // differently (e.g. Cargo uses `bucket_workspace-<hash>`), so a
    ...
    fn basename_contains_ignores_directory_components() {
        assert!(basename_contains(
            "/usr/local/bin/bucket-workspace-server",
            "workspace-server"
        ));
        assert!(basename_contains(
            "C:\\Program Files\\BUCKET-Workspace-Server.exe",
            "workspace-server"
        ));
        assert!(
