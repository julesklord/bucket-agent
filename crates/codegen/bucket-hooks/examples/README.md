...
Sample hooks for Bucket. Copy to `~/.bucket/hooks/` to enable globally, or to `<project>/.bucket/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.bucket/hooks/bin
cp examples/hooks/safe-shell.json ~/.bucket/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.bucket/hooks/bin/
chmod +x ~/.bucket/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.bucket/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.bucket/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.bucket/hooks/bin/
chmod +x ~/.bucket/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.bucket/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.bucket/hooks/bin
cp examples/hooks/session-log.json ~/.bucket/hooks/
cp examples/hooks/bin/session-log.sh ~/.bucket/hooks/bin/
chmod +x ~/.bucket/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.bucket/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.bucket/hooks/bin
cp examples/hooks/tool-logger.json ~/.bucket/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.bucket/hooks/bin/
chmod +x ~/.bucket/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Bucket names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.bucket/hooks/`. The hook stops running on the next session.