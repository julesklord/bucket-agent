...
//!
//! `bucket-tui` uses this module to set up OTLP
...
/// (`AuthManager`, `EndpointsConfig`, `BucketComConfig`).
///
/// Built by the binaries (`bucket-tui`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `bucket_agent_core::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-BUCKET-Token-Auth` header (typically `"bucket-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`bucket-pager`) -> `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"bucket-tui"`, `"bucket-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-bucket-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("bucket-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid BUCKET_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-BUCKET-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &bucket_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-BUCKET-Token-Auth".to_string(), value.to_string());
    }
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-bucket-client-version".to_string(),
...
mod tests {
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("bucket-cli"),
