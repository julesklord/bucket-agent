...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "bucket_code.session_start"),
        (E::SessionEnd, "bucket_code.session_end"),
        (E::UserPrompt, "bucket_code.user_prompt"),
        (E::TurnCompleted, "bucket_code.turn_completed"),
        (E::ApiRequest, "bucket_code.api_request"),
        (E::ApiError, "bucket_code.api_error"),
        (E::ToolResult, "bucket_code.tool_result"),
        (E::ToolDecision, "bucket_code.tool_decision"),
        (E::McpServerConnection, "bucket_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "bucket_code.permission_mode_changed",
        ),
        (E::SkillActivated, "bucket_code.skill_activated"),
        (E::PluginLoaded, "bucket_code.plugin_loaded"),
        (E::Compaction, "bucket_code.compaction"),
        (E::Subagent, "bucket_code.subagent"),
        (E::Auth, "bucket_code.auth"),
        (E::InternalError, "bucket_code.internal_error"),
        (E::ModelSwitched, "bucket_code.model_switched"),
        (E::ContextualTip, "bucket_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("bucket-pager"),
        "bucket-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("bucket-pager".into()),
        model_id: "bucket-4".into(),
        agent_name: "bucket-build-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["bucket_code.session.count".to_owned()]);
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.api_request");
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "bucket_code.error.count")
        .map(|m| match m.data() {
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    emit_event_into(
        ...
        &events::ToolCallCompleted {
            ...
            outcome: bucket_file_utils::events::types::ToolOutcome::Success,
            duration_ms: 42,
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "bucket_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "bucket_code.contextual_tip");
...
/// bucket-workspace sampler events, which live outside this crate and have
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("bucket_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("bucket_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("bucket_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("bucket_code.token.usage"));
    assert!(
