...
//! Shared test utilities for bucket-build crates: mock inference server, SSE
...
//! - [`BucketStdioClient`] — ACP client that drives `bucket agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `bucket agent --leader stdio` (unix)
//! - [`run_headless`] — Run `bucket -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`bucket_binary`] — Resolve the bucket binary path (BUCKET_BINARY env or cargo_bin)
