...
/// `BUCKET_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `BUCKET_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `BUCKET_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `BUCKET_GOAL_VERIFIER_N` / remote
...
/// (via [`bucket_tool_types::format_resume_footer`]): trace tooling
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<bucket_sampling_types::conversation::ConversationItem> {
    use bucket_sampling_types::conversation::{ConversationItem, ToolCall};
    ...
    let footer = bucket_tool_types::format_resume_footer(subagent_id, subagent_type, None);
...
/// the phase via [`bucket_chat_state::ChatStateHandle::flush_harness_trace_turn`]
...
pub(crate) fn record_subagent_trace(
    sink: Option<&(bucket_chat_state::ChatStateHandle, String)>,
    subagent_id: &str,
    subagent_type: &str,
    description: &str,
    prompt: Option<&str>,
...
pub(crate) struct ChannelSpawner {
    ...
    pub(crate) trace_sink: Option<(bucket_chat_state::ChatStateHandle, String)>,
...
mod tests {
    ...
    fn validate_details_path_in_root_keys_on_injected_temp_root() {
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/var/folders/zz/T/bucket-goal-abc/goal-classifier-abc-1.md"),
                mac_like,
            )
            .is_ok(),
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/tmp/bucket-goal-abc/goal-classifier-abc-1.md"),
                Path::new("/tmp"),
            )
            .is_ok(),
    ...
    fn gap_fingerprint_is_stable_across_scratch_path_churn() {
        ...
        let c = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/bucket-goal-1/out.log for criterion 2",
        ]);
        let d = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/bucket-goal-2/out.log for criterion 2",
        ]);
        assert_eq!(c, d);
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        use bucket_tools::implementations::bucket_build::task::types::SubagentTypeSummary;
        let mut tool_names = std::collections::HashMap::new();
        tool_names.insert(
            bucket_tools::types::tool::ToolKind::Read,
            "cursor_read".to_string(),
        );
        tool_names.insert(
            bucket_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            bucket_tools::types::tool::ToolKind::Search,
        ...
        // bucket-build explicit render: no leftover placeholder either.
        let bucket = RoleToolNames::from_summary(&summary_with(&[
            (bucket_tools::types::tool::ToolKind::Read, "read_file"),
            (bucket_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (bucket_tools::types::tool::ToolKind::Search, "grep"),
        ]))
        .apply(GOAL_VERIFIER_PROMPT_TEMPLATE);
        assert_no_tool_placeholders(&bucket);
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (bucket_tools::types::tool::ToolKind::Read, "cursor_read"),
            (bucket_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (bucket_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
    ...
    fn render_skeptic_prompt_substitutes_kind_lens_and_leaves_no_placeholder() {
        let body = render_skeptic_prompt(
            ...
            "/tmp/bucket-goal-x/skeptic-0",
            "/tmp/bucket-goal-x/implementer",
            None,
            &RoleToolNames::inherit_defaults(),
            true,
        );
        ...
        assert!(body.contains("/tmp/bucket-goal-x/skeptic-0"));
        assert!(body.contains("/tmp/bucket-goal-x/implementer"));
        assert!(
    ...
    fn stage_inputs_resume<'a>(
        ...
        VerificationStageInputs {
            ...
            verifier_id,
            attempt,
            model_id: "bucket-test",
            goal_created_at: 0,
            plan_file: None,
            plan_baseline_file: None,
            implementer_scratch_dir: Path::new("/tmp/bucket-goal-test/implementer"),
    ...
    async fn verification_stage_n1_not_refuted_returns_achieved() {
        ...
        assert!(
            p.contains("/tmp/bucket-goal-test/implementer"),
    ...
    fn resolve_goal_verifier_count_env_clamps() {
        unsafe { std::env::set_var("BUCKET_GOAL_VERIFIER_N", "0") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MIN
        );
        unsafe { std::env::set_var("BUCKET_GOAL_VERIFIER_N", "99") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MAX
        );
        unsafe { std::env::set_var("BUCKET_GOAL_VERIFIER_N", "garbage") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_COUNT,
            "invalid env falls through to the default"
        );
        unsafe { std::env::remove_var("BUCKET_GOAL_VERIFIER_N") };
    }
    ...
    fn resolve_goal_verifier_count_precedence_and_clamp() {
        ...
        // env > config.
        unsafe { std::env::set_var("BUCKET_GOAL_VERIFIER_N", "2") };
    ...
    fn resolve_goal_classifier_max_runs_env_clamps_and_no_ceiling() {
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER_MAX", "0") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER_MAX", "999") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER_MAX", "garbage") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::remove_var("BUCKET_GOAL_CLASSIFIER_MAX") };
    }
    ...
    fn resolve_goal_classifier_max_runs_precedence_and_floor() {
        ...
        // env > config.
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER_MAX", "4") };
    ...
    fn resolve_strategist_every_default_tracks_cap_floored_at_one() {
        unsafe { std::env::remove_var("BUCKET_GOAL_STRATEGIST_EVERY") };
    ...
    fn resolve_strategist_every_precedence_and_floor() {
        ...
        // env > config + remote.
        unsafe { std::env::set_var("BUCKET_GOAL_STRATEGIST_EVERY", "7") };
        let r = cfg_strategist(Some(3), Some(4)).resolve_goal_strategist_every(10);
        assert_eq!(r.value, 7);
        assert_eq!(r.source, ConfigSource::Env);
        // invalid env falls through to the default (cap/2).
        unsafe { std::env::set_var("BUCKET_GOAL_STRATEGIST_EVERY", "not-a-number") };
        assert_eq!(
            cfg_strategist(None, None)
                .resolve_goal_strategist_every(10)
                .value,
    ...
    async fn run_one_skeptic_fails_closed_when_scratch_root_squatted() {
        ...
        let inputs = SkepticInputs {
            ...
            attempt: 1,
            kind_lens: "",
            implementer_scratch: "/tmp/bucket-goal-test/implementer",
            scratch_dir_ready: true,
            prior_gaps: None,
        };
