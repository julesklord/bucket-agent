...
//! cargo run -p bucket-voice --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,bucket_voice=debug".into()),
        )
        .init();

    let args = parse_args(std::env::args().skip(1).collect());

    let auth = std::env::var("BUCKET_API_KEY")
        .ok()
        .and_then(StaticVoiceAuth::shared)
        .ok_or_else(|| {
            anyhow::anyhow!("set BUCKET_API_KEY (standalone probe has no login session)")
        })?;
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                bucket_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("BUCKET_BUCKET_API_BASE_URL").ok();
...
Environment:
  BUCKET_API_KEY     required
  RUST_LOG        optional (default info,bucket_voice=debug)

Reads [voice] from ~/.bucket/config.toml unless --config PATH is set.
"#
    );
}
