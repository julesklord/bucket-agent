...
static BASH_CAPABILITIES: LazyLock<bucket_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| bucket_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(bucket_tool_protocol::ToolScope::Write),
        streaming: Some(bucket_tool_protocol::StreamingSpec {
...
fn bash_output_chunk_progress(
    spec: &bucket_tool_protocol::StreamingSpec,
    chunk: &BashOutputChunk,
    last_total: &mut usize,
) -> Option<bucket_tool_runtime::ToolProgress> {
    // `stream_chunk` counts in `u64`; convert at the boundary.
    let mut cursor = *last_total as u64;
    let progress = bucket_tool_runtime::stream_chunk(
        spec,
        &chunk.base.output,
...
pub struct BashParams {
    ...
    /// bucket-build opts up to 10h via bucket-agent-core's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "bucket_build.Bash";
...
impl bucket_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<bucket_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => bucket_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production bucket-build sets it to 10h in bucket-agent-core's
...
/// [`bucket_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `BUCKET_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("BUCKET_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in bucket_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the bucket_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`BUCKET_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `BUCKET_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = bucket_config::shell::chain_separator();
...
impl bucket_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    fn description(
        ...
        bucket_tool_types::ToolDescription::new(
            "run_terminal_cmd",
    ...
    async fn execute(
        &self,
        mut ctx: bucket_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> bucket_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield bucket_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield bucket_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield bucket_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield bucket_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, bucket_tool_runtime::ToolError> {
        ...
        let ampersand = bucket_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(bucket_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing bucket_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (bucket_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &bucket_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            bucket_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let bucket_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = bucket_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, bucket_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                bucket_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = bucket_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = bucket_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(bucket_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // bucket-shell opts up to 10h via max_timeout_secs. Background unaffected.
