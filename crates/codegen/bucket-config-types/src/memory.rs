//! Memory-system configuration value types, extracted from bucket-agent-core
...
//! `bucket-agent-core` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.bucket/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.bucket/memory/` are scanned:
