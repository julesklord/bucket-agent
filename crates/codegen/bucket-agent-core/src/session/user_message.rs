...
// Re-export from bucket-chat-state — canonical definition lives there.
pub use bucket_chat_state::compaction_utils::extract_user_query;
...
pub async fn compute_vcs_status_block(
    ...
    use bucket_workspace::file_system::{git_status, jj_status};
...
// Tests for extract_user_query now live in bucket_chat_state::compaction_utils.

#[cfg(test)]
mod tests {
    use super::*;
    use bucket_workspace::file_system::FsError;
    ...
    async fn construct_user_message_returns_without_git_status_on_bad_dir() {
        let dir = std::path::Path::new("/tmp/nonexistent-bucket-test-dir");
