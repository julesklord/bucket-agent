...
use bucket_telemetry::id::agent_id;
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `bucket sessions delete <id>` CLI path.
...
/// hot-reload watcher when `<cwd>/.bucket/config.toml`,
...
/// Hot-reload the model catalog from `~/.bucket/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by bucket-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .bucket/plugins under it, plus
