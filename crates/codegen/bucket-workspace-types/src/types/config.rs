...
//! agent-session config types in `bucket-config` and friends.
...
/// `bucket-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `bucket_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
