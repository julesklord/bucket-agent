...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("bucket");
        self.last_title.clear();
        self.last_title.push_str("bucket");
...
/// strings (e.g. bucket.com conversation titles), which must not terminate the
...
mod tests {
    ...
    #[test]
    fn bucket_only_produces_just_bucket() {
        let cfg = config_with_items(vec![TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "bucket");
    }

    #[test]
    fn session_name_and_bucket_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - bucket");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - bucket"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "bucket");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - bucket");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - bucket");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    #[test]
    fn empty_items_produces_bucket_fallback() {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("bucket-3"),
            ..idle_state()
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "bucket-3 - bucket");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - bucket");
    }
    ...
    #[test]
    fn reset_clears_state_and_emits_bucket() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Bucket]);
        let mut mgr = TitleManager::new(&cfg);
        let activity = TurnActivity::Thinking;
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "bucket");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("bucket"), "title missing 'bucket': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - bucket-3 - workspace - bucket"
        );
    }
