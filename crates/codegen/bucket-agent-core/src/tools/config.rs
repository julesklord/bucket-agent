...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production bucket-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `BUCKET_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> bucket_tools::implementations::bucket_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("BUCKET_WEB_FETCH_PROXY"))
...
/// from `bucket_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `BucketBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `BucketBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "BucketBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "BucketBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"BucketBuild:read_file"));
        assert!(ids.contains(&"BucketBuild:search_replace"));
        assert!(ids.contains(&"BucketBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"BucketBuildHashline:hashline_read"));
        assert!(ids.contains(&"BucketBuildHashline:hashline_edit"));
        assert!(ids.contains(&"BucketBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            bucket_agent::config::AgentDefinition::plan(),
            bucket_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "BucketBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"BucketBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"BucketBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"BucketBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production bucket-build must set the 10h foreground ceiling"
        );
    }
