...
pub use bucket_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `BUCKET_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    bucket_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use bucket_agent_core::agent::config::{BoolFlag, ConfigSource};
    let resolved = BoolFlag::env("BUCKET_VOICE_MODE")
...
///
/// Uses [`bucket_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `bucket-tui-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`bucket-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&bucket_agent_core::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<bucket_agent_core::agent::models::EarlyPrefetchHandle>,
) -> Option<bucket_agent_core::util::config::RemoteSettings> {
    let handle = handle?;
...
/// should print a message telling the user to relaunch `bucket`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<bucket_updater::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    bucket_tty_utils::redirect_native_stderr();
    ...
    let bucket_com_config =
        match bucket_agent_core::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.bucket_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                bucket_agent_core::auth::BucketComConfig::default()
            }
        };
    let refreshed_auth = bucket_agent_core::auth::try_ensure_fresh_auth(&bucket_com_config).await;
    let early_prefetch =
        bucket_agent_core::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    bucket_agent_core::util::config::cache_remote_auto_mode(
    ...
    {
        let summaries = bucket_agent_core::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("BUCKET_HUNK_TRACKER").ok();
    ...
    let launch_auto = bucket_agent_core::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    cancel.cancel();
    bucket_tty_utils::global_process_scope().kill_all();
    match result {
        Ok(run_result) => {
...
fn print_exit_resume_hint(session_id: &str, minimal: bool, w: &mut impl Write) {
    ...
    if minimal {
        let _ = writeln!(w, "  bucket --minimal --resume {session_id}");
    } else {
        let _ = writeln!(w, "  bucket --resume {session_id}");
    }
}
...
fn disable_mouse_paste_raw() {
    bucket_agent_core::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(bucket_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    bucket_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        bucket_agent_core::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(bucket_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                bucket_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = bucket_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = bucket_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        bucket_crash_handler::disable_terminal_escape_restore();
    })
}
...
/// BEL/ESC (titles can arrive from bucket.com conversation metadata) would
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - bucket"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "bucket");
        assert_eq!(terminal_title_string(""), "bucket");
        assert_eq!(terminal_title_string("My chat"), "My chat - bucket");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["bucket-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["bucket-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["bucket-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["bucket-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["bucket-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["bucket-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = bucket_agent_core::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["bucket-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["bucket-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["bucket-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["bucket-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["bucket-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["bucket-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["bucket-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["bucket-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["bucket-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["bucket-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["bucket-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["bucket-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["bucket-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["bucket-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["bucket-pager", "--no-alt-screen"]).unwrap();
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["bucket-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["bucket-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  bucket --minimal --resume sess-abc\n"
        );
    }
    #[test]
