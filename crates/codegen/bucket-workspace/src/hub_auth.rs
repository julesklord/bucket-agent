//! Hub [`AuthProvider`] from `~/.bucket/auth.json` for the standalone
...
//!
//! The in-leader `bucket workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let bucket = bucket_config::user_bucket_home()
        .ok_or_else(|| anyhow::anyhow!("no user bucket home (set $BUCKET_HOME or $HOME)"))?;
    Ok(bucket.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `bucket login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `bucket login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.bucket/auth.json`).
pub fn provider(
    hub_url: &Url,
...
mod tests {
    ...
    fn read_auth_entry_picks_oidc_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "bucket-plainkey", "user_id": "u1" },
            "oidc": {
    ...
    fn read_auth_entry_rejects_non_oidc() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "api_key": { "key": "bucket-plainkey", "user_id": "u1" }
        }"#,
        );
    ...
    fn write_refreshed_token_updates_jwt_entry() {
        ...
        let path = write_auth_json(
            dir.path(),
            r#"{
            "legacy": { "key": "bucket-old", "user_id": "u1" },
        ...
        assert_eq!(updated["legacy"]["key"], "bucket-old"); // untouched
    }
