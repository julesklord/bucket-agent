...
//! `bucket_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `BUCKET_POOL_*` /
//! `BUCKET_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `BUCKET_SAMPLER_SHARED_CLIENT=0` falls back to
...
use bucket_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `bucket_agent_core::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `bucket_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use bucket_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `bucket-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `bucket-agent-core`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `bucket-telemetry` consumes
// the same type via `bucket_sampler::OriginClientInfo`. The shell-specific
...
/// `BUCKET_CLIENT_VERSION` env vars. Returns `None` when
/// `BUCKET_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`BUCKET_CLIENT_NAME` env var, default `"bucket-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("BUCKET_CLIENT_NAME").unwrap_or_else(|_| "bucket-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`bucket -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-bucket-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `bucket -p`, `run_headless_inner` for
/// `bucket agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn bucket_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(bucket_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "bucket-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "bucket-desktop".to_string(),
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "bucket-pager".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("bucket-desktop/1.2.3 bucket-shell/"));
        ...
        assert!(without_version.starts_with("bucket-web bucket-shell/"));
        assert!(!without_version.starts_with("bucket-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "bucket-shell".to_string(),
        ...
        assert_eq!(ua.render(), "bucket-shell/0.1.171 (macos; aarch64)");
    }
}
