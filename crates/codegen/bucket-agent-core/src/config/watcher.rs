...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.bucket/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** bucket process
    /// sharing the same `~/.bucket` (the writer may also be this process;
...
/// Watches `~/.bucket/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.bucket/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.bucket/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let bucket_home_buf = bucket_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(bucket_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(bucket_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(bucket_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(bucket_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %bucket_home.display(),
                    error = %e,
                    "failed to watch bucket home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.bucket/config.toml` so the loop above
        // watches `<cwd>/.bucket/`), the call below installs a
        ...
        tracing::info!(
            bucket_home = %bucket_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.bucket/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.bucket/config.toml` trigger a
    ...
    /// `<cwd>/.bucket/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.bucket/` does not yet
/// exist at session-open time, the `.bucket/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.bucket/` followed by a
/// write to `<cwd>/.bucket/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let bucket_dir = cwd.join(".bucket");
    if let Err(e) = debouncer
        .watcher()
        .watch(&bucket_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .bucket directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let bucket_dir = cwd.join(".bucket");
    if let Err(e) = debouncer.watcher().unwatch(&bucket_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .bucket directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.bucket/` hasn't
...
/// Watches skill directories (`~/.bucket/skills/`, `<repo>/.bucket/skills/`, etc.)
...
/// recursively: `bucket_home` (`~/.bucket`, or `$BUCKET_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.bucket` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, bucket_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, bucket_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".bucket", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, bucket_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(bucket_home) {
        return true;
    }
...
/// `~/.bucket` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](bucket_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = bucket_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &bucket_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &bucket_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only bucket_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_bucket_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let bucket_home = home.join(".bucket");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &bucket_home, Some(home));

        // bucket_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&bucket_home));
        ...
        assert!(!g(&home.join("repo").join(".bucket")));
    }

    /// Regression for the ~/.bucket inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.bucket/worktrees/`) must not
    ...
    /// A write to `<bucket_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another bucket process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.bucket/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let bucket_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_bucket = cwd.path().join(".bucket");
        fs::create_dir_all(&project_bucket).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_bucket.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_bucket.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.bucket/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.bucket/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_bucket = new_cwd.path().join(".bucket");
        fs::create_dir_all(&project_bucket).unwrap();
        fs::write(project_bucket.join("config.toml"), "").unwrap();
