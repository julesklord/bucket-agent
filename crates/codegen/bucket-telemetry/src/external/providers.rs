...
//! `Authorization`/`X-BUCKET-Token-Auth`/`x-userid`;
...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("bucket_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `bucket-cli`, a wire commitment.
...
mod tests {
    ...
    /// `X-BUCKET-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-bucket-token-auth", "x-userid", "x-teamid"] {
