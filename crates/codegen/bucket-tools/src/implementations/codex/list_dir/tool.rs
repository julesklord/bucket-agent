...
//! absolute paths. See the plan document for the full diff vs the bucket-build
//! `ListDirTool`.
...
impl bucket_tool_runtime::Tool for CodexListDirTool {
    type Args = CodexListDirInput;
    type Output = ListDirOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    ...
    ) -> Result<ListDirOutput, bucket_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = bucket_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = bucket_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
