...
fn api_key_id_for(auth: Option<&crate::auth::BucketAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| bucket_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: BucketAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = BucketAuthCredentials::new(None);
...
///
/// - `"bucket-shell"`   — classic Bucket CLI / TUI (bucket-agent-core)
/// - `"bucket-pager"`   — new Bucket Pager / TUI (bucket-tui)
/// - `"bucket-desktop"` — Bucket Desktop app
/// - `"bucket-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-bucket-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-bucket-client-identifier`
///
/// to the underlying `bucket-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-BUCKET-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> bucket_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn bucket_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        bucket_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(bucket_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = BucketAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in bucket-file-utils)
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = BucketAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        bucket_telemetry::external::set_identity(
            bucket_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `bucket-tui` and `bucket-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> bucket_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let bucket_com_config = crate::auth::BucketComConfig::default();
    let exporter = bucket_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = bucket_com_config.token_header.clone();
    let bucket_home = crate::util::bucket_home::bucket_home();
    let bootstrap = Arc::new(AuthManager::new(&bucket_home, bucket_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    bucket_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    /// RAII guard: pins `BUCKET_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("BUCKET_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("BUCKET_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("BUCKET_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("BUCKET_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> BucketAuth {
        BucketAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<BucketAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::BucketComConfig::default(),
        ));
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use bucket_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("bucket-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("bucket-token-EX").as_str())
        );
        assert!(dep.api_key_id.is_none());
        let api_auth = BucketAuth {
    ...
    /// user token, matching the precedence in `BucketAuthCredentials::apply`.
