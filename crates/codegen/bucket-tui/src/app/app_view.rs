...
/// [`bucket_agent_core::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<bucket_agent_core::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: bucket_agent_core::util::config::WorktreeHintMode) -> Self {
        use bucket_agent_core::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        bucket_agent_core::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            bucket_agent_core::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`bucket_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `BUCKET_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `BUCKET_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("BUCKET_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`bucket_agent_core::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => bucket_agent_core::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: bucket_agent_core::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`bucket_agent_core::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.bucket/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`BUCKET_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `BUCKET_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<bucket_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<bucket_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `BUCKET_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `bucket dashboard`, the
    ...
    /// non-selectable headers. Gated by `BUCKET_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `BUCKET_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `bucket-tui-minimal` crate
    ...
    /// `bucket_agent_core::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `BUCKET_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: bucket_agent_core::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<bucket_agent_core::util::config::ContextualHintsRemote>,
    ...
    /// Login button label from `AuthMethod.name` (e.g., "bucket.com", "Acme Corp").
    ...
    /// `bucket_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, bucket_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `bucket_build_access_gate`. `Some` = blocked.
    pub gate: Option<bucket_agent_core::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperBucket", "Free").
    ...
    pub pending_gate_verification: Option<bucket_agent_core::auth::GateInfo>,
    ...
    pub provider_config_modal:
        Option<crate::views::provider_config_modal::ProviderConfigModalState>,
    ...
    /// `~/.bucket/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `BUCKET_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_config: bucket_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `BUCKET_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<bucket_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<bucket_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &bucket_agent_core::util::config::RemoteSettings,
    ) -> Option<bucket_agent_core::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(bucket_agent_core::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &bucket_agent_core::auth::AuthMeta) {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: bucket_agent_core::agent::config::UiConfig::default(),
            ...
            voice_config: bucket_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<bucket_agent_core::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && bucket_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: bucket_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(bucket_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(bucket_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `BUCKET_FPS` overlay occupies (0 in non-dev builds), so
    ...
    pub(crate) fn handle_input_with_paste_provenance(
        ...
        if let Some(modal) = self.provider_config_modal.as_mut() {
            ...
            let outcome_to_input = |o: ProviderConfigModalOutcome| match o {
                ...
                ProviderConfigModalOutcome::Cancelled => {
                    InputOutcome::Action(Action::ProviderConfigCancel)
                }
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<bucket_agent_core::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    bucket_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSuperbucketUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        bucket_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSuperbucketUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                bucket_agent_core::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            bucket_telemetry::session_ctx::log_event(
                                bucket_telemetry::events::SuperBucketUpsellShown {
                                    source:
                                        bucket_telemetry::events::SuperBucketUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`bucket_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        ...
        let key = bucket_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                bucket_telemetry::session_ctx::log_event(
                    bucket_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: bucket_agent_core::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::ContextualTip {
                tip: bucket_telemetry::events::ContextualTipKind::ImageInput,
                action: bucket_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = bucket_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![bucket_announcements::RemoteAnnouncement {
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = bucket_agent_core::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = bucket_agent_core::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&bucket_agent_core::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&bucket_agent_core::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = bucket_agent_core::auth::AuthMeta {
            subscription_tier: Some("SuperBucket".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&bucket_agent_core::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperBucket".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperBucket")));
        assert!(!is_restricted_tier(Some("SuperBucket Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(bucket_agent_core::auth::GateInfo {
            message: "Subscribe to use Bucket Build".into(),
            url: Some("https://bucket.com/superbucket?referrer=bucket-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = bucket_agent_core::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(bucket_workspace::foreign_sessions::RecentForeignSession {
                tool: bucket_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![bucket_workspace::file_system::FuzzyMatchResult {
