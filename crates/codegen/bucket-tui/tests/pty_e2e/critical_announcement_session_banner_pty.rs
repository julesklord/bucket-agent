...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "BUCKET_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// safe-open path — observed via the `BUCKET_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("BUCKET_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `BUCKET_ANNOUNCEMENTS_OVERRIDE`.
...
/// `BUCKET_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
