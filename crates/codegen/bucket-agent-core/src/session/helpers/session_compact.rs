...
pub use bucket_chat_state::compaction_utils::{
...
///
/// All other agents (bucket-build, etc.) continue to use the detailed
...
pub(crate) use bucket_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_bucket_conv_id = Some(sid.clone());
            message.x_bucket_req_id = Some(format!("bucket-compact-{}", uuid::Uuid::new_v4()));
            message.x_bucket_session_id = Some(sid);
            message.x_bucket_agent_id = Some(bucket_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = bucket_sampling_types::StopReason::from(fr);
                                truncated = matches!(sr, bucket_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_bucket_conv_id: Some(session_id.to_string()),
                x_bucket_req_id: Some(format!("bucket-compact-{}", uuid::Uuid::new_v4())),
                x_bucket_session_id: Some(session_id.to_string()),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            bucket_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            bucket_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: bucket_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            bucket_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, bucket_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        bucket_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            bucket_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            bucket_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// `bucket-chat-state`, rendering the system-reminder synchronously (no
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            bucket_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    /// Regression guard: bucket-build must DROP the working
    /// tail post-compaction. A prior change routed bucket-build to keep `recent_messages`,
    /// which survive only as `Tool call omitted...` stubs (dead tokens). Mirrors
    /// `summary_before_recent_compaction_with_no_user_query_yields_three_messages` for bucket-build
    /// (`summary_before_recent = false`).
    #[tokio::test]
    async fn bucket_build_compaction_drops_working_tail_regression_206460() {
        let conversation = vec![
            ConversationItem::system("You are a helpful assistant."),
            ConversationItem::user(
                "<user_info>\nOS: macos\n</user_info>\n\n<user_query>\nread auth.rs\n</user_query>",
        ...
        assert!(
            ...
            "bucket-build must drop recent_messages post-compaction",
        );
        let compacted = build_compacted_history(
            "You are a helpful assistant.",
            "<user_info>\nOS: macos\n</user_info>",
        ...
        assert!(
            !compacted
                ...
                .any(|i| matches!(i, ConversationItem::ToolResult(_))
                    || i.text_content() == "Tool call omitted..."),
            "no tail (ToolResult or stub) may leak into the bucket-build compacted history",
        );
    }
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use bucket_chat_state::compaction_utils::validate_compacted_history;
