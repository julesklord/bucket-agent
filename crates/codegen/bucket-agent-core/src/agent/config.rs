...
use crate::auth::{AuthManager, BucketComConfig, OidcAuthConfig};
...
pub const DEFAULT_AGENT_TYPE: &str = "bucket-build-plan";
...
pub struct EndpointsConfig {
    ...
    /// Env: `BUCKET_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `BUCKET_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `BUCKET_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `BUCKET_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `BUCKET_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `BUCKET_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `BUCKET_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `BUCKET_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `BUCKET_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `BUCKET_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `BUCKET_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `BUCKET_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `BUCKET_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bucket_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `BUCKET_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bucket_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `BUCKET_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("BUCKET_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `BUCKET_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`bucket setup`): explicit `managed_config_url`,
    ...
    /// 1. `bucket_internal_otlp_traces_endpoint` (verbatim)
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.bucket_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 BUCKET_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `bucket_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.bucket_internal_otlp_headers) {
    ...
    /// `bucket_internal_otlp_*` override shadowed that half.
    ///
    ...
    /// still honored by the internal pipeline even with `BUCKET_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("BUCKET_CLI_CHAT_PROXY_BASE_URL").ok(),
            xai_api_base_url: std::env::var("BUCKET_BUCKET_API_BASE_URL")
                .unwrap_or_else(|_| BUCKET_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("BUCKET_MODELS_BASE_URL"),
            models_list_url: env_string("BUCKET_MODELS_LIST_URL"),
            feedback_base_url: env_string("BUCKET_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("BUCKET_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("BUCKET_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("BUCKET_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("BUCKET_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("BUCKET_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("BUCKET_DEPLOYMENT_KEY"),
            managed_config_url: env_string("BUCKET_MANAGED_CONFIG_URL"),
...
pub use bucket_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use bucket_config::env_bool;
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> bucket_chat_state::CompactionMode {
    use bucket_chat_state::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> bucket_chat_state::CompactionDetail {
    use bucket_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(bucket_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: bucket_tools::types::compat::CompatCell,
...
pub(crate) use bucket_telemetry::config::env_telemetry_mode;
pub use bucket_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.bucket/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                bucket_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> bucket_agent::plugins::discovery::DiscoveryConfig {
        bucket_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `bucket-build-0.1`
...
/// (`bucket workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `bucket workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "BUCKET_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("BUCKET_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("BUCKET_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("BUCKET_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "BUCKET_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "bucket-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.bucket/skills/`,
/// `.agents/skills/`, `~/.bucket/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `bucket inspect`.
    #[serde(skip)]
    pub model_override_warnings: Vec<super::config_model_override_parse::ModelOverrideWarning>,
    pub bucket_com_config: BucketComConfig,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Typed as `BucketComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<BucketComConfig>,
    /// `[desktop]` section — owned by bucket-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array — consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<bucket_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `bucket_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from BUCKET_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<bucket_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `BUCKET_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.bucket/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, bucket_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, bucket_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<bucket_tools::implementations::bucket_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`bucket-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<bucket_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<bucket_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut bucket_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut bucket_agent::config::AgentDefinition) {
...
pub use bucket_agent::config::AgentDefinition;
pub use bucket_agent::config::Effort;
pub use bucket_agent::config::PermissionMode;
pub use bucket_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .bucket/agents/, ~/.bucket/agents/, built-ins)
...
/// 4. `BUCKET_AGENT` env var
/// 5. Default `bucket-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `bucket_agent::discovery::by_name_in_cwd()`.
    /// Examples: "bucket-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.bucket/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            bucket_com_config: BucketComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(bucket_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::bucket_home::bucket_home(),
            self.bucket_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.bucket_com_config.oidc.is_none() {
            config.bucket_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.bucket_com_config.oidc.is_none() && config.bucket_com_config.oauth2.is_none() {
            config.bucket_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("BUCKET_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[bucket_com_config]`.
    /// `[bucket_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[bucket_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("bucket_com_config") {
                ...
                table.insert("bucket_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("BUCKET_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `BUCKET_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("BUCKET_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("BUCKET_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-bucket-doom-loop-check`
    ...
    /// `BUCKET_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<bucket_sampling_types::DoomLoopRecoveryPolicy> {
        use bucket_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("BUCKET_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `BUCKET_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_ASK_USER_QUESTION")
    ...
    ///
    /// Precedence: requirements > `BUCKET_VOICE_MODE` > config/managed
    ...
    pub(crate) fn resolve_voice_mode(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_VOICE_MODE")
    ...
    /// `image_gen` tool gate. Default on; gated only by the `BUCKET_IMAGE_GEN`
    /// env var and managed-config requirement pin.
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        BoolFlag::env("BUCKET_IMAGE_GEN")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use bucket_tools::implementations::bucket_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("BUCKET_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("BUCKET_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("BUCKET_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("BUCKET_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("BUCKET_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_write_file(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_WRITE_FILE")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("BUCKET_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `BUCKET_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> bucket_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("BUCKET_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `BUCKET_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("BUCKET_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `BUCKET_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> bucket_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("BUCKET_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("BUCKET_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use bucket's default OAuth2 (xAI auth.x.ai).
    ///
    ...
    ///
    /// Priority: `--oauth` > BUCKET_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_bucket_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("BUCKET_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.bucket/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (BUCKET_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("BUCKET_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (BUCKET_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `BUCKET_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("BUCKET_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (BUCKET_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("BUCKET_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (BUCKET_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `BUCKET_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("BUCKET_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `BUCKET_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(bucket_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("BUCKET_ERROR_REPORTING"))
...
/// `BUCKET_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn bucket_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("BUCKET_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.bucket/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::bucket_home::bucket_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `BUCKET_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        bucket_config::load_merged_requirements().as_ref(),
        env_bool("BUCKET_EXTERNAL_OTEL"),
...
/// by `bucket setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: bucket_telemetry::external::config::ExternalClientInfo,
) -> Option<bucket_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<bucket_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| bucket_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            bucket_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = bucket_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = bucket_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        bucket_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = bucket_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    bucket_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub struct ModelEntryConfig {
    ...
    /// If not set, falls back to env_key, then BUCKET_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to BUCKET_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Can also be set via the `BUCKET_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// The base URL of the model (session endpoint). e.g. "https://cli-chat-proxy.bucket.com/v1"
    pub base_url: String,
    ...
    /// Always has a value; defaults to `"bucket-build-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<bucket_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Bucket starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `bucket_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`bucket-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`BUCKET_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`BUCKET_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `BUCKET_MCP_AUTO_RESTART` to opt out).
    ///
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: bucket_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
/// Priority: model api_key/env_key > session token > BUCKET_API_KEY.
///
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            bucket_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            bucket_chat_state::AuthType::SessionToken,
        )
        ...
        (Some(key), url, bucket_chat_state::AuthType::ApiKey)
    } else {
...
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == bucket_chat_state::AuthType::ApiKey
        && crate::util::is_first_party_xai_url(&creds.base_url)
    {
        creds.auth_type = bucket_chat_state::AuthType::SessionToken;
        creds.api_key = session_key.map(str::to_owned);
        bucket_telemetry::unified_log::debug(
...
pub use bucket_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.bucket_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
/// on BYOK / non-proxy routes for internal slugs like `bucket-build`).
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: bucket_chat_state::AuthType,
) -> bucket_chat_state::AuthType {
...
///
/// * cli-chat-proxy bases get `X-BUCKET-Token-Auth` and
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            .entry("X-BUCKET-Token-Auth".to_string())
            .or_insert_with(|| "bucket-cli".to_string());
        headers
...
mod tests {
    ...
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_bucket_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use bucket_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "bucket-4.5"
...
"#;
        ...
        let json = serde_json::json!(
            ...
            : "bucket-4.5", "reasoning_effort" : "low" }
        );
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("bucket-4.5"));
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        assert_eq!(
            headers.get("X-BUCKET-Token-Auth").map(String::as_str),
            Some("bucket-cli")
        );
        assert_eq!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        ...
        assert!(headers.get("X-BUCKET-Token-Auth").is_none());
    ...
    fn inject_url_derived_headers_does_not_overwrite_existing_entries() {
        let mut headers = IndexMap::new();
        headers.insert("X-BUCKET-Token-Auth".to_string(), "caller-set".to_string());
        inject_url_derived_headers(&mut headers, None, crate::env::PROD_CLI_CHAT_PROXY_BASE_URL);
        assert_eq!(
            headers.get("X-BUCKET-Token-Auth").map(String::as_str),
            Some("caller-set"),
    ...
    fn finalize_image_describe_sampler_none_uses_active_session_model_not_forced_helper() {
        ...
        assert_ne!(cfg.model, "bucket-build");
    }
    #[test]
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "bucket-4.5");
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: bucket_chat_state::AuthType::ApiKey,
    ...
    fn env_keys_resolve_first_set_wins() {
        ...
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "BUCKET_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "BUCKET_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "BUCKET_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "BUCKET_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "BUCKET_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["BUCKET_TEST_WS_PRIMARY", "BUCKET_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "BUCKET_TEST_WS_PRIMARY" => Some("   ".into()),
                "BUCKET_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("BUCKET_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("BUCKET_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use bucket_test_support::EnvGuard;
        let var = "BUCKET_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        ...
        let primary = "BUCKET_TEST_MULTI_ENV_PRIMARY";
        let alias = "BUCKET_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use bucket_chat_state::AuthType;
        use bucket_test_support::EnvGuard;
        let primary = "BUCKET_TEST_EMPTY_ENV_PRIMARY";
        let alias = "BUCKET_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "BUCKET_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "BUCKET_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(BUCKET_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_BUCKET_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn resolve_credentials_no_session_key_returns_api_key() {
        let model = test_model_entry("m", "https://example.com/v1", None, None, None);
        let creds = resolve_credentials(&model, None);
        assert_eq!(creds.auth_type, bucket_chat_state::AuthType::ApiKey);
    }
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("bucket-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: bucket_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        ...
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("bucket-secret"));
        let mut creds = api_key_creds("https://api.x.ai/v1");
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("bucket-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = bucket_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_adds_api_key_to_default_model() {
        ...
        assert_eq!(
            model.info.base_url, "https://cli-chat-proxy.bucket.com/v1",
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use bucket_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use bucket_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(bucket_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    /// bucket-agent-core/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_bucket_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["bucket[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["bucket["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_bucket_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.bucket_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn bucket_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.bucket_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.bucket_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.bucket_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.bucket_com_config.force_login_team_uuid,
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("BUCKET_API_KEY", "bucket-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("BUCKET_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "BUCKET_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-bucket"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-bucket").unwrap();
        assert_eq!(model.info.model, "bucket-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    fn e2e_default_model_with_session_routes_to_proxy() {
        ...
        assert_eq!(
            sampling.base_url, "https://cli-chat-proxy.bucket.com/v1",
    ...
    fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
        ...
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("BUCKET_API_KEY", "bucket-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("bucket-external-key"));
        assert_eq!(
    ...
    fn e2e_user_config_overrides_prefetched_model() {
        ...
        prefetched.insert(
            dm.to_string(),
            test_model_entry(dm, "https://cli-chat-proxy.bucket.com/v1", None, None, None),
        );
        let (_, models) = resolve_models_from_toml(
            &format!(
                r#"
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        ...
        unsafe { std::env::set_var("BUCKET_API_KEY", "env-key") };
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-bucket").unwrap();
        ...
        assert_eq!(sampling.base_url, "https://cli-chat-proxy.bucket.com/v1",);
    }
    #[test]
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-bucket")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-bucket")),
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            ...
            "BUCKET_FEEDBACK_BASE_URL",
            "BUCKET_TRACE_UPLOAD_URL",
            "BUCKET_MANAGED_CONFIG_URL",
            ...
            "BUCKET_INTERNAL_OTLP_TRACES_ENDPOINT",
            "BUCKET_INTERNAL_OTLP_HEADERS",
            "BUCKET_EXTERNAL_OTEL",
        ] {
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("BUCKET_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("BUCKET_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("BUCKET_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("BUCKET_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("BUCKET_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("BUCKET_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("BUCKET_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("BUCKET_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("BUCKET_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("BUCKET_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("BUCKET_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("BUCKET_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("BUCKET_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("BUCKET_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("BUCKET_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("BUCKET_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("bucket-imagine-image")).resolve_image_gen_model_override(),
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("BUCKET_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("BUCKET_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("BUCKET_GOAL");
            std::env::remove_var("BUCKET_GOAL_CLASSIFIER");
            std::env::remove_var("BUCKET_GOAL_PLANNER");
            std::env::remove_var("BUCKET_GOAL_SUMMARY");
            std::env::remove_var("BUCKET_GOAL_VERIFIER_N");
            std::env::remove_var("BUCKET_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("BUCKET_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("BUCKET_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("BUCKET_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("BUCKET_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("BUCKET_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("BUCKET_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "BUCKET_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "bucket-build", agent_type = "bucket-build-plan" }

[goal.strategist_model]
model = "bucket-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "bucket-build"
agent_type = "bucket-build-plan"
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models[0].model, "bucket-build");
        assert_eq!(
...
planner_model = { agent_type = "bucket-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "bucket-build");
        assert_eq!(cfg.goal.skeptic_models[1].model, "bucket-composer-2.5-fast");
    }
...
classifier_max_runs = 6
planner_model = { model = "bucket-build", agent_type = "bucket-build-plan" }
strategist_model = { model = "bucket-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let bucket_build = crate::util::config::GoalRoleModel {
            model: "bucket-build".into(),
            agent_type: "bucket-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "bucket-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(bucket_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(bucket_build),
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "bucket-token-test"
        "#,
        )
        .unwrap();
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            bucket_internal_otlp_traces_endpoint: None,
            bucket_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `bucket_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_bucket_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                bucket_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: BUCKET_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                bucket_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                bucket_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `bucket_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                bucket_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> bucket_telemetry::external::config::ExternalClientInfo {
        bucket_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("BUCKET_EXTERNAL_OTEL", "1")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("BUCKET_EXTERNAL_OTEL", "0")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `BUCKET_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("BUCKET_SUBAGENTS");
            std::env::remove_var("BUCKET_RESPECT_GITIGNORE");
            std::env::remove_var("BUCKET_WEB_SEARCH_MODEL");
            std::env::remove_var("BUCKET_SESSION_SUMMARY_MODEL");
            std::env::remove_var("BUCKET_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("BUCKET_CURSOR_RULES_ENABLED");
            std::env::remove_var("BUCKET_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("BUCKET_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("BUCKET_CLAUDE_RULES_ENABLED");
            std::env::remove_var("BUCKET_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("BUCKET_MANAGED_MCPS_ENABLED");
            std::env::remove_var("BUCKET_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("BUCKET_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "BUCKET_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "BUCKET_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "BUCKET_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("BUCKET_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("BUCKET_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("BUCKET_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("BUCKET_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("BUCKET_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("BUCKET_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("BUCKET_TELEMETRY_ENABLED") };
...
"#,
        ...
        bucket_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("bucket-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.bucket-build] sets
    /// context_window=500k for model="bucket-4.5", but
    /// [models].default="bucket-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "bucket-build", not key "bucket-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        assert_eq!(
            ...
            500_000,
            "BUG: prefetched 'bucket-4.5' should inherit 500k from \
             sibling 'bucket-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        prefetched.insert("bucket-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("bucket-4.5").unwrap();
        assert_eq!(
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("bucket-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("bucket-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn resolve_model_list_inherits_context_window_from_default_when_prefetched_has_fallback() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("bucket-build", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("bucket-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("bucket-build").expect("model must exist");
        assert_ne!(
    ...
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("bucket-build", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("bucket-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("bucket-build").expect("model must exist");
    ...
    fn resolve_model_list_inherits_agent_type_and_api_backend() {
        ...
        if let Some(default) = defaults.get("bucket-build") {
    ...
    fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
        ...
        if let Some(e) = defs.shift_remove("bucket-build") {
            p.insert("bucket-build".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("bucket-build"));
    ...
    fn resolve_model_list_prefetch_replaces_bundled_entirely() {
        let cfg = Config::default();
        let mut p = IndexMap::new();
        let e = prefetch_model_entry("bucket-4.5", 500_000, ApiBackend::Responses);
        p.insert("bucket-4.5".to_string(), e);
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("bucket-4.5"));
        assert!(!resolved.contains_key("bucket-build"));
    }
    #[test]
    ...
    /// Regression: enterprise managed config aliases bucket-build to their own
    /// endpoint with env_key. The bundled bucket-build has supported_in_api=false.
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("BUCKET_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("BUCKET_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("BUCKET_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("BUCKET_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("BUCKET_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("BUCKET_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("BUCKET_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("BUCKET_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("BUCKET_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("BUCKET_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("BUCKET_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("BUCKET_MCP_RECURSIVE_CONFIG_WATCH", "false") };
