...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.bucket_ws_origin {
        config.bucket_com_config.bucket_ws_origin = v.clone();
    }
    if let Some(v) = &args.bucket_ws_url {
        config.bucket_com_config.bucket_ws_url = v.clone();
    }
}
...
/// Entrypoint tag for `bucket -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(bucket_telemetry::sampling_log::layer())
        .with(bucket_telemetry::instrumentation::layer())
        .with(bucket_telemetry::hooks_log::layer())
        .with(bucket_telemetry::otel_layer::build_otel_layer(
            bucket_telemetry::otel_layer::OtelClientInfo {
                client_name: "bucket-agent",
                client_version: bucket_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            bucket_agent_core::auth::credential_provider::build_default_otel_layer_config(),
        ));
    bucket_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    bucket_telemetry::external::init(
        bucket_agent_core::agent::config::resolve_external_otel_config(
            bucket_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: bucket_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `bucket setup`: rendering + exit codes only; fetch logic lives in `bucket_agent_core::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use bucket_agent_core::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `bucket login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export BUCKET_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:BUCKET_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  bucket setup");
        eprintln!();
        eprintln!("Or add the key to ~/.bucket/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's Bucket administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `bucket setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = bucket_agent_core::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !bucket_agent_core::util::is_bucket_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a bucket process, removing stale lock");
        ...
        if let Err(e) = bucket_agent_core::util::kill_process_by_pid(pid) {
...
) -> Result<(LeaderDescriptor, bucket_agent_core::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = bucket_agent_core::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = bucket_agent_core::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "bucket-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&bucket_agent_core::leader::ControlPayload>,
...
/// Env override for the `bucket workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "BUCKET_WORKSPACE_COMMAND";
/// Resolution of the `bucket workspace` gate. `Unknown` is kept separate from
...
/// The `BUCKET_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&bucket_agent_core::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for bucket on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<bucket_agent_core::util::config::RemoteSettings> {
    join_early_prefetch(bucket_agent_core::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`bucket workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `bucket workspace`. Check your \
             network connection (run `bucket login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.bucket_com_config.bucket_ws_url;
    let socket = socket_path_for_ws_url(ws_url);
    LeaderClient::connect(
        socket,
        "bucket-workspace-cli",
        ClientMode::Stdio,
        ClientCapabilities::default(),
    )
    .await
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a bucket session, or run `bucket workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = bucket_agent_core::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use bucket_agent_core::auth::ensure_authenticated;
    bucket_agent_core::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = bucket_agent_core::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`bucket workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.bucket/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.bucket_com_config,
        false,
        Some("No cached credentials found. Run `bucket login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.bucket_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    bucket_telemetry::sentry::flush_on_shutdown();
    bucket_telemetry::otel_layer::shutdown_otel();
    bucket_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "bucket: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<bucket_tui::app::AgentArgs>,
    ...
    bucket_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => bucket_agent_core::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = bucket_agent_core::agent::models::start_early_prefetch(None);
    bucket_agent_core::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Bucket Build (pager) - v{}",
            bucket_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                bucket_updater::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(bucket_agent_core::sampling::types::parse_canonical_effort_token);
    let launch_yolo = bucket_agent_core::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("bucket: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = bucket_agent_core::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = bucket_acp::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = bucket_agent_core::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
        ...
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(bucket_agent_core::agent::app::LeaderAutoUpdateConfig {
...
/// Sets `BUCKET_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `bucket dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `bucket dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `BUCKET_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !bucket_tui::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.bucket/config.toml and \
             unsetting BUCKET_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("BUCKET_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<bucket_tui::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(bucket_tui::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<bucket_agent_core::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(bucket_agent_core::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    bucket_agent_core::heap_profile::install(bucket_agent_core::heap_profile::HeapProfileHooks {
...
fn main() {
    bucket_tui_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    bucket_tui::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        bucket_tui::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        bucket_tui::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = bucket_tui::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    bucket_tui::memory_trace::start(
        bucket_agent_core::util::bucket_home::bucket_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = bucket_config::validate_requirements() {
        eprintln!("Couldn't start Bucket: {e}");
        eprintln!();
        eprintln!(
            "Update Bucket to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = bucket_telemetry::sentry::init(bucket_telemetry::sentry::Config {
        client: "bucket-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: bucket_agent_core::agent::config::is_error_reporting_disabled_sync(),
    });
    bucket_tui::docs::extract_user_guide_docs(&bucket_agent_core::util::bucket_home::bucket_home());
    bucket_crash_handler::install_terminal_restore_only();
    if bucket_agent_core::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = bucket_agent_core::util::bucket_home::bucket_home().join("crash");
        if let Some(report) = bucket_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("Bucket crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !bucket_crash_handler::install(bucket_crash_handler::CrashHandlerConfig {
    ...
    let crashed = bucket_agent_core::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    bucket_telemetry::debug_log::flush();
    if let Err(e) = result {
        bucket_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("BUCKET_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(
    ...
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(bucket_agent_core::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("BUCKET_DEBUG_LOG", path);
            std::env::remove_var("BUCKET_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("BUCKET_DEBUG_LOG", "1");
        set_if_unset("BUCKET_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        bucket_tui::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return bucket_tui::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        bucket_tui::app::cli::SandboxStartup::Apply(profile) => profile,
        bucket_tui::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    bucket_agent_core::config::apply_sandbox(
        None,
    ...
    bucket_agent_core::http::set_client_name(if is_interactive {
        bucket_workspace::permission::ClientType::BucketPager
    } else {
        bucket_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        bucket_updater::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `bucket-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                bucket_agent_core::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return bucket_tui::mcp_cmd::run(mcp_args).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return bucket_tui::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return bucket_tui::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return bucket_tui::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return bucket_tui::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return bucket_tui::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return bucket_tui::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                bucket_agent_core::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                bucket_agent_core::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = bucket_agent_core::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                bucket_agent_core::auth::run_cli_logout(&config)?;
                bucket_agent_core::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = bucket_tui::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(bucket_tui::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == bucket_tui::headless::OutputFormat::Plain {
                args.output_format = bucket_tui::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return bucket_tui::headless::run_single_turn(
            prompt,
            args.verbatim,
            bucket_tui::headless::HeadlessOptions {
    ...
    let result = bucket_tui::app::run(args, bg_update_rx).await;
    bucket_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `bucket` to start.");
            } else {
                eprintln!("Update did not complete. Run `bucket update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `bucket update` child
...
/// fall back to a fresh blocking `bucket update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = bucket_agent_core::env::BucketBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                bucket_agent_core::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("BUCKET_NPM_REGISTRY"))
        .ok()
        .or_else(bucket_agent_core::util::config::load_npm_registry_sync);
    if let Ok(root) = bucket_agent_core::config::load_effective_config_disk_only()
        && let Some(ch) = bucket_agent_core::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("BUCKET_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<bucket_home>/bin/bucket` resolves to, the
...
/// npm shim hardcodes `~/.bucket`, so a custom `BUCKET_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, bucket_home: &std::path::Path) -> bool {
    if bucket_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = bucket_config::bucket_application_in(bucket_home);
...
/// Handle `bucket-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `bucket update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in bucket_agent_core::leader::discover_leaders().await {
        if d.classification != bucket_agent_core::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match bucket_agent_core::leader::LeaderClient::connect(
            socket_path,
            "bucket-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(bucket_agent_core::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(bucket_agent_core::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            bucket_agent_core::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: bucket_agent_core::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/bucket-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            bucket_agent_core::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!bucket_agent_core::heap_profile::prof_available());
            return;
        }
        assert!(bucket_agent_core::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(bucket_agent_core::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(bucket_agent_core::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        bucket_agent_core::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_bucket_target() {
        ...
        assert!(!is_managed_install(
            Some(home.join("bin").join("bucket")),
            &home
        ));
        ...
        let target = home.join("downloads").join("bucket-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("bucket")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("bucket")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("bucket-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("BUCKET_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("BUCKET_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `bucket dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(BUCKET_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["bucket", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    /// `BUCKET_AGENT_DASHBOARD=0` disables the feature — the subcommand
    /// must error visibly before the TUI starts.
    #[serial_test::serial(BUCKET_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_errors_when_disabled() {
        unsafe { std::env::set_var("BUCKET_AGENT_DASHBOARD", "0") };
        let mut args = PagerArgs::try_parse_from(["bucket", "dashboard"]).unwrap();
        let result = flag_dashboard_at_startup_if_requested(&mut args);
        unsafe { std::env::remove_var("BUCKET_AGENT_DASHBOARD") };
        let err = result.expect_err("disabled dashboard must error");
        assert!(err.to_string().contains("disabled"), "got: {err}");
        assert!(
            std::env::var("BUCKET_OPEN_DASHBOARD_AT_STARTUP").is_err(),
    ...
    #[serial_test::serial(BUCKET_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("BUCKET_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("BUCKET_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("BUCKET_WORKSPACE_COMMAND", "off") };
