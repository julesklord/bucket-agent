...
//!
//! Catalog file lookup, in order: `.bucket-plugin/marketplace.json` (preferred),
//! `.bucket-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.bucket-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.bucket-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_bucket_dir() {
        let dir = tempfile::tempdir().unwrap();
        let bucket_dir = dir.path().join(".bucket-plugin");
        std::fs::create_dir_all(&bucket_dir).unwrap();
        std::fs::write(
            bucket_dir.join("marketplace.json"),
            r#"{"name": "bucket", "plugins": []}"#,
        )
        .unwrap();
    ...
    #[test]
    fn load_index_bucket_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".bucket-plugin", "bucket"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "bucket");
    }
