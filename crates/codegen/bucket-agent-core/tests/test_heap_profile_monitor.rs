...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = BucketComConfig::default().auth_scope();
    let auth = BucketAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[bucket_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), BucketComConfig::default()));
