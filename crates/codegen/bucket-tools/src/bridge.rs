//! ToolBridge: adapter that wraps `bucket-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the bucket_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, bucket_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, bucket_tool_runtime::ToolError> {
        use crate::implementations::bucket_build::scheduler::types::{
        ...
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    bucket_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
...
mod tests {
    ...
    impl bucket_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> bucket_tool_protocol::ToolId {
            bucket_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::bucket_tool_runtime::ListToolsContext,
        ) -> bucket_tool_types::ToolDescription {
            bucket_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + bucket_build's snake_case in one registry
