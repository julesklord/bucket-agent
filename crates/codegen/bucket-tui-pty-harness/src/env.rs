...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build bucket-tui"))?;

    if !output.status.success() {
        bail!(
            "failed to build bucket-tui (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_bucket-tui` (set by `cargo test`)
/// 3. Build locally via `cargo build -p bucket-bin` (the composition-
///    root package that owns the `bucket-tui` binary)
