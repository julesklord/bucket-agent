...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, BucketAuth};
...
pub enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `bucket login` or set BUCKET_API_KEY instead."
    )]
    NotEnabled,
    #[error(transparent)]
...
/// `x-bucket-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-bucket-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
                ("scope", scope_str.as_str()),
                ("referrer", "bucket-build"),
            ]),
        &url,
    )
    .send()
...
///
/// On success, persists credentials to `~/.bucket/auth.json` and returns
/// the authenticated `BucketAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `bucket login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, BucketComConfig};
    ...
    fn auth_manager_with_bucket_home(
        bucket_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(bucket_home, BucketComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let bucket_home = temp_dir.path().join(".bucket");
        std::fs::create_dir_all(&bucket_home).unwrap();
        let auth_manager = auth_manager_with_bucket_home(&bucket_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access bucket-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access bucket-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access bucket-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: BucketComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let bucket_home = temp_dir.path().join(".bucket");
        std::fs::create_dir_all(&bucket_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&bucket_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !bucket_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::BucketAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_bucket_home(temp_dir.path(), "http://127.0.0.1:9");
