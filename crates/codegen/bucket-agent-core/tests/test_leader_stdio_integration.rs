...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, bucket_agent_core::leader::ServerHandle) {
...
async fn test_session_new_valid_default_model_injected() {
    ...
    let meta = &json["params"]["_meta"];
    assert_eq!(meta["modelId"], "bucket-3-fast");
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    invoker
        .send(format!(
            r#"{{"jsonrpc":"2.0","id":42,"method":"session/setModel","params":{{"sessionId":"{}","modelId":"bucket-4"}}}}"#,
            shared_sid
        ))
        .unwrap();
    let received = acp_rx.recv().await.unwrap();
    ...
    assert_eq!(json["params"]["modelId"], "bucket-4");
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"x.ai/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"bucket-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    response_tx.send(broadcast.clone()).unwrap();
    let response = format!(
        r#"{{"jsonrpc":"2.0","result":{{"meta":{{"model":"bucket-4"}}}},"id":"{}"}}"#,
        setmodel_ns_id
    );
    response_tx.send(response).unwrap();
    ...
    assert_eq!(inv1["params"]["update"]["model_id"], "bucket-4");
    ...
    assert_eq!(inv2["result"]["meta"]["model"], "bucket-4");
    ...
    assert_eq!(f["params"]["update"]["model_id"], "bucket-4");
...
async fn test_session_ownership_cleanup_on_disconnect() {
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(bucket_agent_core::leader::ShutdownReason::Manual);
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(bucket_agent_core::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use bucket_agent_core::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != bucket_agent_core::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        bucket_agent_core::leader::DisconnectReason::LeaderShutdown
    );
}
...
///
/// This binary does not sandbox BUCKET_HOME, so on a dev machine these entries
/// land in the real `~/.bucket` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = bucket_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = bucket_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        bucket_agent_core::leader::DisconnectReason::Connected,
