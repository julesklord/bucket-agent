...
//! (SuperBucket, SuperBucket Heavy/Lite, X Premium/+, and any unknown future name)
...
mod tests {
    ...
    fn unrestricted_names() {
        assert!(!is_restricted_tier_name("SuperBucket"));
        assert!(!is_restricted_tier_name("SuperBucket Heavy"));
        assert!(!is_restricted_tier_name("superbucket_lite"));
