...
use bucket_agent_core::env::BucketBuildEnvironment;
...
///
/// Constructed once from `BucketBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `BucketBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.bucket.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.bucket/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (BUCKET_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &BucketBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: bucket_agent_core::auth::BucketComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = bucket_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = BucketVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<BucketVersion>(&version_str)
...
pub use bucket_version::installed as get_installed_bucket_version;

/// Version of the managed bucket binary currently on disk, read from the
/// `~/.bucket/bin/bucket` symlink target (`../downloads/bucket-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `bucket update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.bucket/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = bucket_agent_core::util::bucket_home::bucket_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "bucket")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`bucket-0.1.150-macos-aarch64`, including
/// pre-releases: `bucket-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`bucket-0.1.150`,
/// `bucket-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`bucket-latest`, `bucket-pager-*` when `bin_prefix` is
/// `bucket`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.bucket/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: BucketVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(bucket_version::VERSION, &stable)
    })
}
...
/// `~/.bucket/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(bucket_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = BucketVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`bucket-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("bucket-0.2.46-darwin-arm64", Some("0.2.46")),
            ("bucket-0.1.220-linux-x86_64", Some("0.1.220")),
            ("bucket-0.2.5-windows-x86_64.exe", Some("0.2.5")),
        ...
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "bucket").as_deref(),
                *expected,
        ...
        // prefix but not under "bucket".
        assert_eq!(
            version_from_versioned_binary_name("bucket-pager-0.1.5-darwin-arm64", "bucket-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // BucketVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: BucketVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = BucketVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: BucketVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<BucketVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<BucketVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = BucketVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use bucket_agent_core::env::BucketBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&BucketBuildEnvironment::Production);
