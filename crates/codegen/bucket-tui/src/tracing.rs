...
/// firehose (`BUCKET_DEBUG_LOG` / `BUCKET_LOG_FILE`).
pub use bucket_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `bucket-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use bucket_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "bucket_agent_core=info,bucket_tui=trace,bucket_tools=info,bucket_acp=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = bucket_telemetry::otel_layer::build_otel_layer(
        bucket_telemetry::otel_layer::OtelClientInfo {
            client_name: "bucket-pager",
    ...
    let instrumentation_layer = bucket_telemetry::instrumentation::layer();
    let sampling_log_layer = bucket_telemetry::sampling_log::layer();
    let hooks_log_layer = bucket_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    bucket_telemetry::debug_log::install_firehose(registry, "tui");
