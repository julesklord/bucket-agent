...
fn external_stream_end_to_end() {
    ...
    let mut cfg = bucket_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "BUCKET_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = bucket_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    bucket_telemetry::external::init(Some(cfg));
    assert!(bucket_telemetry::external::is_active());
    ...
    assert!(!bucket_telemetry::is_enabled());
    bucket_telemetry::log_event(bucket_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: bucket_telemetry::enums::PermissionMode::Ask,
    });
    bucket_telemetry::log_event(bucket_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    bucket_telemetry::log_event(bucket_telemetry::events::PromptSubmitted {
    ...
    bucket_telemetry::log_event(bucket_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("bucket-cli")),
        "service.name=bucket-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "bucket_code.session_start",
        "bucket_code.user_prompt",
        "bucket_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "bucket_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "bucket_code.session.count" {
    ...
    bucket_telemetry::external::shutdown();
    assert!(
