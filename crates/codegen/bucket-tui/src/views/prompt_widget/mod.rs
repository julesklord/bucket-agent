...
//!  bucket-3 · yolo                             ← info line (optional)
//! ```
//!
...
pub struct PromptWidget {
    ...
    hovered_image_element_id: Option<bucket_ratatui_textarea::ElementId>,
    /// Image shown immediately after insertion without moving the edit cursor.
    post_insert_image_preview: Option<(bucket_ratatui_textarea::ElementId, usize)>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod bucket-4` yields `/model bucket-4`, not `/model  bucket-4`
            ...
            // Replace the args range (e.g., "gr" → "Bucket 4 Fast").
...
impl PromptWidget {
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use bucket_telemetry::events::BackspaceNoEffect;
    ...
    pub fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> PromptEvent {
        use bucket_ratatui_textarea::{MouseAction, TextElementEventKind};
    ...
    fn image_element_near_cursor(&self) -> Option<&bucket_ratatui_textarea::TextElement> {
    ...
    pub fn textarea(&self) -> &bucket_ratatui_textarea::TextArea {
        &self.textarea
    }
...
fn strip_all_elements(
    text: &str,
    cursor: usize,
    textarea: &bucket_ratatui_textarea::TextArea,
) -> (String, usize) {
...
fn map_clean_offset_to_raw(
    text: &str,
    clean_offset: usize,
    elements: &[bucket_ratatui_textarea::TextElement],
) -> usize {
    if elements.is_empty() {
