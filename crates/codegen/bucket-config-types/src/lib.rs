...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-bucket-doom-loop-check` header and parse the reported
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `bucket_build_settings`
    ...
    /// default (on). Sits below env `BUCKET_FOLDER_TRUST`, user
    ...
    /// remote settings `bucket_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `bucket_build_settings` remote settings key) to enable it. See
    ...
    /// `bucket_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// `BUCKET_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `bucket_build_settings`).
    ...
    /// ghost text), from the `bucket_build_settings` remote settings flag. Sits below
    /// env (`BUCKET_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `bucket-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `bucket-imagine-image`) instead of the default quality model
    /// (`bucket-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `BUCKET_VOICE_MODE`. Free-tier SuperBucket upsell
    ...
    /// User's subscription tier from remote settings `bucket_build_access_gate`.
    /// E.g. "free", "premium", "superbucket", "superbucket_heavy".
    ...
    /// Whether the user is allowed to use Bucket Build. Set by remote settings
    /// `bucket_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "SuperBucket", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `bucket_build_usage_redirect_url`
    ...
    /// `bucket_build_settings`. Per-model override on `ModelInfo`
    /// (`bucket_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`BUCKET_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`BUCKET_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`BUCKET_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`BUCKET_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `bucket workspace` CLI command (Computer Hub
    /// workspace exposure), from `bucket_build_settings.workspace_command_enabled`.
...
/// only guaranteed to work with a compatible harness (cursor vs bucket-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "bucket-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "bucket-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-bucket-build toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "bucket-build"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("bucket-build"));
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "bucket-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "bucket-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "bucket-4", "agent_type": "general-purpose"},
            {"model": "bucket-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "bucket-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "bucket-4.5", "agent_type": "cursor"},
            "default_model": "bucket-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        assert_eq!(
        ...
        assert_eq!(s.default_model.as_deref(), Some("bucket-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "bucket-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
