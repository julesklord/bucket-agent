...
4. The `BUCKET_API_KEY` environment variable (global fallback)
...
```toml
[models]
web_search = "bucket-4.20-multi-agent"
```
...
curl -s https://api.example.com/v1/models -H "Authorization: Bearer $BUCKET_API_KEY"
```
