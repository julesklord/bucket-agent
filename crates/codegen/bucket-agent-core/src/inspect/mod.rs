//! `bucket inspect` — configuration introspection.
//!
//! Shows everything Bucket discovers in the current directory: project
...
/// Enterprise login-hardening policy resolved from `[bucket_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `BUCKET_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .bucket/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `bucket inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = bucket_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = bucket_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = bucket_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        bucket_version: bucket_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".bucket")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = bucket_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        bucket_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let bucket_home = Some(crate::util::bucket_home::bucket_home());
    ...
    // `bucket_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both Bucket TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use bucket_workspace::permission::resolution;
...
/// (`[bucket_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let bucket_com_config = config
        .map(|c| c.bucket_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: bucket_com_config.api_key_auth_disabled(),
        disable_api_key_auth: bucket_com_config.disable_api_key_auth,
        force_login_team_uuid: bucket_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[bucket_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = bucket_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        bucket_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&bucket_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &bucket_agent::plugins::PluginRegistry,
    skills_config: &bucket_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = bucket_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &bucket_home);
...
/// `<bucket_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
fn skill_entry_source(
    s: &bucket_agent::prompt::skills::SkillInfo,
    bucket_home: &Path,
) -> ConfigSource {
    use bucket_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, bucket_home) {
...
fn list_agents(
    ...
    let agents = bucket_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[bucket_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                bucket_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                bucket_agent::plugins::PluginScope::Project => Scope::Project,
                bucket_agent::plugins::PluginScope::User => Scope::User,
                bucket_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: bucket_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    bucket_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = bucket_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `bucket mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.bucket/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_bucket_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.bucket_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `bucket mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use bucket_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use bucket_tools::implementations::skills::types::SkillScope;
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.bucket/requirements.toml";
    ...
    fn model_override_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("bucket-4.5"));

        let human = render_model_override_warnings(&warnings);
        assert!(human.contains("Model Overrides"), "{human}");
        assert!(
            human.contains("[model.\"bucket-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"bucket-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["modelKey"], "bucket-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.bucket");

        let s = skill_fixture("a", "/repo/.bucket/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.bucket/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.bucket/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.bucket/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<bucket_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.bucket/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Bundled name in a user dir outside <bucket_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <bucket_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.bucket/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <bucket_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.bucket/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.bucket dirs.
        ...
        let registry = bucket_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
