...
pub(super) fn build_middleware_client(
    ...
    bucket_http::with_auth_retry(bucket_http::shared_client(), credentials)
}
...
fn build_static_middleware_client(
    ...
    let provider: std::sync::Arc<dyn bucket_auth::AuthCredentialProvider> = std::sync::Arc::new(
        bucket_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl bucket_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = bucket_http::shared_client()
