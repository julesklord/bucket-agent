...
// The `Shared` summarizer reuses bucket-build's full-replace summarization core
...
/// `FullReplace` strategy (default): bucket-build's full-replace — summarize the
...
/// `<bucket_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching bucket-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through bucket-build's shared core.
    ...
    //     Shared with Bucket chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<bucket_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<bucket_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<bucket_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): bucket-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<bucket_user_queries>` preamble so
...
/// — bucket-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and bucket-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // bucket-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // bucket-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Bucket chat's
...
/// the literal error messages produced by the Bucket chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Bucket chat sampler when the
    ...
    /// — guards the forwarding that Bucket chat (`Arc<BucketTurn>`) relies on.
    #[test]
