//! Parse marketplace sources from `~/.bucket/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "xAI Official"
//! git = "https://github.com/bucket-org/bucket-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// instead of `~/.bucket`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("bucket-org") && branch.as_deref() == Some("main"))
        );
    }
