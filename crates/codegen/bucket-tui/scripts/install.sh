#!/bin/bash
#
# Bucket CLI installer — https://x.ai/cli/install.sh
#
# Auth: BUCKET_DEPLOYMENT_KEY (takes precedence) or ~/.bucket/auth.json from `bucket login`.
# Env: BUCKET_CHANNEL (stable|alpha|enterprise, default: stable), BUCKET_BIN_DIR, BUCKET_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   BUCKET_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.bucket/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_bucket_token() {
    local auth_file="$HOME/.bucket/auth.json"
    local scope="$1"
...
# Resolve auth: BUCKET_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$BUCKET_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_bucket_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_bucket_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.bucket/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.bucket/auth.json." >&2
    fi
fi
...
BASE_URL_FALLBACK="https://storage.googleapis.com/bucket-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.bucket/downloads"
BIN_DIR="${BUCKET_BIN_DIR:-$HOME/.bucket/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${BUCKET_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Bucket $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Bucket $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/bucket-$platform"
artifact_base="${BASE_URL}/bucket-${version}-${platform}"
...
echo "  Downloading bucket ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Bucket is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in bucket.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/bucket.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded bucket failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.bucket/bin and ~/.bucket/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/bucket"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/bucket and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.bucket/completions/bash" "$HOME/.bucket/completions/zsh"
"$BIN_DIR/bucket" completions bash > "$HOME/.bucket/completions/bash/bucket.bash" 2>/dev/null || true
"$BIN_DIR/bucket" completions zsh  > "$HOME/.bucket/completions/zsh/_bucket"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/bucket" completions fish > "$HOME/.config/fish/completions/bucket.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.bucket/config.toml"
...
if [ -n "$BUCKET_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${BUCKET_PROXY_URL:-https://cli-chat-proxy.bucket.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$BUCKET_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.bucket/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.bucket/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.bucket/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.bucket/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Bucket $version installed to $BIN_DIR/bucket.exe" >&2
else
    echo "Bucket $version installed to $BIN_DIR/bucket" >&2
fi

# --- Ensure bucket is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so bucket works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/bucket" "$candidate/bucket"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/bucket -> $BIN_DIR/bucket" >&2
...
# Also update shell config so ~/.bucket/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> bucket installer >>>
fish_add_path $HOME/.bucket/bin
# <<< bucket installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> bucket installer >>>
export PATH="$HOME/.bucket/bin:$PATH"
fpath=(~/.bucket/completions/zsh $fpath)
...
# <<< bucket installer <<<'
    ...
    if grep -qs "bucket installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> bucket installer >>>/ { skip=1; next }
            /# <<< bucket installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'bucket' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'bucket' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'bucket' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.bucket/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use bucket from cmd.exe or PowerShell, add %USERPROFILE%\\.bucket\\bin to your PATH." >&2
fi
