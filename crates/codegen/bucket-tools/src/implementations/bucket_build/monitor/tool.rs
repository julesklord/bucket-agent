...
impl bucket_tool_runtime::Tool for MonitorTool {
    type Args = MonitorInput;
    type Output = MonitorOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("monitor").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: MonitorInput,
    ) -> Result<MonitorOutput, bucket_tool_runtime::ToolError> {
