...
Hooks let you run a script or send an HTTP request at key moments in a Bucket session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Bucket calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.bucket/hooks
   ```

2. Create a hook file, e.g. `~/.bucket/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Bucket session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Bucket session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.bucket/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.bucket/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.bucket/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.bucket/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.bucket/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`BUCKET_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Bucket accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Bucket skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Bucket maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Bucket sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `BUCKET_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `BUCKET_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `BUCKET_SESSION_ID`     | The unique identifier of the current Bucket session. |
| `BUCKET_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `BUCKET_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Bucket additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `BUCKET_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `BUCKET_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`BUCKET_PLUGIN_ROOT`, `BUCKET_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/bucket-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Bucket re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.bucket/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.bucket/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug BUCKET_LOG_FILE=/tmp/bucket.log bucket`, then check `/tmp/bucket.log`.
