...
//! its own `OnceLock<BUCKET_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the BUCKET_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes BUCKET_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// BUCKET_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `BUCKET_HOME`, initialized exactly once per test
/// binary. Once initialized, `bucket_config::bucket_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("BUCKET_INSTALLER");
            std::env::remove_var("BUCKET_MANAGED_BY_NPM");
            std::env::remove_var("BUCKET_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `BUCKET_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_bucket_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("BUCKET_TEST_VERSION", v) };
}
...
/// Minimal [`bucket_updater::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> bucket_updater::UpdateConfig {
    bucket_updater::UpdateConfig {
...
/// Backdate every file in `BUCKET_HOME/downloads` by ~2 hours.
///
