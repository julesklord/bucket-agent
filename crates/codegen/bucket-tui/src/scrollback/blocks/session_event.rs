...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "bucket-4.5".into(),
            new_model_id: "bucket-build".into(),
            reason: "Model \"bucket-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"bucket-4.5\" is no longer available. Switched to \"bucket-build\"."
        );
    }
