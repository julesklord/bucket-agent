//! `pty-bench` — PTY benchmark CLI for `bucket-tui`.
//!
...
//! ```bash
//! cargo bench -p bucket-tui-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/bucket-${VERSION}-linux-x86_64 \
//!   cargo bench -p bucket-tui-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for bucket-tui",
    long_about = None,
)]
struct Cli {
