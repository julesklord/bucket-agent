//! Headless markdown analysis sharing Bucket Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `bucket-markdown` so analysis matches what Bucket
//! Build actually renders 1:1.
//!
//! After parsing, Bucket applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Bucket Build uses to render markdown.
///
...
/// Offset event stream from Bucket's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Bucket Build's options; count elements and flag structural issues.
