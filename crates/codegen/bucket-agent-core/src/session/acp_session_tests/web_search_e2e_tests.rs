...
use bucket_tools::registry::types::{SessionContext, ToolConfig, ToolServerConfig};
...
async fn web_search_uses_model_override_from_config_end_to_end() {
    ...
    let config = ToolServerConfig {
        tools: vec![ToolConfig {
            id: "BucketBuild:web_search".into(),
            params: None,
            name_override: None,
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("bucket-web-search-e2e"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("bucket-web-search-e2e/state.json"),
        memory_backend: None,
        web_search_config: bucket_tools::implementations::web_search::WebSearchConfig::Enabled {
...
async fn web_search_errors_when_configured_model_cannot_be_resolved() {
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: std::env::temp_dir(),
        session_folder: std::env::temp_dir().join("bucket-web-search-disabled"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("bucket-web-search-disabled/state.json"),
        memory_backend: None,
        web_search_config: bucket_tools::implementations::web_search::WebSearchConfig::Disabled,
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: Default::default(),
        video_gen_config: Default::default(),
