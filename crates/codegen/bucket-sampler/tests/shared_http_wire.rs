...
/// ambient shell exports (`BUCKET_SAMPLER_SHARED_CLIENT=0`,
/// `BUCKET_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("BUCKET_SAMPLER_SHARED_CLIENT");
            std::env::set_var("BUCKET_POOL_MAX_IDLE", "2");
            std::env::set_var("BUCKET_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
