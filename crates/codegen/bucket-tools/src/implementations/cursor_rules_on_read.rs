...
mod tests {
    ...
    use crate::implementations::bucket_build::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as BucketReadFileTool,
    };
    ...
    fn resources_with_bucket_rules_on_read(root: &Path) -> SharedResources {
        let mut resources = Resources::new();
        resources.insert(Cwd(root.to_path_buf()));
        resources.insert(FileSystem(Arc::new(LocalFs)));
        resources.insert(NotificationHandle(ToolNotificationHandle::noop()));
    ...
    #[tokio::test]
    async fn bucket_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let shared = resources_with_bucket_rules_on_read(root);
        ...
        let first = bucket_tool_runtime::Tool::run(&BucketReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
