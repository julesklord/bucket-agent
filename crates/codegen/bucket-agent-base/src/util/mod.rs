...
pub mod tips;
pub mod uname;
pub use bucket_shared::clipboard;
pub use bucket_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True if `pid` is a bucket process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_bucket_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("bucket"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.bucket.com.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.bucket.com/v11/chat/completions"
        ));
    }
    #[test]
    ...
    #[test]
    fn is_bucket_process_self_true_impossible_pid_false() {
        assert!(is_bucket_process(std::process::id()));
        assert!(!is_bucket_process(u32::MAX));
    }
}
