                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        bucket_telemetry::session_ctx::log_event(
                            bucket_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question =
            bucket_tools::implementations::bucket_build::ask_user_question::Question {
                question: "Pick one?".to_string(),
                options: vec![
                bucket_tools::implementations::bucket_build::ask_user_question::QuestionOption
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/bucket-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
