...
//! field rename/retype in `bucket-tools.proto` cannot silently break those
//! wire contracts (the producer and consumer live in separate services).

use bucket_tools_api::ToolConfigEntry;

fn full_entry() -> ToolConfigEntry {
    ToolConfigEntry {
        id: "BucketBuild:grep".to_owned(),
...
fn minimal_entry_deserializes_from_id_only() {
    ...
    let back: ToolConfigEntry =
        serde_json::from_value(serde_json::json!({"id": "BucketBuild:read_file"}))
            .expect("deserialize minimal");
    assert_eq!(back.id, "BucketBuild:read_file");
