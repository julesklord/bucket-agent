//! Built-binary end-to-end tests for the bucket (bucket-tui) binary.
//!
//! These tests verify that the built bucket binary works end-to-end against a mock
...
//!
//! The tests exercise:
//! - **Smoke** (`bucket --version`): binary loads without crashing
//! - **ACP stdio** (`bucket agent stdio`): full protocol lifecycle via ClientSideConnection
//!
//! Tests are `#[ignore]`d by default — they require a pre-built binary.
//!
//! Run locally (auto-builds the binary if not already present):
//! ```bash
//! cargo test -p bucket-agent-core --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `BUCKET_BINARY` to point at the release artifact:
//! ```bash
//! BUCKET_BINARY=./artifacts/bucket-0.1.159-linux-x86_64 \
//!   cargo test -p bucket-agent-core --test test_built_binary_e2e -- --ignored
//! ```
...
async fn bucket_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("bucket-4.5", "bucket-build")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("BUCKET_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "bucket --version with BUCKET_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "bucket -p in git repo", Some(&server));
...
/// Verify bucket works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "bucket -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = bucket_build_server().await;
    ...
    assert_headless_success(&result, "bucket -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "bucket -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Bucket Build usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use bucket_test_support::scripted::SseEvent;

    let server = single_model_server("bucket-4.5", "chat_completions").await;
    let chunk = |body: serde_json::Value| SseEvent::data(body.to_string());
    server.enqueue_response(
        "/v1/chat/completions",
        bucket_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "bucket -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        bucket_test_support::scripted::ScriptedResponse::sse(
            bucket_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    let server = single_model_server("bucket-4.5", "responses").await;
    server.set_response(r#"{"name":"Alice","age":30}"#);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "bucket -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "bucket -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (bucket agent stdio)
//
// These test the agent as a server: spawn `bucket agent stdio`, speak the full
...
/// Isolated headless run with a custom `~/.bucket/`. Clean env (no leaked
/// host credentials). Write config files into `bucket_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".bucket")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("BUCKET_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("BUCKET_TELEMETRY_ENABLED".into(), "false".into()),
                ("BUCKET_FEEDBACK_ENABLED".into(), "false".into()),
                ("BUCKET_TRACE_UPLOAD".into(), "false".into()),
                ("BUCKET_INSTRUMENTATION".into(), "disabled".into()),
                ("BUCKET_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn bucket_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".bucket")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        ...
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // Windows resolves `~` via USERPROFILE, not HOME — pin the bucket
            // home explicitly so the sandbox holds on all platforms (see
            // `test_env_cmd_tokio`).
            .env("BUCKET_HOME", self.bucket_dir())
...
/// Enterprise BYOK: managed_config.toml overrides bucket-build with a custom
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    let server = MockInferenceServer::start_with_required_auth(
        vec![MockModelEntry::new("bucket-4.5")],
    ...
    std::fs::write(
        h.bucket_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
...
env_key = "BUCKET_TEST_BYOK_TOKEN"
model = "bucket-4.5"
...
"#,
    ...
    .unwrap();
    h.env("BUCKET_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "bucket -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "bucket -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "bucket -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
