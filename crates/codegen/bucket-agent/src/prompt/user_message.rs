...
//! - `Default` -- the legacy Bucket Build prefix (built by the shell layer).
...
pub enum UserMessageTemplate {
    /// Legacy Bucket Build prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .bucket/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `bucket_agent_core::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.bucket/, ~/.claude/).
    ...
    /// (`~/.bucket/projects/<encoded-cwd>/mcps`). Surfaced in
