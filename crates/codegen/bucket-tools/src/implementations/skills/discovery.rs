...
///
/// `skills` is the standard layout (`.bucket/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Bucket Build sessions.
...
/// name (e.g. `~/.bucket/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Bucket Build.
...
/// `cwd` (exclusive). At each directory, checks for `.bucket/skills/`,
...
/// visits known config roots (`.bucket`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.bucket` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".bucket", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.bucket is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.bucket/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.bucket/ → kept (user content).
        let bucket_shell = tmp.path().join(".bucket").join("skills").join("shell");
        std::fs::create_dir_all(&bucket_shell).unwrap();
        std::fs::write(
            bucket_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (bucket_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.bucket/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .bucket skill in an intermediate dir.
        ...
        assert!(
            names_on.contains(&"bucket-dyn"),
            "bucket-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only bucket-dyn discovered.
        ...
        assert!(
            names_off.contains(&"bucket-dyn"),
            "bucket-dyn missing: {names_off:?}"
        );
        assert!(
