...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = bucket_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == bucket_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = bucket_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
/// `BUCKET_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut bucket_agent_core::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = bucket_agent_core::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn set_auto_dark_theme(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        // No prior config: fall back to BucketNight (the default).
        .unwrap_or_else(|| crate::theme::ThemeKind::BucketNight.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
...
pub(in crate::app::dispatch) fn set_auto_light_theme(
    ...
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        .unwrap_or_else(|| crate::theme::ThemeKind::BucketDay.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
        Some(c) if c != crate::theme::ThemeKind::Auto.display_name() => c,
        _ => {
            tracing::error!(
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "Bucket Build")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / BUCKET_CHAT_MODE) catalogs use opaque `/rest/modes`
    // slugs that must not become the global Build `default_model`.
    let mut effects: Vec<Effect> = Vec::new();
    if !bucket_agent_core::agent::chat_modes::process_chat_mode_enabled() {
...
/// (`bucket_agent_core::models::default_model()`).
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = bucket_agent_core::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
