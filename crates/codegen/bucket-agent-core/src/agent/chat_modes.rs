//! bucket.com chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching bucket-web's refetch cadence.
...
pub const BUCKET_CHAT_MODE_ENV: &str = "BUCKET_CHAT_MODE";
...
impl ChatModesManager {
    ...
    /// The active bucket.com identity, or `None` when unauthenticated. Modes are
...
/// Maps bucket.com modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        let mut m = available("bucket-4.5", "");
        m.title = "   ".to_owned();
        let resp = ListModesResponse {
            modes: vec![m],
            default_mode_id: String::new(),
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "bucket-4.5");
    }
}
