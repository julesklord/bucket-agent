...
//!
//! Fetches the authenticated user's Bucket Build billing configuration
...
/// Current billing configuration for Bucket Build coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `BucketBuildBillingConfig` fields
...
/// against both the new `GetBucketCreditsConfig` and the legacy
/// `GetBucketBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetBucketCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `BucketCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetBucketBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/bucket/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperBucket Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.bucket/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    ...
    // forwards to the backend `GetBucketCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-BUCKET-Token-Auth",
            crate::auth::BucketComConfig::default().token_header,
        )
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperBucket");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetBucketCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_BUCKET_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
