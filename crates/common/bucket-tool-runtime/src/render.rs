...
//! ```rust
//! use serde::Serialize;
//! use bucket_tool_runtime::render::ToolOutput;
...
/// Lets `bucket_tool_types::TaskOutputOutput` be used directly as a `Tool::Output`
/// (handy for stub/test tools and pass-through proxies).
impl ToolOutput for bucket_tool_types::TaskOutputOutput {}

/// Lets `bucket_tool_types::SubagentCompletedOutput` be used directly as a
/// `Tool::Output` (the `task` tool's structured completion output).
impl ToolOutput for bucket_tool_types::SubagentCompletedOutput {}

/// Lets `bucket_tool_types::KillTaskOutput` be used directly as a `Tool::Output`
/// (the `kill_task` tool's typed result / not-found output).
impl ToolOutput for bucket_tool_types::KillTaskOutput {}
...
pub fn extract_content_blocks(value: &Value) -> Vec<ContentBlock> {
    ...
    if let Some(obj) = value.as_object() {
        // bucket-build `ToolRunResult`: the model sees `prompt_text` (reminders
