...
use bucket_acp::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a bucket-shell instance.
    pub is_bucket_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<bucket_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `bucket.com`).
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<bucket_agent_core::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_bucket_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(bucket_agent_core::auth::AuthManager::new(
        ...
        agent_config.bucket_com_config.clone(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    bucket_agent_core::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = bucket_agent_core::util::bucket_home::bucket_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a bucket-shell agent
    let is_bucket_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("bucketShell"))
...
///
/// Matches TUI startup behavior: if the first method is `bucket.com`, defer auth
...
/// for a `bucket.com` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "bucketShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "bucketShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_bucket_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("bucket.com", "bucket.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("bucket.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "bucket.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_bucket_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("bucket.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_bucket_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.bucket/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use bucket_agent_core::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `bucket.com`
    ...
    /// `startup_auth_bucket_com_no_provider_needs_login_pending` above and
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        ...
        let methods = vec![
            ...
            make_auth_method(BUCKET_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with bucket.com first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("bucket.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("bucket.com", "bucket.com", Some(meta))];
