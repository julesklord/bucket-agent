...
//!
//! Extracted from `bucket-agent-core::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$BUCKET_HOME` is resolved through `bucket-config::bucket_home`.
...
/// This function caches the result in `$BUCKET_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = bucket_config::bucket_home().join("agent_id");
...
/// Returns true when workspace marker env vars (`BUCKET_ROOT` and `BUCKET_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("BUCKET_ROOT").is_ok() && std::env::var("BUCKET_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `BUCKET_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("BUCKET_TELEMETRY_SPECIAL_USER").as_deref(),
