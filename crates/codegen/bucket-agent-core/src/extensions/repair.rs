...
//!
//! Repairs via [`bucket_chat_state::compaction_utils::repair_history`]. Resident
...
async fn repair_on_disk(bucket_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    ...
    let storage = JsonlStorageAdapter::with_root(bucket_root.to_path_buf());
    ...
    let report = bucket_chat_state::compaction_utils::repair_history(&mut chat_history);
