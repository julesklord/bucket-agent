...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.bucket/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.bucket/`, `~/.claude/`,
/// `~/.bucket/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<bucket_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<bucket_agent::prompt::user_message::RuleEntry>,
    Vec<bucket_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.bucket/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = bucket_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for bucket ordering), so a failed offload never
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(bucket_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: bucket_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use bucket_agent::prompt::agents_md::read_agents_config_with_paths;
        use bucket_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use bucket_workspace::file_system::{git_status_short, jj_status};
        use bucket_workspace::session::git::VcsKind;
    ...
    ) -> Vec<bucket_agent::prompt::user_message::McpServerEntry> {
        use bucket_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &bucket_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = bucket_sampler::SamplingClient::new(sampler_config).map_err(|e| {
