...
//! startup latency, under either strategy (`BUCKET_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p bucket-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! BUCKET_FSNOTIFY_PER_DIR=0 cargo run --release -p bucket-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! BUCKET_FSNOTIFY_PER_DIR=1 cargo run --release -p bucket-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "bucket holds 55k inotify watches" report).
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("BUCKET_FSNOTIFY_PER_DIR").unwrap_or_default();
