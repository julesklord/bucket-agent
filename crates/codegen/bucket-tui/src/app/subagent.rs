...
thread_local! {
    static REPLAY_BUCKET_HOME : std::cell::RefCell < Option < std::path::PathBuf >> = const
    { std::cell::RefCell::new(None) };
}
/// Override bucket home for disk-replay unit tests (thread-local; production never sets this).
#[cfg(test)]
pub(crate) fn set_replay_bucket_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_BUCKET_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_bucket_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_BUCKET_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_bucket_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_bucket_home_for_tests(None);
    }
    #[test]
    ...
    fn subagent_meta_all_fields() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), Some("analyst"), Some("bucket-3")),
            " (researcher \u{00b7} analyst \u{00b7} bucket-3)"
        );
    }
    #[test]
    fn subagent_meta_partial_skips_nones() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), None, Some("bucket-3")),
            " (researcher \u{00b7} bucket-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_collapses_duplicate_persona_role() {
        assert_eq!(
            format_subagent_meta(Some("reviewer"), Some("reviewer"), Some("bucket-3")),
            " (reviewer \u{00b7} bucket-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_drops_both_empty_persona_role() {
        assert_eq!(
            format_subagent_meta(Some(""), Some(" "), Some("bucket-3")),
            " (bucket-3)"
        );
    }
    #[test]
