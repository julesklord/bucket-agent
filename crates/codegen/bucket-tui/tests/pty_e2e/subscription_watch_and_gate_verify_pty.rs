...
const PAID_TIER: &str = "SuperBucketPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "SuperBucket Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `superbucket_heavy` = live
/// `/user` string `SuperBucketPro`. Must match
...
/// `BUCKET_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let bucket_home = content.home().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create temp .bucket");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("BUCKET_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("bucket-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("BUCKET_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://bucket.com/superbucket?referrer=bucket-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("BUCKET_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
