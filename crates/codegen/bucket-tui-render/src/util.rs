...
pub use bucket_config::bucket_home;

/// Path to `$BUCKET_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    bucket_home().join("pager.toml")
}

/// User-facing label for the user bucket directory (``~/.bucket`` or ``$BUCKET_HOME``).
///
/// Derived from resolved [`bucket_home()`] vs `bucket_config::default_bucket_home()`,
/// not from whether `BUCKET_HOME` is set in the environment.
pub fn display_bucket_home_prefix() -> String {
    if bucket_home() == bucket_config::default_bucket_home() {
        "~/.bucket".to_string()
    } else {
        "$BUCKET_HOME".to_string()
    }
}

/// User-facing path under [`bucket_home()`], e.g. ``~/.bucket/config.toml``.
pub fn display_user_bucket_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_bucket_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`bucket_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let bucket = bucket_home();
    if let Ok(rest) = path_buf.strip_prefix(&bucket) {
        let prefix = display_bucket_home_prefix();
...
/// True when `path` is under user [`bucket_home()`] (not project `{cwd}/.bucket`).
pub fn is_under_user_bucket_home(path: &Path) -> bool {
    path.starts_with(bucket_home())
}
...
mod tests {
    ...
    #[test]
    fn display_bucket_home_prefix_default_install() {
        if std::env::var("BUCKET_HOME").is_ok() {
            return;
        }
        assert_eq!(display_bucket_home_prefix(), "~/.bucket");
    }

    #[test]
    fn display_user_bucket_path_joins_relative() {
        let path = display_user_bucket_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".bucket") || path.contains("$BUCKET_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_bucket() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.bucket/memory/MEMORY.md");
