...
//! `bucket-workspace`. Wire types live in
//! `bucket_workspace_types::rpc`; this crate adds the connected-state
...
//! own their timeouts.
use bucket_hub_sdk::harness::ToolHarness;
use bucket_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
use bucket_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use bucket_workspace_types::rpc::code_nav::{
...
use bucket_workspace_types::rpc::fs::{
...
use bucket_workspace_types::rpc::git::{
...
use bucket_workspace_types::rpc::hunks::{
...
use bucket_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use bucket_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use bucket_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
...
use bucket_workspace_types::rpc::worktree::{
...
use bucket_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
...
pub async fn consume_stream_terminal(
    stream: &mut bucket_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, bucket_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](bucket_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &bucket_tool_runtime::ToolError) -> bool {
    match err.kind {
        bucket_tool_runtime::ToolErrorKind::NetworkError => true,
        bucket_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = bucket_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    use super::*;
    use bucket_hub_sdk::harness::LocalRegistry;
    use bucket_tool_protocol::{SessionId, ToolId};
    use bucket_tool_runtime::{Tool, ToolError};
    use bucket_tool_types::ToolDescription;
    use bucket_workspace_types::rpc::skills::SkillScope;
    ...
    impl bucket_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::bucket_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.bucket/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = bucket_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: bucket_tool_runtime::ToolStream<TypedToolOutput> =
            bucket_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = bucket_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
