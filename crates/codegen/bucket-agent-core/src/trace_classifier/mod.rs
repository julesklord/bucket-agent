...
/// two-function shape in `bucket-tools` (F29).
...
/// `bucket-tools` — existing items keep their prior content when
...
/// binary. Wraps a `bucket_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: bucket_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: bucket_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_bucket_conv_id: session_id_str.clone(),
        x_bucket_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_bucket_session_id: session_id_str,
        x_bucket_agent_id: Some(bucket_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// Bucket-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `bucket_agent_core::util::bucket_home::bucket_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub bucket_home: Option<PathBuf>,
}
...
/// Resolve the API key from `--api-key` → `$BUCKET_API_KEY` →
...
/// provider command (if `BUCKET_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, bucket_home: &Path) -> Result<String> {
    if let Some(k) = explicit.map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    let env_key = std::env::var("BUCKET_API_KEY").ok();
    if let Some(k) = env_key.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    if let Some(key) = non_interactive_auth_key(bucket_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set BUCKET_API_KEY, or run `bucket login` to populate \
         <bucket-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `bucket_home`, install the
...
/// explicit `bucket_home` so `--bucket-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `bucket_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(bucket_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, BucketComConfig};
    ...
    let config = BucketComConfig::default();
    let auth_provider_command = config.auth_provider_command.clone();
    let manager = std::sync::Arc::new(AuthManager::new(bucket_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `bucket login` to re-authenticate, \
             or pass --api-key / set $BUCKET_API_KEY to bypass auth.json."
        )),
    }
}
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
    bucket_home: Option<&Path>,
) -> Result<bucket_sampler::SamplingClient> {
    let default_home;
    let bucket_home_path: &Path = match bucket_home {
        Some(p) => p,
        None => {
            default_home = crate::util::bucket_home::bucket_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, bucket_home_path).await?;
    let config = bucket_sampler::SamplerConfig {
    ...
    bucket_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use bucket_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(req.x_bucket_conv_id.as_deref(), Some("sess-x"));
        assert_eq!(req.x_bucket_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_bucket_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_bucket_agent_id.as_deref(),
            Some(bucket_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// OIDC scope (the same scope `bucket login` writes today and
    /// `AuthManager` reads). `auth_mode: api_key` skips the refresh
    /// path entirely — useful for "plain key, no refresh wanted"
    /// fixtures.
    fn write_auth_json(bucket_home: &Path, key: &str) {
        let scope = crate::auth::BucketComConfig::default().auth_scope();
        ...
        std::fs::write(bucket_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(bucket_home: &Path) {
        std::fs::write(bucket_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(bucket_home: &Path, key: &str) {
        let scope = crate::auth::BucketComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(bucket_home: &Path, key: &str) {
        let scope = crate::auth::BucketComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        write_empty_auth_json(bucket_home);
        ...
        assert!(
            resolve_api_key(None, bucket_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), bucket_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), bucket_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), bucket_home)
                .await
                .expect("ok"),
            "from-flag",
        );

        unsafe { std::env::set_var("BUCKET_API_KEY", "from-env") };
        assert_eq!(
            resolve_api_key(None, bucket_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), bucket_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), bucket_home).await.expect("ok"),
            "from-env",
        ...
        unsafe { std::env::set_var("BUCKET_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, bucket_home).await.is_err(),
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let bucket_home = tmp.path();
        write_auth_json(bucket_home, "from-auth-json");
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("BUCKET_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, bucket_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, bucket_home)
                .await
        ...
        assert!(
            msg.contains("--api-key")
                && msg.contains("BUCKET_API_KEY")
                && msg.contains("bucket login")
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(bucket_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, bucket_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(bucket_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, bucket_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let bucket_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(bucket_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, bucket_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `BUCKET_API_KEY` — would return early from `resolve_api_key`.
    /// * `BUCKET_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `BUCKET_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.bucket/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `BUCKET_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("BUCKET_API_KEY").ok();
        unsafe { std::env::remove_var("BUCKET_API_KEY") };
        let tmp = tempfile::tempdir().expect("tempdir");
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            bucket_home: Some(tmp.path().to_path_buf()),
        };
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("BUCKET_API_KEY", v) },
            None => unsafe { std::env::remove_var("BUCKET_API_KEY") },
        }
    }
