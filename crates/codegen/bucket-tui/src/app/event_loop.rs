...
/// [`decide`](bucket_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("BUCKET_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = bucket_agent_core::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(bucket_agent_core::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = bucket_agent_core::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match bucket_agent_core::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = bucket_config::env_bool("BUCKET_PLUGIN_CTA")
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the bucket.com method from the advertised list
            let bucket_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "bucket.com");
            if let Some(method) = bucket_com {
                ...
                // No bucket.com method available, use the first method as fallback
        ...
        // fail-closed: do not invent bucket.com / auto-start OIDC.
        tracing::info!(
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<bucket_agent_core::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == bucket_agent_core::agent::auth_method::BUCKET_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = bucket_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = bucket_agent_core::config::load_merged_requirements();
    let user_config = bucket_agent_core::config::load_from_disk().ok();
    let managed_config = bucket_agent_core::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match bucket_agent_core::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = bucket_agent_core::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > BUCKET_BUCKET_API_BASE_URL env > default).
            let endpoints_base =
                bucket_agent_core::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                bucket_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to bucket-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = bucket_agent_core::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        bucket_agent_core::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        bucket_agent_core::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        bucket_agent_core::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use bucket_agent_core::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = bucket_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = bucket_agent_core::util::tips::pick_and_advance(&app.tips, &bucket_home);
        }
    }

    let hints = bucket_agent_core::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.bucket/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        bucket_agent_core::util::config::clamped_display_permission_mode(
            bucket_agent_core::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        bucket_agent_core::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = bucket_agent_core::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = bucket_agent_core::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // BUCKET_SCROLL_SPEED/_MODE/_LINES + BUCKET_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<bucket_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`bucket "fix the bug"`). When
    ...
    // `bucket dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `BUCKET_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("BUCKET_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(bucket_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart bucket");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.bucket/config.toml [ui]`
                // rather than `~/.bucket/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = bucket_agent_core::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> bucket_agent_core::agent::config::UiConfig {
    use bucket_agent_core::agent::config::UiConfig;
    let Ok(root) = bucket_agent_core::config::load_effective_config() else {
