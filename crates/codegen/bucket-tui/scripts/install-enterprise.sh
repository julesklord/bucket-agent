#!/bin/bash
#
# Bucket CLI installer (enterprise channel) — https://x.ai/cli/enterprise-install.sh
#
...
# Env: BUCKET_BIN_DIR, BUCKET_PROXY_URL
#
# Usage:
...
#   BUCKET_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/bucket"
