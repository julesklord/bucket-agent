...
//!
//! The tool runtime contract (`Tool` trait) lives in `bucket_tool_runtime`.
...
///
/// Serializes to snake_case (`bucket_build`, `mcp`, …) for the
...
/// (e.g. `"BucketBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "BucketBuild")]
    BucketBuild,
    #[serde(alias = "BucketBuildConcise")]
    BucketBuildConcise,
    #[serde(alias = "BucketBuildHashline")]
    BucketBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
