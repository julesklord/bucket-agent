...
- **bucket inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct bucket --minimal --resume command.
...
- **Bucket no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **bucket setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Bucket above the footer shortcuts.
...
- **`bucket worktree ls`** now works as a short alias for `bucket worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.bucket/config.toml`.
...
- **Resumed bucket.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.bucket/worktrees/.
...
- **Terminal command cards** on bucket.com now correctly settle after foreground bash tasks.
...
- **bucket inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain bucket launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive bucket agent stdio without silent parse drops on session/* calls.
...
- **bucket --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by bucket.com" link in the Extensions modal is now clickable and underlined.
...
- **bucket --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **bucket update** on Windows no longer fails when a previous.old executable is still running.
...
- **bucket models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `bucket plugin marketplace add`.
...
- **Plan files** now default to.bucket/plan.md to match Bucket conventions.
...
- **--minimal** flag now shows in `bucket --help`.
...
- **Tool result previews** no longer paint opaque panels in `bucket --minimal`.
- **bucket wrap** now correctly handles quoted strings and shell aliases.
...
- **Bucket.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the bucket.com model catalog** in the picker.
...
- **`bucket update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `bucket wrap`.
...
- **bucket agent stdio** now checks for updates in the background like other modes.
...
- **bucket wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `bucket agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `bucket wrap`** to run any command with local clipboard support.
...
- **Fixed `bucket agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or bucket-desktop.
...
- You can now pass --json-schema to bucket -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `BUCKET_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with BUCKET_API_KEY present no longer lose refresh on idle.
...
- **bucket -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in bucket -p and bucket agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `bucket` after running `bucket update`.
- Headless (`bucket -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`bucket plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **bucket sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **bucket quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, bucket sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, BUCKET_HUNK_TRACKER, or config.
...
- **Bucket** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`bucket dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`bucket mcp add`** now accepts positional arguments (e.g. `bucket mcp add filesystem -- npx...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`bucket update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.bucket.
...
- **MCP servers menu** and slash commands now work when starting bucket outside a project directory.
...
- **bucket --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `bucket --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `bucket update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`bucket --debug`** now produces per-session log files under ~/.bucket/debug/ even with a leader process.
...
- **`bucket update`** now restarts any older running leader so all clients get the new binary.
...
- **`bucket login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `bucket agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the BUCKET_IMAGE_GEN_HARNESS env var.
...
- **bucket inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via BUCKET_CURSOR_MCPS_ENABLED / BUCKET_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Bucket and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the bucket-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality bucket-imagine-image-quality model.
...
- **Version output** now shows [alpha] or [stable] in `--version`, banners, `/session-info`, and `bucket inspect`.
...
- **New `/export` slash command** and `bucket export` CLI command let you save or copy conversation transcripts as Markdown.
...
- **New `bucket plugin` commands** let you manage plugins and marketplaces directly from the terminal.
...
- **New `bucket plugin` commands** let you manage plugins and marketplaces directly from the terminal.
- New `/export` command and `bucket export` let you save or copy conversation transcripts as Markdown.
...
- **Bucket** now reminds itself to take action in very long conversations while still obeying instructions like "just plan".
...
- **Bucket** now reminds itself to execute plans in long conversations while strictly following instructions like "just plan".
...
- **Added `bucket logout` subcommand** to sign out from the terminal without launching the TUI.
...
- **API-key users** no longer see unavailable models such as bucket-build that 404 on selection.
- **Image and video generation** now recover better from network issues and large response bodies.
- **Improved error messages** when hitting a subscription wall with an API key set, suggesting `bucket logout`.
...
- **Web search and X search** can now execute server-side with full session persistence when BUCKET_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the BUCKET_GOAL environment variable.
...
- **Bucket CLI** now runs natively on Intel Macs via x86_64 builds.
...
- **Paywall** now lifts automatically after subscribing via bucket login or external purchase without needing to re-login.
...
- **Model list** no longer gets stuck on only bucket-build after waking from sleep on macOS.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the BUCKET_GOAL environment variable.
...
- **New `bucket logout` command** clears your cached login session directly from the terminal.
...
- **Fixed model list** getting stuck on only bucket-build after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **API key users** no longer see unavailable models like bucket-build in the model list.
- **Image and video generation** now handles slow or interrupted network responses more reliably without generic decode errors.
- **Fixed error messages** when hitting a subscription wall with an API key set, now suggesting `bucket logout`.
- **Fixed a crash** on exit when using MCP servers that require token refresh.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on Bucket Build.
...
- **`bucket sessions search`** now finds local and remote sessions correctly, and **`bucket -r <id>`** restores remote sessions reliably.
...
- **AI shell command suggestions** enabled via BUCKET_SUGGESTIONS_AI=1 environment variable.
...
- **Fixed git and bash commands** failing in Bucket Desktop on Windows.
...
- **`bucket -w my-worktree`** creates a custom-named worktree folder.
...
- **`/usage manage`** now opens billing page; **`bucket login`** enforces tiers correctly after refresh.
...
- **Legacy web login** no longer treated as first-party xAI account for sharing and billing; prompts **`bucket login`** upgrade.
...
- **Large tool outputs** shortened when sent to Bucket, full text saved to file in session folder.
...
- **Legacy authentication** removed; run **`bucket login`** to upgrade to OAuth.
...
- **Legacy web login** no longer treated as premium xAI account; run **`bucket login`** instead.
...
- **New `bucket-build-v2` agent** available in config.toml for prompt/tool testing.
...
- **Fixed `bucket sessions list`** auth failures.
...
- **New `bucket-build-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **`bucket sessions list`** now works reliably after login.
...
- **Added bucket memory clear** CLI command with scope selection and confirmation.
...
- **Computer Hub integration** into bucket-agent-core
...
- **bucket memory clear command** with scope and confirmation
...
- **Best-of-N mode** for bucket-build
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like bucket-build.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like bucket-build.
...
- **bucket-build model** now defaults to Chat Completions API.
...
- **Shell commands now work** in Bucket Desktop on Windows.
...
- **New `bucket-build-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `bucket import`** resumes Claude Code sessions with `bucket import <id/path>`.
- **New `bucket-build-orchestrator`** coordinates subagents for complex builds.
...
- **Fixed `bucket share`** failing with backend ZodError due to duplicate request headers.
...
- **New `bucket login --devbox`** option for signing in from headless devbox environments without browser access.
...
- **New `bucket mcp doctor`** diagnoses MCP servers and suggests fixes.
...
- **New `bucket mcp doctor`** diagnoses MCP server configuration and connectivity.
...
- **After plan approval**, Bucket suggests `/implement` for structured coding when available.

## Bug Fixes

- **Model lists and pickers** now show only models accessible via your `BUCKET_CODE_BUCKET_API_KEY`.
...
- **`bucket models`** and TUI picker now show team-specific models when **`BUCKET_CODE_BUCKET_API_KEY`** is set.
...
- **Native Windows support** now available experimentally for running Bucket without WSL.
...
- **Clearer login error messages** now suggest `bucket login`, API key env var, or config.toml.
...
- **Native Windows support** (opt-in) lets you run Bucket CLI directly without WSL, with MCP server compatibility.
...
- **Authentication errors** now clearly suggest `bucket login`, env var, or config.toml api_key.
- **Fixed false-positive loop warnings** for different shell commands during edit-test cycles.
...
- **Import Claude settings** from `.claude/` into `.bucket/config.toml` via welcome screen ('i') or `/import-claude`.
- **Skills reload automatically** when editing/saving SKILL.md files without restarting `bucket`.
...
- **Improved `bucket login`** with automatic code delivery from browser consent page.
...
- **New `bucket trace`** command uploads or exports session data for debugging.
...
- **New `bucket trace`** command exports or uploads session data for debugging.
...
- **`bucket models`** now lists available models and the default.
...
- **`bucket models`** command now lists available models and authentication status.
...
- **X Premium and Premium+** tiers now grant access to Bucket Build.
...
- **Model picker** now displays config key names like `bucket-build` for aliased models.
...
- **Welcome screen** shows **SuperBucket upsell** without active subscription.
...
- **Welcome screen** restricts actions and upsells SuperBucket without subscription.
...
- **SuperBucket upsell** shown on welcome screen without subscription.
...
- **`bucket sessions search`** now finds sessions by content with scores and snippets.
...
- **`bucket sessions search`** now supports **full-text queries** across chat content and tools.
- **Session picker** now combines fuzzy matching with **deep content search** results.
- **Unified logs** now saved to `~/.bucket/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Auth.json hot-reload** fixed for bucket login in other terminals.
...
- **Auto-update restart** now launches the correct new version.
- **Auth.json hot-reload** now detects `bucket login` from other terminals.
...
- **External `bucket login`** now hot-reloads auth without app restart.
...
- **External `bucket login`** now hot-reloads auth without restarting.
...
- **New `bucket ssh`** command enables clipboard copy from remote sessions in Apple Terminal.
...
- **`bucket login --device-auth`** enables login from SSH, Docker, and other headless environments without port forwarding.
...
- **Added** `bucket login --device-auth` **flag** for headless environments like SSH and Docker.
...
- **Bundled agents** discovered from ~/.bucket/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.bucket/managed_config.toml/requirements.toml with `bucket inspect` and `bucket setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **bucket login defaults to OAuth** instead of legacy relay; use --legacy or BUCKET_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old bucket/bucket-pager binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old bucket/bucket-pager binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.bucket/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **Parallel tool dispatch** via BUCKET_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **OAuth login option** via --oauth/--legacy flags, BUCKET_OAUTH_ENABLED env, feature flag (legacy default).
...
- **Restores web search functionality** by reinjecting required proxy headers like x-bucket-client-version.
...
- **BucketNight default theme** with **runtime color quantization** ensures correct rendering across truecolor, 256-color, and 16-color terminals.
...
- **Legacy relay auth flow** via `bucket login --legacy` restores pre-OIDC accounts.x.ai token exchange.
...
- **OIDC OAuth replaces relay auth**; removed `--auth-signin-url`/`exchange-code-url`/`redirect-target` flags, run `bucket login` to migrate.

## Features

- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.bucket/lsp.json`.
...
- **Web search model override** via CLI, config.toml or env defaults to bucket-4-1-fast-reasoning.
...
- **Image generation tool** gated behind a server-side flag or `BUCKET_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.bucket/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.bucket/config.toml before existing checks.
...
- **Suppresses config warning** for [desktop] section owned by bucket-desktop using opaque serde sink field.
...
- **Skills discovery expands** to.agents/skills directories alongside.bucket and.claude.
...
- **OIDC OAuth2 replaces legacy relay login** for `bucket login`; add `--legacy` flag for old flow and remove `--auth-signin-url` etc. CLI flags (migrate by deleting auth.json).
- **Adds `bucket login --legacy`** fallback; removes `--auth-signin-url` etc flags (use env/config defaults, no migration needed for most users).
...
- **Dynamic discovery of nested skills** from.bucket/skills/ subdirs at runtime via reminders.
...
- **Silent [desktop] config handling** consumes bucket-desktop section without spurious 'unrecognized key' warnings.
...
- **Question-answer panel** in pager with **BucketBuildPlanNoSubagents** mode excluding subagent tools.
...
- **Discover skills from**.claude/skills directories alongside.bucket/skills.
...
- **Codex grep_files uses** shared rg_path() matching bucket_build pattern.
...
- **Auto-injected managed MCPs** from bucket.com for WebLogin, deduped with config.toml opt-out.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP x.ai/auth/logout** removes scopes from ~/.bucket/auth.json.
- **Custom npm registry** via config.toml or BUCKET_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.bucket/config.toml** to answer MCP server and model configuration queries.
- **Deployment keys supported** on all API proxy endpoints including storage and sessions.
- **Public install-bucket.sh script** supports deployment keys and channels without VPN.
...
- **Bundled /help skill** extracts README.md to ~/.bucket/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.bucket/personas/*.toml with fail-closed resolution.
...
- **Question-answer panel** adds pager UI for interactive permissions and subagent-free BucketBuildPlan agent.
...
- **Auto-injects managed MCPs** from bucket.com into CLI/TUI for WebLogin users.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.bucket/auth.json.
- **Discovers skills from.claude/skills** alongside.bucket/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching bucket_build and opencode patterns.
...
- **Configurable bash tool params** like timeouts injected from config.toml `[toolset.bash]` into BucketBuild actors.

## Bug Fixes

- **Precise io::ErrorKind mapping** from ACP file errors enables consistent NotFound and PermissionDenied handling.
- **Silent bucket-pager installation** hides internal details from install and auto-update user output.
...
- **SSE stream parsing** handles flat error format from Bucket proxy without deserialization failure.
...
- **Post-A/B session hooks** via BUCKET_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `BUCKET_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `BUCKET_USER_METADATA`.
- **Plan mode tools** add `enter_plan_mode`, `exit_plan_mode`, and `ask_user_question` with notifications.
- **Auto-installs bucket-pager** alongside bucket during internal and GitHub release updates.
...
- **Independent feedback gating** via BUCKET_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **Bucket 4.20 default model** with CLI/env/config/remote-settings overrides for catalog models.
...
- **`bucket completions` subcommand** generates bash/zsh completion scripts, with fast-path exit before any network or auth warmup.
...
- **use_tool works in all server contexts** by dispatching through ToolCallContext instead of session-actor interception, fixing silent failures in bucket-tools-server.
...
- **`bucket memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boost in hybrid search.
...
- **User-defined subagents** via .bucket/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **Session token routing fixed** for default models — `bucket login` users no longer get 400 errors on proxy-routed models.
...
- **Restored remote sessions create distinct local children** with parent tracking, preventing identity reuse and duplicate restores on repeated `bucket -r`.
...
- **Shell completions** via `bucket completions <shell>`, leader CLI, and stdio reconnect replay.
- **User-defined subagents** via.bucket/agents/*.md with config toggles.
...
- **Disables web search tool** via `--disable-web-search` flag or `BUCKET_DISABLE_WEB_SEARCH=1` for benchmarks.
- **hashline_read tool** annotates file output with anchors like `LINE:LOCAL:CONTEXT→CONTENT`.
- **Bucket 4.20 default model** with CLI/env/config/remote-settings overrides for web search and summaries.
- **Unauthenticated feedback submission** via optional tokens and `BUCKET_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.bucket/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via BUCKET_FEEDBACK_ENABLED.
- **Leader CLI** adds `bucket leader list/info/profile/kill/dev` for discovery and CPU profiling.
...
- **Bash tool params from config.toml** override schema defaults like `timeout_secs` for BucketBuild.
- **Live subagent progress** in TUI tasks panel via ACP polling and push notifications.
- **ACP extension resumes sessions in worktrees** via single call matching `bucket -w -r` CLI flow.
...
- **SSE parsing handles flat errors** from Bucket proxy alongside OpenAI-standard nested format.
...
- **Silent bucket-pager installation** hides implementation details from install/update output.
...
- **Subagent support** gated behind `--subagents` flag, `BUCKET_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **`bucket memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boosting for retrieved memory chunks.
...
- **Cross-devbox session restore** via `bucket sessions list/search` CLI subcommands and hardened `bucket -r` with upload-ordering safety, cwd-scoped lookups, and staged-vs-unstaged correctness.
...
- **Replay notifications now guaranteed before session/load response** via `forward_with_completion()` drain; removes public `acp_send_fire_and_forget` from bucket-acp.
...
- **`BUCKET_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Headless mode (`bucket -p`) exit panic** fixed by flushing telemetry before runtime teardown.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.bucket/bin symlink at spawn time.
...
- **Sandbox profile configurable via BUCKET_SANDBOX env variable**, defaulting to workspace profile on devboxes.
- **Agent version exposed in InitializeResponse** metadata for relay and client version discovery.
- **`bucket worktree` subcommand** for listing, inspecting, removing, and garbage-collecting session worktrees.
...
- **ACP file operations** were silently bypassing the client filesystem backend, breaking ask-mode write rejection in BucketCode.
...
- **Reliable CLI updates** now refresh bucket-latest during installs, ensuring symlink-based setups launch the newly installed binary in fresh shells.
...
- **Agent environment metadata becomes queryable** by merging BUCKET_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **Prompt introspection from the CLI** is available via `bucket prompt`, with JSON or section output and persisted session prompt_context snapshots.
...
- **Installation diagnostics** help troubleshoot conflicting bucket binaries by listing canonical and invoked paths, versions, update status, and optional JSON output.
...
- **npm installs now use a canonical Bucket binary path** via postinstall copy to `~/.bucket/bin/bucket`, preventing installer conflicts and version confusion.
