...
//!
//! `bucket_hooks` pulls in `git2`/`reqwest`/`bucket-tools`, too heavy for
...
/// Wire mirror of `bucket_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `bucket_hooks::config::HookSpec`.
///
...
/// `bucket-hooks`, so a server-side test (`bucket-workspace`'s
...
/// Wire mirror of `bucket_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.bucket/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });
