...
pub use bucket_workspace_client::consume_stream_terminal;

/// Extract a `WorkspaceEvent` from a custom `ToolNotificationFrame`.
pub fn extract_workspace_event(
    frame: &bucket_tool_protocol::ToolNotificationFrame,
) -> Option<WorkspaceEvent> {
    use bucket_tool_protocol::WireToolNotification;
...
mod tests {
    use super::*;
    use bucket_tool_protocol::{ToolId, ToolNotificationFrame, WireToolNotification};
    ...
    // consume_stream_terminal tests live in bucket-workspace-client.
}
