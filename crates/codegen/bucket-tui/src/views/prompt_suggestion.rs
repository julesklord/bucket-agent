...
/// Env override for the whole feature: `BUCKET_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "BUCKET_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `BUCKET_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "BUCKET_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "bucket-build-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `BUCKET_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| bucket_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `bucket-build-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `bucket-build-0.1` default): the shell
