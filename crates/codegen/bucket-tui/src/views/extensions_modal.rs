...
pub fn fuzzy_matches_hook(hook: &bucket_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &bucket_hooks_plugins_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[bucket_hooks_plugins_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<bucket_hooks_plugins_types::PluginOrigin>,
) -> bucket_hooks_plugins_types::PluginInfo {
    bucket_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: bucket_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: bucket_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: bucket_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a bucket_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &bucket_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use bucket_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectBucket) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserBucket) => PluginGroup::new(2, "origin:user", "User"),
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [bucket_hooks_plugins_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a bucket_hooks_plugins_types::MarketplaceScanResult,
        plugin: &'a bucket_hooks_plugins_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [bucket_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[bucket_hooks_plugins_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> bucket_telemetry::events::ExtensionsModalTab {
        use bucket_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(bucket_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(bucket_hooks_plugins_types::PluginsAction),
    ...
    AddMcpServer {
        name: String,
        config: Box<bucket_agent_core::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open bucket.com connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(bucket_hooks_plugins_types::MarketplaceAction),
...
pub enum ModalMessage {
    ...
    Confirmation {
        message: String,
        action: bucket_hooks_plugins_types::PluginsAction,
    },
    /// A confirmation prompt for a marketplace action (install/uninstall/update).
    MarketplaceConfirmation {
        message: String,
        action: bucket_hooks_plugins_types::MarketplaceAction,
    },
}
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use bucket_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use bucket_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            bucket_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<bucket_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<bucket_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<bucket_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<bucket_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[bucket_hooks_plugins_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let bucket = bucket_config::bucket_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user bucket home (BUCKET_HOME-aware)
    // or a project-scoped `{cwd}/.bucket/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User bucket home (BUCKET_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(bucket.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.bucket/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".bucket" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $BUCKET_HOME/hooks
    let global_hooks = bucket.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.bucket/hooks") || source_dir.contains("/.bucket/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&bucket) {
            let prefix = crate::util::display_bucket_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            bucket_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_bucket_home(path) {
                    crate::util::display_user_bucket_path("skills")
            ...
            bucket_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            bucket_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &bucket_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use bucket_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &bucket_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[bucket_hooks_plugins_types::ComponentItem]); 6] {
    use bucket_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &bucket_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &bucket_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.bucket/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // bucket-home branch is BUCKET_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.bucket/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.bucket/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("bucket_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by bucket.com")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "bucket_com_linear"),
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> bucket_tools::implementations::skills::types::SkillInfo {
        bucket_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: bucket_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = bucket_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(bucket_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> bucket_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: bucket_hooks_plugins_types::PluginOrigin,
    ) -> bucket_hooks_plugins_types::PluginInfo {
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    bucket_agent_core::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    bucket_agent_core::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                bucket_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> bucket_hooks_plugins_types::HookInfo {
        bucket_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: bucket_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: bucket_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> bucket_hooks_plugins_types::MarketplaceScanResult {
        bucket_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> bucket_hooks_plugins_types::MarketplaceScanResult {
        bucket_hooks_plugins_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.bucket/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<bucket_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> bucket_hooks_plugins_types::MarketplacePluginEntry {
            ...
            bucket_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                bucket_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                bucket_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(bucket_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = bucket_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> bucket_hooks_plugins_types::ComponentItem {
        bucket_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> bucket_hooks_plugins_types::PluginComponents {
        bucket_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(bucket_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = bucket_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<bucket_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
    ...
    fn plugin_group_maps_each_origin_variant() {
        use bucket_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectBucket, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserBucket, 2, "origin:user", "User"),
            (
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = bucket_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = bucket_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = bucket_hooks_plugins_types::PluginScope::Config;
        ...
        assert_eq!(group.key, "bucket-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            bucket_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("xAI Official".into());
        assert_eq!(plugin_group(&unknown).key, "bucket-mp:xAI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserBucket),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use bucket_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserBucket),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use bucket_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserBucket);
