...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), BucketComConfig::default()));
    am.hot_swap(BucketAuth {
...
/// attribution.
fn auth_error() -> bucket_sampler::SamplingErrorInfo {
    bucket_sampler::SamplingErrorInfo {
        kind: bucket_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: bucket_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        bucket_chat_state::AuthType::SessionToken => "cached_token",
        bucket_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `BUCKET_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(bucket_chat_state::Credentials {
...
/// `BUCKET_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> bucket_sampler::SamplingErrorInfo {
    bucket_sampler::SamplingErrorInfo {
            kind: bucket_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model bucket-build does not exist or your team does not have access".into(),
...
async fn legacy_auth_hint_on_404_model_not_found() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                msg.contains("bucket logout"),
                "hint must mention `bucket logout`, got: {msg}"
            );
            assert!(
                msg.contains("bucket login"),
                "hint must mention `bucket login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> bucket_sampler::SamplingErrorInfo {
    bucket_sampler::SamplingErrorInfo {
            kind: bucket_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.bucket.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=bucket-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
// transiently collapsed to `ApiKey` (session-token cache miss + `BUCKET_API_KEY`)
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = bucket_sampler::SamplerConfig {
