...
//! other concurrently running bucket processes (which may wipe or downgrade
...
/// User-configurable overrides via `~/.bucket/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple bucket processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    bucket_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
