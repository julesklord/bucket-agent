...
/// Claude.md / `.bucket/rules/*.md`) discovered from the workspace root up to
/// the git root, plus `~/.bucket` and compat dirs.
...
/// Mirrors the serde shape of `bucket-agent`'s `AgentConfigFile`.
