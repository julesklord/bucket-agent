//! Minimal (scrollback-native) render mode — `bucket --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `bucket_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `bucket-tui` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`bucket_tui::minimal_hook`]) of function pointers, and the
//! composition-root binary (`bucket-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `bucket-tui`.
///
...
/// subsequent calls are ignored (see [`bucket_tui::minimal_hook`]).
pub fn install() {
    bucket_tui::minimal_hook::install(bucket_tui::minimal_hook::MinimalHooks { draw });
}
