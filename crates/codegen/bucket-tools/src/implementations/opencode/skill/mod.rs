...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.bucket/skills/ or .bucket/skills/)
${%- endif %}
</available_skills>"#;
...
impl bucket_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, bucket_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "skill");
