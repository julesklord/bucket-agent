...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".bucket", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
fn has_excluded_component(path: &Path) -> bool {
    for component in path.components() {
        if let std::path::Component::Normal(name) = component {
            ...
            if name_lower.starts_with(".bucket-") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn bucket_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".bucket")));
                assert!(!is_project_dir(&home.join(".bucket/bin")));
            }
        }

        #[test]
        fn bucket_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".bucket-proxy-work")));
            }
        }
