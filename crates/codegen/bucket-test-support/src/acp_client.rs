//! ACP stdio clients for testing bucket sessions end-to-end: the typed
//! [`BucketStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
use crate::env::{bucket_binary, test_env_cmd_tokio};
use crate::headless::stderr_tail;
use crate::mock_server::MockInferenceServer;
use crate::process::spawn_piped_with_stderr_capture;

/// Spawn `bucket agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`BucketStdioClient`])
...
/// kill-list so a test can still set e.g. `BUCKET_DEBUG_LOG=1` explicitly.
...
/// Drives `bucket agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`BucketStdioClient`] (`ClientSideConnection`,
