...
//!
//! Extracted from `bucket-agent-core/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `bucket-chat-state::compaction_utils::strip_system_tags` which
...
/// `bucket-agent-core/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use bucket_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(bucket_sampling_types::synthesized_reasoning_item(text))
    }
