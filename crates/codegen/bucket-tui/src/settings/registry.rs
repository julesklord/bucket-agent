...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"bucketnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"Bucket Night"`).
...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: bucket_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`bucket_voice::canonicalize_stt_language`] so the pager and
/// the STT client share one catalog (official Bucket STT languages + client-only
/// `auto`). Unknown/blank/`None` → `en`.
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    bucket_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `BUCKET_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = bucket_agent_core::models::default_model();
...
mod tests {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in bucket-tools"
                    );
                }
    ...
    /// Spot-check the delegation to `bucket_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = bucket_voice::stt_language_by_code(c.canonical)
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: bucket_agent_core::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
