...
//!
//! Reads OIDC credentials from `~/.bucket/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use bucket_workspace::config::WorkspaceServerMetadata;
use bucket_workspace::daemonize;
use bucket_workspace::diag_server;
use bucket_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
use clap::Parser;
...
const SERVICE_NAME: &str = "prod_bucket_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<bucket_tool_protocol::ServerId>()
        .err()
        .map(|e| format!("{INVALID_SERVER_ID_MARKER} {id:?}: {e}"))
}
#[derive(Parser)]
#[command(name = "bucket-workspace-server")]
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.bucket.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `BUCKET_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "BUCKET_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Override with `BUCKET_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "BUCKET_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/bucket-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = bucket_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("BUCKET_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match bucket_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use bucket_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("BUCKET_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized BUCKET_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if bucket_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via BUCKET_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = bucket_sandbox::is_active();
            ...
            tracing::info!(
                ...
                bucket_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = bucket_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("BUCKET_SESSION_ID").ok();
    ...
    bucket_workspace::init_metrics();
    let ws_handle = bucket_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = bucket_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(grace_budget, bucket_workspace::handle::DrainReason::Sigterm)
        .await;
    ...
    bucket_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["bucket-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["bucket-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["bucket-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["bucket-workspace-server", "--ready-file", "/tmp/x.ready"])
                .unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["bucket-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://bucket.com/preview-auth")
        );
