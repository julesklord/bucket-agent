...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] bucket_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `bucket-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `bucket_tools::ToolError`) happens
