//! Session-level fs-watch policy over [`bucket_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `bucket_fsnotify`. This module
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> bucket_workspace::file_system::FileIndexDelta {
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = bucket_workspace::session::git::get_branch(&self.cwd).await;
        let commit = bucket_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = bucket_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = bucket_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: bucket_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || bucket_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = bucket_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(bucket_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: bucket_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
