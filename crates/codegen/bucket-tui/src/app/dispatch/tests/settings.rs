...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let response = bucket_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![bucket_hooks_plugins_types::MarketplaceScanResult {
            source_name: bucket_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: bucket_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn slash_model_valid_dispatches_set_default_model_with_switch_and_persist() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_id = acp::ModelId::new(std::sync::Arc::from("bucket-4.5"));
    app.agents
        .get_mut(&id)
        .unwrap()
        .session
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Bucket 4.5".to_string()),
        );
    let effects = dispatch(Action::SendPrompt("/model Bucket 4.5".into()), &mut app);
    assert_eq!(
        effects.len(),
        2,
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("bucket-test"));
    let info = acp::ModelInfo::new(id.clone(), "Bucket Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("bucket-4.5"));
    let info = acp::ModelInfo::new(id.clone(), "Bucket 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
    ...
    assert!(
        matches!(& effects[0], Effect::PersistSetting { key : "default_model", value :
        crate ::settings::SettingValue::String(s), .. } if s == "bucket-4.5")
    );
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("bucket-already"));
    let info = acp::ModelInfo::new(id.clone(), "Bucket Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `clamp_to_terminal` might fold to BucketNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        crate::theme::cache::set(crate::theme::ThemeKind::BucketNight);
        let effects = dispatch(Action::SetTheme("bucketday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("bucketday"));
                assert_eq!(*rollback_value, SettingValue::Enum("bucketnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("bucketday"));
        assert!(
...
/// for the auto-dark sibling. Uses `bucketday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("bucketday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("bucketday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("bucketnight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("bucketnight"));
                assert_eq!(*rollback_value, SettingValue::Enum("bucketday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="bucketnight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="bucketday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on BucketNight even though we're committing a
/// different theme. Uses `bucketday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("bucketnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::BucketNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("bucketday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::BucketNight,
...
///
/// Uses `BucketDay` (non-truecolor-requiring) for the dark-mode
...
/// BucketNight. Using a non-truecolor theme avoids the clamp
...
/// Uses `bucketnight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from BucketDay, the
...
fn set_auto_dark_theme_does_not_apply_when_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoDarkTheme("bucketnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::BucketDay,
...
/// (`bucketnight`) for the same clamp reason as the dark variant.
#[test]
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("bucketnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::BucketNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("bucketday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Bucket Day"),
            "toast must use display name `Bucket Day`, not canonical `bucketday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Bucket Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Bucket Night"));
    });
}
...
///
/// Uses non-truecolor themes (`bucketday` ↔ `bucketnight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("bucketnight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("bucketday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
