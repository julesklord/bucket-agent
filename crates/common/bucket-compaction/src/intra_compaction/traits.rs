...
pub enum CompactionTarget {
    ...
    /// with a single summary — bucket-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Bucket chat's
/// `StreamProcessor` (`Item = Arc<BucketTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<BucketTurn>`) already satisfies it.
