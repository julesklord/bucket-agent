...
//!
//! `ToolSearchIndex` is defined in `bucket-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `bucket-agent-core`
...
/// in `bucket-agent-core` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `bucket-agent-core` after MCP initialization.
#[derive(Clone)]
