//! Unified PTY harness for bucket-tui.
//!
...
//!    `bucket-tui/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `BUCKET_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use bucket_tui_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through bucket's stdout. Used to reproduce the doubled-line class of bugs
    /// where bucket's diff renderer never re-asserts a region it didn't write
