...
//! [`MockInferenceServer`] from `bucket-test-support` and provides the
...
pub use bucket_test_support::mock_server::LogEntry;
pub use bucket_test_support::mock_server::MockModelEntry as MockModel;
pub use bucket_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use bucket_test_support::sse;
pub use bucket_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.bucket
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit BUCKET_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // BUCKET_HOME is set in the test runner's env).
            ("BUCKET_HOME".into(), bucket_home),
            ("BUCKET_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("BUCKET_BUCKET_API_BASE_URL".into(), self.url()),
            ("BUCKET_API_KEY".into(), "test-key-for-ci".into()),
            ...
            ("BUCKET_PROMPT_SUGGESTIONS".into(), "false".into()),
            ...
            ("BUCKET_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    /// on the SuperBucket upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("BUCKET_HOME").as_deref(),
            content.home().join(".bucket").to_str()
        );
        assert_eq!(get("BUCKET_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("BUCKET_BUCKET_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("BUCKET_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("BUCKET_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("BUCKET_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("BUCKET_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("BUCKET_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("BUCKET_MAX_RETRIES").as_deref(), Some("0"));
