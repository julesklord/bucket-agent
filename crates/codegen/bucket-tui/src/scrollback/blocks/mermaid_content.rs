...
//! the generic [`CodeBlockSpan`](bucket_markdown::CodeBlockSpan) API) and
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](bucket_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](bucket_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a bucket_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `BucketDay` is the only light theme; every other concrete theme (and the
/// `BucketNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `bucket_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::BucketDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_bucketday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::BucketDay),
            "BucketDay is the light theme"
        );
        for dark in [
            ThemeKind::BucketNight,
