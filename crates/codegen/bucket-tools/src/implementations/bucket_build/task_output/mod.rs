...
use crate::implementations::bucket_build::task::TaskTool;
use crate::implementations::bucket_build::task::backend::SubagentBackendResource;
use crate::implementations::bucket_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::bucket_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `BUCKET_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("BUCKET_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let bucket_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: bucket_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: bucket_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
    vec![bucket_build_bash, bucket_build_concise_bash, opencode_bash]
}
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, bucket_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<bucket_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = bucket_tool_types::task_output_waits(timeout_ms);
...
pub(crate) use bucket_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&bucket_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
/// [`bucket_tool_types::build_task_output_description`] builder so the CLI and
...
impl bucket_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default bucket-build
    ...
    async fn multi_task_ids_poll_returns_multi_result_mode_poll() {
        let resources = resources_with_terminal(None);
        let tool = TaskOutputTool;
        let out = bucket_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
            TaskOutputToolInput {
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::bucket_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::bucket_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::bucket_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::bucket_build::task::types::SubagentEvent,
    ) -> crate::implementations::bucket_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::bucket_build::task::types::SubagentEvent::Query(q) => q,
