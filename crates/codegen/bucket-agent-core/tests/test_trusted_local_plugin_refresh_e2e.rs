...
//! 4. Smoke-validate session JSON under `$BUCKET_HOME/sessions/` after exit.
//!
//! Requires a built `bucket` binary (`BUCKET_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p bucket-agent-core --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p bucket-agent-core --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _bucket_guard = EnvVarGuard::set("BUCKET_HOME", &bucket_home);
    ...
    let mut registry = InstallRegistry::empty(bucket_home.join("installed-plugins"));
    ...
    // bucket_home() caches the first BUCKET_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        bucket_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built bucket binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/BUCKET_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via bucket_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _bucket_guard = EnvVarGuard::set("BUCKET_HOME", &bucket_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("BUCKET_HOME", &bucket_home);
    ...
    // Smoke: session storage under BUCKET_HOME/sessions has JSON artifacts.
