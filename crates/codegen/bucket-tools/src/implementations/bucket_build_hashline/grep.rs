...
use crate::implementations::bucket_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl bucket_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = bucket_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(bucket_tool_runtime::Cwd(cwd));
        let mut result = bucket_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                bucket_tool_runtime::ToolError::execution(
                    bucket_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
        ...
        if matches!(output_mode, OutputMode::Content) && result.exit_code == 0 {
            ...
            let (fs, scheme) = {
                ...
                let scheme = params
                    .0
                    .build_scheme()
                    .map_err(bucket_tool_runtime::ToolError::invalid_arguments)?;
                (fs, scheme)
            };
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
