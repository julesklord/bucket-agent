...
fn resolve_config(cfg: &AgentConfig, auth_manager: &AuthManager) -> AgentConfig {
    ...
    if cfg.remote_settings.is_none()
        && let Some(handle) =
            crate::agent::models::start_early_prefetch(Some(cfg.bucket_com_config.clone()))
    {
        match handle.join() {
            Ok(result) => {
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (bucket -p, subagents).
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_bundled_files(&bucket_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&bucket_home);
        }
