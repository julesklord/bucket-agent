...
pub async fn git_cli(cwd: &Path, args: &[&str]) -> Result<String> {
    ...
    for &(key, val) in bucket_tty_utils::GIT_AUTH_SUPPRESSION_ENVS.iter() {
        cmd.env(key, val);
    }
...
///
/// ```
/// use bucket_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
///
/// 1. `supplied_cwd` is a bucket-managed worktree (`~/.bucket/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = bucket_tools::util::bucket_home::bucket_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "BUCKET_WORKSPACE_REWIND_GIT";
...
pub fn git_rewind_enabled() -> bool {
    bucket_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
mod tests {
    ...
    fn test_resolve_persisted_session_git_metadata_collects_sorted_unique_remotes() {
        ...
        .unwrap();
        repo.remote("backup", "https://gitlab.com/bucket-org/example.git")
            .unwrap();
        repo.remote("duplicate", "https://github.com/bucket-org/example.git")
            .unwrap();
        ...
        assert_eq!(
            ...
            vec![
                "https://github.com/bucket-org/example.git".to_string(),
                "https://gitlab.com/bucket-org/example.git".to_string(),
            ]
        );
    }
    #[test]
    fn test_resolve_persisted_session_git_metadata_captures_head() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "https://github.com/bucket-org/example.git")
            .unwrap();
        let metadata = resolve_persisted_session_git_metadata_sync(tmp.path());
    ...
    fn test_resolve_persisted_session_git_metadata_worktree_resolves_remotes() {
        ...
        assert_eq!(
            metadata.git_remotes,
            vec!["https://github.com/bucket-org/example.git".to_string()],
        );
    }
    #[test]
    ...
    fn test_effective_worktree_cwd_empty_offset() {
        let result =
            effective_worktree_cwd("/home/user/.bucket/worktrees/repo/ab-123-a", Path::new(""));
        assert_eq!(result, "/home/user/.bucket/worktrees/repo/ab-123-a");
    }
    #[test]
    ...
    fn test_effective_worktree_cwd_nested_offset() {
        let result = effective_worktree_cwd(
            "/home/user/.bucket/worktrees/repo/ab-123-b",
            Path::new("packages/frontend/src"),
        );
        assert_eq!(
            result,
            "/home/user/.bucket/worktrees/repo/ab-123-b/packages/frontend/src"
        );
    }
    #[test]
    ...
    fn test_effective_cwd_roundtrip_with_compute_offset() {
        ...
        let worktree_root = "/home/user/.bucket/worktrees/myrepo/ab-test-a";
    ...
    fn normalize_ssh_scp_url() {
        assert_eq!(
            normalize_repo_url("git@github.com:bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_https_url() {
        assert_eq!(
            normalize_repo_url("https://github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_and_https_produce_same_result() {
        let ssh = normalize_repo_url("git@github.com:bucket-org/example.git");
        let https = normalize_repo_url("https://github.com/bucket-org/example.git");
        assert_eq!(ssh, https);
    }
    #[test]
    fn normalize_https_without_git_suffix() {
        assert_eq!(
            normalize_repo_url("https://github.com/bucket-org/example"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_https_with_credentials() {
        assert_eq!(
            normalize_repo_url("https://x-access-token:secret@github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_url() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_ssh_scheme_with_port() {
        assert_eq!(
            normalize_repo_url("ssh://git@github.com:22/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_http_url() {
        assert_eq!(
            normalize_repo_url("http://github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    ...
    fn normalize_git_plus_ssh_scheme() {
        assert_eq!(
            normalize_repo_url("git+ssh://git@github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_git_plus_https_scheme() {
        assert_eq!(
            normalize_repo_url("git+https://github.com/bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    fn normalize_scp_no_user() {
        assert_eq!(
            normalize_repo_url("github.com:bucket-org/example.git"),
            Some("github.com/bucket-org/example".into()),
        );
    }
    #[test]
    ...
    fn resolve_normalized_remote_urls_deduplicates_across_transports() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = git2::Repository::init(tmp.path()).unwrap();
        repo.remote("origin", "git@github.com:bucket-org/example.git")
            .unwrap();
        repo.remote("https-mirror", "https://github.com/bucket-org/example.git")
            .unwrap();
        let urls = resolve_normalized_remote_urls(tmp.path());
        assert_eq!(urls, vec!["github.com/bucket-org/example"]);
    }
}
#[cfg(test)]
...
mod restore_code_tests {
    ...
    async fn stash_before_destructive_op_dirty_tracked_returns_ref() {
        ...
        assert!(
            list.contains("bucket: pre-test sess-2"),
    ...
    fn restore_code_checkout_allowed_worktree_cwd_is_allowed() {
        let worktrees = Path::new("/home/u/.bucket/worktrees");
        assert!(restore_code_checkout_allowed_in(
            Path::new("/home/u/.bucket/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            Some("/home/u/repo"),
            worktrees,
        ));
    }
    ...
    fn restore_code_checkout_allowed_source_repo_with_worktree_session_is_refused() {
        ...
        assert!(!restore_code_checkout_allowed_in(
            Path::new("/home/u/repo"),
            Some("/home/u/.bucket/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            worktrees,
        ));
    }
    #[test]
