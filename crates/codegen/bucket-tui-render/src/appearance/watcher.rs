...
//!
//! In dev mode, watches ~/.bucket/pager.toml for changes and hot-reloads.
...
///
/// In dev mode: reads from ~/.bucket/pager.toml, watches for changes.
...
impl ConfigWatcher {
    ...
    ///
    /// - In dev mode: reads/creates ~/.bucket/pager.toml, watches for changes
    ...
    fn start_static() -> io::Result<Self> {
        let config = bucket_config::user_bucket_home()
