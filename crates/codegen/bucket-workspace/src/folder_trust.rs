...
//! [`crate::trust::TrustStore`] (`~/.bucket/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `bucket-agent-core`.
//!
...
//! provisional and re-checked rather than cached — is a `bucket-agent-core`
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.bucket`).
...
/// this binary — true on a local/dev build (no `BUCKET_VERSION` release stamp).
///
/// THE single security short-circuit: every explicit trust auto-grant site calls
/// this (greppable via `folder_trust_inert`). When true a self-built bucket never
...
/// (`BUCKET_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `bucket-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned BUCKET_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(bucket_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("BUCKET_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `BUCKET_VERSION` release stamp) the feature is OFF
/// regardless of env/config/remote — a self-built bucket auto-trusts (never
...
/// env `BUCKET_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit BUCKET_FOLDER_TRUST/config
    // opt-in is ignored here so a self-built bucket never prompts.
    if is_local_build {
        return false;
    }
    ...
    let user = bucket_config::load_from_disk().ok();
    let managed = bucket_config::load_managed_config().ok();
    BoolFlag::env("BUCKET_FOLDER_TRUST")
...
/// same `~/.bucket/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `.claude` / `.bucket/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = bucket_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.bucket/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = bucket_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.bucket/lsp.json`.
    if cwd.join(".bucket").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.bucket/hooks/evil.json`) would
    ...
    if hook_root.join(".bucket").join("hooks").is_dir()
    ...
    // that must be gated — else a plugin clone (e.g. `.bucket/plugins/evil/`, even
    // one in a subdir launched via `cd sub && bucket`) would resolve trusted and
    ...
    if !bucket_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.bucket/agents` / `.claude/agents`): a project agent
    ...
    if !bucket_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .bucket/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_bucket_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_bucket_lsp_json() {
        let tmp = repo_tmp();
        let bucket = tmp.path().join(".bucket");
        std::fs::create_dir_all(&bucket).unwrap();
        std::fs::write(bucket.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.bucket/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".bucket").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".bucket").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".bucket").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let bucket = tmp.path().join(".bucket");
        std::fs::create_dir_all(&bucket).unwrap();
        std::fs::write(bucket.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_bucket_config_plugins_paths() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.bucket/agents` — even when launched from a SUBDIR (the cwd→git-root walk
        // that `first_only` shares). Guards against silent drift between the two.
        let tmp = repo_tmp();
        let bucket = tmp.path().join(".bucket");
        std::fs::create_dir_all(bucket.join("agents")).unwrap();
        std::fs::write(bucket.join("config.toml"), "[plugins]\npaths = [\"./x\"]\n").unwrap();
    ...
    // BUCKET_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`bucket-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(bucket_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvVarGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty BUCKET_HOME (no
        // config.toml/managed_config.toml) + BUCKET_FOLDER_TRUST unset. nextest's
        // process-per-test makes bucket_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvVarGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // BUCKET_FOLDER_TRUST=1 does NOT enable the feature (so a self-built bucket
        // never prompts). BUCKET_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvVarGuard::set("BUCKET_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty BUCKET_HOME (no config.toml/managed config) +
        // BUCKET_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _flag = EnvVarGuard::unset("BUCKET_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned BUCKET_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(bucket_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no BUCKET_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(bucket_version::TEST_VERSION_ENV);
        if option_env!("BUCKET_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); BUCKET_HOME-isolated
        // and ENV_LOCK-serialized so toggling BUCKET_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _unset = EnvVarGuard::unset(bucket_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // BUCKET_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // BUCKET_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _sim = simulate_release_build();
