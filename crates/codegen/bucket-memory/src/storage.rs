...
//! `~/.bucket/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// Write-operation scope. Distinct from `bucket_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.bucket/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.bucket/memory/bucket-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.bucket/memory/`
    global_dir: PathBuf,
    /// `~/.bucket/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.bucket/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| bucket_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        bucket_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.bucket/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by Bucket's memory system.\n\
