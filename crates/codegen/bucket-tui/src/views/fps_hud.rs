//! Release-safe FPS readout — `/debug fps`, `BUCKET_FPS` on release builds.
//!
//! The full frame profiler (`render::frame_metrics`, `BUCKET_FPS`) is compiled
...
//!
//! `BUCKET_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` as
...
//! HUD from startup, so `BUCKET_FPS=1` is never a silent no-op
//! ([`HONORS_BUCKET_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `BUCKET_FPS` env gate: only where the dev
/// `FrameMetrics` overlay is compiled out. In debug/dev builds the env keeps
/// feeding that overlay alone.
const HONORS_BUCKET_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD. `BUCKET_FPS` enables it at startup on
/// release binaries ([`HONORS_BUCKET_FPS_ENV`]); `/debug fps` toggles it
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("BUCKET_FPS").ok())
    }
    /// `env` is the raw `BUCKET_FPS` value; the truthiness rule (nonempty and
    /// not `"0"`) matches `FrameMetrics` and `BUCKET_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_BUCKET_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `BUCKET_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {
...
mod tests {
    ...
    /// asserting against [`HONORS_BUCKET_FPS_ENV`] keeps the test true under
    /// both cfgs (the dev half is pinned by the constant's shape, the same
    /// limitation as the `/debug` visibility test).
    #[test]
    fn bucket_fps_env_enables_hud_where_dev_overlay_absent() {
        for truthy in ["1", "full", " "] {
            assert_eq!(
                FpsHud::with_env(Some(truthy.into())).enabled(),
                HONORS_BUCKET_FPS_ENV,
                "BUCKET_FPS={truthy:?} must track the env-gate owner"
            );
        }
