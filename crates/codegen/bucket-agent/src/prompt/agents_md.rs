...
//!
//! Searches from cwd to repo root, plus `~/.bucket/`. Also discovers
//! `*.md` files in `.bucket/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.bucket/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.bucket/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = bucket_tools::util::bucket_home::bucket_home();
    ...
    // Always include bucket_home (~/.bucket/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `bucket-agent-core` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .bucket/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.bucket/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            bucket_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
