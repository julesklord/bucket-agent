...
//!   into `\\.\pipe\bucket-leader-<hash>` so callers keep their path-based API.
//!
#[cfg(unix)]
...
mod windows_impl {
    ...
    /// Deterministic leaf name (`bucket-leader-<hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("bucket-leader-{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_is_deterministic() {
            let a = path_to_pipe_name(Path::new("/tmp/bucket.sock"));
            let b = path_to_pipe_name(Path::new("/tmp/bucket.sock"));
            assert_eq!(a, b);
        }
        ...
        fn pipe_name_has_correct_prefix() {
            ...
            assert!(s.starts_with(r"\\.\pipe\bucket-leader-"), "got: {s}");
        }
        ...
        fn pipe_name_is_bounded() {
            ...
            // \\.\pipe\bucket-leader- (20 chars) + 16 hex chars = 36 total
