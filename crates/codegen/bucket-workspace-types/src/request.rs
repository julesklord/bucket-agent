...
//!   here would force every consumer of `bucket-workspace-types`
...
//! (`bucket-workspace`) wraps this in its own `Request<T>` that
//! adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `bucket-workspace-types`
