...
impl bucket_tool_runtime::Tool for WebSearchTool {
    type Args = WebSearchInput;
    type Output = WebSearchOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("web_search").expect("valid tool id")
    }
    ...
    ) -> Result<WebSearchOutput, bucket_tool_runtime::ToolError> {
        ...
        let (content, citations) = client
            ...
            .map_err(|e| {
                bucket_tool_runtime::ToolError::execution(
                    bucket_tool_protocol::ToolId::new("web_search").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "web_search");
        assert!(
