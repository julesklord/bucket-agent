...
//!
//! Simplified to only support BucketShell (in-process) mode.
...
/// Spawn a BucketShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_bucket_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<bucket_agent_core::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.bucket/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        bucket_agent_core::agent::init::bootstrap(&agent_config, &auth_manager, None)
