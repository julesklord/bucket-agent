...
> {
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        bucket_workspace::permission::types::PermissionConfig::new(
        ...
        let hub_permission = if bucket_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn bucket_workspace::permission::PermissionHookTransport,
                        >
                });
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let resolved_task_output =
        ...
        .await;
    let resolved_read =
        bucket_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, bucket_agent::AgentBuildError>,
    >();
