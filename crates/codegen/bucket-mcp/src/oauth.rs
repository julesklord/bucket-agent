...
const MCP_OAUTH_CLIENT_NAME: &str = "Bucket";
...
// mode disabled, multiple `bucket` invocations).
//
// Layer 1 (cross-process): filesystem lock at $BUCKET_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    bucket_config::bucket_home().join(format!("mcp_auth_{safe}.lock"))
}
