//! Permission resolution engine: merges native `.bucket/config.toml`,
...
/// (fail-safe) with a warn + skip record for `bucket inspect`.
...
/// user `~/.bucket` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    bucket_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.bucket/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.bucket/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `bucket-agent-core::config::find_project_configs`.
fn find_project_bucket_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".bucket").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".bucket").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
/// Load `[permission]` rules from native Bucket TOML config files:
///
///   * `~/.bucket/config.toml` (lowest priority)
///   * Each `.bucket/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.bucket/config.toml` first (lowest priority within this layer).
    // Gated on user_bucket_home() so a project's .bucket/config.toml is never read as
    // global permissions when neither BUCKET_HOME nor a home dir resolves.
    if let Some(global_path) = bucket_config::user_bucket_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match bucket_config::load_config_file(&global_path) {
    ...
    for path in find_project_bucket_configs(cwd) {
        match bucket_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[bucket_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Bucket and Claude sources.
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `bucket inspect`.
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &bucket_config::managed_config_layers(),
            ),
        }
    }
}
...
/// bypass is pinned off via bucket `requirements.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `bucket inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in find_claude_settings_paths(cwd) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `bucket inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `bucket inspect` via the outer resolver.
...
/// [`bucket_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = bucket_config::claude_managed_settings_path() else {
...
/// not consulted: bucket must not inherit a host-wide always-approve lockdown from
/// that file. bucket still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in bucket use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = bucket_config::requirements_layers();
...
/// without `~/.bucket`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (bucket.com-injected) MCP server names. Defined
/// here (shell depends on workspace) and re-exported by shell's `to_managed_name`
/// so the prefix and policy matching never drift.
pub const MANAGED_MCP_PREFIX: &str = "bucket_com_";

/// Max `char` length of a managed runtime name (`bucket_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `bucket_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `BUCKET_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // BUCKET_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("BUCKET_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_BUCKET_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate BUCKET_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate BUCKET_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "bucket_com_foo"));
        assert!(mcp_name_matches("bucket_com_foo", "foo"));
        assert!(mcp_name_matches("bucket_com_foo", "bucket_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "bucket_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "bucket_com_slack"));
        assert!(mcp_name_matches("My Server", "bucket_com_my_server"));
        assert!(mcp_name_matches("bucket_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("bucket_com_", "bucket_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        let managed = http_named("bucket_com_foo", "https://foo.example.com/mcp");
        assert!(al.is_server_denied(&managed));
        assert!(!al.is_server_allowed(&managed));

        // Name match is transport-agnostic.
        let stdio = stdio_named("bucket_com_foo", "npx");
        ...
        for unrelated in ["foobar", "bucket_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("bucket_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("bucket_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("bucket_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "bucket_com_foo" } ]
        }));
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/bucket/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/bucket/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.bucket/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/bucket/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.bucket/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/bucket/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/bucket.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for bucket inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            bucket_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            bucket_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
