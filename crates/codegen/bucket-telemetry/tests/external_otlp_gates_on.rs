...
const SECRET_MODEL: &str = "bucket-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    bucket_telemetry::log_event(bucket_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "bucket_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.bucket_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("bucket-cli"),
        "service.name=bucket-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("bucket_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let tool = col::find_event(&collected, "bucket_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "bucket_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "bucket_code.session.count");
