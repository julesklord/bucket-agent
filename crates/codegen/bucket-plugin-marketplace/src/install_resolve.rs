//! Pure resolution logic for `bucket plugin install <name>` marketplace refs.
...
mod tests {
    ...
    fn parse_name_with_owner_repo_qualifier() {
        assert_eq!(
            parse_marketplace_ref("sentry@bucket-org/plugin-marketplace"),
            Some(MarketplaceRef {
                name: "sentry".into(),
                qualifier: Some("bucket-org/plugin-marketplace".into()),
            })
        );
    }
    ...
    fn parse_rejects_fragment() {
        ...
        assert_eq!(
            parse_marketplace_ref("sentry@bucket-org/marketplace#sub"),
            None
        );
    }
    ...
    fn slugify_lowercases_and_hyphenates_spaces() {
        ...
        assert_eq!(slugify("xAI Official"), "bucket-official");
    }
    ...
    fn resolve_qualifier_normalizes_dot_git_in_qualifier() {
        ...
        assert_eq!(
            resolve_qualified_source("bucket-org/plugin-marketplace.git", &sources),
            Ok(0)
        );
    }
