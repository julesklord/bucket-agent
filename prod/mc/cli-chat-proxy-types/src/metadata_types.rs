...
/// v1.18: Added `shell_version`: the bucket-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like bucket-desktop.
...
/// OS-level sandbox state for a trace turn (local `bucket-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// The agent type / harness name for this session (e.g. "bucket-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the bucket-shell agent binary that handled this turn
    /// (`bucket_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like bucket-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
