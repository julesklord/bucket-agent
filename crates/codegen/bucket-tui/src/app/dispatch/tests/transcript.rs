...
fn plugins_list_response() -> bucket_hooks_plugins_types::PluginsListResponse {
    use crate::views::extensions_modal::test_plugin_info;
    bucket_hooks_plugins_types::PluginsListResponse {
        plugins: vec![
            test_plugin_info(
                "user-tool",
                Some(bucket_hooks_plugins_types::PluginOrigin::UserBucket),
            ),
            test_plugin_info(
                "claude-tool",
                Some(bucket_hooks_plugins_types::PluginOrigin::UserClaude),
            ),
        ],
    }
}
