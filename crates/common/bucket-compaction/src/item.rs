...
//! harness implements [`CompactionItem`] for its `BucketTurn`;
//! bucket-build implements it for `bucket_sampling_types::ConversationItem`.
//!
...
//! `<bucket_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `BucketRole` (Bucket chat) and the
/// `ConversationItem` variants (bucket-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Bucket chat) — maps to System on harnesses without a
...
/// extraction (`<bucket_file id=".." name=".." />` lines in the
/// `<bucket_user_queries>` preamble).
...
///
/// Implementors:
/// - Bucket chat: `BucketTurn`
/// - bucket-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Bucket chat's
    /// `BucketTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Bucket chat: a
    ...
    /// strips their `<bucket_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<bucket_file>` lines in the
    /// `<bucket_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Bucket chat: a `Developer` turn with category
    ...
    /// requests/results (Bucket chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up bucket-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Bucket chat's
/// `BucketTurn`; adding these constructors to it as required methods would break
/// that impl. They are also bucket-build-specific (Bucket chat's tail-keep path
...
///
/// The bucket-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Bucket chat stores turns as `Arc<BucketTurn>`).
