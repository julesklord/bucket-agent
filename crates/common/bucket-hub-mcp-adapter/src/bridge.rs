...
use bucket_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use bucket_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`bucket_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`bucket_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl bucket_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            bucket_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == bucket_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
