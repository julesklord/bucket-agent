...
//! `bucket-agent-core` and `bucket-tui` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `bucket-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `bucket-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Bucket's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `bucket-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.bucket/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.bucket/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.bucket/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.bucket/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserBucket),
            conflict: None,
        };
