...
#[ignore = "PTY e2e; run with cargo test -p bucket-tui --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared BUCKET_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let bucket_home = content.home().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create BUCKET_HOME");
    std::fs::write(
        bucket_home.join("config.toml"),
        format!("[models]\ndefault = \"{CONFIG_MODEL}\"\n"),
    )
    .expect("write config.toml");
    let socket = bucket_home.join("leader-e2e.sock");
    let socket = socket.to_str().expect("socket path is utf-8").to_owned();

    // Session (OAuth) auth, not the harness's default BUCKET_API_KEY: the
    ...
    let state_path = bucket_home.join("campaigns_state.json");
