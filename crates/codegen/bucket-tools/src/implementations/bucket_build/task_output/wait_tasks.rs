...
use crate::implementations::bucket_build::task_output::{
...
use bucket_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
/// [`bucket_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl bucket_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
