...
pub fn loaded_plugin_to_info(plugin: &bucket_agent::plugins::LoadedPlugin) -> PluginInfo {
    use bucket_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &bucket_agent::plugins::PluginOrigin) -> PluginOrigin {
    use bucket_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectBucket => PluginOrigin::ProjectBucket,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserBucket => PluginOrigin::UserBucket,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/plugins/action" => {
            let req: bucket_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use bucket_agent::plugins::PluginOrigin as AgentOrigin;
    use bucket_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> bucket_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        bucket_agent::plugins::LoadedPlugin {
