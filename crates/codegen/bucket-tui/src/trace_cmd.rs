...
use bucket_agent_core::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $BUCKET_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
    ...
    if !agent_config.is_trace_upload_enabled() {
        ...
        if !args.json {
            eprintln!(
                ...
                crate::util::display_user_bucket_path("config.toml")
            );
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            bucket_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    bucket_agent_core::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_bucket_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    bucket_home().join("trace-exports")
}
...
async fn run_upload(
    ...
    let upload_method = match upload_method {
        ...
        None => {
            ...
            anyhow::bail!(
                "No upload credentials. Run `bucket login` or set a deployment key. \
                 See {} for upload overrides.",
                crate::util::display_user_bucket_path("docs/user-guide")
            );
        }
    };
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            bucket_agent_core::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
            "No trace upload bucket configured. Set `BUCKET_TELEMETRY_GCS_BUCKET`, \
             `BUCKET_TRACE_UPLOAD_BUCKET`, or `endpoints.trace_upload_bucket` in \
    ...
    let upload_config = bucket_agent_core::session::repo_changes::TraceExportConfig {
...
impl UploadAttempt<'_> {
    ...
    pub fn handle_failure(&self, error: &anyhow::Error) -> anyhow::Error {
        ...
        if self.json {
            ...
            eprintln!("  Retry:  bucket trace {}", self.session_id);
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "Bucket version: {}", env!("VERSION_WITH_COMMIT"));
        let _ = writeln!(
            log,
            "OS:           {} {}",
            std::env::consts::OS,
...
async fn upload_with_retries(
    config: &bucket_agent_core::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
    ...
    (|| async {
        tokio::time::timeout(
            UPLOAD_TIMEOUT,
            bucket_file_utils::gcs::upload_bytes(config, object_path, archive, "application/gzip"),
        )
        .await
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = bucket_agent_core::auth::ensure_authenticated_or_noninteractive(
