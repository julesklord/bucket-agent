...
//!
//! `MemoryBackend` is defined in `bucket-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `bucket-tools`.
/// Mirrors `bucket_agent_core::session::memory_log::TARGET`.
