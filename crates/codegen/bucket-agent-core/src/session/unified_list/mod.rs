...
/// Env lane (desktop `BUCKET_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`BUCKET_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source: "local".into(),
            model_id: Some("bucket-build".into()),
            num_messages: 7,
            ...
            source_workspace_dir: Some("/Users/me/bucket-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/bucket-src");
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = bucket_test_support::EnvGuard::set(
            "BUCKET_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = bucket_test_support::EnvGuard::unset("BUCKET_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = bucket_test_support::EnvGuard::set("BUCKET_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = bucket_test_support::EnvGuard::set("BUCKET_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::BUCKET_CHAT_MODE_ENV;
        let _chat_off = bucket_test_support::EnvGuard::unset(BUCKET_CHAT_MODE_ENV);
        let _desktop_off =
            bucket_test_support::EnvGuard::unset("BUCKET_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                bucket_test_support::EnvGuard::set("BUCKET_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = bucket_test_support::EnvGuard::set(BUCKET_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = bucket_test_support::EnvGuard::unset(BUCKET_CHAT_MODE_ENV);
        ...
        {
            let _on = bucket_test_support::EnvGuard::set(BUCKET_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
