...
/// Re-exported from `bucket-announcements` for backward compatibility.
pub use bucket_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `BUCKET_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("BUCKET_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid BUCKET_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `BUCKET_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
