...
///
/// Replaces `${BUCKET_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${BUCKET_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`bucket_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    bucket_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".bucket-plugin")).unwrap();

        // Write manifest in .bucket-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".bucket-plugin/plugin.json"),
