...
pub struct PermissionViewState {
    ...
    pub request: bucket_acp::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `bucket-agent-core`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell Bucket what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from bucket\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from bucket\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p bucket-tui --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/bucket-tui
