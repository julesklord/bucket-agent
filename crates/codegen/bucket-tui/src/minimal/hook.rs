...
//! render mode.
//!
//! `bucket-tui` cannot depend on `bucket-tui-minimal`: that crate reads
...
//!
//! The composition-root binary (`bucket-bin`) wires it up once at
//! startup by calling `bucket_tui_minimal::install()`. When nothing has
