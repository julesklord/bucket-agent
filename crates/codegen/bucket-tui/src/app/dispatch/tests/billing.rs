...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("superbucket_heavy")));
    assert!(is_max_tier(Some("SuperBucket Heavy")));
    assert!(is_max_tier(Some("SUPERBUCKET_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("superbucket")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("superbucket_heav")));
    assert!(!is_max_tier(Some("superbucket_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_superbucket() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("superbucket"), "got: {url}");
    assert!(url.contains("referrer=bucket-build"), "got: {url}");
}
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superbucket_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superbucket_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: bucket_telemetry::events::SuperBucketUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        (
            "Upgrade to SuperBucket",
            "For everyday coding and productivity tasks",
            Some(UPSELL_URL_UPGRADE),
        ),
        (
            "Upgrade to SuperBucket Heavy",
            "Get the most out of Bucket Build. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use bucket_agent_core::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option SuperBucket
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: bucket_telemetry::events::SuperBucketUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with SuperBucket.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to SuperBucket");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to SuperBucket Heavy");
...
/// `BUCKET_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
#[serial_test::serial(BUCKET_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "bucket-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("BUCKET_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("BUCKET_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("bucket-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("BUCKET_TEST_OPEN_URL_FILE", &url_file) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "bucket-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("BUCKET_TEST_OPEN_URL_FILE", &bad) };
