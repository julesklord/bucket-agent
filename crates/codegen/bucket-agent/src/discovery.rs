...
//!
//! Searches `.bucket/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.bucket/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.bucket/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".bucket/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.bucket/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.bucket/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), bucket.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, bucket_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.bucket/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.bucket/agents/` walking from `cwd` up to repo root
/// 2. `~/.bucket/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.bucket/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user bucket agents, `.claude`
/// compat agents, then bundled. `.bucket` dirs resolve from `bucket_home`
/// (BUCKET_HOME-aware) plus the legacy literal `~/.bucket` when BUCKET_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    bucket_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.bucket, included only when it differs from bucket_home
    // (i.e. BUCKET_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_bucket = home
        .map(|h| h.join(".bucket"))
        .filter(|legacy| bucket_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = bucket_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_bucket {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let bucket = bucket_config::user_bucket_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), bucket.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, bucket_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let bucket = bucket_config::user_bucket_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), bucket.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, bucket_home);
...
///
/// Project-level `.bucket/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let bucket = bucket_config::user_bucket_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), bucket.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, bucket_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, bucket_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Bucket
...
/// Load project agent definitions from every `.bucket/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectBucket,
            PluginScope::User => PluginOrigin::UserBucket,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_bucket_when_bucket_home_differs() {
        let home = Path::new("/home/u");
        let bucket = Path::new("/custom/buckethome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(bucket))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&bucket.join("agents")));
        assert!(paths.contains(&home.join(".bucket").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&bucket.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".bucket").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_bucket_home_is_dot_bucket() {
        let home = Path::new("/home/u");
        let bucket = home.join(".bucket");
        let count = user_agent_dirs(Some(home), Some(&bucket))
            .into_iter()
            .filter(|(p, _)| *p == bucket.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.bucket/agents when bucket_home == ~/.bucket"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_bucket_build() {
        let def = by_name("bucket-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "bucket-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".bucket").join("agents");
        let agents_dir_2 = inner_dir.join(".bucket").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".bucket").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".bucket")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".bucket")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".bucket").join("agents");
        let bundled_dir = home.join(".bucket").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".bucket")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".bucket")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".bucket").join("agents");
        let bundled_dir = home.join(".bucket").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "bucket-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "bucket-build.md",
            "bucket-build",
        ...
        let def = by_name_in_cwd("bucket-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "bucket-build");
        assert_eq!(def.description, "Custom bucket-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .bucket/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("bucket-build-orchestrator")
            .expect("from_str must resolve bucket-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::BucketBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "bucket-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("bucket-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find bucket-build-orchestrator");
        assert_eq!(def.name, "bucket-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.bucket/agents/) agent named "explore" should NOT shadow
