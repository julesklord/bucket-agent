...
//! `off`. Custom profiles via `~/.bucket/sandbox.toml` or `.bucket/sandbox.toml`.
...
/// Load sandbox config from `~/.bucket/sandbox.toml` and `.bucket/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.bucket/sandbox.toml
    let global_path = bucket_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.bucket/sandbox.toml (additive only)
    let project_path = workspace.join(".bucket").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&bucket_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".bucket").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.bucket/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.bucket/sandbox.toml or .bucket/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
