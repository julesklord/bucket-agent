...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> bucket_hub_sdk::ObservabilityBridge {
    bucket_hub_sdk::ObservabilityBridge::new(
        None,
        bucket_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> bucket_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> bucket_agent::Agent {
    use bucket_tools::implementations::bucket_build::update_goal::UpdateGoalTool;
    use bucket_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Bucket-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_bucket_build_agent_with_todo() -> bucket_agent::Agent {
    use bucket_tools::implementations::bucket_build::todo::TodoWriteTool;
    use bucket_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> bucket_agent::Agent {
    use bucket_tools::implementations::bucket_build::enter_plan_mode::EnterPlanModeTool;
    use bucket_tools::implementations::bucket_build::exit_plan_mode::ExitPlanModeTool;
    use bucket_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<bucket_tools::registry::types::ToolConfig>,
) -> bucket_agent::Agent {
    test_agent_from_config(
        bucket_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        bucket_agent::AgentDefinition::default_bucket_build(),
        std::sync::Arc::new(bucket_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: bucket_agent::AgentDefinition,
    backend: std::sync::Arc<dyn bucket_tools::computer::types::TerminalBackend>,
) -> bucket_agent::Agent {
    use bucket_tools::computer::local::LocalFs;
    use bucket_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("bucket-test"),
    ...
    bucket_agent::Agent::new(
        definition,
        bucket_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        bucket_agent::ReminderPolicy::default(),
        bucket_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<bucket_acp::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = bucket_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(bucket_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let actor = SessionActor {
        ...
        permissions: bucket_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
