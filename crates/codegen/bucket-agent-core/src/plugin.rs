...
use bucket_agent::plugins::git_install::{self, UpdateStatus};
use bucket_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use bucket_plugin_marketplace::git::{self, SourceCacheLease};
use bucket_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.bucket/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &bucket_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `bucket plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `bucket plugin install ./{name}`, or add a \
                     source with `bucket plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `bucket plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `bucket plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &bucket_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.bucket/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user bucket via user_bucket_home() (None when no home resolves) and
    // home separately, so removal still runs from $BUCKET_HOME when no home dir
    // exists, and never touches a cwd-relative .bucket.
    ...
    if let Some(ref bucket) = bucket {
        settings_candidates.push(bucket.join("settings.local.json"));
        settings_candidates.push(bucket.join("settings.json"));
    }
    ...
    if let Some(ref bucket) = bucket {
        known_candidates.push(bucket.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            ...
            "xAI Official (bucket-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "xAI Official (pin: sentry@bucket-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "xAI Official (bucket-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - xAI Official (bucket-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "bucket-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("bucket plugin install ./sentry"), "{msg}");
        assert!(msg.contains("bucket plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec!["xAI Official (pin: sentry@bucket-org/plugin-marketplace)".into()],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - xAI Official (pin: sentry@bucket-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("bucket plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/bucket-org/plugin-marketplace.git";
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "xAI Official (bucket-org/plugin-marketplace)".to_string(),
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> bucket_workspace::permission::resolution::MarketplaceAllowlist {
        bucket_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = bucket_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "bucket-org/plugin-marketplace")
