...
//!
//! Credentials are stored in `$BUCKET_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$BUCKET_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$BUCKET_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$BUCKET_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user bucket home (set $BUCKET_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$BUCKET_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(bucket_config::user_bucket_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$BUCKET_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("bucket-mcp-credentials-test");
