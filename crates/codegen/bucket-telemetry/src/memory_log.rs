...
//! file logger that writes to `~/.bucket/logs/memory.log`.
//!
//! ## When to use
//!
...
//! BUCKET_MEMORY_LOG=0 bucket                # disable even when enabled
//! tail -f ~/.bucket/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "BUCKET_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.bucket/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `BUCKET_MEMORY_LOG=0` to disable, `BUCKET_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || bucket_home().join("logs").join("memory.log");
