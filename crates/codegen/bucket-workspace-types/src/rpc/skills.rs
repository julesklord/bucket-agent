...
//! `bucket-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.bucket/skills
    Local,
    /// repo_root/.bucket/skills
    Repo,
    /// ~/.bucket/skills
    User,
    /// ~/.bucket/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.bucket/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the bucket-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.bucket/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.bucket/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.bucket/plugins/infra-plugin",
            "plugin_data": "/root/.bucket/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "bucket-4",
            "effort": "high",
            "user_invocable": true,
