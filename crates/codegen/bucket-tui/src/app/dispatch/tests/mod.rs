...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("bucket.com"),
            "Bucket".to_string(),
        ))],
        ...
        welcome_models_picker_state: crate::views::picker::PickerState::input_active(),
...
fn cta_entry(name: &str, status: &str) -> bucket_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: bucket_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `bucket_home()` (OnceLock-cached;
/// do not rely on setting `BUCKET_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = bucket_agent_core::util::bucket_home::bucket_home();
    let encoded = bucket_agent_core::util::bucket_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: bucket_tools::implementations::bucket_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    bucket_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
fn push_synthetic_permission(
    ...
    let request = bucket_acp::AcpArgs {
