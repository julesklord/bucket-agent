...
/// Display label for a section header, e.g. `"Managed by bucket.com (3)"`.
pub fn section_label(section: &McpSectionId, count: usize) -> String {
    match section {
        McpSectionId::Managed => format!("Managed by bucket.com ({count})"),
        McpSectionId::Plugin(name) => format!("Plugin: {name} ({count})"),
        McpSectionId::Local => format!("Local ({count})"),
    }
}

/// Base bucket.com connectors URL (no team). Prefer [`managed_connectors_url`] when opening.
pub const MANAGED_SECTION_CONNECTORS_URL: &str = "https://bucket.com/connectors";
...
///
/// Priority: `bucket_com_` prefix or managed wire source → Managed; else plugin
...
pub fn section_for(server: &McpServerInfo) -> McpSectionId {
    if server.name.starts_with("bucket_com_") || server.wire_source == McpWireSource::Managed {
        McpSectionId::Managed
...
pub fn is_removable(server: &McpServerInfo) -> bool {
    server.wire_source == McpWireSource::Local && !server.name.starts_with("bucket_com_")
}
...
mod tests {
    ...
    fn section_description_lines_managed_includes_connectors_url() {
        ...
        assert_eq!(lines[1], "[bucket.com/connectors]");
        assert!(
        ...
        assert_eq!(with_team[1], "[bucket.com/connectors?teamId=team-1]");
    }
    ...
    #[test]
    fn section_for_bucket_com_with_plugin_label_is_managed() {
    ...
    #[test]
    fn is_removable_rejects_bucket_com_prefix() {
        let server = server_from_wire("bucket_com_slack", Some("local"), None);
        assert!(!is_removable(&server));
    }
    ...
    fn convert_list_response_classifies_managed_gateway_only_for_gateway_rows() {
        ...
        let legacy_managed = server_from_wire("bucket_com_slack", Some("managed"), None);
