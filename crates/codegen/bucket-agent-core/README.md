...
- **Using Bucket**
  ...
  - [SSH Passthrough](#ssh-passthrough-bucket-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`bucket inspect`)](#introspection)
...
- [Building with Bucket](#building-with-bucket) — headless API, ACP SDK integration
...
Credentials are stored in `~/.bucket/auth.json` and persist across sessions. Tokens expire after 7 days; Bucket will prompt you to re-authenticate when needed.
...
export BUCKET_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export BUCKET_CLI_CHAT_PROXY_BASE_URL="https://bucket-proxy.acme.com/v1"
```

**3. Run `bucket`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.bucket/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Bucket is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
1. Bucket runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Bucket and saved to `~/.bucket/auth.json` as the access token
5. exit 0 → success; exit non-zero → Bucket falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Bucket reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Bucket to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Bucket when to proactively refresh. Without it, Bucket assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Bucket captures this)
...
export BUCKET_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export BUCKET_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Bucket cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with bucket.com". In headless mode (`bucket -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=bucket-cli")
...
When Bucket needs to refresh an expired token, it re-runs your binary with `BUCKET_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:
...
`BUCKET_AUTH_EXPIRED` is optional — if your binary ignores it, Bucket still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Bucket supports automatic credential refresh for external auth providers and OIDC. When Bucket detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Bucket handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Bucket re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Bucket re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Bucket silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Bucket refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export BUCKET_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `bucket login` before starting — Bucket runs your binary automatically on first launch. You _can_ run `bucket login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Bucket tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
bucket --debug-file /tmp/bucket-auth.log -p "hello"
tail -f /tmp/bucket-auth.log
```
...
If you've authenticated with `bucket login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the bucket CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.bucket.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.bucket/auth.json)" \
  -H "X-BUCKET-Token-Auth: bucket-cli" \
  -H "x-bucket-model-override: bucket-build" \
  -d '{
    "model": "bucket-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.bucket/auth.json` (set by `bucket login`)                                                                                                                              |
| `X-BUCKET-Token-Auth: bucket-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-bucket-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `bucket-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `bucket-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `bucket login` to refresh.
...
bucket --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
bucket --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
bucket --always-approve --prompt "Format all files"
```
...
Run Bucket non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `bucket-build`)               |
...
bucket -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
bucket -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
bucket -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
bucket -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
bucket -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
bucket -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
bucket -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
bucket -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
bucket -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
bucket --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
bucket -p "What does this project do?"

# Use a specific model
bucket -p "Optimize this function" -m bucket-build

# Get JSON output for parsing
bucket -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
bucket -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
bucket -p "Remember: the secret number is 42" --output-format json
bucket -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
bucket -p "Continue where we left off" -c

# Run in a different directory
bucket -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
bucket -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
bucket -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
bucket -p "Generate a README" > README.md

# Parse JSON output with jq
bucket -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
bucket -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | bucket -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  bucket -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
bucket -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Bucket as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `bucket-build`)                           |
...
```bash
bucket agent headless --bucket-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`bucket ssh`)

Use `bucket ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
bucket ssh user@host

# With SSH flags
bucket ssh -t user@host
bucket ssh -L 8080:localhost:8080 user@host

# With remote command
bucket ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `bucket ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Bucket can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `bucket -p` and parses JSON output.
...
class BucketChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["bucket", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="bucket-build", stream=False):
...
class BucketChat {
  ...
  async create(
    ...
    { model = "bucket-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("bucket", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await BucketACPChat(cwd=".").init()
...
class BucketACPChat {
  ...
  async init() {
    this.proc = spawn("bucket", ["agent", "stdio"]);
...
// Usage
const client = await new BucketACPChat(".").init();
...
Bucket implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Bucket reads configuration from `~/.bucket/config.toml`. If the file doesn't exist, Bucket uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "bucket-build"           # model used for new sessions
web_search = "bucket-4.20-multi-agent"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: BUCKET_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: BUCKET_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: BUCKET_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: BUCKET_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: BUCKET_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: BUCKET_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: BUCKET_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: BUCKET_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `BUCKET_TELEMETRY_BUILD_EVENTS_URL`, `BUCKET_TELEMETRY_BUILD_EVENTS_API_KEY`, and `BUCKET_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Bucket can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Bucket language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Bucket can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Bucket can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Bucket looks for server definitions in:

- project config: `<repo>/.bucket/lsp.json`
- user config: `~/.bucket/lsp.json`
...
- LSP tools are enabled (`BUCKET_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
BUCKET_LSP_TOOLS=1 bucket
```
...
If LSP tools are enabled but no usable server config is found, Bucket emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Bucket does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `BUCKET_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
name = "Bucket Build Latest (Proxy)"
...
With this config, `bucket` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Bucket reads these files and appends their contents to the system prompt.

Bucket scans for agent rules in this order:

1. `~/.bucket/` (global rules)
...
Within each directory, Bucket checks for these filenames:
...
Skills are reusable prompt packages that extend Bucket with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Bucket discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.bucket/skills/`           | Local | Highest  |
| `<repo_root>/.bucket/skills/` | Repo  | Medium   |
| `~/.bucket/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.bucket/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Bucket decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Bucket invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Bucket automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Bucket discovers agent definitions from `.bucket/agents/` (project), `~/.bucket/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `BUCKET_AGENT` env var
4. Default `bucket-build` agent
...
```bash
bucket --agent-profile ./my-agent.md
# or
export BUCKET_AGENT="my-custom-agent"
```
...
```bash
export BUCKET_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "bucket-build"              # route explore to a lighter model
```
...
model = "bucket-build"
prompt_file = ".bucket/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".bucket/personas/concise.md"  # or load from file
```

Both are also discovered from `.bucket/roles/*.toml` and `.bucket/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Bucket with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.bucket/plugins/`            | Project |
| `~/.bucket/plugins/`          | User    |
...
Bucket discovers hooks from `.bucket/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `BUCKET_API_KEY`. If neither `api_key` nor `env_key` is set, Bucket falls back to the global `BUCKET_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Bucket falls back to built-in defaults for known models.
...
[model.bucket-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Bucket starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `BUCKET_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `bucket models` output). You also need a `[model.*]` entry so Bucket knows
...
>
> [model.my-custom-model]              # 2. tell Bucket how to reach it
...
Point Bucket at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `BUCKET_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `BUCKET_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `BUCKET_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |
...
Bucket fetches the model list from `{BUCKET_MODELS_BASE_URL}/models` on startup and sends inference requests to `BUCKET_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `BUCKET_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.bucket/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Bucket uses API key auth (`Authorization: Bearer`) instead of session auth. `bucket login` is not required — only the API key.

---

## MCP Servers

Extend Bucket's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.bucket/config.toml`:
...
MCP servers can also be configured per-project in `.bucket/config.toml`. Bucket walks from the current directory up to the git repo root, loading `.bucket/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.bucket/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.bucket/config.toml` | This repository   | ↑        |
| `<cwd>/.bucket/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.bucket/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.bucket/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.bucket/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.bucket/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `BUCKET_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Bucket remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.bucket/memory/`:
- **Global** (`~/.bucket/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.bucket/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.bucket/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `bucket-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Bucket saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.bucket/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Bucket searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.bucket/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.bucket/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Bucket emits the `bucket-shell-memory_injection`
...
Bucket can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.bucket/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.bucket/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.bucket/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.bucket/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.bucket/sandbox.toml` (global) or `.bucket/sandbox.toml` (per-project):
...
```bash
bucket --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire bucket process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Bucket logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.bucket/sandbox-events.jsonl`
...
Use `bucket inspect` to see everything Bucket discovers in the current directory:

```bash
bucket inspect          # human-readable output
bucket inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.bucket/skills/`, `~/.bucket/skills/`, plugins, and config paths
...
Bucket automatically discovers configuration from Claude Code directories alongside native `.bucket/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Bucket uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.bucket/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Bucket at runtime.

---

## Built-in Tools

Bucket includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `BUCKET_WEB_FETCH=1`. 
...
Bucket automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.bucket/sessions/`, organized by URL-encoded working directory:

```
~/.bucket/sessions/<encoded-cwd>/<session-id>/
...
bucket -p "Remember: X=42" -s my-session
bucket -p "What is X?" -s my-session

# Resume existing session (errors if not found)
bucket -p "Continue" -r my-session

# Continue most recent session in current directory
bucket -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
bucket -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.bucket/config.toml` | Configuration file                                  |
| `~/.bucket/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.bucket/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.bucket/memory/`     | Cross-session memory files and index                |
| `~/.bucket/skills/`     | User-scoped skill definitions                       |
| `~/.bucket/plugins/`    | User-scoped plugins                                 |
| `~/.bucket/agents/`     | User-scoped agent definitions                       |
| `.bucket/config.toml`   | Project-scoped config (MCP servers)                 |
...
| `.bucket/hooks/`        | Project-scoped hooks                                |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `BUCKET_API_KEY`         | API key from [console.x.ai](https://console.x.ai). Used for custom endpoint auth and API key login      |
| `BUCKET_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.bucket.com/v1`)                          |
| `BUCKET_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `BUCKET_MODELS_LIST_URL`          | Override the model list URL if it differs from `{BUCKET_MODELS_BASE_URL}/models`                                              |
| `BUCKET_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `BUCKET_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `BUCKET_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `BUCKET_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `BUCKET_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `BUCKET_HOME`                     | Override config directory (default: `~/.bucket`)                                                           |
| `BUCKET_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `BUCKET_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `BUCKET_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `BUCKET_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `BUCKET_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `BUCKET_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `BUCKET_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `BUCKET_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `BUCKET_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `BUCKET_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.bucket/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `BUCKET_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `bucket` commands and flags.
...
bucket completions bash > ~/.local/share/bash-completion/completions/bucket
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Bucket-managed location):

```bash
mkdir -p ~/.bucket/completions/bash
bucket completions bash > ~/.bucket/completions/bash/bucket.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.bucket/completions/bash/bucket.bash" ]] && source "$HOME/.bucket/completions/bash/bucket.bash"
```
...
bucket completions zsh > ~/.zsh/completions/_bucket
```
...
```bash
mkdir -p ~/.bucket/completions/zsh
bucket completions zsh > ~/.bucket/completions/zsh/_bucket
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.bucket/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `bucket` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `bucket --debug` or `BUCKET_LOG_FILE` instead:

```bash
# Per-session debug log (~/.bucket/debug/<sessionId>.txt)
bucket --debug

# Log to a custom path
BUCKET_LOG_FILE=/tmp/bucket-debug.log bucket

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.bucket/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `BUCKET_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
BUCKET_LOG_FILE=/tmp/bucket-debug.log RUST_LOG="info,bucket_agent_core::auth=debug" bucket
```
...
cat ~/.bucket/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.bucket/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.bucket/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
