...
use bucket_tools::registry::types::FinalizedToolset;
use bucket_tools::types::resources::Terminal;
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "bucket_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "bucket_workspace_rpc_duration_seconds",
...
) -> Vec<bucket_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use bucket_tools::computer::types::TaskKind;
    use bucket_tools::types::resources::Terminal;
    use bucket_tools::types::tool::ToolKind;
    use bucket_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &bucket_tools::registry::types::FinalizedToolset,
) -> Vec<bucket_workspace_types::rpc::workspace::TodoSummaryWire> {
    use bucket_tools::implementations::bucket_build::todo::{TodoState, TodoStatus};
    use bucket_tools::types::resources::State;
    use bucket_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use bucket_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use bucket_workspace_types::rpc::search::FuzzyStatusReq;
        use bucket_workspace_types::rpc::skills::DiscoverPluginsReq;
        use bucket_workspace_types::rpc::workspace::{
        ...
        use bucket_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<bucket_tool_runtime::SessionContext>();
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use bucket_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<bucket_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use bucket_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use bucket_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                ...
                Some(bucket_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(bucket_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            bucket_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "bucket_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use bucket_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
