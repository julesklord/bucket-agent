...
// ── Bucket state directory ────────────────────────────────────────────────────

/// Bucket state directory — always writable (`$BUCKET_HOME` or `~/.bucket`).
pub(crate) fn bucket_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), bucket_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.bucket + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
#[cfg(all(feature = "enforce", unix))]
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![bucket_home()];
    paths.extend(temp_writable_paths());
    paths
}
