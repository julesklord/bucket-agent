...
/// Format: https://bucket.com/build/{sessionId}
pub fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("BUCKET_CODE_WEB_URL").unwrap_or_else(|_| "https://bucket.com".to_string());
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://bucket.com/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://bucket.com/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
