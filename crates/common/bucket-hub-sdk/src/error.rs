...
//!
//! Wire-level [`bucket_tool_protocol::ToolErrorWire`] variants and JSON-RPC
...
pub enum ClientError {
    ...
    /// [`bucket_tool_runtime::ToolCallContext::default`], which does so)
...
mod tests {
    ...
    fn sdk_reexported_recognizer_matches_decoded_error() {
        ...
        let err = bucket_hub_core::error_from_envelope(workspace_gone_envelope());
    ...
    fn sdk_reexported_recognizer_rejects_unrelated_custom_error() {
        ...
        let err = bucket_hub_core::error_from_envelope(env);
        assert!(!crate::is_workspace_unavailable(&err));
    }
}
