...
/// auth.json scope key for plain API key auth (desktop login, `bucket login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from bucket-desktop login or `bucket login --api-key`)
    ApiKey,
}
...
pub struct BucketAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_bucket_code_access: Option<bool>,
...
impl std::fmt::Debug for BucketAuth {
...
impl BucketAuth {
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &BucketAuth) {
...
#[cfg(test)]
impl BucketAuth {
    /// Returns a `BucketAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// BucketAuth { key: "my-key".into(), ..BucketAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, BucketAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `bucket login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<BucketAuth> {
...
/// Early-invalidation buffer. Override with `BUCKET_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("BUCKET_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &BucketAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &BucketAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> BucketAuth {
        BucketAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperBucketPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperBucketPro"));
    }
