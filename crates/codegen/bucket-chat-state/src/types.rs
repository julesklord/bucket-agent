...
pub enum AuthType {
    /// From AuthManager (bucket login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, BUCKET_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `bucket_sampling_types::SamplingConfig` (which is secret-free).
