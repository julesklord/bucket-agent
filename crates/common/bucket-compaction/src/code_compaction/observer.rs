//! Observability seam for the full-replace (bucket-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (bucket-build:
...
//!
//! Emission points are part of the behavior contract: the bucket-build observer
