...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<bucket_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    pub context_state: Option<bucket_agent_core::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<bucket_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(bucket_telemetry::events::CreditLimitChoice::PayAsYouGo);
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::events::SuperBucketUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use bucket_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        bucket_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = bucket_tools::util::bucket_home::bucket_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &bucket_tools::implementations::bucket_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
