//! Detects whether bucket is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` bucket is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `bucket-tui-pty-harness/src/pty.rs`
/// (test-env hygiene).
