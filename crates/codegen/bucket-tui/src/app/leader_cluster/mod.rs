...
//!
//! Env sandboxing follows this crate's `serial(BUCKET_HOME)` idiom; note
//! `bucket_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_bucket_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `BUCKET_API_KEY`,
//! `BUCKET_LEADER_SOCKET`, `BUCKET_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `bucket_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p bucket-tui --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The bucket home the agent actually persisted under: `bucket_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_bucket_home() -> PathBuf {
    bucket_config::bucket_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same BUCKET_HOME (two agents on one updates.jsonl is the
    ...
    _bucket_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(BUCKET_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = bucket_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("BUCKET_HOME", bucket_home.path()),
            crate::test_util::EnvVarGuard::set("BUCKET_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("BUCKET_BUCKET_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("BUCKET_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("BUCKET_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("BUCKET_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("BUCKET_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (bucket_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                bucket_agent_core::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(bucket_agent_core::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // bucket-agent-core/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: bucket-agent-core/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same BUCKET_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `BUCKET_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    bucket_ws_url: String::new(),
                    bucket_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
