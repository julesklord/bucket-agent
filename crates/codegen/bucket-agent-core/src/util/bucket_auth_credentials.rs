...
use std::sync::Arc;
/// Credentials for authenticating with bucket backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-BUCKET-Token-Auth: bucket-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct BucketAuthCredentials {
...
impl std::fmt::Debug for BucketAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BucketAuthCredentials")
            .field(
                "user_token",
...
impl BucketAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your BUCKET_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `bucket login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> BucketAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> BucketAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-BUCKET-Token-Auth"),
                    obfstr::obfstr!("bucket-cli"),
                )
        } else {
            builder
        };
...
impl bucket_auth::HttpAuth for BucketAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        BucketAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = BucketAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = BucketAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = BucketAuthCredentials::new(Some("static-token".into()));
