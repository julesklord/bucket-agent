//! Headless single-turn mode (`bucket -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_bucket_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use bucket_acp::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use bucket_agent_core::agent::auth_method::AuthMethodKind;
use bucket_agent_core::agent::config::Config as AgentConfig;
use bucket_agent_core::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use bucket_agent_core::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use bucket_agent_core::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use bucket_agent_core::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_bucket_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<bucket_workspace::permission::types::PermissionRule>> {
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<bucket_agent_core::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = bucket_agent_core::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut bucket_agent_core::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    bucket_agent_core::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `bucket login` to authenticate \
         (or `bucket login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         bucket login --device-code\n\n\
         Alternatively, set the BUCKET_API_KEY environment variable \
         or run `bucket login` on a machine with a browser."
            .to_string()
    }
}
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &bucket_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'bucket models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    bucket_agent_core::http::set_process_client_mode_headless();
    ...
    // hub_url lives on `bucket workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = bucket_agent_core::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        bucket_agent_core::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_bucket_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("BUCKET_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = bucket_agent_core::active_sessions::register(
            bucket_agent_core::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(bucket_agent_core::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(bucket_agent_core::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.bucket can't hang exit.
        let _ = bucket_agent_core::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = bucket_agent_core::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &bucket_acp::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
...
mod tests {
    ...
    use super::*;
    use bucket_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            bucket_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> bucket_acp::AcpArgsBox<acp::ExtNotification> {
