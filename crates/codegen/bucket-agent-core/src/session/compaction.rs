...
fn prefire_lead_percent() -> u64 {
    std::env::var("BUCKET_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("BUCKET_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        bucket_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("BUCKET_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG BUCKET_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(bucket_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`bucket-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                bucket_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            bucket_telemetry::session_ctx::log_event(
                bucket_telemetry::events::AutoCompactSuppressed {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    bucket_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    bucket_chat_state::estimate_conversation_tokens(&full_conv),
                );
    ...
    async fn run_compact_inner(
        ...
        trigger: bucket_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            bucket_telemetry::events::CompactionTrigger::Manual => "manual",
            bucket_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = bucket_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            bucket_hooks::event::HookEventName::PreCompact,
            bucket_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            bucket_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            bucket_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            bucket_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<bucket_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            bucket_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<bucket_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
        ...
        let estimated_input_tokens =
            bucket_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, bucket_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = bucket_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match bucket_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(bucket_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(bucket_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(bucket_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            bucket_telemetry::session_ctx::log_event(
                                bucket_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = bucket_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    bucket_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                bucket_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                bucket_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use bucket_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn bucket_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn bucket_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : bucket_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        bucket_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            bucket_hooks::event::HookEventName::PostCompact,
            bucket_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if bucket_token_estimation::exceeds_threshold(
            ...
            let percentage = bucket_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &bucket_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = bucket_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        bucket_telemetry::session_ctx::log_event(bucket_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                bucket_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<bucket_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<bucket_acp::AcpClientMessage>,
        ...
        let hunk_tracker_handle = bucket_hunk_tracker::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            bucket_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = bucket_chat_state::ChatStateActor::spawn(
            vec![],
            bucket_sampling_types::SamplingConfig {
            ...
            Box::new(bucket_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: bucket_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: bucket_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: bucket_workspace::session::git::VcsKind::Git,
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<bucket_acp::AcpClientMessage>();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("Bucket Build usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> bucket_sampler::SamplingErrorInfo {
        bucket_sampler::SamplingErrorInfo {
            kind: bucket_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
        ...
        local
            .run_until(async {
                let (gateway_tx, _) = mpsc::unbounded_channel::<bucket_acp::AcpClientMessage>();
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = bucket_sampler::SamplingErrorInfo {
                    kind: bucket_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = bucket_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = bucket_chat_state::CompactionMode::Summary;
