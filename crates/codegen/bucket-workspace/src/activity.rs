...
/// `BUCKET_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `BUCKET_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `BUCKET_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: bucket_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
