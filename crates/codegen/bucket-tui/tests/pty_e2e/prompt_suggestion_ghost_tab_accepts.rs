...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`bucket-build-0.1`, the shell's built-in default)
    ...
    let content = ContentController::start_with_models(vec![
        ...
        MockModel::new("bucket-build-0.1"),
    ])
    .await
    ...
    env.retain(|(k, _)| k != "BUCKET_PROMPT_SUGGESTIONS");
    env.push(("BUCKET_PROMPT_SUGGESTIONS".into(), "true".into()));
