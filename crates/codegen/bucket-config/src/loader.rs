...
use crate::paths::{system_config_dir, user_bucket_home};
...
/// bucket config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_bucket_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_bucket_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.bucket/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.bucket` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/bucket`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `BUCKET_CAMPAIGNS_OVERRIDE` env layer.
...
/// `BUCKET_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("BUCKET_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`bucket_agent_core::util::config::load_effective_config`) when remote campaigns
/// or `BUCKET_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `BUCKET_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match bucket_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("bucket-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("bucket-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("bad.toml");
        // Duplicate key: the message names the key; the secret-bearing source line is only in Display.
        std::fs::write(
            &path,
            "api_key = \"bucket-secretmustnotleak\"\napi_key = \"bucket-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("bucket-secretmustnotleak"),
    ...
    /// `BUCKET_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `BUCKET_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("BUCKET_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads BUCKET_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("BUCKET_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("BUCKET_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("BUCKET_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("BUCKET_CAMPAIGNS") },
        }
    }
}
