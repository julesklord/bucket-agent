...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("BUCKET_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        // per-hook extra_env (plugin vars), or Bucket's own process env. The
        ...
        #[cfg(not(unix))]
        {
            let inv = bucket_config::shell::shell_command_argv(&command_str);
    ...
    // `bucket_tools::util::detach_command`: Unix uses the same setsid /
    ...
    // plugin) can spoof `BUCKET_HOOK_EVENT`, `BUCKET_HOOK_NAME`, `BUCKET_SESSION_ID`,
    // `BUCKET_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("BUCKET_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("BUCKET_HOOK_NAME", &spec.name)
        .env("BUCKET_SESSION_ID", ctx.session_id)
        .env("BUCKET_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `BUCKET_WORKSPACE_ROOT`; native `.bucket` hooks should use
...
///   hooks),
/// * the Bucket process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.bucket/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
            Some(std::path::PathBuf::from(
                "/project/.bucket/hooks/bin/check.sh"
            ))
        );
    }
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${BUCKET_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${BUCKET_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.bucket-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".bucket-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.bucket-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.bucket-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.bucket-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
