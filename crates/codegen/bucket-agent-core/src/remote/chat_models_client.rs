...
pub enum ChatModelsError {
    #[error("no bucket.com credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("BUCKET_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("BUCKET_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("BUCKET_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| BUCKET_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid bucket.com bearer — deliberately NOT `is_xai_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-BUCKET-Token-Auth",
                self.auth.bucket_com_config().token_header.clone(),
            )
        ...
        let builder = bucket_file_utils::trace_context::inject_trace_context_into_request(builder);
