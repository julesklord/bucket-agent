...
//! handshake against rmcp 2.1 in `bucket-mcp/tests/repro_sse_flood.rs`): a
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<bucket_acp::AcpClientMessage>,
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use bucket_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `bucket_com_*` tools enter entry-B.
