...
fn local_bucket_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("bucket-tui{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_bucket_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "bucket-tui", "--bin", "bucket-tui"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build bucket-tui: {e}"));

    assert!(
        output.status.success(),
        "failed to build bucket-tui for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "bucket-tui build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve bucket binary: `BUCKET_BINARY` env (CI) or a locally built `bucket-tui` binary.
pub fn bucket_binary() -> PathBuf {
    if let Ok(path) = std::env::var("BUCKET_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "BUCKET_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_bucket-tui") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_bucket_binary_path();
    ensure_local_bucket_binary(&binary);
    binary
}
...
/// Point bucket at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox bucket on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit BUCKET_HOME every spawned child shares the real
        // `%USERPROFILE%\.bucket` — test 1's models_cache.json (which embeds
        ...
        .env("BUCKET_HOME", home.join(".bucket"))
        .env("BUCKET_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("BUCKET_BUCKET_API_BASE_URL", mock_url)
        .env("BUCKET_API_KEY", "test-key-for-ci")
        .env("BUCKET_TELEMETRY_ENABLED", "false")
        .env("BUCKET_FEEDBACK_ENABLED", "false")
        .env("BUCKET_TRACE_UPLOAD", "false")
        .env("BUCKET_INSTRUMENTATION", "disabled")
        ...
        .env("BUCKET_DISABLE_AUTOUPDATER", "1");
}
