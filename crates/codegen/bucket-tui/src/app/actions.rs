...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: bucket_agent_core::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open bucket.com in the browser for SuperBucket subscription upsell.
    OpenSuperbucketUrl,
    ...
    /// Open bucket.com managed connectors, appending session teamId when set.
    ...
    OpenTranscriptPager,
    /// Minimal mode (`bucket --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: bucket_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(bucket_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(bucket_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(bucket_hooks_plugins_types::MarketplaceAction),
    ...
    AnnouncementsOpenCta(bucket_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"bucketnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `bucket dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.bucket/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: bucket_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.bucket/config.toml`.
    ...
    /// Persist a typed setting to `~/.bucket/config.toml`. On failure,
    ...
    /// registry (`~/.bucket/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`bucket-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<bucket_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<bucket_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<bucket_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<bucket_agent_core::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<bucket_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<bucket_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<bucket_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<bucket_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<bucket_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<bucket_agent_core::session::SessionInfoResponse>,
        text: String,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<bucket_agent_core::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
