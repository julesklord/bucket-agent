//! Integration tests for bucket-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use bucket_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use bucket_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!bucket_sandbox::should_restrict_child_network());
}
