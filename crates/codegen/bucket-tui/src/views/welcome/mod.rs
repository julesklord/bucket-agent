...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a bucket_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Bucket Build** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Bucket Build Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Bucket Build Beta**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = bucket_updater::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", bucket_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a bucket_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a bucket_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [bucket_agent_core::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a bucket_agent_core::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("bucket.com");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Bucket Agent is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Bucket Build
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Bucket Agent may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &bucket_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let menu_items: &[(&str, &str)] = if !p.has_access {
        ...
        items.push(("ctrl+p", "Configure Provider"));
        items.push(("ctrl+m", "Configure Model"));
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("SuperBucket subscription required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use bucket_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = bucket_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.x.ai/oauth2/authorize?client_id=bucket";
    ...
    fn long_ann() -> bucket_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = bucket_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
