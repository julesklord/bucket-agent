...
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("BUCKET_CODE_WEB_URL").unwrap_or_else(|_| BUCKET_CODE_WEB_URL.to_string());
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::bucket_auth_credentials::BucketAuthCredentials::new(
    ...
    bucket_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("BUCKET_CODE_BACKEND_URL")
                .unwrap_or_else(|_| BUCKET_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&BucketAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn bucket_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<BucketAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-BUCKET-Token-Auth so nginx auth subrequest routes to authenticate_bucket_cli_token.
    /// See: crates/codegen/bucket-agent-core/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-bucket-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            "x-bucket-client-version",
            HeaderValue::from_static(bucket_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = bucket_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `bucket_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-bucket-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(bucket_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-bucket-agent-id", agent_id)
        .header("x-bucket-client-version", bucket_version::VERSION)
        .header(
            "x-bucket-client-identifier",
            crate::http::process_client_identifier(),
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_xai_api_key_env()
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set BUCKET_API_KEY.".into(),
                    )
                })?;
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-BUCKET-Token-Auth", "bucket-cli")
                .header("x-userid", &auth.user_id)
                .header("x-bucket-client-version", bucket_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| bucket_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(bucket_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-bucket-agent-id"),
                            client_identifier: header_str(&headers, "x-bucket-client-identifier"),
                            client_version: header_str(&headers, "x-bucket-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-bucket-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-bucket-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-bucket-client-version"
        );
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("bucket-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        let value = serde_json::json!(
            { "id" : "bucket-3", "object" : "model", "owned_by" : "xai", "context_window" :
            131072 }
        );
        let result = parse_remote_model_value(&value, "https://api.x.ai/v1").unwrap();
        assert_eq!(result.model, "bucket-3");
        assert_eq!(result.base_url, "https://api.x.ai/v1");
        assert_eq!(result.name.as_deref(), Some("bucket-3"));
    }
    #[test]
    ...
    fn parse_reads_reasoning_effort_fields() {
        ...
        let value = serde_json::json!(
            { "model" : "bucket-4.5", "context_window" : 1_000_000,
        ...
        let value = serde_json::json!(
            { "model" : "bucket-4.5", "contextWindow" : 1_000_000,
    ...
    fn parse_reads_reasoning_efforts_list() {
        use bucket_sampling_types::ReasoningEffort;
        let value = serde_json::json!(
            { "model" : "bucket-4.5", "context_window" : 1_000_000, "reasoning_efforts" :
    ...
    fn parse_remote_model_value_no_laziness_detector_block_yields_default() {
        let value = serde_json::json!(
            { "model" : "bucket-4", "context_window" : 256_000, }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert_eq!(
    ...
    fn parse_remote_model_value_parses_camelcase_key() {
        let value = serde_json::json!(
            { "model" : "bucket-4", "context_window" : 256_000, "lazinessDetector" : {
    ...
    fn parse_remote_model_value_parses_snake_case_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "bucket-4", "context_window" : 256_000, "laziness_detector" : {
    ...
    fn parse_remote_model_value_parses_meta_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "bucket-4", "context_window" : 256_000, "_meta" : {
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "bucket-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "bucket-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "bucket-build", "context_window" : 256_000, "_meta" : {
    ...
    fn list_url_defaults_to_proxy_models() {
        ...
        assert_eq!(
            ...
            "https://proxy.bucket.com/v1/models"
        );
    }
    #[test]
    ...
    /// REGRESSION: `bucket setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("BUCKET_DEPLOYMENT_KEY", "bucket-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("BUCKET_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
