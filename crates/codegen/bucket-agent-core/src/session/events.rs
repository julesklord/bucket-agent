...
//! `bucket-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use bucket_file_utils::events::tracker::EventTracker;
pub(crate) use bucket_file_utils::events::types::{
...
) -> Option<bucket_sampling_types::PriorTurnInterrupt> {
    use bucket_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run bucket-build flavor. Distinct from
...
/// (`bucket-file-utils`). The two enums are intentional mirrors and live in
