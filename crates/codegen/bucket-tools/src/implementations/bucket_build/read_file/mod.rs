...
mod versions;
use crate::types::schema::BucketIntegerSchema;
...
crate::register_resource!("bucket_build", "ReadFile", ReadFileParams);
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<bucket_tool_protocol::ToolCapabilities> =
...
async fn handle_pptx(
    file_bytes: Vec<u8>,
    path: &std::path::Path,
) -> Result<ReadFileOutput, bucket_tool_runtime::ToolError> {
...
/// an empty window (harness-aligned; not a Bucket-line clamp).
...
impl bucket_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> bucket_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield bucket_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
        Box::pin(async_stream::stream! {
            ...
            bucket_tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            bucket_tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            bucket_tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
            bucket_tool_runtime::ToolStreamItem::Terminal(Err(e)), }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &bucket_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), bucket_tool_runtime::ToolError> {
        let resources = crate::types::tool_metadata::shared_resources(ctx)?;
        let cwd_override = ctx
            .extensions
            .get::<bucket_tool_runtime::Cwd>()
            .map(|c| c.0.clone());
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = bucket_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = bucket_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".bucket/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".bucket/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        bucket_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = bucket_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// phantom field only (start past any `split_inclusive` line), so Bucket
