...
/// `x-bucket-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p bucket-agent-core --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = bucket_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let bucket_home = home.path().join(".bucket");
    std::fs::create_dir_all(&bucket_home).expect("create .bucket home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(bucket_test_support::bucket_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    bucket_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("BUCKET_HOME", bucket_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("BUCKET_LEADER_SOCKET");

    let result = bucket_test_support::run_headless_with_cmd(cmd).await;
    bucket_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-bucket-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-bucket-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-bucket-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-bucket-doom-loop-check"),
            None::<&str>,
