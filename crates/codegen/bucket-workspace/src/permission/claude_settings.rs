...
pub struct ClaudeSettings {
    ...
    /// bucket-only root legacy). Recognized values: `acceptEdits`,
...
///     under `permissions.*`. Root-level keys are **bucket legacy only** (not in
...
pub fn load_claude_settings(path: &Path) -> Option<ClaudeSettings> {
    ...
    // Canonical vendor settings store these under `permissions`; root is bucket legacy only.
...
///
/// Root `defaultMode` is bucket-only back-compat for older tests / hand-written
...
pub(crate) fn extract_default_mode(value: &serde_json::Value, path: &Path) -> Option<String> {
    ...
    // Nested key absent — optional bucket legacy root.
        ...
        Some(dm) => match dm.as_str() {
            ...
            None => {
                warn!(
                    path = %path.display(),
                    actual_type = %dm.type_of(),
                    "root defaultMode (bucket legacy): expected string, ignoring"
                );
                None
            }
        },
...
fn extract_additional_directories(value: &serde_json::Value, path: &Path) -> Option<Vec<String>> {
    ...
    // when it is present but the wrong type, do *not* resurrect the bucket-legacy
...
// permissions), consider extracting to a shared helper (e.g., in bucket-hooks
...
// =============================================================================
//
// `bucket-agent-core::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in bucket-agent-core (inaccessible from here at runtime). They also
    // set this env var so the workspace-resident gate honours the override
    // without a cross-crate dependency.
    if std::env::var("_BUCKET_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = bucket_config::user_bucket_home().map(|g| g.join("config.toml")) else {
        return false;
    };
