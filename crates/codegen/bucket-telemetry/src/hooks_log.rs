...
//! file logger that writes to `~/.bucket/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `bucket_hooks` or `bucket_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! BUCKET_HOOKS_LOG=1 bucket              # enable, write to ~/.bucket/logs/hooks.log
//! BUCKET_HOOKS_LOG=/tmp/h.log bucket     # write to custom path
//! BUCKET_HOOKS_LOG=0 bucket              # explicitly disable
//! tail -f ~/.bucket/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "BUCKET_HOOKS_LOG";
...
///
/// Writes to `~/.bucket/logs/hooks.log` (or custom path via `BUCKET_HOOKS_LOG`).
/// Filters to hooks (`bucket_hooks`) and plugins (`bucket_agent::plugins`) targets.
/// Set `BUCKET_HOOKS_LOG=0` to disable, `BUCKET_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "bucket_hooks=debug,bucket_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || bucket_home().join("logs").join("hooks.log");
