...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `BUCKET_HOME`
//! tempdir so we can verify the full install pipeline:
//!   fetch version → download bucket binary → chmod → atomic symlink →
//!   cleanup_old_downloads → persist installer config.
//!
//! The function reads `bucket_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `BUCKET_HOME` and run serially via
//! `#[serial]`.
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main bucket binary download.
    Mock::given(method("GET"))
        .and(path(format!("/bucket-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let symlink = home.join("bin").join("bucket");
    assert!(symlink.is_symlink(), "bucket symlink created");
    let target = std::fs::read_link(&symlink).unwrap();
    assert_eq!(
        target.file_name().unwrap(),
        format!("bucket-0.1.181-{platform}").as_str()
    );

    // `bucket` and `agent` move together — see `swap_managed_bin_links`.
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    let old_binary = download_dir.join(format!("bucket-0.1.180-{platform}"));
    std::fs::write(&old_binary, b"#!/bin/sh\nexit 0\n").unwrap();
    let rel_old = std::path::Path::new("..")
        .join("downloads")
        .join(format!("bucket-0.1.180-{platform}"));
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("bucket")).unwrap();
...
/// Rollback regression: if `agent` swap fails after `bucket` succeeded,
/// `bucket` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_bucket_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // bucket must be rolled back to the prior version.
    let bucket_target = std::fs::read_link(bin_dir.join("bucket")).unwrap();
    assert_eq!(
        bucket_target.file_name().unwrap(),
        format!("bucket-0.1.180-{platform}").as_str(),
        "bucket must be rolled back when agent swap fails"
    );
}

/// Absent-prior rollback regression: fresh install (no prior `bucket` /
/// `agent`), sabotaged `agent` swap must *remove* the just-created `bucket`
/// link so we don't leave it on the new binary while `agent` is absent.
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_bucket_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // No prior `bucket`. Sabotage `agent` swap: non-empty directory → EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("bucket").exists() && !bin_dir.join("bucket").is_symlink(),
        "precondition: bucket must not exist before install",
    );
    ...
    let bucket_path = bin_dir.join("bucket");
    assert!(
        !bucket_path.is_symlink() && !bucket_path.exists(),
        "bucket must be removed on rollback when there was no prior link",
    );
}
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate bucket-pager binary. Verify the
    // update removes the stale symlink from ~/.bucket/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("bucket-pager");
    ...
    assert!(
        ...
        "stale bucket-pager symlink should be removed"
    );
}
...
async fn install_internal_alpha_channel_resolves_max_of_alpha_and_stable() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/bucket-0.1.181-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_bucket_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    let target = std::fs::read_link(home.join("bin").join("bucket")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("bucket")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_bucket_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
