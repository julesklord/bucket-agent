        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            SessionUpdate::Xai(notification) => {
                map.serialize_entry("method", BUCKET_SESSION_UPDATE_METHOD)?;
                map.serialize_entry("params", notification)?;
            }
        }
        map.end()
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        ...
        match update {
            ...
            SessionUpdate::Xai(notification) => Ok(Self {
                timestamp,
                method: BUCKET_SESSION_UPDATE_METHOD.to_string(),
                params: serde_json::to_value(notification)?,
            }),
        }
    }
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific bucket home.
