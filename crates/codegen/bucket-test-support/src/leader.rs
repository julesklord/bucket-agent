//! Leader-mode (`bucket agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`bucket_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "BUCKET_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`bucket_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "BUCKET_BINARY_CLIENT";
...
/// (`BUCKET_BINARY_LEADER`, else the shared [`bucket_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`BUCKET_BINARY_CLIENT`,
/// else the shared [`bucket_binary`] resolution).
...
/// A `bucket agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&bucket_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export BUCKET_* vars
            // (e.g. BUCKET_LEADER_SOCKET pointing at a REAL leader on this
            ...
            // ~/.bucket/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "bucket_agent_core=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".bucket").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".bucket").join("leader.log")).unwrap_or_default()
}
