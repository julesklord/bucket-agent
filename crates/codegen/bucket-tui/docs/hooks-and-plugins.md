...
Bucket Agent supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.bucket/hooks/`
- **Project hooks** — from `.bucket/hooks/` in your repo
...
Plugin hooks automatically receive `BUCKET_PLUGIN_ROOT` and `BUCKET_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.bucket/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `bucket plugin marketplace add <source>`)
...
Sources land in `~/.bucket/config.toml`:
...
Or in `~/.bucket/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../bucket-hooks/examples/README.md) — ready-to-use sample hooks
