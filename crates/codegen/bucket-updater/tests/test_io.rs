...
//!
//! These tests touch global process state — `BUCKET_HOME` (a `OnceLock` in
//! `bucket-config`), `BUCKET_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `BUCKET_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the BUCKET_HOME isolation used in other
//! integration tests.
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_bucket_home() {
    let _ = test_home();
    reset();
...
/// Write a `BucketVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `BucketVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under BUCKET_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every bucket launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_bucket_version env override
//
// The function honors `BUCKET_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_bucket_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("BUCKET_TEST_VERSION", "9.9.9");
    }
    let v = bucket_updater::version::get_installed_bucket_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("BUCKET_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        bucket_updater::version::get_installed_bucket_version()
    };

    unsafe {
        std::env::set_var("BUCKET_TEST_VERSION", "0.0.0-test");
    }
    let overridden = bucket_updater::version::get_installed_bucket_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("BUCKET_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = bucket_updater::version::get_installed_bucket_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("BUCKET_TEST_VERSION", "not-a-version");
    }
    let v = bucket_updater::version::get_installed_bucket_version();
