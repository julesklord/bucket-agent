...
//! ## Plan File
//!
//! The plan file path defaults to `.bucket/plan.md` relative to the session
...
Use this after you have finished writing your plan to the plan file in plan mode."#
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        use crate::implementations::bucket_build::enter_plan_mode::EnterPlanModeTool;
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(
                &EnterPlanModeTool,
            )
            .to_string(),
            id: bucket_tool_runtime::Tool::id(&EnterPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl bucket_tool_runtime::Tool for ExitPlanModeTool {
    type Args = ExitPlanModeInput;
    type Output = ExitPlanModeOutput;

    fn id(&self) -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("exit_plan_mode").expect("valid tool id")
    }
    ...
    ) -> Result<ExitPlanModeOutput, bucket_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(bucket_tool_runtime::Tool::id(&tool).as_str(), "exit_plan_mode");
    ...
    fn tool_is_read_only() {
        assert!(bucket_tool_runtime::Tool::capabilities(&ExitPlanModeTool).is_read_only);
    }
    ...
    async fn exit_with_plan_content() {
        ...
        let plan_dir = tmp.path().join(".bucket");
        ...
        match result {
            ExitPlanModeOutput::PlanReady {
                ...
                assert!(plan_file_path.ends_with(".bucket/plan.md"));
            }
    ...
    async fn sends_plan_mode_exited_notification_with_content() {
        ...
        match notification {
            ToolNotification::PlanModeExited(exited) => {
                ...
                assert!(exited.plan_file_path.ends_with(".bucket/plan.md"));
            }
    ...
    async fn prompt_format_includes_plan_content() {
        ...
        assert!(prompt.contains(".bucket/plan.md"));
