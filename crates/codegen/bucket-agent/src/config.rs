//! Agent definition types — parsed from `.bucket/agents/*.md` files.
...
/// Orchestrator-specific prompt body appended to the standard BucketBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&bucket_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&bucket_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&bucket_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&bucket_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&bucket_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_bucket_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_bucket_build_toolset() -> ToolServerConfig {
    let mut tools = default_bucket_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&bucket_build::EnterPlanModeTool).into());
    tools.push((&bucket_build::ExitPlanModeTool).into());
    tools.push((&bucket_build::AskUserQuestionTool).into());
    tools.push((&bucket_build::WebSearchTool).into());
    tools.push((&bucket_build::ImageGenTool).into());
    tools.push((&bucket_build::ImageToVideoTool).into());
    tools.push((&bucket_build::ReferenceToVideoTool).into());
    tools.push((&bucket_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&bucket_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `bucket-computer` (workspace/sandbox) preset.
fn bucket_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&bucket_build::ReadFileTool).into(),
        (&bucket_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&bucket_build::ListDirTool).into(),
        (&bucket_build::GrepTool).into(),
        (&bucket_build::KillTerminalCommandTool).into(),
        (&bucket_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("bucket-build", workspace_bucket_build_toolset()),
        ("bucket-build-concise", bucket_build_concise_toolset()),
        ("bucket-build-plan", bucket_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("bucket-computer", bucket_computer_toolset()),
    ]
}
...
fn default_bucket_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&bucket_build::ReadFileTool).into(),
            ...
            (&bucket_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&bucket_build::SchedulerCreateTool).into(),
            (&bucket_build::SchedulerDeleteTool).into(),
            (&bucket_build::SchedulerListTool).into(),
            (&bucket_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&bucket_build::UpdateGoalTool).into(),
        ],
        behavior_preset: None,
    }
}
fn bucket_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&bucket_build_concise::BashConciseTool).into(),
            (&bucket_build_concise::ReadFileConciseTool).into(),
            (&bucket_build_concise::SearchReplaceConciseTool).into(),
            (&bucket_build::ListDirTool).into(),
            (&bucket_build::GrepTool).into(),
            kill_task_tool_config(),
            (&bucket_build::TodoWriteTool).into(),
...
pub fn bucket_build_hashline_toolset(
    hashline_tools: Vec<bucket_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<bucket_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&bucket_build::WebSearchTool).into(),
        (&bucket_build::SchedulerCreateTool).into(),
        (&bucket_build::SchedulerDeleteTool).into(),
        (&bucket_build::SchedulerListTool).into(),
        (&bucket_build::MonitorTool).into(),
...
/// Bucket Build + plan mode toolset.
///
/// Extends the default `bucket-build` toolset with plan mode tools:
...
fn bucket_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&bucket_build::ReadFileTool).into(),
            ...
            (&use_tool::UseTool).into(),
            (&bucket_build::UpdateGoalTool).into(),
            (&bucket_build::EnterPlanModeTool).into(),
            (&bucket_build::ExitPlanModeTool).into(),
            (&bucket_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&bucket_build::MonitorTool).into(),
            (&bucket_build::WebSearchTool).into(),
            (&bucket_build::WebFetchTool).into(),
            (&bucket_build::ImageGenTool).into(),
            (&bucket_build::ImageToVideoTool).into(),
            (&bucket_build::ReferenceToVideoTool).into(),
...
/// Bucket Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `bucket_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn bucket_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&bucket_build::ReadFileTool).into(),
...
/// Default Bucket Build toolset + `ask_user_question`.
///
/// Same as `default_bucket_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn bucket_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&bucket_build::ReadFileTool).into(),
...
/// - `model: bucket-3-fast` → `Override("bucket-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "bucket-build-orchestrator")]
    BucketBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::BucketBuild => AgentDefinition::default_bucket_build(),
            Self::BucketBuildConcise => AgentDefinition::bucket_build_concise(),
            Self::BucketBuildPlan => AgentDefinition::bucket_build_plan(),
            Self::BucketBuildPlanNoSubagents => AgentDefinition::bucket_build_plan_no_subagents(),
            Self::BucketBuildAskUser => AgentDefinition::bucket_build_ask_user(),
            ...
            Self::BucketBuildOrchestrator => AgentDefinition::bucket_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .bucket/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_bucket_build_toolset")]
    ...
    pub capability_mode: Option<bucket_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .bucket/agents/ (project-level, highest priority)
    Project,
    /// ~/.bucket/agents/ (user-level)
    User,
    /// ~/.bucket/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_bucket_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.bucket/agent-memory/<name>/`
    User,
    /// `<project>/.bucket/agent-memory/<name>/`
    Project,
    /// `<project>/.bucket/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: bucket_config::bucket_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".bucket/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".bucket/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `bucket_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let bucket = bucket_config::user_bucket_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), bucket.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".bucket/agents/") || path_str.contains(".bucket\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".bucket/bundled/agents/")
            || path_str.contains(".bucket\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `bucket-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<bucket_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["BucketBuild:read_file", "BucketBuildHashline:hashline_read"],
            [
                "BucketBuild:search_replace",
                "BucketBuildHashline:hashline_edit",
            ],
            ["BucketBuild:grep", "BucketBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_bucket_build_toolset(),
    ...
    pub fn bucket_build_concise() -> Self {
        Self {
            tool_config: bucket_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
    ...
    /// Bucket Build agent with plan mode tools.
    pub fn bucket_build_plan() -> Self {
        Self {
    ...
    /// Bucket Build + plan mode WITHOUT subagent tools.
    pub fn bucket_build_plan_no_subagents() -> Self {
        Self {
    ...
    /// Default Bucket Build agent with the `ask_user_question` tool.
    pub fn bucket_build_ask_user() -> Self {
        Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: bucket_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: bucket_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: bucket_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Bucket Build Orchestrator — GBL model with full BucketBuild tools
    ...
    pub fn bucket_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::BucketBuildOrchestrator,
                "BucketBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_bucket_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("bucket-build").unwrap();
    ...
    fn bucket_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&bucket_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&bucket_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn bucket_computer_preset_is_curated_bucket_build_subset() {
        let gc = toolset_for_preset("bucket-computer").unwrap();
        let gb = toolset_for_preset("bucket-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = bucket_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "bucket-computer tool `{}` must also ship in the bucket-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "bucket-computer should be a curated subset of bucket-build"
        );
    }
    #[test]
    fn bucket_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("bucket-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&bucket_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&bucket_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&bucket_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&bucket_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&bucket_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&bucket_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&bucket_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The bucket-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the bucket-build
    ...
    #[test]
    fn bucket_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("bucket-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "bucket-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn bucket_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("bucket-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&bucket_build::LspTool).id,
            ToolConfig::from(&bucket_build::EnterPlanModeTool).id,
            ToolConfig::from(&bucket_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "bucket-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_bucket_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_bucket_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::BucketBuildOrchestrator => true,
            BuiltinAgentName::BucketBuild
            | BuiltinAgentName::BucketBuildConcise
            | BuiltinAgentName::BucketBuildPlan
            | BuiltinAgentName::BucketBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "bucket-build-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("bucket-3-fast".to_string()).to_string(),
            "bucket-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("bucket-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.bucket/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.bucket/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_bucket_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have bucket_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"bucket-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("bucket-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("bucket-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"bucket-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("bucket-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "bucket-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("bucket-code-fast-1".to_string())
        );
    }
    #[test]
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_bucket_build();
