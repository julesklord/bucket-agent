//! Canonical external-settings tool name ↔ Bucket tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`bucket-hooks`) needs the
//! Bucket tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`bucket-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `bucket` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Bucket, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Bucket [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Bucket tool names this Claude tool maps to (empty when there is no direct
    /// Bucket tool — the entry then only contributes a `kind`).
    bucket: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, bucket: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        bucket,
...
const fn match_only(claude: &'static str, bucket: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        bucket,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (bucket-build reads SKILL.md)
...
/// The Bucket [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Bucket tool names a Claude matcher term fires on.
pub fn bucket_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.bucket)
...
/// The Claude names that map to `bucket_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(bucket_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.bucket.contains(&bucket_name))
        .map(|t| t.claude)
}

/// Every distinct Bucket name the table references, for the `bucket-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn bucket_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.bucket.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Bucket name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.bucket.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
