//! Compacted-history assembly (bucket-build's rebuild structure, generic).
//!
//! Moved from `bucket-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! bucket-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // bucket-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn bucket_build_order_recent_before_summary() {
