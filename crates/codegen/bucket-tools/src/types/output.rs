...
use crate::implementations::bucket_build::todo::{TodoItem, TodoState};
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl bucket_tool_runtime::ToolOutput,
) -> bucket_tool_runtime::TypedToolOutput {
...
/// protocol boundary in `bucket-agent-core`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the bucket_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the bucket_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when BucketBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
impl bucket_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<bucket_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => bucket_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::bucket_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::bucket_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::bucket_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::bucket_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::bucket_build::update_goal::UpdateGoalOutput),
...
impl bucket_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<bucket_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => bucket_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl bucket_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<bucket_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(bucket_tool_runtime::ToolChatCompletionResponse {
            result: Some(bucket_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(bucket_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl bucket_tool_runtime::ToolOutput for GrepSearchOutput {}
impl bucket_tool_runtime::ToolOutput for ReadFileOutput {}
impl bucket_tool_runtime::ToolOutput for ListDirOutput {}
impl bucket_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl bucket_tool_runtime::ToolOutput for TodoWriteOutput {}
impl bucket_tool_runtime::ToolOutput for WebSearchOutput {}
impl bucket_tool_runtime::ToolOutput for WebFetchOutput {}
impl bucket_tool_runtime::ToolOutput for SkillOutput {}
impl bucket_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl bucket_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl bucket_tool_runtime::ToolOutput for SearchToolOutput {}
impl bucket_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl bucket_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl bucket_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl bucket_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/bucket-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/bucket-worktree/wt-agent</worktree_path>"),
    ...
    fn assert_cer(
        resp: &bucket_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = bucket_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            bucket_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = bucket_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = bucket_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            bucket_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = bucket_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            bucket_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> bucket_tool_protocol::ToolId {
        bucket_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(bucket_tool_protocol::ToolId::new("text").unwrap());
