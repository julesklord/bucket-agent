...
use bucket_workspace::permission::is_enable_always_approve_option;
...
///
/// 1. `BUCKET_DEFAULT_SELECTED_PERMISSION` env var (headless / agent testing —
...
pub fn load_default_selected_permission() -> DefaultSelectedPermission {
    CONFIG_LOADED.with(|loaded| {
        if !loaded.get() {
            let resolved = std::env::var("BUCKET_DEFAULT_SELECTED_PERMISSION")
                .ok()
...
mod tests {
    use super::*;
    use bucket_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
