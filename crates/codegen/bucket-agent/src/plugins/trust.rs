...
//!
//! Plugins from project directories (`.bucket/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.bucket/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.bucket/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.bucket/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_bucket_home() so a project's `.bucket/trusted-plugins` is never
        // read as the user trust store when neither BUCKET_HOME nor a home dir resolves.
        let Some(bucket) = bucket_config::user_bucket_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = bucket.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.bucket/trusted-plugins`.
    ...
    /// rewrites `~/.bucket/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.bucket/trusted-plugins`.
